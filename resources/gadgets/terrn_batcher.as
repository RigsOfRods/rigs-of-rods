...
void drawMainPanel()
{
    ...
    Ogre::SceneManager@ sceneMgr = findSceneManager(root, "main_scene_manager");
    if (@sceneMgr == null)
    {
    ...
    Ogre::SceneNode@ terrnNode = findChildSceneNode(rootNode, "Terrain: " + terrain.getTerrainName());
    if (@terrnNode==null)
    {
    ...
    Ogre::SceneNode@ outputsNode = findChildSceneNode(terrnNode, tbuiOutputsNodeName);
    if (@outputsNode==null)
    {
    ...
    drawSelectionBasket(terrnNode);
    drawSelectionBatchControls(terrnNode);
    ...
    drawTreeNodeOgreSceneNodeRecursive(terrnNode);
}
...
dictionary uniqueSubmeshSetHits; // dictionary { mesh name + ':' + submesh index => int numHits } 
    ...
    for (uint i=0; i<tobjNodes.length(); i++)
    {
        ...
        Ogre::SceneNode@ tobjNode = findChildSceneNode(terrnNode,tobjNodes[i]);
        if (@tobjNode == null)
        {
            ImGui::SameLine();
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    basketProcessSingleInput(materialSet, uniqueSubmeshSet, uniqueSubmeshSetHits, tobjNode, j);
                }
            }
        }
    }
    ...
    for (uint i = 0; i<materialKeys.length(); i++)
    {
        ...
        {
            Ogre::TexturePtr tex = getTrivialCaseMaterialTexture(mat);
            ImGui::SameLine();
...
void basketProcessSingleInput(dictionary&inout materialSet, dictionary&inout subMeshSet, dictionary&inout subMeshSetHits, Ogre::SceneNode@ tobjNode, int childIndex)
{
    ...
    else if (movables.length() == 1 && movables[0].getMovableType() == "Entity")
    {
        ...
        for (uint k=0; k<subEntities.length(); k++)
        {
            ...
            {
                materialSet[mat.getName()] = (isMaterialTrivialCase(mat)) ? int(materialSet[mat.getName()])+1 : -1; 
...
{
    if (isSceneNodePicked(key, index))
    {
...
void drawTreeNodeOgreSceneNodeRecursive(Ogre::SceneNode@ snode, uint indexUnderParent=0)
{
    ...
    else
    {
        ...
        {
            drawInlineSceneNodeControlsToInspector(snode, indexUnderParent);
        }
        else 
        {
...
void drawTreeNodeOgreMovableObject(Ogre::MovableObject@ movable)
{
    if (ImGui::TreeNode(movable.__getUniqueName()))
    {
        ...
        if (movable.getMovableType() == "Entity")
        {
            ...
            {
                drawTreeNodeOgreSubEntity(subEntities, i);
            }
            // END +TerrnBatcher
        }
...
void drawInlineSceneNodeControlsToInspector(Ogre::SceneNode@ snode, uint indexUnderParent)
{
    ...
    ImGui::SameLine();
    if (isSceneNodePicked(snode.getParentSceneNode().getName(), indexUnderParent))
    {
        if(ImGui::SmallButton("-UnPick"))
        {
            unPickSceneNode(snode.getParentSceneNode().getName(), indexUnderParent);
        }
    }
    else
    {
        if (ImGui::SmallButton("+Pick!")) 
        {
            ...
            pickSceneNode(snode.getParentSceneNode().getName(), indexUnderParent);
        }
    }
...
{
    return !getTrivialCaseMaterialTexture(mat).isNull();
}
...
void addSceneNodeAttachedMeshesToBatch(Ogre::ManualObject@ mo, Ogre::SceneNode@ pickedNode, string&inout foundMatName)
{
    ...
    for (uint iMovas = 0; iMovas < movables.length(); iMovas++)
    {
        ...
        for (uint iSubent=0; iSubent<subEntities.length(); iSubent++)
        {
            ...
            addSingleSubMeshToBatch(mo, subMesh, pickedNode.getPosition(), pickedNode.getOrientation(), pickedNode.getScale());
        }            
...
void batchSelectedMeshes(Ogre::SceneNode@ terrnNode)
{
    ...
    string moName = composeUniqueId("batch #" + tbuiNumBatchesCreated++);
    ...
    {
        Ogre::SceneNode@ tobjNode = findChildSceneNode(terrnNode, tobjNodes[iNode]);
        if (@tobjNode == null)
        {
        ...
        for (uint iChild=0; iChild<nodeIndices.length(); iChild++)
        {
            ...
            addSceneNodeAttachedMeshesToBatch(mo, childNode, /*[inout]*/foundMatName);
            if (tbuiHideOriginal)
            {
