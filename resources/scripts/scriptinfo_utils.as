...
namespace scriptinfo_utils
{
	...
	{
		scriptinfo_internal::ScriptInfoHelper h;
		...
		while (state != -1 && pos < body.length())
		{
			...
			switch (state)
			{
				...
				case 4:  // blanks after slashes
					if (!h.isblank()) {
						if (h.ischar('\\')) { capture_startpos = pos; state=5; }  else { capture_startpos = pos; state=8; } }
					else if (h.isEol()) { state=0;}
					break;
				case 5: // cmd, like '\file'
					if (h.isblank()) { 
						string cmd = body.substr(capture_startpos, pos-capture_startpos);
						//game.log('DBG cmd: "'+cmd+'"'); 
						if (h.isEol()) {state=0;} 
						else if (cmd=='\\title') { state=6; capture_startpos = pos; } 
						else if (cmd=='\\brief') {state=7;capture_startpos = pos; } 
						else {state=8;capture_startpos = pos; } }
					break;
				case 6:
...
namespace scriptinfo_internal // Don't touch!
{
	class ScriptInfoHelper
	{
		...
		{ 
			...
			//game.log("DBG trimStart(): s='"+s+"', found:"+found+", dryrun: '"+s.substr(found)+"'");
			if (found > 0) 
			{
