...
Node::Ref SequentialImporter::ResolveNodeByIndex(unsigned int index_in)
{
    ...
    {
        std::stringstream msg;
            msg << "Node with index [" << index_in << "] resolves to index [" << index_out << "]";
...
void SequentialImporter::ProcessModule(boost::shared_ptr<RigDef::File::Module> module)
{
    ...
    FOR_EACH (File::KEYWORD_FLEXBODIES, module->flexbodies, flexbody,
    {
        ...
        ResolveNodeRanges(flexbody->forset);
    });
...
{                                                        \
    ...
    if (COUNT_VAR != 0) { STREAM << " (start index:" << std::setw(4) << offset << ", end index:" << (offset + COUNT_VAR) - 1 << ")"; } \
}

void SequentialImporter::LogNodeStatistics()
{
    std::stringstream msg;
    msg << "\n~~~ Node statistics: ~~~"
...
void SequentialImporter::IterateAndLogAllNodes()
{
    std::stringstream report;
    report << "\n~~~ Iterating all nodes (total: " << m_all_nodes.size() << ") ~~~\n";
    ...
    this->AddMessage(Message::TYPE_INFO, report.str());
}
