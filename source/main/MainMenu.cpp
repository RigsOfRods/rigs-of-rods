...
namespace RoR
{
    ...
    void MainMenu::EnterMainMenuLoop()
    {
        ...
        while (App::app_state.GetPending() == AppState::MAIN_MENU)
        {
            ...
            if (!rw->isActive() && rw->isVisible()) rw->update(); // update even when in background !
        }
    ...
    void MainMenu::MainMenuLoopUpdate(float seconds_since_last_frame)
    {
...
#ifdef USE_SOCKETW
        if (App::mp_state.GetPending() == MpState::CONNECTED && App::mp_state.GetActive() == MpState::DISABLED)
        ...
        while (!events.empty())
        {
            ...
            case Networking::NetEvent::Type::SERVER_KICK:
                ...
                App::GetGuiManager()->ShowMessageBox(_LC("Network", "Network disconnected"), events.front().message.c_str());
                break;
            ...
            case Networking::NetEvent::Type::CONNECT_FAILURE:
                ...
                App::GetGuiManager()->ShowMessageBox(_LC("Network", "Multiplayer: connection failed"),
        ...
        if (App::mp_state.GetActive() == MpState::CONNECTED) { App::GetGuiManager()->GetMpClientList()->update(); }
    ...
    void MainMenu::MainMenuLoopUpdateEvents(float seconds_since_last_frame)
    {
        ...
        {
            RoR::App::GetOverlayWrapper()->update(
                seconds_since_last_frame); // TODO: What's the meaning of this? It only updates some internal timer. ~ only_a_ptr
        }

        if (RoR::App::GetInputEngine()->getEventBoolValueBounce(
                EV_COMMON_QUIT_GAME)) // TODO: EV_COMMON_QUIT_GAME should really be EV_COMMON_ESCAPE ~ only_a_ptr, 12/2017
        {
