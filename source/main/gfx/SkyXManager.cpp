...
{
    if (mBasicController != nullptr) return mBasicController->getSunDirection();
...
bool SkyXManager::UpdateSkyLight()
{
    ...
    Ogre::Vector3 sunPos =
        gEnv->mainCamera->getDerivedPosition() - lightDir * mSkyX->getMeshManager()->getSkydomeRadius(gEnv->mainCamera);
    ...
    if (App::GetSimTerrain()->getWater()) { App::GetSimTerrain()->getWater()->WaterSetSunPosition(sunPos * 0.1); }
    ...
    /*	Ogre::Vector3 sunCol = mSunGradient.getColor(point);
        ...
        if (App::GetSimTerrain()->getWater()) App::GetSimTerrain()->getWater()->setFadeColour(Ogre::ColourValue(sunCol.x,
       sunCol.y, sunCol.z));
        */
