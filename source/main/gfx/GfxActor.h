    ...
    class GfxActor
    {
      ...
      public:
        ...
        struct NodeGfx
        {
            ...
            float
                     nx_wet_time_sec; //!< 'Wet' means "already out of water, producing dripping particles". Set to -1 when not 'wet'.
        ...
        struct Rod
        {
            ...
            uint16_t rod_beam_index;  //!< Index of the associated `beam_t` instance; assumes Actor has at most 65536 beams (RoR
                                      //!< doesn't have a soft limit now, but until v0.4.8 it was 5000 beams).
            uint16_t rod_diameter_mm; //!< Diameter in millimeters

            // Assumption: Actor has at most 65536 nodes (RoR doesn't have a soft limit right now, but until v0.4.8 it was 1000
            // nodes).
        ...
        GfxActor(Actor *actor, ActorSpawner *spawner, std::string ogre_resource_group, std::vector<NodeGfx> &gfx_nodes,
        ...
        void AddRod(int beam_index, int node1_index, int node2_index, const char *material_name, bool visible,
        ...
        int  GetNumFlexbodies() const
        {
            return static_cast<int>(m_flexbodies.size());
        }
        ...
        void SortFlexbodies();
        void AddFlexbody(FlexBody *fb)
        {
            m_flexbodies.push_back(fb);
        }
        Attributes &GetAttributes()
        {
            return m_attr;
        }
        inline Ogre::MaterialPtr &GetCabTransMaterial()
        {
            return m_cab_mat_visual_trans;
        }
        inline VideoCamState GetVideoCamState() const
        {
            return m_vidcam_state;
        }
        inline DebugViewType GetDebugView() const
        {
            return m_debug_view;
        }
        ...
        NodeData *GetSimNodeBuffer()
        {
            return m_simbuf.simbuf_nodes.get();
        }
        void SetSurveyMapEntity(SurveyMapEntity *e)
        {
            m_survey_map_entity = e;
        }
        SurveyMapEntity *GetSurveyMapEntity()
        {
            return m_survey_map_entity;
        }
        std::set<GfxActor *> GetLinkedGfxActors()
        {
            return m_linked_gfx_actors;
        }
        Ogre::String GetResourceGroup()
        {
            return m_custom_resource_group;
        }
        bool HasDriverSeatProp() const
        {
            return m_driverseat_prop_index != -1;
        }
        ...
        void CalcPropAnimation(const int flag_state, float &cstate, int &div, float timer, const float lower_limit,
