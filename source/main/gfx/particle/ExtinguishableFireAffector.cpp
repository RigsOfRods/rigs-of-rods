...
namespace Ogre
{
    ...
    //-----------------------------------------------------------------------
    ExtinguishableFireAffector::ExtinguishableFireAffector(ParticleSystem *psys) : ParticleAffector(psys), mPsys(psys)
    {
        ...
        if (createParamDictionary("ExtinguishableFireAffector"))
        {
            ...
            dict->addParameter(ParameterDef("middle_point",
                                            "The middle point of the sphere on which water particles will interact.", PT_VECTOR3),
                               &msMiddlePointCmd);
            dict->addParameter(ParameterDef("intensity",
                                            ...
                                            PT_REAL),
                               &msIntensityCmd);
            dict->addParameter(ParameterDef("max_intensity", "The maximum intensity the fire can grow to.", PT_REAL),
                               &msMaxIntensityCmd);
            dict->addParameter(
                ParameterDef("intensity_growth", "The amount by which the intensity of the fire grows per second.", PT_REAL),
                &msMaxIntensityCmd);
            dict->addParameter(ParameterDef("radius", "The radius of the sphere.", PT_REAL), &msRadiusCmd);
        }
    ...
    void ExtinguishableFireAffector::_affectParticles(ParticleSystem *pSystem, Real timeElapsed)
    {
        ...
        if (mIntensity < 0)
        {
            // The fire is extinguished (normally, this particleSystem should get deleted by the fire extinguisher, before this
            // can happen)
        ...
        else
        {
            ...
            {
                // update the fire
                mPsys->setDefaultDimensions(originalDimensions.x * mIntensity / originalIntensity,
                                            originalDimensions.y * mIntensity / originalIntensity);
                updateIntensityRequired = false;
                ScriptEngine::getSingleton().fireEvent(objectInstanceName, mIntensity);
            }
        }
    ...
    {
        return StringConverter::toString(static_cast<const ExtinguishableFireAffector *>(target)->getMiddlePoint());
    }
    void ExtinguishableFireAffector::CmdMiddlePoint::doSet(void *target, const String &val)
    {
        static_cast<ExtinguishableFireAffector *>(target)->setMiddlePoint(StringConverter::parseVector3(val));
    }
    //-----------------------------------------------------------------------
    String ExtinguishableFireAffector::CmdIntensity::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ExtinguishableFireAffector *>(target)->getIntensity());
    }
    void ExtinguishableFireAffector::CmdIntensity::doSet(void *target, const String &val)
    {
        static_cast<ExtinguishableFireAffector *>(target)->setIntensity(StringConverter::parseReal(val));
    }
    //-----------------------------------------------------------------------
    String ExtinguishableFireAffector::CmdMaxIntensity::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ExtinguishableFireAffector *>(target)->getMaxIntensity());
    }
    void ExtinguishableFireAffector::CmdMaxIntensity::doSet(void *target, const String &val)
    {
        static_cast<ExtinguishableFireAffector *>(target)->setMaxIntensity(StringConverter::parseReal(val));
    }
    //-----------------------------------------------------------------------
    String ExtinguishableFireAffector::CmdIntensityGrowth::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ExtinguishableFireAffector *>(target)->getIntensityGrowth());
    }
    void ExtinguishableFireAffector::CmdIntensityGrowth::doSet(void *target, const String &val)
    {
        static_cast<ExtinguishableFireAffector *>(target)->setIntensityGrowth(StringConverter::parseReal(val));
    }
    //-----------------------------------------------------------------------
    String ExtinguishableFireAffector::CmdRadius::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ExtinguishableFireAffector *>(target)->getRadius());
    }
    void ExtinguishableFireAffector::CmdRadius::doSet(void *target, const String &val)
    {
        static_cast<ExtinguishableFireAffector *>(target)->setRadius(StringConverter::parseReal(val));
    }
    //------------------------------------------------------------------------
