...
namespace Ogre
{
    ...
    ShaderParticleRenderer::ShaderParticleRenderer()
        : mDefaultParticleSize(1, 1), mKeepInLocalSpace(false), mParentNode(NULL), mParentIsTagPoint(false),
          mSortMode(SM_DISTANCE), mLightListUpdated(0), mRadius(0.0f), mRenderQueueID(RENDER_QUEUE_MAIN),
          mVertexFormatTexture(false), mVertexFormatSize(false), mVertexFormatRotation(false), mVertexFormatRotationSpeed(false),
          mVertexFormatDirection(false), mVertexFormatColour(false), mVertexFormatTTL(false), mVertexFormatTotalTTL(false),
          mVertexFormatTimeFragment(false), mVertexFormatTimeFragmentInv(false), mVertexSize(0)
    {
        if (createParamDictionary("ShaderParticleRenderer"))
        {
            ParamDictionary *dict = getParamDictionary();
            dict->addParameter(ParameterDef("diffuse_colour", "Adds diffuse colour to vertex format (type = float4)", PT_BOOL),
                               &msVertexFmtColour);

            dict->addParameter(
                ParameterDef("texture_coord", "Adds general texture coordinate to vertex format (type = float2)", PT_BOOL),
                &msVertexFmtTexture);

            dict->addParameter(ParameterDef("particle_size", "Adds particle size to vertex format (type = float2)", PT_BOOL),
                               &msVertexFmtSize);

            dict->addParameter(
                ParameterDef("particle_rotation",
    ...
    //////////////////////////////////////////////////////////////////////////
    void ShaderParticleRenderer::_updateRenderQueue(RenderQueue *queue, Ogre::list<Particle *>::type &currentParticles,
                                                    bool cullIndividually)
    {
        ...
        if (!currentParticles.empty())
        {
            ...
            for (Ogre::list<Particle *>::type::iterator it = currentParticles.begin(); it != currentParticles.end(); ++it)
            {
                ...
                float fDist = (mParentNode != NULL) ? mParentNode->getPosition().distance(pParticle->mPosition)
                                                    : pParticle->mPosition.length();
                if (fDist > mRadius) mRadius = fDist;
            }
            pVB->unlock();
        }
    ...
    {
        if (mParentNode != NULL) { return cam->getRealPosition().squaredDistance(mParentNode->getPosition()); }
        else
        {
            return 0.0f;
        }
    ...
    bool ShaderParticleRenderer::allocateBuffers(size_t iNumParticles)
    {
        ...
        if (mVertexData->vertexDeclaration->getElementCount() == 0)
        {
            ...
            if (mVertexFormatColour) ofs += pDecl->addElement(0, ofs, VET_FLOAT4, VES_DIFFUSE).getSize(); // diffuse colour
            ...
            {
                if (mVertexFormatRotation && mVertexFormatRotationSpeed)
                    ofs += pDecl->addElement(0, ofs, VET_FLOAT2, VES_TEXTURE_COORDINATES, ix++)
                               .getSize(); // current rotation and rotation speed
                else
                    ofs += pDecl->addElement(0, ofs, VET_FLOAT1, VES_TEXTURE_COORDINATES, ix++)
                               .getSize(); // current rotation or rotation speed
            }

            if (mVertexFormatDirection)
                ofs += pDecl->addElement(0, ofs, VET_FLOAT3, VES_TEXTURE_COORDINATES, ix++)
                           .getSize(); // particle direction (as speed)
            ...
            switch (iNumTimes)
            {
            case 1: ofs += pDecl->addElement(0, ofs, VET_FLOAT1, VES_TEXTURE_COORDINATES, ix++).getSize(); break;
            case 2: ofs += pDecl->addElement(0, ofs, VET_FLOAT2, VES_TEXTURE_COORDINATES, ix++).getSize(); break;
            case 3: ofs += pDecl->addElement(0, ofs, VET_FLOAT3, VES_TEXTURE_COORDINATES, ix++).getSize(); break;
            case 4: ofs += pDecl->addElement(0, ofs, VET_FLOAT4, VES_TEXTURE_COORDINATES, ix++).getSize(); break;
            }
        ...
        {
            pIB = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(
                Ogre::HardwareIndexBuffer::IT_16BIT, iNumParticles * 6, Ogre::HardwareBuffer::HBU_STATIC_WRITE_ONLY);
            if (pIB.isNull()) return false;
    ...
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatColour());
    }
    void ShaderParticleRenderer::CmdVertexFormatColour::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatColour(StringConverter::parseBool(val));
    }
    ...
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatTexture());
    }
    void ShaderParticleRenderer::CmdVertexFormatTexture::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatTexture(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatSize::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatSize());
    }
    void ShaderParticleRenderer::CmdVertexFormatSize::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatSize(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatRotation::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatRotation());
    }
    void ShaderParticleRenderer::CmdVertexFormatRotation::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatRotation(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatRotationSpeed::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatRotationSpeed());
    }
    void ShaderParticleRenderer::CmdVertexFormatRotationSpeed::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatRotationSpeed(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatDirection::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatDirection());
    }
    void ShaderParticleRenderer::CmdVertexFormatDirection::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatDirection(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatTTL::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatTTL());
    }
    void ShaderParticleRenderer::CmdVertexFormatTTL::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatTTL(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatTotalTTL::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatTotalTTL());
    }
    void ShaderParticleRenderer::CmdVertexFormatTotalTTL::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatTotalTTL(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatTimeFrag::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatTimeFragment());
    }
    void ShaderParticleRenderer::CmdVertexFormatTimeFrag::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatTimeFragment(StringConverter::parseBool(val));
    }

    //////////////////////////////////////////////////////////////////////////
    String ShaderParticleRenderer::CmdVertexFormatTimeFragInv::doGet(const void *target) const
    {
        return StringConverter::toString(static_cast<const ShaderParticleRenderer *>(target)->getVertexFormatTimeFragmentInv());
    }
    void ShaderParticleRenderer::CmdVertexFormatTimeFragInv::doSet(void *target, const String &val)
    {
        static_cast<ShaderParticleRenderer *>(target)->setVertexFormatTimeFragmentInv(StringConverter::parseBool(val));
    }
} // namespace Ogre
