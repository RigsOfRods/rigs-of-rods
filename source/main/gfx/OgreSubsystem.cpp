...
namespace RoR
{
    ...
    OgreSubsystem::OgreSubsystem() : m_ogre_root(nullptr), m_render_window(nullptr), m_viewport(nullptr)
    {
    }
    ...
    bool OgreSubsystem::Configure()
    {
        ...
        catch (...)
        {
            ...
            if (dialog->display()) m_ogre_root->saveConfig();
        }
        ...
        m_render_window =
            Ogre::Root::getSingleton().createRenderWindow("Rigs of Rods version " + Ogre::String(ROR_VERSION_STRING), width,
                                                          height, ropts["Full Screen"].currentValue == "Yes", &miscParams);
    ...
    bool OgreSubsystem::LoadOgrePlugins(Ogre::String const &pluginsfile)
    {
        ...
        catch (Ogre::Exception &e)
        {
            Ogre::LogManager::getSingleton().logMessage(
                pluginsfile + " not found, automatic plugin loading disabled. Message: " + e.getFullDescription());
            return false;
        }
        ...
        if (pluginDir.empty()) { pluginDir = RoR::App::sys_process_dir.GetActive(); }
    ...
    void OgreSubsystem::WindowResized(Ogre::Vector2 const &size)
    {
        ...
        if (m_viewport->getCamera()) { m_viewport->getCamera()->setAspectRatio(Ogre::Real(size.x) / Ogre::Real(size.y)); }
    }
