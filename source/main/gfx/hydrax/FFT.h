        ...
        class FFT : public Noise
        {
          public:
            ...
            struct Options
            {
                 ...
                 */
                Options()
                    : Resolution(128), PhysicalResolution(32.0f), Scale(0.25f), WindDirection(Ogre::Vector2(4, 5)),
                      AnimationSpeed(1), KwPower(6.0f), Amplitude(1.0f), GPU_Strength(2.0f),
                 ...
                 */
                Options(const int &_Resolution, const float &_PhysicalResolution, const float &_Scale,
                        const Ogre::Vector2 &_WindDirection, const float &_AnimationSpeed, const float &_KwPower,
                        const float &_Amplitude)
                    : Resolution(_Resolution), PhysicalResolution(_PhysicalResolution), Scale(_Scale),
                      WindDirection(_WindDirection), AnimationSpeed(_AnimationSpeed), KwPower(_KwPower), Amplitude(_Amplitude),
                      GPU_Strength(2.0f), GPU_LODParameters(Ogre::Vector3(0.5f, 50, 150000))
                {
                }
                ...
                Options(const int &_Resolution, const float &_PhysicalResolution, const float &_Scale,
                        const Ogre::Vector2 &_WindDirection, const float &_AnimationSpeed, const float &_KwPower,
                        const float &_Amplitude, const float &_GPU_Strength, const Ogre::Vector3 &_GPU_LODParameters)
                    : Resolution(_Resolution), PhysicalResolution(_PhysicalResolution), Scale(_Scale),
                      WindDirection(_WindDirection), AnimationSpeed(_AnimationSpeed), KwPower(_KwPower), Amplitude(_Amplitude),
                      GPU_Strength(_GPU_Strength), GPU_LODParameters(_GPU_LODParameters)
                {
                }
            };
          ...
          private:
             ...
             */
            const float _getPhillipsSpectrum(const Ogre::Vector2 &waveVector, const Ogre::Vector2 &wind,
                                             const float &kwPower_ = 2.0f) const;
            ...
            /// the data of the simulation at time t, which is formed using the data at time 0 and the angular frequencies at time
            /// t
