...
namespace Hydrax
{
    GodRaysManager::GodRaysManager(Hydrax *h)
        : mCreated(false), mManualGodRays(0), mProjectorCamera(0), mProjectorSN(0), mPerlin(0), mNoiseDerivation(3),
          mNoisePositionMultiplier(50), mNoiseYNormalMultiplier(10), mNoiseNormalMultiplier(0.175), mSimulationSpeed(5.0f),
          mNumberOfRays(100), mRaysSize(0.03f), mObjectsIntersections(false), mHydrax(h)
    {
        for (int k = 0; k < 2; k++)
        {
    ...
    void GodRaysManager::create(const HydraxComponent &HC)
    {
        ...
        if (mObjectsIntersections) { _createDepthRTT(); }
    ...
    {
        if (!mCreated || !mHydrax->_isCurrentFrameUnderwater()) { return; }
    ...
    Ogre::Vector2 GodRaysManager::_calculateRayPosition(const int &RayNumber)
    {
        ...
        Ogre::Vector2 RayPos = Ogre::Vector2( // X coord
        ...
        Ogre::Vector2 Position =
            RayPos * mNoisePositionMultiplier + Ogre::Vector2(mProjectorSN->getPosition().x, mProjectorSN->getPosition().z);
    ...
    {
        if (!mObjectsIntersections) { return; }
        ...
        Ogre::Matrix4 TexViewProj = PROJECTIONCLIPSPACE2DTOIMAGESPACE_PERSPECTIVE *
                                    mProjectorCamera->getProjectionMatrixWithRSDepth() * mProjectorCamera->getViewMatrix();
        ...
        for (TechIt = mDepthTechniques.begin(); TechIt != mDepthTechniques.end(); TechIt++)
        {
            ...
            (*TechIt)->getPass(0)->getFragmentProgramParameters()->setNamedConstant("uLightPosition",
                                                                                    mProjectorSN->getPosition());
            (*TechIt)->getPass(0)->getFragmentProgramParameters()->setNamedConstant("uLightFarClipDistance",
    ...
    void GodRaysManager::_createMaterials(const HydraxComponent &HC)
    {
        ...
        if (_isComponent(HC, HYDRAX_COMPONENT_CAUSTICS)) { NumberOfDepthChannels++; }
        ...
        case MaterialManager::SM_GLSL:
        {
            ...
            // main function
            VertexProgramData +=
                Ogre::String("void main()\n") + "{\n" + "gl_Position = gl_ModelViewProjectionMatrix * gl_Vertex;\n";
            if (mObjectsIntersections)
            {
                VertexProgramData += Ogre::String("vec4 wPos = uWorld * gl_Vertex);\n") + "Position_  = wPos.xyz;\n" +
        ...
        case MaterialManager::SM_CG:
        {
            if (mObjectsIntersections)
                ...
                FragmentProgramData += Ogre::String("void main_fp(\n") +
                                       // OUT
                                       "out float4 oColor    : COLOR)\n" + "{\n" + "oColor = float4(float3(" +
                                       GB[NumberOfDepthChannels] + ") * 0.1, 1);\n" + "}\n";
        }
        break;
        ...
        case MaterialManager::SM_GLSL:
        {
            if (mObjectsIntersections)
                FragmentProgramData +=
                    ...
                    GB[NumberOfDepthChannels] + ") * 0.1, 1.0);\n" + "}\n" + "}\n";
            else
                FragmentProgramData += Ogre::String("\n") +
                                       ...
                                       // OUT
                                       // main function
                                       "void main()\n" + "{\n" + "gl_FragColor = vec4(vec3(" + GB[NumberOfDepthChannels] +
                                       ") * 0.1, 1.0);\n" + "}\n";
        }
        break;
        }
        ...
        // Build our material
        mMaterials[0] = Ogre::MaterialManager::getSingleton().create(_def_GodRays_Material_Name,
        ...
        GR_Technique0_Pass0->createTextureUnitState(_def_GodRays_Depth_Map)
        ...
        // Build our material
        mMaterials[1] = Ogre::MaterialManager::getSingleton().create(_def_GodRaysDepth_Material_Name,
                                                                     Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
        ...
        mMaterialManager->fillGpuProgramsToPass(GRD_Technique0_Pass0, GpuProgramNames, mHydrax->getShaderMode(), EntryPoints,
                                                GpuProgramsData);
    ...
    void GodRaysManager::addDepthTechnique(Ogre::Technique *Technique, const bool &AutoUpdate)
    {
        if (!Ogre::MaterialManager::getSingleton().resourceExists(_def_GodRaysDepth_Material_Name))
        {
            HydraxLOG("GodRaysManager::addDepthTechnique(...) Objects intersection must be enabled and Hydrax::create() already "
                      "called, skipping...");
    ...
    {
        mProjectorRTT = Ogre::TextureManager::getSingleton().createManual(
            _def_GodRays_Depth_Map, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D,
            // 256*256 must be sufficient
            256, 256, 0,
            // Only one channel
            Ogre::PF_L8, Ogre::TU_RENDERTARGET);
    ...
    void GodRaysManager::DepthMapListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent &evt)
    {
        Ogre::SceneManager::MovableObjectIterator EntityIterator =
            mGodRaysManager->mHydrax->getSceneManager()->getMovableObjectIterator("Entity");
