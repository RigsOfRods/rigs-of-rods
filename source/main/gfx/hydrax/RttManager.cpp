...
namespace Hydrax
{
    RttManager::RttManager(Hydrax *h) : mHydrax(h), mPlanesSceneNode(0), mReflectionDisplacementError(0.5f)
    {
        Ogre::String RttNames[6] = {_def_Hydrax_Reflection_Rtt_Name, _def_Hydrax_Refraction_Rtt_Name,
                                    _def_Hydrax_Depth_Rtt_Name,      _def_Hydrax_Depth_Reflection_Rtt_Name,
                                    _def_Hydrax_API_Rtt_Name,        _def_Hydrax_GPU_Normal_Map_Rtt_Name};
    ...
    void RttManager::initialize(const RttType &Rtt)
    {
        ...
        case RTT_REFRACTION:
        {
            ...
            _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y, Ogre::ColourValue(WC.x, WC.y, WC.z), RenderSkyBox,
                           &mRefractionListener);
        }
        break;
        ...
        case RTT_DEPTH:
        {
            _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y, Ogre::ColourValue::Black, false, &mDepthListener, "HydraxDepth",
                           false);
        }
        break;
        ...
        {
            _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y, Ogre::ColourValue::Black, false, &mDepthReflectionListener,
                           "HydraxDepth", false);
        }
        break;
        ...
        {
            _initializeRtt(Rtt, Ogre::Vector3::NEGATIVE_UNIT_Y, Ogre::ColourValue(0.5, 1, 0.5), false, &mGPUNormalMapListener, "",
                           false);
        }
        break;
        }
                                    ...
                                    const bool &ShadowsEnabled)
    {
        ...
        Ogre::MeshManager::getSingleton().createPlane(
            mRttOptions[Rtt].Name + "ClipPlane", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, *mPlanes[Rtt],
        ...
        mTextures[Rtt] = Ogre::TextureManager::getSingleton().createManual(
            mRttOptions[Rtt].Name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, TSize.Width,
            TSize.Height, 0, getPixelFormat(Rtt), Ogre::TU_RENDERTARGET);
        ...
        if (MaterialScheme != "") { RT_Texture_Viewport->setMaterialScheme(MaterialScheme); }
    ...
    void RttManager::CReflectionListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent &evt)
    {
        ...
        // Underwater
        if (mHydrax->_isCurrentFrameUnderwater() && (mHydrax->getCamera()->getDerivedPosition().y >
                                                     mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->getPosition().y))
        {
            mCameraPlaneDiff    = 0;
        ...
        // Overwater
                 ...
                 (mHydrax->getCamera()->getDerivedPosition().y <
                  mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->getPosition().y))
        {
            mCameraPlaneDiff = mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->getPosition().y -
                               mHydrax->getCamera()->getDerivedPosition().y + mRttManager->mReflectionDisplacementError;
        ...
        if (IsInUnderwaterError) { mCReflectionQueueListener.mActive = false; }
        else
        {
    ...
    void RttManager::CReflectionListener::postRenderTargetUpdate(const Ogre::RenderTargetEvent &evt)
    {
        ...
        if (mCameraPlaneDiff != 0) { mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->translate(0, mCameraPlaneDiff, 0); }
                                                                                           ...
                                                                                           &skipThisInvocation)
    {
            ...
            mActive)
        {
            ...
            Ogre::Root::getSingleton().getRenderSystem()->_setProjectionMatrix(
                mRttManager->mHydrax->getCamera()->getProjectionMatrixRS());
        }
    }
    ...
    void RttManager::CDepthListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent &evt)
    {
        ...
        while (EntityIterator.hasMoreElements())
        {
            ...
            for (k = 0; k < CurrentEntity->getNumSubEntities(); k++)
            {
                ...
                CurrentEntity->getSubEntity(k)->setMaterialName(
                    mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_DEPTH)->getName());
            }
        ...
        if (mHydrax->_isCurrentFrameUnderwater())
        {
            ...
            mHydrax->getMesh()->getEntity()->setMaterialName(
                mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_SIMPLE_RED)->getName());
    ...
    void RttManager::CDepthReflectionListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent &evt)
    {
        ...
        if (mHydrax->getCamera()->getDerivedPosition().y >
            mRttManager->mPlanes[RTT_DEPTH_REFLECTION]->getParentNode()->getPosition().y)
        {
            mCameraPlaneDiff    = 0;
            IsInUnderwaterError = true;
        }
        ...
        if (!IsInUnderwaterError) { mHydrax->getCamera()->enableCustomNearClipPlane(mRttManager->mPlanes[RTT_DEPTH_REFLECTION]); }
    }
    ...
    void RttManager::CGPUNormalMapListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent &evt)
    {
        ...
        mRttManager->mHydrax->getMesh()->setMaterialName(
            mRttManager->mHydrax->getGPUNormalMapManager()->getNormalMapMaterial()->getName());
