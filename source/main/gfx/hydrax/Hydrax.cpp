...
namespace Hydrax
{
    ...
    Hydrax::Hydrax(Ogre::SceneManager *sm, Ogre::Camera *c, Ogre::Viewport *v)
        : mSceneManager(sm), mCamera(c), mViewport(v), mCreated(false), mVisible(true), mPolygonMode(Ogre::PM_SOLID),
          mShaderMode(MaterialManager::SM_HLSL), mPosition(Ogre::Vector3(0, 0, 0)), mPlanesError(0),
          mFullReflectionDistance(99999997952.0), mGlobalTransparency(0.05), mWaterColor(Ogre::Vector3(0, 0.1, 0.172)),
          mNormalDistortion(0.09), mSunPosition(Ogre::Vector3(5000, 3000, 1)), mSunStrength(1.75), mSunArea(150),
          mSunColor(Ogre::Vector3(1, 0.75, 0.25)), mFoamMaxDistance(75000000.0), mFoamScale(0.0075), mFoamStart(0),
          mFoamTransparency(1), mDepthLimit(0), mSmoothPower(30), mCausticsScale(20), mCausticsPower(15), mCausticsEnd(0.55),
          mGodRaysExposure(Ogre::Vector3(0.76, 2.46, 2.29)), mGodRaysIntensity(0.015), mUnderwaterCameraSwitchDelta(0.1f),
          mCurrentFrameUnderwater(false), mMesh(new Mesh(this)), mMaterialManager(new MaterialManager(this)),
          mRttManager(new RttManager(this)), mTextureManager(new TextureManager(this)), mGodRaysManager(new GodRaysManager(this)),
          mDecalsManager(new DecalsManager(this)), mGPUNormalMapManager(new GPUNormalMapManager(this)),
          mCfgFileManager(new CfgFileManager(this)), mModule(0), mComponents(HYDRAX_COMPONENTS_NONE)
    {
    ...
    void Hydrax::create()
    {
        ...
        if (isComponent(HYDRAX_COMPONENT_DEPTH)) { mRttManager->initialize(RttManager::RTT_DEPTH); }
    ...
    void Hydrax::_checkVisible()
    {
        ...
        else
        {
            ...
            if (mModule->getNormalMode() == MaterialManager::NM_RTT) { mGPUNormalMapManager->create(); }
    ...
    void Hydrax::setComponents(const HydraxComponent &Components)
    {
            ...
            isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS))
        {
            ...
            {
                HydraxComponent s = HYDRAX_COMPONENTS_NONE, f = HYDRAX_COMPONENTS_NONE, u = HYDRAX_COMPONENTS_NONE,
                                ur = HYDRAX_COMPONENTS_NONE;

                if (isComponent(HYDRAX_COMPONENT_SUN)) { s = HYDRAX_COMPONENT_SUN; }
                if (isComponent(HYDRAX_COMPONENT_FOAM)) { f = HYDRAX_COMPONENT_FOAM; }
                if (isComponent(HYDRAX_COMPONENT_UNDERWATER)) { u = HYDRAX_COMPONENT_UNDERWATER; }
                if (isComponent(HYDRAX_COMPONENT_UNDERWATER_REFLECTIONS)) { ur = HYDRAX_COMPONENT_UNDERWATER_REFLECTIONS; }
        ...
        if (isComponent(HYDRAX_COMPONENT_UNDERWATER_REFLECTIONS) || isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS))
        {
            ...
            if (!isComponent(HYDRAX_COMPONENT_UNDERWATER))
            {
                HydraxComponent s = HYDRAX_COMPONENTS_NONE, f = HYDRAX_COMPONENTS_NONE, d = HYDRAX_COMPONENTS_NONE,
                                sm = HYDRAX_COMPONENTS_NONE, c = HYDRAX_COMPONENTS_NONE;

                if (isComponent(HYDRAX_COMPONENT_SUN)) { s = HYDRAX_COMPONENT_SUN; }
                if (isComponent(HYDRAX_COMPONENT_FOAM)) { f = HYDRAX_COMPONENT_FOAM; }
                if (isComponent(HYDRAX_COMPONENT_DEPTH)) { d = HYDRAX_COMPONENT_DEPTH; }
                if (isComponent(HYDRAX_COMPONENT_SMOOTH)) { sm = HYDRAX_COMPONENT_SMOOTH; }
                ...
                {
                    HydraxLOG("Underwater reflections component needs underwater component... underwater reflections component "
                              "desactivated.");
                }
                if (isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS))
        ...
        if (isComponent(HYDRAX_COMPONENT_DEPTH))
        {
            ...
            if (isComponent(HYDRAX_COMPONENT_CAUSTICS)) { NumberOfDepthChannels++; }

            if (isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS)) { NumberOfDepthChannels++; }
        }

        if (NumberOfDepthChannels > 0)
        {
            mRttManager->setNumberOfChannels(RttManager::RTT_DEPTH,
                                             static_cast<RttManager::NumberOfChannels>(NumberOfDepthChannels));
        }

        if (!mCreated || !mModule) { return; }

        if (isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS)) { mGodRaysManager->create(mComponents); }
        else
        {
            mGodRaysManager->remove();
        }
        ...
        // Check for Rtt's
        if (mCurrentFrameUnderwater && isComponent(HYDRAX_COMPONENT_UNDERWATER) &&
        ...
        if (!isComponent(HYDRAX_COMPONENT_UNDERWATER)) { mCurrentFrameUnderwater = false; }

        mMesh->setMaterialName(mCurrentFrameUnderwater ? mMaterialManager->getMaterial(MaterialManager::MAT_UNDERWATER)->getName()
                                                       : mMaterialManager->getMaterial(MaterialManager::MAT_WATER)->getName());

        if (mCurrentFrameUnderwater) { mMaterialManager->setCompositorEnable(MaterialManager::COMP_UNDERWATER, true); }
    }
    ...
    void Hydrax::setModule(Module::Module *Module, const bool &DeleteOldModule)
    {
        if (mModule)
        {
            ...
            if (mModule->getNormalMode() == MaterialManager::NM_RTT && mModule->isCreated() &&
                mModule->getNoise()->areGPUNormalMapResourcesCreated())
            { mModule->getNoise()->removeGPUNormalMapResources(mGPUNormalMapManager); }

            if (DeleteOldModule) { delete mModule; }
            else
            {
                mModule->remove();
            }
    ...
    {
        if (mComponents & Component) { return true; }

        if (Component == HYDRAX_COMPONENTS_NONE && mComponents == HYDRAX_COMPONENTS_NONE) { return true; }

        if (Component == HYDRAX_COMPONENTS_ALL && mComponents == HYDRAX_COMPONENTS_ALL) { return true; }
        ...
        if (getHeigth(mCamera->getDerivedPosition()) > mCamera->getDerivedPosition().y - mUnderwaterCameraSwitchDelta)
        {
            ...
            if (mMesh->getMaterialName() != mMaterialManager->getMaterial(MaterialManager::MAT_UNDERWATER)->getName())
            {
                ...
                if (isComponent(HYDRAX_COMPONENT_UNDERWATER_REFLECTIONS))
                {
                    ...
                    if (isComponent(HYDRAX_COMPONENT_DEPTH)) { mRttManager->initialize(RttManager::RTT_DEPTH_REFLECTION); }
                }
                else
                {
                    mRttManager->remove(RttManager::RTT_REFLECTION);
            ...
            // Update god rays
            if (isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS)) { mGodRaysManager->update(timeSinceLastFrame); }
        }
        else
        {
            mCurrentFrameUnderwater = false;
    ...
    void Hydrax::setPosition(const Ogre::Vector3 &Position)
    {
        ...
        if (isComponent(HYDRAX_COMPONENT_DEPTH))
        {
            mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_VERTEX, MaterialManager::MAT_DEPTH, "uPlaneYPos",
                                                     Position.y);
        }

        mMesh->getSceneNode()->setPosition(Position.x - mMesh->getSize().Width / 2, Position.y,
                                           Position.z - mMesh->getSize().Height / 2);
    ...
    void Hydrax::_setStrength(const Ogre::Real &Strength)
    {
        ...
        {
            mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uFoamRange",
                                                     Strength);

            if (isComponent(HYDRAX_COMPONENT_UNDERWATER))
            {
                mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_UNDERWATER,
                                                         "uFoamRange", Strength);
            }
        }
    ...
    void Hydrax::setFullReflectionDistance(const Ogre::Real &FullReflectionDistance)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER,
    ...
    void Hydrax::setWaterColor(const Ogre::Vector3 &WaterColor)
    {
        ...
        if (!isComponent(HYDRAX_COMPONENT_DEPTH)) { return; }

        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uWaterColor",
                                                 WaterColor);
    ...
    void Hydrax::setNormalDistortion(const Ogre::Real &NormalDistortion)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uNormalDistortion",
                                                 NormalDistortion);
    ...
    {
        if (!isComponent(HYDRAX_COMPONENT_SUN)) { return; }

        mSunPosition = SunPosition;

        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uSunPosition",
                                                 mMesh->getObjectSpacePosition(SunPosition));
    ...
    void Hydrax::setSunStrength(const Ogre::Real &SunStrength)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uSunStrength",
                                                 SunStrength);
    ...
    void Hydrax::setSunArea(const Ogre::Real &SunArea)
    {
        ...
        if (isComponent(HYDRAX_COMPONENT_UNDERWATER))
        {
            mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_UNDERWATER, "uSunArea",
                                                     SunArea);
        }
    }
    ...
    void Hydrax::setSunColor(const Ogre::Vector3 &SunColor)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uSunColor",
                                                 SunColor);

        if (isComponent(HYDRAX_COMPONENT_UNDERWATER))
        {
            mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_UNDERWATER, "uSunColor",
                                                     SunColor);
        }
    }
    ...
    {
        if (!isComponent(HYDRAX_COMPONENT_FOAM)) { return; }

        mFoamMaxDistance = FoamMaxDistance;

        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uFoamMaxDistance",
                                                 FoamMaxDistance);
    ...
    void Hydrax::setFoamScale(const Ogre::Real &FoamScale)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uFoamScale",
                                                 FoamScale);
    ...
    void Hydrax::setFoamStart(const Ogre::Real &FoamStart)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uFoamStart",
                                                 FoamStart);
    ...
    void Hydrax::setFoamTransparency(const Ogre::Real &FoamTransparency)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uFoamTransparency",
                                                 FoamTransparency);
    ...
    void Hydrax::setDepthLimit(const Ogre::Real &DepthLimit)
    {
        ...
        if (mDepthLimit <= 0) { mDepthLimit = 1; }

        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_DEPTH, "uDepthLimit",
                                                 1 / mDepthLimit);
    }
    ...
    void Hydrax::setDistLimit(const Ogre::Real &DistLimit)
    {
        ...
        if (mDistLimit <= 0) { mDistLimit = 1; }

        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_DEPTH, "uDistLimit",
                                                 1 / mDistLimit);
    }

    void Hydrax::setSmoothPower(const Ogre::Real &SmoothPower)
    {
        if (!isComponent(HYDRAX_COMPONENT_SMOOTH)) { return; }

        mSmoothPower = SmoothPower;

        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uSmoothPower",
                                                 SmoothPower);
    }

    void Hydrax::setCausticsScale(const Ogre::Real &CausticsScale)
    {
        if (!isComponent(HYDRAX_COMPONENT_CAUSTICS)) { return; }

        mCausticsScale = CausticsScale;

        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_DEPTH, "uCausticsScale",
                                                 CausticsScale);
    }
    ...
    void Hydrax::setCausticsPower(const Ogre::Real &CausticsPower)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_WATER, "uCausticsPower",
                                                 CausticsPower);
        ...
        if (isComponent(HYDRAX_COMPONENT_UNDERWATER))
        {
            ...
            mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_UNDERWATER_COMPOSITOR,
                                                     "uCausticsPower", CausticsPower);
        }
    }
    ...
    void Hydrax::setCausticsEnd(const Ogre::Real &CausticsEnd)
    {
        ...
        mMaterialManager->setGpuProgramParameter(MaterialManager::GPUP_FRAGMENT, MaterialManager::MAT_DEPTH, "uCausticsEnd",
                                                 CausticsEnd);
    }
} // namespace Hydrax
