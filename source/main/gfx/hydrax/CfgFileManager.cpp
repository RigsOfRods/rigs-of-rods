...
namespace Hydrax
{
    CfgFileManager::CfgFileManager(Hydrax *h) : mHydrax(h)
    {
    }
    ...
    const bool CfgFileManager::load(const Ogre::String &File) const
    {
        ...
        if (!CfgFileResult.first) { return false; }

        Ogre::ConfigFile &CfgFile = CfgFileResult.second;

        if (!_checkVersion(CfgFile)) { return false; }
        ...
        if (mHydrax->getModule())
        {
            ...
            if (mHydrax->getModule()->getNoise()) { mHydrax->getModule()->getNoise()->loadCfg(CfgFile); }
        }
    ...
    const bool CfgFileManager::save(const Ogre::String &File, const Ogre::String &Path) const
    {
        ...
        if (mHydrax->getModule())
        {
            ...
            if (mHydrax->getModule()->getNoise()) { mHydrax->getModule()->getNoise()->saveCfg(Data); }
        }
    ...
    const void CfgFileManager::_loadCfgFile(const Ogre::String &File, std::pair<bool, Ogre::ConfigFile> &Result) const
    {
        try
        {
            Result.second.load(Ogre::ResourceGroupManager::getSingleton().openResource(
                File, Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME));
            Result.first = true;
        }
        catch (...)
        {
    ...
    {
        return "<vector2>" + Name + "=" + Ogre::StringConverter::toString(Value.x) + "x" +
               Ogre::StringConverter::toString(Value.y) + "\n";
    }

    Ogre::String CfgFileManager::_getCfgString(const Ogre::String &Name, const Ogre::Vector3 &Value)
    {
        return "<vector3>" + Name + "=" + Ogre::StringConverter::toString(Value.x) + "x" +
               Ogre::StringConverter::toString(Value.y) + "x" + Ogre::StringConverter::toString(Value.z) + "\n";
    }

    Ogre::String CfgFileManager::_getCfgString(const Ogre::String &Name, const Size &Value)
    {
        return "<size>" + Name + "=" + Ogre::StringConverter::toString(Value.Width) + "x" +
               Ogre::StringConverter::toString(Value.Height) + "\n";
    }
    ...
    const void CfgFileManager::_loadComponentsSettings(Ogre::ConfigFile &CfgFile) const
    {
        ...
        HydraxComponent ComponentsToLoad[8] = {HYDRAX_COMPONENTS_NONE, HYDRAX_COMPONENTS_NONE, HYDRAX_COMPONENTS_NONE,
                                               HYDRAX_COMPONENTS_NONE, HYDRAX_COMPONENTS_NONE, HYDRAX_COMPONENTS_NONE,
                                               HYDRAX_COMPONENTS_NONE, HYDRAX_COMPONENTS_NONE};

        if (_isStringInList(Cmpnts, "Sun")) { ComponentsToLoad[0] = HYDRAX_COMPONENT_SUN; }
        if (_isStringInList(Cmpnts, "Foam")) { ComponentsToLoad[1] = HYDRAX_COMPONENT_FOAM; }
        if (_isStringInList(Cmpnts, "Depth")) { ComponentsToLoad[2] = HYDRAX_COMPONENT_DEPTH; }
        if (_isStringInList(Cmpnts, "Smooth")) { ComponentsToLoad[3] = HYDRAX_COMPONENT_SMOOTH; }
        if (_isStringInList(Cmpnts, "Caustics")) { ComponentsToLoad[4] = HYDRAX_COMPONENT_CAUSTICS; }
        if (_isStringInList(Cmpnts, "Underwater")) { ComponentsToLoad[5] = HYDRAX_COMPONENT_UNDERWATER; }
        if (_isStringInList(Cmpnts, "UnderwaterReflections")) { ComponentsToLoad[6] = HYDRAX_COMPONENT_UNDERWATER_REFLECTIONS; }
        if (_isStringInList(Cmpnts, "UnderwaterGodRays")) { ComponentsToLoad[7] = HYDRAX_COMPONENT_UNDERWATER_GODRAYS; }

        mHydrax->setComponents(static_cast<HydraxComponent>(ComponentsToLoad[0] | ComponentsToLoad[1] | ComponentsToLoad[2] |
                                                            ComponentsToLoad[3] | ComponentsToLoad[4] | ComponentsToLoad[5] |
                                                            ComponentsToLoad[6] | ComponentsToLoad[7]));
        ...
        if (_isStringInList(Cmpnts, "Smooth")) { mHydrax->setSmoothPower(_getFloatValue(CfgFile, "SmoothPower")); }
    ...
    {
        return _getCfgString("Rtt_Quality_Reflection", mHydrax->getRttManager()->getTextureSize(RttManager::RTT_REFLECTION)) +
               ...
               _getCfgString("Rtt_Quality_GPUNormalMap",
                             mHydrax->getRttManager()->getTextureSize(RttManager::RTT_GPU_NORMAL_MAP));
    }
    ...
    const void CfgFileManager::_loadRttSettings(Ogre::ConfigFile &CfgFile) const
    {
        ...
        mHydrax->getRttManager()->setTextureSize(RttManager::RTT_GPU_NORMAL_MAP,
                                                 _getSizeValue(CfgFile, "Rtt_Quality_GPUNormalMap"));
    }
    ...
    Ogre::Vector2 CfgFileManager::_getVector2Value(Ogre::ConfigFile &CfgFile, const Ogre::String Name)
    {
        ...
        if (Value == "") { return Ogre::Vector2(0, 0); }
        else
        {
    ...
    Ogre::Vector3 CfgFileManager::_getVector3Value(Ogre::ConfigFile &CfgFile, const Ogre::String Name)
    {
        ...
        if (Value == "") { return Ogre::Vector3(0, 0, 0); }
        else
        {
