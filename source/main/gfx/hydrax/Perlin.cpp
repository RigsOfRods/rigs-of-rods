...
namespace Hydrax
{
    namespace Noise
    {
        Perlin::Perlin() : Noise("Perlin", true), time(0), r_noise(0), magnitude(n_dec_magn * 0.085f), mGPUNormalMapManager(0)
        {
        }

        Perlin::Perlin(const Options &Options)
            : Noise("Perlin", true), mOptions(Options), time(0), r_noise(0), magnitude(n_dec_magn * Options.Scale),
        ...
        bool Perlin::createGPUNormalMapResources(GPUNormalMapManager *g)
        {
            ...
            Ogre::TexturePtr mPerlinTexture0 = Ogre::TextureManager::getSingleton().createManual(
                "_Hydrax_Perlin_Noise0", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, np_size,
                np_size, 0, Ogre::PF_L16, Ogre::TU_DYNAMIC_WRITE_ONLY);

            Ogre::TexturePtr mPerlinTexture1 = Ogre::TextureManager::getSingleton().createManual(
                "_Hydrax_Perlin_Noise1", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, np_size,
                np_size, 0, Ogre::PF_L16, Ogre::TU_DYNAMIC_WRITE_ONLY);
        ...
        bool Perlin::loadCfg(Ogre::ConfigFile &CfgFile)
        {
            ...
            setOptions(Options(CfgFileManager::_getIntValue(CfgFile, "Perlin_Octaves"),
        ...
        void Perlin::_initNoise()
        {
            ...
            int i, frame, v, u, v0, v1, v2, u0, u1, u2, f;
            ...
            for (frame = 0; frame < noise_frames; frame++)
            {
                for (v = 0; v < n_size; v++)
                {
                    for (u = 0; u < n_size; u++)
                    {
                        ...
                        temp = (1.0f / 14.0f) * (tempnoise[f + v0 + u0] + tempnoise[f + v0 + u1] + tempnoise[f + v0 + u2] +
        ...
        {
            int i, o, v, u, multitable[max_octaves], amount[3], iImage;
            ...
            if (_def_PackedNoise)
            {
                ...
                for (o = 0; o < mOptions.Octaves; o += n_packsize)
                {
                    for (v = 0; v < np_size; v++)
                    {
                        for (u = 0; u < np_size; u++)
                        {
                            p_noise[v * np_size + u + octavepack * np_size_sq] =
        ...
        {
            int iu, iup, iv, ivp, fu, fv, ut01, ut23, ut;
        ...
        float Perlin::_getHeigthDual(float u, float v)
        {
            ...
            int ui = u * magnitude, vi = v * magnitude, i, value = 0, hoct = mOptions.Octaves / n_packsize;
        ...
        int Perlin::_mapSample(const int &u, const int &v, const int &upsamplepower, const int &octave)
        {
            int magnitude = 1 << upsamplepower,
                ...
                pu = u >> upsamplepower, pv = v >> upsamplepower,

                fu = u & (magnitude - 1), fv = v & (magnitude - 1),

                fu_m = magnitude - fu, fv_m = magnitude - fv,
