...
namespace Hydrax
{
    TextureManager::TextureManager(Hydrax *h) : mCreated(false), mHydrax(h)
    {
        for (int k = 0; k < 1; k++)
        {
            mTextures[k].setNull();
    ...
    bool TextureManager::_updateNormalMap(Image &Image)
    {
        ...
        if (Texture->getWidth() != ImageSize.Width || Texture->getHeight() != ImageSize.Height)
        {
            HydraxLOG("Error in TextureManager::update, Update size doesn't correspond to " + getTextureName(TEX_NORMAL_ID) +
                      " texture size");
    ...
    bool TextureManager::_createTexture(Ogre::TexturePtr &Texture, const Ogre::String &Name, const Size &Size)
    {
        try
        {
            ...
            Texture = Ogre::TextureManager::getSingleton().createManual(
                Name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, Size.Width, Size.Height, 0,
