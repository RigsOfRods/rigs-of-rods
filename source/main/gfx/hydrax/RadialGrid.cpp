...
namespace Hydrax
{
    namespace Module
    {
        ...
        {
            if (NormalMode == MaterialManager::NM_VERTEX) { return Mesh::VT_POS_NORM; }
        ...
        {
            if (NormalMode == MaterialManager::NM_VERTEX) { return "Vertex"; }

            return "Rtt";
        }

        RadialGrid::RadialGrid(Hydrax *h, Noise::Noise *n, const MaterialManager::NormalMode &NormalMode)
            : Module("RadialGrid" + _RG_getNormalModeString(NormalMode), n,
                     Mesh::Options(0, Size(200), _RG_getVertexTypeFromNormalMode(NormalMode)), NormalMode),
              mHydrax(h), mVertices(0), mVerticesChoppyBuffer(0)
        {
        }

        RadialGrid::RadialGrid(Hydrax *h, Noise::Noise *n, const MaterialManager::NormalMode &NormalMode, const Options &Options)
            : Module("RadialGrid" + _RG_getNormalModeString(NormalMode), n,
                     Mesh::Options(0, Size(Options.Radius * 2), _RG_getVertexTypeFromNormalMode(NormalMode)), NormalMode),
              mHydrax(h), mVertices(0), mVerticesChoppyBuffer(0)
        {
            setOptions(Options);
        }
        ...
        void RadialGrid::setOptions(const Options &Options)
        {
            ...
            if (isCreated())
            {
                ...
                if (getNormalMode() == MaterialManager::NM_VERTEX)
                {
                    ...
                    float r_scale = mOptions.Radius / (mOptions.StepSizeLin * mOptions.Circles +
                    ...
                    {
                        float r = r_scale * (mOptions.StepSizeLin * (y + 1) + mOptions.StepSizeCube * Ogre::Math::Pow(y + 1, 3) +
                                             mOptions.StepSizeFive * Ogre::Math::Pow(y + 1, 5));

                        for (x = 0; x < mOptions.Steps; x++)
                        {
                            Vertices[1 + y * mOptions.Steps + x].x =
                                mOptions.Radius + r * Ogre::Math::Cos(Ogre::Math::TWO_PI * x / mOptions.Steps);
                            Vertices[1 + y * mOptions.Steps + x].z =
                                mOptions.Radius + r * Ogre::Math::Sin(Ogre::Math::TWO_PI * x / mOptions.Steps);
                        }
                    }
        ...
        const bool RadialGrid::_createGeometry(Mesh *mMesh) const
        {
            ...
            case Mesh::VT_POS_NORM:
            {
                ...
                mMesh->getHardwareVertexBuffer() = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
                    sizeof(Mesh::POS_NORM_VERTEX), numVertices, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
            }
            break;
            ...
            case Mesh::VT_POS:
            {
                ...
                mMesh->getHardwareVertexBuffer() = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
                    sizeof(Mesh::POS_VERTEX), numVertices, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
            }
            break;
            }
            ...
            for (int k = 0; k < mOptions.Steps; k++)
            {
                ...
                if (k != mOptions.Steps - 1) { indexbuffer[k * 3] = k + 2; }
                else
                {
                    indexbuffer[k * 3] = 1;
                }
            ...
            mMesh->getHardwareIndexBuffer() = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(
                Ogre::HardwareIndexBuffer::IT_32BIT, numEle, Ogre::HardwareBuffer::HBU_STATIC, true);

            mMesh->getHardwareIndexBuffer()->writeData(0, mMesh->getHardwareIndexBuffer()->getSizeInBytes(), indexbuffer, true);
        ...
        void RadialGrid::remove()
        {
            ...
            if (mVerticesChoppyBuffer) { delete[] mVerticesChoppyBuffer; }
        }
        ...
        {
            if (!Module::loadCfg(CfgFile)) { return false; }

            setOptions(
                Options(CfgFileManager::_getIntValue(CfgFile, "RG_Steps"), CfgFileManager::_getIntValue(CfgFile, "RG_Circles"),
                        CfgFileManager::_getFloatValue(CfgFile, "RG_Radius"), CfgFileManager::_getBoolValue(CfgFile, "RG_Smooth"),
        ...
        void RadialGrid::update(const Ogre::Real &timeSinceLastFrame)
        {
            ...
            if (getNormalMode() == MaterialManager::NM_VERTEX)
            {
                ...
                {
                    Vertices[i].y =
                        mNoise->getValue(HydraxPos.x + Vertices[i].x, HydraxPos.z + Vertices[i].z) * mOptions.Strength;
                }
            }
            else if (getNormalMode() == MaterialManager::NM_RTT)
            {
                    ...
                    for (y = 1; y < mOptions.Circles - 1; y++)
                    {
                        for (x = 1; x < mOptions.Steps - 1; x++)
                        {
                            Vertices[y * mOptions.Steps + x].y =
                                0.2f * (Vertices[y * mOptions.Steps + x].y + Vertices[y * mOptions.Steps + x + 1].y +
                                        Vertices[y * mOptions.Steps + x - 1].y + Vertices[(y + 1) * mOptions.Steps + x].y +
        ...
        {
            if (getNormalMode() != MaterialManager::NM_VERTEX) { return; }
            ...
            vec2 = Ogre::Vector3(Vertices[1].x - Vertices[1 + Steps_4 * 2].x, Vertices[1].y - Vertices[1 + Steps_4 * 2].y,
                                 Vertices[1].z - Vertices[1 + Steps_4 * 2].z);

            vec1 = Ogre::Vector3(Vertices[1 + Steps_4].x - Vertices[1 + Steps_4 * 3].x,
            ...
            for (x = 0; x < mOptions.Steps; x++)
            {
                vec2 = Ogre::Vector3(Vertices[x + 2].x - Vertices[x].x, Vertices[x + 2].y - Vertices[x].y,
                                     Vertices[x + 2].z - Vertices[x].z);

                vec1 =
                    Ogre::Vector3(Vertices[0].x - Vertices[mOptions.Steps + x].x, Vertices[0].y - Vertices[mOptions.Steps + x].y,
            ...
            for (y = 1; y < mOptions.Circles - 1; y++)
            {
                for (x = 0; x < mOptions.Steps; x++)
                {
                    vec2 = Ogre::Vector3(Vertices[y * mOptions.Steps + x + 2].x - Vertices[y * mOptions.Steps + x].x,
                    ...
                    vec1 = Ogre::Vector3(Vertices[(y - 1) * mOptions.Steps + x + 1].x - Vertices[(y + 1) * mOptions.Steps + x].x,
        ...
        {
            if (getNormalMode() != MaterialManager::NM_VERTEX || !mOptions.ChoppyWaves) { return; }

            int x, y, Underwater = 1;

            if (mHydrax->_isCurrentFrameUnderwater()) { Underwater = -1; }

            Mesh::POS_NORM_VERTEX *Vertices = static_cast<Mesh::POS_NORM_VERTEX *>(mVertices);

            Ogre::Vector2 Current, NearStep, CircleStep, Proportion, Dir, Perp, Norm2;
            Ogre::Vector3 Norm;
            ...
            for (y = 0; y < mOptions.Circles - 1; y++)
            {
                ...
                for (x = 0; x < mOptions.Steps; x++)
                {
                    Dir = Ogre::Vector2(Vertices[1 + y * mOptions.Steps + x].nx, Vertices[1 + y * mOptions.Steps + x].nz)
                              .normalisedCopy();
                    ...
                    Norm = Ogre::Vector3(Vertices[1 + y * mOptions.Steps + x].nx, Vertices[1 + y * mOptions.Steps + x].ny,
                                         Vertices[1 + y * mOptions.Steps + x].nz)
                               .normalisedCopy();

                    Norm2 =
                        Ogre::Vector2(Norm.x, Norm.z) * ((Dir * Proportion.x) + (Perp * Proportion.y)) * mOptions.ChoppyStrength;
