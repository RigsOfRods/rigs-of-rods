...
namespace Hydrax
{
    namespace Module
    {
        ...
        ProjectedGrid::ProjectedGrid(Hydrax *h, Noise::Noise *n, const Ogre::Plane &BasePlane,
            ...
            : Module("ProjectedGrid" + _PG_getNormalModeString(NormalMode), n,
                     Mesh::Options(256, Size(0), _PG_getVertexTypeFromNormalMode(NormalMode)), NormalMode),
              mHydrax(h), mVertices(0), mVerticesChoppyBuffer(0), mBasePlane(BasePlane), mNormal(BasePlane.normal),
              mPos(Ogre::Vector3(0, 0, 0)), mProjectingCamera(0), mTmpRndrngCamera(0), mRenderingCamera(h->getCamera())
        {
        }

        ProjectedGrid::ProjectedGrid(Hydrax *h, Noise::Noise *n, const Ogre::Plane &BasePlane,
                                     const MaterialManager::NormalMode &NormalMode, const Options &Options)
            : Module("ProjectedGrid" + _PG_getNormalModeString(NormalMode), n,
                     Mesh::Options(Options.Complexity, Size(0), _PG_getVertexTypeFromNormalMode(NormalMode)), NormalMode),
              mHydrax(h), mVertices(0), mVerticesChoppyBuffer(0), mBasePlane(BasePlane), mNormal(BasePlane.normal),
              mPos(Ogre::Vector3(0, 0, 0)), mProjectingCamera(0), mTmpRndrngCamera(0), mRenderingCamera(h->getCamera())
        {
            setOptions(Options);
        ...
        bool ProjectedGrid::loadCfg(Ogre::ConfigFile &CfgFile)
        {
            ...
            setOptions(Options(
                CfgFileManager::_getIntValue(CfgFile, "PG_Complexity"), CfgFileManager::_getFloatValue(CfgFile, "PG_Strength"),
                CfgFileManager::_getFloatValue(CfgFile, "PG_Elevation"), CfgFileManager::_getBoolValue(CfgFile, "PG_Smooth"),
        ...
        void ProjectedGrid::update(const Ogre::Real &timeSinceLastFrame)
        {
            ...
            if (mLastPosition != RenderingCameraPos || mLastOrientation != mRenderingCamera->getDerivedOrientation() ||
                    ...
                    if (mOptions.ChoppyWaves)
                    {
                        for (int i = 0; i < mOptions.Complexity * mOptions.Complexity; i++)
                        {
                            ...
                            Vertices[i].y = -mBasePlane.d + mNoise->getValue(RenderingCameraPos.x + Vertices[i].x,
                                                                             RenderingCameraPos.z + Vertices[i].z) *
                                                                mOptions.Strength;
                        }
                    }
                    else
                ...
                if (mOptions.Smooth)
                {
                    if (getNormalMode() == MaterialManager::NM_VERTEX)
                    {
                        ...
                        for (int v = 1; v < (mOptions.Complexity - 1); v++)
                        {
                            ...
                            {
                                Vertices[v * mOptions.Complexity + u].y = 0.2f * (Vertices[v * mOptions.Complexity + u].y +
        ...
        bool ProjectedGrid::_renderGeometry(const Ogre::Matrix4 &m, const Ogre::Matrix4 &_viewMat, const Ogre::Vector3 &WorldPos)
        {
            ...
            float du = 1.0f / (mOptions.Complexity - 1), dv = 1.0f / (mOptions.Complexity - 1), u, v = 0.0f,
                  // _1_u = (1.0f-u)
                _1_u, _1_v = 1.0f, divide;
            ...
            if (getNormalMode() == MaterialManager::NM_VERTEX)
            {
                ...
                for (iv = 0; iv < mOptions.Complexity; iv++)
                {
                    ...
                    for (iu = 0; iu < mOptions.Complexity; iu++)
                    {
                        ...
                        Vertices[i].y =
                            -mBasePlane.d + mNoise->getValue(WorldPos.x + result.x, WorldPos.z + result.z) * mOptions.Strength;
            ...
            if (mOptions.Smooth)
            {
                if (getNormalMode() == MaterialManager::NM_VERTEX)
                {
                    ...
                    for (iv = 1; iv < (mOptions.Complexity - 1); iv++)
                    {
                        ...
                        {
                            Vertices[iv * mOptions.Complexity + iu].y = 0.2f * (Vertices[iv * mOptions.Complexity + iu].y +
        ...
        void ProjectedGrid::_calculeNormals()
        {
            ...
            for (v = 1; v < (mOptions.Complexity - 1); v++)
            {
                ...
                {
                    vec1 =
                        Ogre::Vector3(Vertices[v * mOptions.Complexity + u + 1].x - Vertices[v * mOptions.Complexity + u - 1].x,
            ...
            for (v = 1; v < (mOptions.Complexity - 1); v++)
            {
                Dis1 = (Ogre::Vector2(mVerticesChoppyBuffer[v * mOptions.Complexity + 1].x,
                        ...
                        Ogre::Vector2(mVerticesChoppyBuffer[(v + 1) * mOptions.Complexity + 1].x,
                                      mVerticesChoppyBuffer[(v + 1) * mOptions.Complexity + 1].z))
                           .length();
                ...
                for (u = 1; u < (mOptions.Complexity - 1); u++)
                {
                    Dis2 = (Ogre::Vector2(mVerticesChoppyBuffer[v * mOptions.Complexity + u].x,
                            ...
                            Ogre::Vector2(mVerticesChoppyBuffer[v * mOptions.Complexity + u + 1].x,
                                          mVerticesChoppyBuffer[v * mOptions.Complexity + u + 1].z))
                               .length();
                    /*
                    ...
                    Norm = Ogre::Vector3(Vertices[v * mOptions.Complexity + u].nx, Vertices[v * mOptions.Complexity + u].ny,
                                         Vertices[v * mOptions.Complexity + u].nz)
                               .normalisedCopy();

                    Norm2 = Ogre::Vector2(Norm.x, Norm.z) * ((Dir * Dis1) + (Perp * Dis2)) * mOptions.ChoppyStrength;

                    Vertices[v * mOptions.Complexity + u].x =
                        mVerticesChoppyBuffer[v * mOptions.Complexity + u].x + Norm2.x * Underwater;
                    Vertices[v * mOptions.Complexity + u].z =
                        mVerticesChoppyBuffer[v * mOptions.Complexity + u].z + Norm2.y * Underwater;
                }
            }
        }

        // Check the point of intersection with the plane (0,1,0,0) and return the position in homogenous coordinates
        Ogre::Vector4 ProjectedGrid::_calculeWorldPosition(const Ogre::Vector2 &uv, const Ogre::Matrix4 &m,
        ...
        bool ProjectedGrid::_getMinMax(Ogre::Matrix4 *range)
        {
            ...
            bool keep_it_simple = false, underwater = false;

            if (height_in_plane < 0.0f) { underwater = true; }

            if (keep_it_simple) { mProjectingCamera->setDirection(mTmpRndrngCamera->getDerivedDirection()); }
            else
            {
                ...
                if (height_in_plane < (mOptions.Strength + mOptions.Elevation))
                {
                    if (underwater)
                    {
                        mProjectingCamera->setPosition(mProjectingCamera->getRealPosition() +
                                                       mLowerBoundPlane.normal *
                    ...
                    ((mBasePlane.normal).dotProduct(mTmpRndrngCamera->getDerivedPosition()) < 0.0f))
                {
                    ...
                    if (!_result.first) { _result.second = -_result.second; }

                    aimpoint = mTmpRndrngCamera->getDerivedPosition() + _result.second * mTmpRndrngCamera->getDerivedDirection();
                }
                else
                {
                    Ogre::Vector3 flipped = mTmpRndrngCamera->getDerivedDirection() -
                                            2 * mNormal * (mTmpRndrngCamera->getDerivedDirection()).dotProduct(mNormal);
                    flipped.normalise();
            ...
            if (n_points > 0)
            {
                ...
                Ogre::Matrix4 pack(x_max - x_min, 0, 0, x_min, 0, y_max - y_min, 0, y_min, 0, 0, 1, 0, 0, 0, 0, 1);
