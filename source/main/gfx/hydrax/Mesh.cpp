...
{
    Mesh::Mesh(Hydrax *h)
        : mHydrax(h), mCreated(false), mMesh(), mSubMesh(0), mEntity(0), mNumFaces(0), mNumVertices(0), mVertexBuffer(),
          mIndexBuffer(), mSceneNode(0), mMaterialName("_NULL_")
    {
    }
    ...
    void Mesh::setOptions(const Options &Options)
    {
        if (mCreated)
        {
            ...
            {
                meshBounds = Ogre::AxisAlignedBox(-1000000, -Options.MeshStrength / 2, -1000000, 1000000,
                                                  Options.MeshStrength / 2, 1000000);
            }
            else
            {
                meshBounds = Ogre::AxisAlignedBox(0, -Options.MeshStrength / 2, 0, Options.MeshSize.Width,
                                                  Options.MeshStrength / 2, Options.MeshSize.Height);
            }
    ...
    void Mesh::setMaterialName(const Ogre::String &MaterialName)
    {
        ...
        if (mEntity) { mEntity->setMaterialName(mMaterialName); }
    }
    ...
    void Mesh::create()
    {
        ...
        mMesh =
            Ogre::MeshManager::getSingleton().createManual("HydraxMesh", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
        mSubMesh                    = mMesh->createSubMesh();
        mSubMesh->useSharedVertices = false;

        if (mHydrax->getModule())
        {
            if (!mHydrax->getModule()->_createGeometry(this)) { _createGeometry(); }
        }
        ...
        {
            meshBounds =
                Ogre::AxisAlignedBox(-1000000, -mOptions.MeshStrength / 2, -1000000, 1000000, mOptions.MeshStrength / 2, 1000000);
        }
        else
        {
            meshBounds = Ogre::AxisAlignedBox(0, -mOptions.MeshStrength / 2, 0, mOptions.MeshSize.Width,
                                              mOptions.MeshStrength / 2, mOptions.MeshSize.Height);
        }
    ...
    void Mesh::_createGeometry()
    {
        ...
        case VT_POS_NORM_UV:
        {
            ...
            mVertexBuffer = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
                sizeof(POS_NORM_UV_VERTEX), numVertices, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
        }
        break;
        ...
        case VT_POS_NORM:
        {
            ...
            mVertexBuffer = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
                sizeof(POS_NORM_VERTEX), numVertices, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
        }
        break;
        ...
        case VT_POS_UV:
        {
            ...
            mVertexBuffer = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
                sizeof(POS_UV_VERTEX), numVertices, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
        }
        break;
        ...
        case VT_POS:
        {
            ...
            mVertexBuffer = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
                sizeof(POS_VERTEX), numVertices, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
        }
        break;
        }
        ...
        mIndexBuffer = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(Ogre::HardwareIndexBuffer::IT_32BIT, numEle,
    ...
    {
        if (numVer != mVertexBuffer->getNumVertices() || !mCreated) { return false; }

        if (verArray) { mVertexBuffer->writeData(0, mVertexBuffer->getSizeInBytes(), verArray, true); }
    ...
    bool Mesh::isPointInGrid(const Ogre::Vector2 &Position)
    {
        ...
        // |           |
        // a-----------b
        Ogre::Vector3 a = WordMeshBox.getCorner(Ogre::AxisAlignedBox::FAR_LEFT_BOTTOM),
        ...
        Ogre::Vector2 Corners2D[4] = {Ogre::Vector2(a.x, a.z), Ogre::Vector2(b.x, b.z), Ogre::Vector2(c.x, c.z),
        ...
        if (NumberOfCollisions == 1) { return true; }
    ...
    {
        if (mOptions.MeshSize.Width == 0 && mOptions.MeshSize.Height == 0) { return Position; }

        if (!isPointInGrid(Position)) { return Ogre::Vector2(-1, -1); }
        ...
        // Transform all corners to Ogre::Vector2 array
        Ogre::Vector2 Corners2D[3] = {Ogre::Vector2(a.x, a.z), Ogre::Vector2(b.x, b.z), Ogre::Vector2(c.x, c.z)};

        // Get segments AB and AC
        Ogre::Vector2 AB = Corners2D[1] - Corners2D[0], AC = Corners2D[2] - Corners2D[0];

        // Find the X/Y position projecting the Position point to AB and AC segments.
        Ogre::Vector2 XProjectedPoint = Position - AC, YProjectedPoint = Position - AB;
        ...
        // Find lengths
        Ogre::Real ABLength = AB.length(), ACLength = AC.length(), XLength = (XPoint - Corners2D[0]).length(),
    ...
    const Ogre::Vector3 Mesh::getObjectSpacePosition(const Ogre::Vector3 &WorldSpacePosition) const
    {
        ...
        if (mCreated) { mWorldMatrix = mEntity->getParentSceneNode()->_getFullTransform(); }
        else
        {
