...
namespace Hydrax
{
    Image::Image(const Size &Size)
        : mSize(Hydrax::Size(Size.Width + 1, Size.Height + 1)), mChannels(static_cast<int>(TYPE_RGBA)), mData(0)
    {
        _Initialize(0);
    }

    Image::Image(const Size &Size, const Type &Type)
        : mSize(Hydrax::Size(Size.Width + 1, Size.Height + 1)), mChannels(static_cast<int>(Type)), mData(0)
    {
        _Initialize(0);
    }
    ...
    const float &Image::getValue(const int &x, const int &y, const int &c) const
    {
        ...
        if (c < 0 || c > mChannels || x < 0 || x > mSize.Width || y < 0 || y > mSize.Height)
        {
            HydraxLOG("Error in Image::getValue, x = " + Ogre::StringConverter::toString(x) +
                      " y = " + Ogre::StringConverter::toString(y) + " Channel = " + Ogre::StringConverter::toString(c));
    ...
    float Image::getValueLI(const float &x, const float &y, const int &c) const
    {
        ...
        int xINT = static_cast<int>(x), yINT = static_cast<int>(y);

        float xDIFF = x - xINT, yDIFF = y - yINT, _xDIFF = 1 - xDIFF, _yDIFF = 1 - yDIFF;

        //   A      B
        //
        //
        //   C      D
        float A = mData[(yINT * mSize.Width + xINT) * mChannels + c], B = mData[(yINT * mSize.Width + xINT + 1) * mChannels + c],
              C = mData[((yINT + 1) * mSize.Width + xINT) * mChannels + c],
              D = mData[((yINT + 1) * mSize.Width + xINT + 1) * mChannels + c];

        return A * _xDIFF * _yDIFF + B * xDIFF * _yDIFF + C * _xDIFF * yDIFF + D * xDIFF * yDIFF;
    }
    ...
    Image::Pixel Image::getPixel(const int &x, const int &y) const
    {
        ...
        for (int k = 0; k < 4; k++)
        {
            if (mChannels >= k) { v[k] = getValue(x, y, k); }
            else
            {
                v[k] = 0;
            }
    ...
    Image::Pixel Image::getPixelLI(const float &x, const float &y) const
    {
        ...
        for (int k = 0; k < 4; k++)
        {
            if (mChannels >= k) { v[k] = getValueLI(x, y, k); }
            else
            {
                v[k] = 0;
            }
    ...
    {
        mData = new float[(mSize.Width) * (mSize.Height) * mChannels];
