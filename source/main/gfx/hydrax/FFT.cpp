...
namespace Hydrax
{
    namespace Noise
    {
        ...
        FFT::FFT()
            : Noise("FFT", true), resolution(128), re(0), img(0), maximalValue(2), initialWaves(0), currentWaves(0),
              angularFrequencies(0), time(10), mGPUNormalMapManager(0)
        {
        }

        FFT::FFT(const Options &Options)
            : Noise("FFT", true), mOptions(Options), resolution(128), re(0), img(0), maximalValue(2), initialWaves(0),
              currentWaves(0), angularFrequencies(0), time(10), mGPUNormalMapManager(0)
        {
        }
        ...
        void FFT::remove()
        {
            ...
            if (currentWaves) { delete[] currentWaves; }
            if (re) { delete[] re; }
            if (img) { delete[] img; }
            if (initialWaves) { delete[] initialWaves; }

            if (angularFrequencies) { delete[] angularFrequencies; }
        ...
        void FFT::setOptions(const Options &Options)
        {
            if (isCreated())
            {
                if (mOptions.Resolution != Options.Resolution || mOptions.Amplitude != Options.Amplitude ||
                    mOptions.KwPower != Options.KwPower || mOptions.PhysicalResolution != Options.PhysicalResolution ||
                    mOptions.WindDirection != Options.WindDirection)
                {
                    ...
                    if (mGPUNormalMapManager) { createGPUNormalMapResources(mGPUNormalMapManager); }
        ...
        {
            if (!Noise::createGPUNormalMapResources(g)) { return false; }

            mGPUNormalMapManager = g;

            // Create our FFT texture
            Ogre::TexturePtr mFFTTexture = Ogre::TextureManager::getSingleton().createManual(
                "_Hydrax_FFT_Noise", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, resolution,
                resolution, 0, Ogre::PF_L16, Ogre::TU_DYNAMIC_WRITE_ONLY);
            ...
            mNormalMapMaterial                    = Ogre::MaterialManager::getSingleton().create(
            ...
            mMaterialManager->fillGpuProgramsToPass(Technique0_Pass0, GpuProgramNames, g->getHydrax()->getShaderMode(),
        ...
        bool FFT::loadCfg(Ogre::ConfigFile &CfgFile)
        {
            ...
            setOptions(Options(CfgFileManager::_getIntValue(CfgFile, "FFT_Resolution"),
        ...
        void FFT::update(const Ogre::Real &timeSinceLastFrame)
        {
            ...
            if (areGPUNormalMapResourcesCreated()) { _updateGPUNormalMapResources(); }
        }
        ...
        void FFT::_initNoise()
        {
            ...
            for (u = 0; u < resolution; u++)
            {
                ...
                for (v = 0; v < resolution; v++)
                {
                    ...
                    *pInitialWavesData++ =
                        std::complex<float>(_getGaussianRandomFloat() * temp, _getGaussianRandomFloat() * temp);
        ...
        void FFT::_calculeNoise(const float &delta)
        {
            ...
            for (u = 0; u < resolution; u++)
            {
                for (v = 0; v < resolution; v++)
                {
                    ...
                    const std::complex<float> &negative_h0 =
                        initialWaves[(resolution - 1 - u) * (resolution) + (resolution - 1 - v)];
                    ...
                    realVal = positive_h0.real() * coswt - positive_h0.imag() * sinwt + negative_h0.real() * coswt -
                              (-negative_h0.imag()) * (-sinwt),
                    imagVal = positive_h0.real() * sinwt + positive_h0.imag() * coswt + negative_h0.real() * (-sinwt) +
                              (-negative_h0.imag()) * coswt;
        ...
        const float FFT::_getPhillipsSpectrum(const Ogre::Vector2 &waveVector, const Ogre::Vector2 &wind,
                                              const float &kwPower_) const
        {
            // Compute the length of the vector
            float k = waveVector.length();

            // To avoid division by 0
            if (k < 0.0000001f) { return 0; }
            else
            {
                float windVelocity = wind.length(), l = pow(windVelocity, 2.0f) / 9.81f, dot = waveVector.dotProduct(wind);

                return mOptions.Amplitude *
                       (Ogre::Math::Exp(-1 / pow(k * l, 2)) / (Ogre::Math::Pow(k, 2) * Ogre::Math::Pow(k, 2))) *
                       Ogre::Math::Pow(-dot / (k * windVelocity), kwPower_);
            }
        }
        ...
        void FFT::_executeInverseFFT()
        {
            ...
            for (x = 0; x < resolution; x++)
            {
                for (y = 0; y < resolution; y++)
                {
                    if (((x + y) & 0x1) == 1) { re[x * resolution + y] *= 1; }
                    else
                    {
                        re[x * resolution + y] *= -1;
                    }
        ...
        void FFT::_normalizeFFTData(const float &scale)
        {
            ...
            if (scale == 0.0f)
            {
                float min = re[0], max = re[0], currentMax = maximalValue;
                ;
        ...
        {
            // Scale world coords
            float xScale = x * mOptions.Scale, yScale = y * mOptions.Scale;

            // Convert coords from world-space to data-space
            int xs = static_cast<int>(xScale) % resolution, ys = static_cast<int>(yScale) % resolution;
            ...
            float xDIFF = xScale - xINT, yDIFF = yScale - yINT, _xDIFF = 1 - xDIFF, _yDIFF = 1 - yDIFF;

            // To adjust the index if coords are out of range
            int xxs = (xs == resolution - 1) ? -1 : xs, yys = (ys == resolution - 1) ? -1 : ys;

            //   A      B
            //
            //
            //   C      D
            float A = re[(ys * resolution + xs)], B = re[(ys * resolution + xxs + 1)], C = re[((yys + 1) * resolution + xs)],
                  D = re[((yys + 1) * resolution + xxs + 1)];

            // Return the result of the linear interpolation
            return (A * _xDIFF * _yDIFF + B * xDIFF * _yDIFF + C * _xDIFF * yDIFF + D * xDIFF * yDIFF) // Range [-0.3, 0.3]
                       * 0.6f -
                   0.3f;
        }
    } // namespace Noise
