...
namespace SkyX
{
    AtmosphereManager::AtmosphereManager(SkyX *s) : mSkyX(s), mOptions(Options())
    {
    }
    ...
    void AtmosphereManager::_update(const Options &NewOptions, const bool &ForceToUpdateAll)
    {
        ...
        if (NewOptions.InnerRadius != mOptions.InnerRadius || NewOptions.OuterRadius != mOptions.OuterRadius || ForceToUpdateAll)
        {
            ...
            float Scale      = 1.0f / (mOptions.OuterRadius - mOptions.InnerRadius),
                  ScaleDepth = (mOptions.OuterRadius - mOptions.InnerRadius) / 2.0f, ScaleOverScaleDepth = Scale / ScaleDepth;
            ...
            mGPUManager->setGpuProgramParameter(
                GPUManager::GPUP_VERTEX, "uCameraPos",
                Ogre::Vector3(0, mOptions.InnerRadius + (mOptions.OuterRadius - mOptions.InnerRadius) * mOptions.HeightPosition,
                              0));
        ...
        if (NewOptions.HeightPosition != mOptions.HeightPosition || ForceToUpdateAll)
        {
        ...
        if (NewOptions.RayleighMultiplier != mOptions.RayleighMultiplier || NewOptions.SunIntensity != mOptions.SunIntensity ||
            ForceToUpdateAll)
        {
        ...
        if (NewOptions.MieMultiplier != mOptions.MieMultiplier || NewOptions.SunIntensity != mOptions.SunIntensity ||
            ForceToUpdateAll)
        {
            mOptions.MieMultiplier = NewOptions.MieMultiplier;

            float Km4PI = mOptions.MieMultiplier * 4.0f * Ogre::Math::PI, KmESun = mOptions.MieMultiplier * mOptions.SunIntensity;
        ...
        if (NewOptions.NumberOfSamples != mOptions.NumberOfSamples || ForceToUpdateAll)
        {
            ...
            mGPUManager->setGpuProgramParameter(GPUManager::GPUP_VERTEX, "uSamples",
                                                static_cast<Ogre::Real>(mOptions.NumberOfSamples));
        }

        if (NewOptions.WaveLength != mOptions.WaveLength || ForceToUpdateAll)
        {
        ...
        if (NewOptions.G != mOptions.G || ForceToUpdateAll)
        {
    ...
    const Ogre::Vector3 AtmosphereManager::getColorAt(const Ogre::Vector3 &Direction) const
    {
        ...
        double Scale      = 1.0f / (mOptions.OuterRadius - mOptions.InnerRadius),
               ...
               KrESun = mOptions.RayleighMultiplier * mOptions.SunIntensity,
               Km4PI = mOptions.MieMultiplier * 4.0f * Ogre::Math::PI, KmESun = mOptions.MieMultiplier * mOptions.SunIntensity;

        // --- Start vertex program simulation ---
        Ogre::Vector3 uLightDir = mSkyX->getController()->getSunDirection(), v3Pos = Direction.normalisedCopy(),
                      uCameraPos = Ogre::Vector3(
                          0, mOptions.InnerRadius + (mOptions.OuterRadius - mOptions.InnerRadius) * mOptions.HeightPosition, 0),
                      uInvWaveLength = Ogre::Vector3(1.0f / Ogre::Math::Pow(mOptions.WaveLength.x, 4.0f),
                                                     1.0f / Ogre::Math::Pow(mOptions.WaveLength.y, 4.0f),
                                                     1.0f / Ogre::Math::Pow(mOptions.WaveLength.z, 4.0f));
        ...
        double        fHeight = uCameraPos.y, fStartAngle = v3Ray.dotProduct(v3Start) / fHeight,
        ...
        // Init loop variables
        double fSampleLength = fFar / (double)mOptions.NumberOfSamples, fScaledLength = fSampleLength * Scale, fHeight_, fDepth_,
               fLightAngle, fCameraAngle, fScatter;
        Ogre::Vector3 v3SampleRay = v3Ray * fSampleLength, v3SamplePoint = v3Start + v3SampleRay * 0.5f,
        ...
        for (int i = 0; i < mOptions.NumberOfSamples; i++)
        {
            ...
            v3Attenuate = Ogre::Vector3(Ogre::Math::Exp(-fScatter * (uInvWaveLength.x * Kr4PI + Km4PI)),
        ...
        // Outputs
        Ogre::Vector3 oRayleighColor = color * (uInvWaveLength * KrESun), oMieColor = color * KmESun,
                      oDirection = uCameraPos - v3Pos;

        // --- End vertex program simulation ---
        // --- Start fragment program simulation ---

        double cos = uLightDir.dotProduct(oDirection) / oDirection.length(), cos2 = cos * cos,
               rayleighPhase = 0.75 * (1.0 + 0.5 * cos2), g2 = mOptions.G * mOptions.G,
               miePhase = 1.5f * ((1.0f - g2) / (2.0f + g2)) * (1.0f + cos2) /
        ...
        if (mSkyX->getLightingMode() == SkyX::LM_LDR)
        {
            ...
            oColor += Ogre::Math::Clamp<Ogre::Real>(((1 - std::max(oColor.x, std::max(oColor.y, oColor.z)) * 10)), 0, 1) *
                      (Ogre::Vector3(0.05, 0.05, 0.1) * (2 - 0.75f * Ogre::Math::Clamp<Ogre::Real>(-uLightDir.y, 0, 1)) *
        ...
        else
        {
            ...
            Ogre::Real nightmult =
                Ogre::Math::Clamp<Ogre::Real>(((1 - std::max(oColor.x, std::max(oColor.y, oColor.z)) * 10)), 0, 1);

            // For night rendering
            Ogre::Vector3 nightCol = Ogre::Vector3(0.05, 0.05, 0.1) *
