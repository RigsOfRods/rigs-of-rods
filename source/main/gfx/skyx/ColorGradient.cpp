...
namespace SkyX
{
    ColorGradient::ColorGradient() : mMalFormed(true)
    {
    }
    ...
    const Ogre::Vector3 ColorGradient::getColor(const Ogre::Real &p) const
    {
        ...
        if (CFrameVector.size() == 0) { return Ogre::Vector3(0, 0, 0); }
        ...
        float range = maxBound.second - minBound.second, rangepoint = (p - minBound.second) / range;
    ...
    const bool ColorGradient::_checkBounds() const
    {
        ...
        for (unsigned int k = 0; k < CFrameVector.size(); k++)
        {
            if (CFrameVector.at(k).second == 0)
            {
                ...
                if (existbounds.first) { return false; }

                existbounds.first = true;
            }

            if (CFrameVector.at(k).second < 0 || CFrameVector.at(k).second > 1) { return false; }
        }
        ...
        for (unsigned int k = 0; k < CFrameVector.size(); k++)
        {
            ...
            {
                // More than one min bound
                if (existbounds.second) { return false; }

                existbounds.second = true;
            }
        }

        if (!existbounds.first || !existbounds.second) { return false; }
