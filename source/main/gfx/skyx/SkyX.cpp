...
namespace SkyX
{
    SkyX::SkyX(Ogre::SceneManager *sm, Controller *c)
        : Ogre::FrameListener(), Ogre::RenderTargetListener(), mSceneManager(sm), mController(c), mCamera(0),
          mMeshManager(new MeshManager(this)), mAtmosphereManager(new AtmosphereManager(this)), mGPUManager(new GPUManager(this)),
          mMoonManager(new MoonManager(this)), mCloudsManager(new CloudsManager(this)),
          mRenderQueueGroups(RenderQueueGroups(Ogre::RENDER_QUEUE_SKIES_EARLY, Ogre::RENDER_QUEUE_3, Ogre::RENDER_QUEUE_4,
                                               Ogre::RENDER_QUEUE_2)),
          mCreated(false), mLastCameraPosition(Ogre::Vector3(0, 0, 0)), mLastCameraFarClipDistance(-1),
          mInfiniteCameraFarClipDistance(100000), mVisible(true), mLightingMode(LM_LDR), mStarfield(true), mTimeMultiplier(0.1f),
          mTimeOffset(0.0f)
    {
    ...
    SkyX::~SkyX()
    {
        ...
        if (mCfgFileManager) delete mCfgFileManager;

        if (mController->getDeleteBySkyX()) { delete mController; }
    }
    ...
    void SkyX::create()
    {
        ...
        if (mStarfield) { mGPUManager->setGpuProgramParameter(GPUManager::GPUP_FRAGMENT, "uTime", mTimeOffset * 0.5f, false); }
    ...
    void SkyX::setVisible(const bool &visible)
    {
        ...
        if (mVCloudsManager->isCreated()) { mVCloudsManager->getVClouds()->setVisible(mVisible); }
    }
    ...
    void SkyX::setRenderQueueGroups(const RenderQueueGroups &rqg)
    {
        ...
        mVCloudsManager->getVClouds()->setRenderQueueGroups(VClouds::VClouds::RenderQueueGroups(
            mRenderQueueGroups.vclouds, mRenderQueueGroups.vcloudsLightningsUnder, mRenderQueueGroups.vcloudsLightningsOver));
