...
namespace SkyX
{
    MeshManager::MeshManager(SkyX *s)
        : mSkyX(s), mCreated(false), mMesh(static_cast<Ogre::Mesh *>(0)), mSubMesh(0), mEntity(0), mVertexBuffer(), mVertices(0),
          mIndexBuffer(), mSceneNode(0), mSteps(70), mCircles(95), mUnderHorizonCircles(12), mUnderHorizonFading(true),
          mUnderHorizonFadingExponent(0.75), mUnderHorizonFadingMultiplier(2), mRadiusMultiplier(0.95f), mMaterialName("_NULL_")
    {
    }
    ...
    void MeshManager::create()
    {
        ...
        mMesh =
            Ogre::MeshManager::getSingleton().createManual("SkyXMesh", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
    ...
    void MeshManager::updateGeometry(Ogre::Camera *cam)
    {
        ...
        for (y = mCircles - mUnderHorizonCircles; y < mCircles; y++)
        {
            ...
            op = Ogre::Math::Clamp<Ogre::Real>(
                Ogre::Math::Pow(static_cast<Ogre::Real>(mCircles - y - 1) / mUnderHorizonCircles, mUnderHorizonFadingExponent) *
        ...
        // Update bounds
        Ogre::AxisAlignedBox meshBounds = Ogre::AxisAlignedBox(-Radius, 0, -Radius, Radius, Radius, Radius);
    ...
    void MeshManager::_createGeometry()
    {
        ...
        mVertexBuffer = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
            sizeof(VERTEX), numVertices, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
        ...
        for (int k = 0; k < mSteps; k++)
        {
            ...
            if (k != mSteps - 1) { indexbuffer[k * 3 + 2] = k + 2; }
            else
            {
                indexbuffer[k * 3 + 2] = 1;
            }
        ...
        mIndexBuffer = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(Ogre::HardwareIndexBuffer::IT_16BIT, numEle,
    ...
    void MeshManager::setUnderHorizonParams(const int &UnderHorizonCircles, const bool &UnderHorizonFading,
    ...
    void MeshManager::setMaterialName(const Ogre::String &MaterialName)
    {
        ...
        if (mCreated) { mEntity->setMaterialName(MaterialName); }
    }
    ...
    const float MeshManager::getSkydomeRadius(Ogre::Camera *c) const
    {
        ...
        if (!cameraFarClipDistance) { cameraFarClipDistance = mSkyX->getInfiniteCameraFarClipDistance(); }
