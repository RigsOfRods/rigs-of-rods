...
{
    /// -------------- CloudLayer -----------------
    CloudLayer::CloudLayer(SkyX *s) : mSkyX(s), mOptions(Options()), mCloudLayerPass(0)
    {
    ...
    CloudLayer::CloudLayer(SkyX *s, const Options &o) : mSkyX(s), mOptions(o), mCloudLayerPass(0)
    {
        mAmbientGradient = ColorGradient();
        mAmbientGradient.addCFrame(ColorGradient::ColorFrame(Ogre::Vector3(1, 1, 1) * 0.95f, 1.0f));
        mAmbientGradient.addCFrame(ColorGradient::ColorFrame(Ogre::Vector3(0.7, 0.7, 0.65), 0.625f));
    ...
    void CloudLayer::_registerCloudLayer(Ogre::Pass *CloudLayerPass)
    {
        ...
        if (mSkyX->getLightingMode() == SkyX::LM_LDR) { CloudLayerPass->setFragmentProgram("SkyX_Clouds_LDR_FP"); }
        else
        {
    ...
    void CloudLayer::_updatePassParameters()
    {
        ...
        mCloudLayerPass->getFragmentProgramParameters()->setNamedConstant("uCloudLayerVolumetricDisplacement",
    ...
    void CloudLayer::_updateInternalPassParameters()
    {
        ...
        mCloudLayerPass->getFragmentProgramParameters()->setNamedConstant("uExposure",
        ...
        -mSkyX->getAtmosphereManager()->getSunDirection()*mSkyX->getMeshManager()->getSkydomeRadius());
        ...
        for (int k = 0; k < 3; k++)
        {
            ...
            AmbientColor += mSkyX->getAtmosphereManager()->getColorAt(Ogre::Vector3(Coords.x,
        mOptions.Height/mSkyX->getMeshManager()->getSkydomeRadius(), Coords.y).normalisedCopy());
        }
    ...
    /// ------------- CloudsManager ---------------
    CloudsManager::CloudsManager(SkyX *s) : mSkyX(s)
    {
    }
    ...
    CloudLayer *CloudsManager::add(const CloudLayer::Options &o)
    {
        ...
        // TODO
        NewCloudLayer->_registerCloudLayer(static_cast<Ogre::MaterialPtr>(Ogre::MaterialManager::getSingleton().getByName(
    ...
    void CloudsManager::unregister(CloudLayer *cl)
    {
        for (CloudLayersIt = mCloudLayers.begin(); CloudLayersIt != mCloudLayers.end(); CloudLayersIt++)
        {
            if ((*CloudLayersIt) == cl) { (*CloudLayersIt)->_unregister(); }
        }
    }
