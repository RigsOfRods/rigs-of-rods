...
namespace SkyX
{
    CfgFileManager::CfgFileManager(SkyX *s, BasicController *c, Ogre::Camera *d) : mSkyX(s), mController(c), mCamera(d)
    {
    }
    ...
    const bool CfgFileManager::load(const Ogre::String &File) const
    {
        ...
        mSkyX->getAtmosphereManager()->setOptions(
            AtmosphereManager::Options(_getFloatValue(CfgFile, "_InnerRadius"), _getFloatValue(CfgFile, "_OuterRadius"),
                                       _getFloatValue(CfgFile, "_HeightPosition"), _getFloatValue(CfgFile, "_RayleighMultiplier"),
                                       _getFloatValue(CfgFile, "_MieMultiplier"), _getFloatValue(CfgFile, "_SunIntensity"),
                                       _getVector3Value(CfgFile, "_WaveLength"), _getFloatValue(CfgFile, "_G"),
                                       _getFloatValue(CfgFile, "_Exposure"), _getFloatValue(CfgFile, "_NumerOfSamples")));
        ...
        if (_getBoolValue(CfgFile, "layeredclouds") && !_getBoolValue(CfgFile, "volClouds"))
        {
            ...
            if (mSkyX->getCloudsManager()->getCloudLayers().empty())
            {
                mSkyX->getCloudsManager()->add(CloudLayer::Options(
                    _getFloatValue(CfgFile, "lHeight"), _getFloatValue(CfgFile, "Scale"),
                    _getVector2Value(CfgFile, "WindDirection"), _getFloatValue(CfgFile, "TimeMultiplier"),
                    _getFloatValue(CfgFile, "DistanceAttenuation"), _getFloatValue(CfgFile, "DetailAttenuation"),
                    _getFloatValue(CfgFile, "HeightVolume"), _getFloatValue(CfgFile, "VolumetricDisplacement")));
            }
        }
        else
        {
            ...
            vclouds->setWheater(_getVector2Value(CfgFile, "Wheater").x, _getVector2Value(CfgFile, "Wheater").y,
                                _getBoolValue(CfgFile, "DelayedResponse"));
            ...
            if (vclouds->getLightningManager()->isEnabled())
            {
                vclouds->getLightningManager()->setAverageLightningApparitionTime(
                    _getFloatValue(CfgFile, "AverageLightningApparitionTime"));
    ...
    {
        Ogre::String Data = "#SkyX cfg file.\n\n";
    ...
    {
        return "<vector4>" + Name + "=" + Ogre::StringConverter::toString(Value.x) + "x" +
               Ogre::StringConverter::toString(Value.y) + "x" + Ogre::StringConverter::toString(Value.z) + "x" +
               Ogre::StringConverter::toString(Value.w) + "\n";
    }
    ...
    Ogre::Vector4 CfgFileManager::_getVector4Value(Ogre::ConfigFile &CfgFile, const Ogre::String Name)
    {
        ...
        if (Value == "") { return Ogre::Vector4(0, 0, 0, 0); }
        else
        {
            return Ogre::Vector4(Ogre::StringConverter::parseReal(Ogre::StringUtil::split(Value, "x")[0]),
                                 Ogre::StringConverter::parseReal(Ogre::StringUtil::split(Value, "x")[1]),
                                 Ogre::StringConverter::parseReal(Ogre::StringUtil::split(Value, "x")[2]),
