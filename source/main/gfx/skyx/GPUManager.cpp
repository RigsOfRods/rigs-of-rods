...
namespace SkyX
{
    GPUManager::GPUManager(SkyX *s) : mSkyX(s), mGroundPasses(std::vector<Ogre::Pass *>())
    {
    ...
    void GPUManager::addGroundPass(Ogre::Pass *GroundPass, const Ogre::Real &AtmosphereRadius, const Ogre::SceneBlendType &SBT)
    {
        ...
        if (mSkyX->getLightingMode() == SkyX::LM_LDR) { GroundPass->setFragmentProgram("SkyX_Ground_LDR_FP"); }
        else
        {
    ...
    void GPUManager::setGpuProgramParameter(const GpuProgram &GpuP, const Ogre::String &Name, const int &Value,
                                            const bool &UpdateGroundPasses)
    {
        if (!mSkyX->getMeshManager()->isCreated()) { return; }
    ...
    void GPUManager::setGpuProgramParameter(const GpuProgram &GpuP, const Ogre::String &Name, const Ogre::Real &Value,
                                            const bool &UpdateGroundPasses)
    {
        if (!mSkyX->getMeshManager()->isCreated()) { return; }
    ...
    void GPUManager::setGpuProgramParameter(const GpuProgram &GpuP, const Ogre::String &Name, const Ogre::Vector2 &Value,
                                            const bool &UpdateGroundPasses)
    {
        if (!mSkyX->getMeshManager()->isCreated()) { return; }
    ...
    void GPUManager::setGpuProgramParameter(const GpuProgram &GpuP, const Ogre::String &Name, const Ogre::Vector3 &Value,
                                            const bool &UpdateGroundPasses)
    {
        if (!mSkyX->getMeshManager()->isCreated()) { return; }
    ...
    const Ogre::String GPUManager::getSkydomeMaterialName() const
    {
        ...
        return (mSkyX->getLightingMode() == SkyX::LM_LDR) ? "SkyX_Skydome_" + starfield + "LDR"
                                                          : "SkyX_Skydome_" + starfield + "HDR";
    }
