...
{
	const std::string PrecipitationController::COMPOSITOR_NAME = "Caelum/PrecipitationCompositor";
	const std::string PrecipitationController::MATERIAL_NAME =   "Caelum/PrecipitationMaterial";
            ...
            Ogre::SceneManager *sceneMgr)
    {
        ...
        checkCompositorSupported(COMPOSITOR_NAME);
	}
	...
	void PrecipitationController::setTextureName (const std::string& textureName) {
        mPresetType = PRECTYPE_CUSTOM;
		mTextureName = textureName;
	}
	
	const std::string PrecipitationController::getTextureName () const {
		return mTextureName;
	}

	void PrecipitationController::setSpeed (float speed) {
        mPresetType = PRECTYPE_CUSTOM;
		mSpeed = speed;
	}

	float PrecipitationController::getSpeed () const {
		return mSpeed;
	}
	...
	bool PrecipitationController::isPresetTypeValid (PrecipitationType type) {
	...
	float PrecipitationController::getIntensity () const {
		return mIntensity;
	}

	void PrecipitationController::update (float secondsSinceLastFrame, Ogre::ColourValue colour) {
	...
	void PrecipitationInstance::notifyMaterialRender(uint pass_id, Ogre::MaterialPtr &mat)
	{
        if (mAutoCameraSpeed) {
            ...
            if (cam != mLastCamera) {
                ...
                float timeDiff = mParent->getSecondsSinceLastFrame ();
            ...
            const Ogre::Vector3& camSpeed) 
    {
        ...
        float appTime = static_cast<float>(fmod(mParent->mInternalTime, static_cast<float>(4523.893416f)));

        ColourValue sceneColour = mParent->mSceneColour;
		float sceneLum = (sceneColour.r + sceneColour.g + sceneColour.b) / 3;
        ...
        Ogre::TextureUnitState* tus = mat->getTechnique(0)->getPass(0)->getTextureUnitState(1);
        if (tus && mParent->mTextureName != "" && tus->getTextureName() != mParent->mTextureName) {
            tus->setTextureName(mParent->mTextureName);
        }
    }
