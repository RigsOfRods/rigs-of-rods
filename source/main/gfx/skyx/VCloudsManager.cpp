...
namespace SkyX
{
    VCloudsManager::VCloudsManager(SkyX *s)
        : mSkyX(s), mVClouds(0), mHeight(Ogre::Vector2(-1, -1)), mWindSpeed(800.0f), mAutoupdate(true), mCreated(false),
          mCurrentTimeSinceLastFrame(0)
    {
        mVClouds = new VClouds::VClouds(mSkyX->getSceneManager());
        mVClouds->setRenderQueueGroups(VClouds::VClouds::RenderQueueGroups(mSkyX->getRenderQueueGroups().vclouds,
                                                                           mSkyX->getRenderQueueGroups().vcloudsLightningsUnder,
                                                                           mSkyX->getRenderQueueGroups().vcloudsLightningsOver));
    ...
    void VCloudsManager::_setLightParameters()
    {
        ...
        // Moon
        if (SunDir.y > 0.175f) { SunDir = -mSkyX->getController()->getMoonDirection(); }
    ...
    void VCloudsManager::_updateWindSpeedConfig()
    {
        ...
        if (mAutoupdate) { mVClouds->setWindSpeed(mSkyX->getTimeMultiplier() * mWindSpeed); }
        else
        {
