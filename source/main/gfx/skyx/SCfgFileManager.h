...
namespace SkyX
{
    ...
    /** Config file manager.
        Class to load/save all SkyX options from/to a config file
     */
    ...
    private:
        ...
        /** Load a cfg file in an Ogre::ConfigFile
            @param File File name
            @param Result, std::pair<bool, Ogre::ConfigFile&> First: False if the file isn't in the SkyX resource group, Second: Ogre::ConfigFile
         */
        const void _loadCfgFile(const Ogre::String& File, std::pair<bool,Ogre::ConfigFile> &Result) const;

        /** Get SkyX version cfg string
            @return SkyX version cfg string
         */
        const Ogre::String _getVersionCfgString() const;

        /** Check SkyX version cfg file
