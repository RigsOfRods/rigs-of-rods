...
namespace SkyX
{
    namespace VClouds
    {
        ...
        LightningManager::LightningManager(VClouds *vc)
            : mVClouds(vc), mLightnings(std::vector<Lightning *>()), mSceneNodes(std::vector<Ogre::SceneNode *>()),
              mEnabled(false), mLightningColor(Ogre::Vector3(1, 0.925f, 0.85f)), mLightningTimeMultiplier(2.0f),
              mAverageLightningApparitionTime(1.5f), mRemainingTime(1.5f), mVolCloudsLightningMaterial(Ogre::MaterialPtr()),
              mLightningMaterial(Ogre::MaterialPtr()), mListeners(std::vector<Listener *>()), mCreated(false)
        {
        }
        ...
        void LightningManager::create()
        {
            ...
            mLightningMaterial =
                static_cast<Ogre::MaterialPtr>(Ogre::MaterialManager::getSingleton().getByName("SkyX_Lightning"));
            ...
            if (mEnabled) { mVClouds->getGeometryManager()->_setMaterialName("SkyX_VolClouds_Lightning"); }
            else
            {
                    ...
                    if (!mVClouds->_getCamerasData().empty())
                    {
                        ...
                        if (prob < 0.5)
                        {
                            addLightning(
                                // Ray position
                                Ogre::Vector3(
                                    c->getDerivedPosition().x +
                                        Ogre::Math::RangeRandom(-c->getFarClipDistance() * 0.5, c->getFarClipDistance() * 0.5) /
                                ...
                                Ogre::Vector3(
                                    ...
                                    c->getDerivedPosition().z +
                                        Ogre::Math::RangeRandom(-c->getFarClipDistance() * 0.5, c->getFarClipDistance() * 0.5) /
                                            Ogre::Math::RangeRandom(1, 5)),
                                // Ray direction
                                Ogre::Vector3(Ogre::Math::RangeRandom(-1, 1), Ogre::Math::RangeRandom(-0.1, 0.1),
        ...
        Lightning *LightningManager::addLightning(const Ogre::Vector3 &p, const Ogre::Vector3 &d, const Ogre::Real l,
                                                  const Ogre::uint32 &div)
        {
            if (!mCreated || mLightnings.size() == 3) { return static_cast<Lightning *>(NULL); }

            Ogre::SceneNode *sn = mVClouds->getSceneManager()->getRootSceneNode()->createChildSceneNode();
            sn->setPosition(p);

            Lightning *lightning = new Lightning(mVClouds->getSceneManager(), sn, Ogre::Vector3(0, 0, 0), d, l, div, 3,
                                                 mLightningTimeMultiplier, mVClouds->getGeometrySettings().Radius / 9500);
            lightning->create();
            lightning->_updateRenderQueueGroup(mVClouds->getGeometryManager()->_getCurrentDistance().y <
                                                       mVClouds->getGeometryManager()->getHeight().y / 2
                                                   ? mVClouds->getRenderQueueGroups().vcloudsLightningsUnder
                                                   : mVClouds->getRenderQueueGroups().vcloudsLightningsOver);
        ...
        void LightningManager::updateMaterial()
        {
            ...
            for (Ogre::uint32 k = 0; k < 3; k++)
            {
                ...
                {
                    pos = mVClouds->getGeometryManager()->getSceneNode()->_getFullTransform().inverse() *
                ...
                {
                    mVolCloudsLightningMaterial->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->setNamedConstant(
                        "uLightning" + Ogre::StringConverter::toString(k), Ogre::Vector4(0, 0, 0, 0));
                }
            }
        }
        ...
        void LightningManager::setLightningColor(const Ogre::Vector3 &c)
        {
            ...
            mVolCloudsLightningMaterial->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->setNamedConstant(
                "uLightningColor", mLightningColor);

            mLightningMaterial->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->setNamedConstant("uColor",
                                                                                                              mLightningColor);
        }
