...
namespace SkyX
{
    namespace VClouds
    {
        DataManager::DataManager(VClouds *vc)
            : mVClouds(vc), mCellsCurrent(0), mCellsTmp(0), mFFRandom(0), mNx(0), mNy(0), mNz(0), mCurrentTransition(0),
              mUpdateTime(10.0f), mStep(0), mXStart(0), mXEnd(0), mMaxNumberOfClouds(250), mVolTexToUpdate(true), mCreated(false)
        {
            for (int k = 0; k < 2; k++)
            {
        ...
        void DataManager::update(const Ogre::Real &timeSinceLastFrame)
        {
            if (mVolTexToUpdate)
            {
                ...
                {
                    if (mXStart != mXEnd) { _performCalculations(mNx, mNy, mNz, mXEnd / mNx, mXStart % mNx, mXEnd % mNx); }
                }
        ...
        void DataManager::setWheater(const float &Humidity, const float &AverageCloudsSize, const bool &delayedResponse)
        {
            ...
            Ogre::Vector3 maxcloudsize =
                AverageCloudsSize * Ogre::Vector3(mNx / 14, mNy / 14, static_cast<int>(static_cast<float>(mNz) / 2.75));
            ...
            for (mEllipsoidsIt = mEllipsoids.begin(); mEllipsoidsIt != mEllipsoids.end(); mEllipsoidsIt++)
            {
                ...
                (*mEllipsoidsIt)
                    ->setPosition(Ogre::Vector3(
                        ...
                        static_cast<int>(Ogre::Math::RangeRandom(currentdimensions.z + 2, mNz - currentdimensions.z - 2))));
            }
            ...
            {
                newclouddimensions =
                    maxcloudsize * Ogre::Vector3(Ogre::Math::RangeRandom(0.5, 2), Ogre::Math::RangeRandom(0.5, 2),
                                                 Ogre::Math::RangeRandom(0.75, 1));
                addEllipsoid(new Ellipsoid(newclouddimensions.x, newclouddimensions.y, newclouddimensions.z, mNx, mNy, mNz,
                                           (int)Ogre::Math::RangeRandom(0, mNx), (int)Ogre::Math::RangeRandom(0, mNy),
                                           static_cast<int>(
                                               Ogre::Math::RangeRandom(newclouddimensions.z + 2, mNz - newclouddimensions.z - 2)),
                                           Ogre::Math::RangeRandom(1, 5.0f)),
                             false);
            }
        ...
        void DataManager::addEllipsoid(Ellipsoid *e, const bool &UpdateProbabilities)
        {
            ...
            if (UpdateProbabilities) { e->updateProbabilities(mCellsCurrent, mNx, mNy, mNz); }
        }
        ...
        void DataManager::_updateProbabilities(Cell ***c, const int &nx, const int &ny, const int &nz,
        ...
        const Ogre::Real DataManager::_getLightAbsorcionAt(Cell ***c, const int &nx, const int &ny, const int &nz, const int &x,
                                                           ...
                                                           const float &att) const
        {
            ...
            int           u, v, uu, vv, current_iteration = 0, max_iterations = 8;
        ...
        void DataManager::_performCalculations(const int &nx, const int &ny, const int &nz, const int &step, const int &xStart,
                                               const int &xEnd)
        {
            int u, v, w;
                    ...
                    for (v = 0; v < ny; v++)
                    {
                        for (w = 0; w < nz; w++)
                        {
                            ...
                            mCellsCurrent[u][v][w].hum =
                                mCellsCurrent[u][v][w].hum || (mFFRandom->get() < mCellsCurrent[u][v][w].phum);
                            mCellsCurrent[u][v][w].cld =
                                mCellsCurrent[u][v][w].cld && (mFFRandom->get() > mCellsCurrent[u][v][w].pext);
                            mCellsCurrent[u][v][w].act =
                                mCellsCurrent[u][v][w].act || (mFFRandom->get() < mCellsCurrent[u][v][w].pact);
                ...
                for (u = xStart; u < xEnd; u++)
                {
                    for (v = 0; v < ny; v++)
                    {
                        for (w = 0; w < nz; w++)
                        {
                            ...
                            mCellsCurrent[u][v][w].act = !mCellsCurrent[u][v][w].act && mCellsCurrent[u][v][w].hum &&
                    ...
                    for (v = 0; v < ny; v++)
                    {
                        for (w = 0; w < nz; w++)
                        {
                            mCellsCurrent[u][v][w].dens =
                                _getDensityAt(mCellsCurrent, nx, ny, nz, u, v, w, 1 /*TODOOOO!!!*/, 1.15f);
            ...
            case 3:
            {
                // Light scattering
                Ogre::Vector3 SunDir =
                    Ogre::Vector3(mVClouds->getSunDirection().x, mVClouds->getSunDirection().z, mVClouds->getSunDirection().y);
                    ...
                    for (v = 0; v < ny; v++)
                    {
                        for (w = 0; w < nz; w++)
                        {
                            mCellsCurrent[u][v][w].light =
                                _getLightAbsorcionAt(mCellsCurrent, nx, ny, nz, u, v, w, SunDir, 0.15f /*TODO!!!!*/);
                        }
                    }
                }
            }
        ...
        const bool DataManager::_fact(Cell ***c, const int &nx, const int &ny, const int &nz, const int &x, const int &y,
                                      const int &z) const
        {
            bool i1m, j1m, k1m, i1r, j1r, k1r, i2r, i2m, j2r, j2m, k2r;
        ...
        const float DataManager::_getDensityAt(Cell ***c, const int &nx, const int &ny, const int &nz, const int &x, const int &y,
                                               const int &z, const int &r, const float &strength) const
        {
            int zr = ((z - r) < 0) ? 0 : z - r, zm = ((z + r) >= nz) ? nz : z + r, u, uu, v, vv, w, clouds = 0, div = 0;
        ...
        {
            mVolTextures[static_cast<int>(TexId)] = Ogre::TextureManager::getSingleton().createManual(
                "_SkyX_VolCloudsData" + Ogre::StringConverter::toString(TexId),
                Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_3D, nx, ny, nz, 0, Ogre::PF_BYTE_RGBA);

            mVolTextures[static_cast<int>(TexId)]->load();

            static_cast<Ogre::MaterialPtr>(Ogre::MaterialManager::getSingleton().getByName("SkyX_VolClouds"))
                ->getTechnique(0)
                ->getPass(0)
                ->getTextureUnitState(static_cast<int>(TexId))
                ->setTextureName("_SkyX_VolCloudsData" + Ogre::StringConverter::toString(TexId), Ogre::TEX_TYPE_3D);
            static_cast<Ogre::MaterialPtr>(Ogre::MaterialManager::getSingleton().getByName("SkyX_VolClouds_Lightning"))
                ->getTechnique(0)
                ->getPass(0)
                ->getTextureUnitState(static_cast<int>(TexId))
                ->setTextureName("_SkyX_VolCloudsData" + Ogre::StringConverter::toString(TexId), Ogre::TEX_TYPE_3D);
