...
namespace SkyX
{
    namespace VClouds
    {
        ...
        GeometryManager::GeometryManager(VClouds *vc)
            : mVClouds(vc), mCreated(false), mHeight(Ogre::Vector2()), mRadius(0), mAlpha(0), mBeta(0), mPhi(0),
              mNumberOfBlocks(0), mNa(0), mNb(0), mNc(0), mA(0), mB(0), mC(0), mWorldOffset(Ogre::Vector2(0, 0)),
        ...
        void GeometryManager::create(const Ogre::Vector2 &Height, const float &Radius, const Ogre::Radian &Alpha,
                                     const Ogre::Radian &Beta, const int &NumberOfBlocks, const int &Na, const int &Nb,
                                     const int &Nc)
        {
            remove();
        ...
        void GeometryManager::updateGeometry(Ogre::Camera *c, const Ogre::Real &timeSinceLastCameraFrame)
        {
            ...
            mSceneNode->setPosition(mVClouds->getCamera()->getDerivedPosition().x, mHeight.x,
        ...
        void GeometryManager::_createGeometry()
        {
            ...
            for (int k = 0; k < mNumberOfBlocks; k++)
            {
                mGeometryBlocks.push_back(
                    new GeometryBlock(mVClouds, mHeight.y, mAlpha, mBeta, mRadius, mPhi, mNa, mNb, mNc, mA, mB, mC, k));
        ...
        void GeometryManager::_updateGeometry(Ogre::Camera *c, const Ogre::Real &timeSinceLastFrame)
        {
            ...
            {
                if ((*currentCameraDataIt).camera == c) { break; }
            }
            ...
            float         offset =
                -CameraDirection.dotProduct(mVClouds->getWindDirectionV2()) * mVClouds->getWindSpeed() * timeSinceLastFrame;

            // Calculate camera offset
            Ogre::Vector2 CameraOffset = Ogre::Vector2(c->getDerivedPosition().x - currentCameraData.lastPosition.x,
                                                       c->getDerivedPosition().z - currentCameraData.lastPosition.z);
            ...
            if (currentCameraData.geometryDisplacement.z < 0 || currentCameraData.geometryDisplacement.z > (mC - mB) / mNc)
            {
                currentCameraData.geometryDisplacement.z -=
                    ((mC - mB) / mNc) * Ogre::Math::IFloor((currentCameraData.geometryDisplacement.z) / ((mC - mB) / mNc));
            }
            ...
            if (currentCameraData.geometryDisplacement.y < 0 || currentCameraData.geometryDisplacement.y > (mB - mA) / mNb)
            {
                currentCameraData.geometryDisplacement.y -=
                    ((mB - mA) / mNb) * Ogre::Math::IFloor((currentCameraData.geometryDisplacement.y) / ((mB - mA) / mNb));
            }
            ...
            if (currentCameraData.geometryDisplacement.x < 0 || currentCameraData.geometryDisplacement.x > mA / mNa)
            {
                currentCameraData.geometryDisplacement.x -=
                    (mA / mNa) * Ogre::Math::IFloor((currentCameraData.geometryDisplacement.x) / (mA / mNa));
            }
