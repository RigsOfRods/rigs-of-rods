...
namespace SkyX
{
    namespace VClouds
    {
        Lightning::Lightning(Ogre::SceneManager *sm, Ogre::SceneNode *sn, const Ogre::Vector3 &orig, const Ogre::Vector3 &dir,
                             const Ogre::Real &l, const Ogre::uint32 &d, const Ogre::uint32 &rec, const Ogre::Real &tm,
                             const Ogre::Real &wm, const Ogre::Vector2 &b)
            : mOrigin(orig), mDirection(dir), mLength(l), mRealLength(0), mDivisions(d), mRecursivity(rec), mTime(0),
              mTimeMultiplier(tm), mIntensity(0), mWidthMultiplier(wm), mBounds(b),
              mAngleRange(Ogre::Vector2(Ogre::Math::RangeRandom(0.3, 0.5), Ogre::Math::RangeRandom(0.6, 0.8))),
              mTimeMultipliers(Ogre::Vector3(Ogre::Math::RangeRandom(1.75, 4.25), Ogre::Math::RangeRandom(0.4, 1.25f),
                                             Ogre::Math::RangeRandom(0.2, 1.0f))),
              mSegments(std::vector<Lightning::Segment>()), mChildren(std::vector<Lightning *>()), mBillboardSet(0),
              mSceneManager(sm), mSceneNode(sn), mCreated(false), mFinished(false)
        {
        }
        ...
        void Lightning::create()
        {
            ...
            for (Ogre::uint32 k = 1; k < mDivisions + 1; k++)
            {
                ...
                current +=
                    (mLength / (mDivisions * 3)) *
                    Ogre::Vector3(Ogre::Math::RangeRandom(-1, 1), Ogre::Math::RangeRandom(-1, 1), Ogre::Math::RangeRandom(-1, 1));
            ...
            Ogre::Real width = mWidthMultiplier * 3 * (static_cast<Ogre::Real>(mRecursivity) / 4 + 1) *
            ...
            if (mRecursivity > 0)
            {
                ...
                for (Ogre::uint32 k = 0; k < mDivisions - 1; k++)
                {
                    angle = (mSegments.at(k).b - mSegments.at(k).a)
                                .normalisedCopy()
                                .dotProduct(((mSegments.at(k + 1).b - mSegments.at(k + 1).a).normalisedCopy()));
                    ...
                    if (angle < Ogre::Math::RangeRandom(mAngleRange.x, mAngleRange.y))
                    {
                        ...
                        Lightning *lightning = new Lightning(mSceneManager, mSceneNode, mSegments.at(k).b, dir,
                                                             lengthMult * mLength, 2 + mDivisions * lengthMult, mRecursivity - 1,
                                                             mTimeMultiplier, mWidthMultiplier, bounds);
                        lightning->create();
        ...
        {
            Ogre::Vector4 params =
                Ogre::Vector4(alpha, currentPos, (3 - mRecursivity) * 0.075f + (mBounds.x * 1.5f + 0.2f) * 0.85f, 0);

            if (parentTime > 1 && parentTime < 2.2f) { params.z *= Ogre::Math::Clamp<Ogre::Real>(1.5 - parentTime, 0.2f, 1); }
