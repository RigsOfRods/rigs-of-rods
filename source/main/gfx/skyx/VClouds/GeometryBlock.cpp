...
namespace SkyX
{
    namespace VClouds
    {
        GeometryBlock::GeometryBlock(VClouds *vc, const float &Height, const Ogre::Radian &Alpha, const Ogre::Radian &Beta,
                                     const float &Radius, const Ogre::Radian &Phi, const int &Na, const int &Nb, const int &Nc,
                                     const int &A, const int &B, const int &C, const int &Position)
            : mVClouds(vc), mCreated(false), mSubMesh(0), mEntity(0), mVertices(0), mNumberOfTriangles(0), mVertexCount(0),
              mHeight(Height), mAlpha(Alpha), mBeta(Beta), mRadius(Radius), mPhi(Phi), mNa(Na), mNb(Nb), mNc(Nc), mA(A), mB(B),
              mC(C), mPosition(Position), mDisplacement(Ogre::Vector3(0, 0, 0)), mWorldOffset(Ogre::Vector2(0, 0)), mCamera(0),
              mDistance(Ogre::Vector3(0, 0, 0)), mLastFallingDistance(0)
        {
            _calculateDataSize();
        }
        ...
        void GeometryBlock::create()
        {
            ...
            mMesh =
                Ogre::MeshManager::getSingleton().createManual("_SkyX_VClouds_Block" + Ogre::StringConverter::toString(mPosition),
            ...
            // Create entity
            mEntity =
                mVClouds->getSceneManager()->createEntity("_SkyX_VClouds_BlockEnt" + Ogre::StringConverter::toString(mPosition),
        ...
        const Ogre::AxisAlignedBox GeometryBlock::_buildAABox(const float &fd) const
        {
            ...
            Ogre::Vector2 V2 =
                mRadius * Ogre::Vector2(Ogre::Math::Cos(mPhi * (mPosition + 1)), Ogre::Math::Sin(mPhi * (mPosition + 1)));

            Ogre::Vector2 Max = Ogre::Vector2(std::max<float>(std::max<float>(V1.x, V2.x), Center.x),
                                              std::max<float>(std::max<float>(V1.y, V2.y), Center.y));
            Ogre::Vector2 Min = Ogre::Vector2(std::min<float>(std::min<float>(V1.x, V2.x), Center.x),
        ...
        void GeometryBlock::_createGeometry()
        {
            ...
            mVertexBuffer = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
                sizeof(VERTEX), mVertexCount, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY);
            ...
            mIndexBuffer = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(
                Ogre::HardwareIndexBuffer::IT_16BIT, mNumberOfTriangles * 3, Ogre::HardwareBuffer::HBU_STATIC, true);
        ...
        void GeometryBlock::updateGeometry(Ogre::Camera *c, const Ogre::Vector3 &displacement, const Ogre::Vector3 &distance)
        {
            ...
            float fallingDistance = mVClouds->getDistanceFallingParams().x *
                                    (mEntity->getParentSceneNode()->_getDerivedPosition().y - c->getDerivedPosition().y);
        ...
        void GeometryBlock::_updateZoneCSlice(const int &n)
        {
            ...
            if (n == 0) { opacity = 1 - mDisplacement.z / ((mC - mB) / mNc); }
            else if (n == mNc - 1)
            {
                opacity = mDisplacement.z / ((mC - mB) / mNc);
            }

            Ogre::Vector2 x1 = Radius * mV2Cos, x2 = Radius * mBetaSin * mV2Cos, z1 = Radius * mV2Sin,
                          z2 = Radius * mBetaSin * mV2Sin;

            Ogre::Vector3 or0 = Ogre::Vector3(x1.x, 0, z1.x), or1 = Ogre::Vector3(x1.y, 0, z1.y);

            float y0 = Radius * Ogre::Math::Sin(mAlpha), d = Ogre::Vector2(x1.x - x2.x, z1.x - z2.x).length(),
                  ang = Ogre::Math::ATan(y0 / d).valueRadians(), hip = mHeight / Ogre::Math::Sin(ang);
        ...
        void GeometryBlock::_updateZoneBSlice(const int &n)
        {
            ...
            if (n == 0) { opacity = 1 - mDisplacement.y / ((mB - mA) / mNb); }
            else if (n == mNb - 1)
            {
            ...
            Ogre::Vector2 x3 = Radius * mAlphaSin * mV2Cos, z3 = Radius * mAlphaSin * mV2Sin;

            Ogre::Vector3 or0 = Ogre::Vector3(x2.x, y0, z2.x), or1 = Ogre::Vector3(x2.y, y0, z2.y);

            float y1 = Radius * Ogre::Math::Sin(mBeta), y3 = y1 - y0, d = Ogre::Vector2(x3.x - x2.x, z3.x - z2.x).length(),
                  ang = Ogre::Math::ATan(y3 / d).valueRadians(), hip = (mHeight - y0) / Ogre::Math::Sin(ang);
        ...
        void GeometryBlock::_setVertexData(const int &index, const Ogre::Vector3 &p, const float &o)
        {
            ...
            Ogre::Vector3 origin =
                Ogre::Vector3(0,
                              -(mEntity->getParentSceneNode()->_getDerivedPosition().y - mCamera->getDerivedPosition().y) -
                                  mRadius * (0.5f + 0.5f * Ogre::Vector2(p.x, p.z).length() / mRadius),
                              0);
