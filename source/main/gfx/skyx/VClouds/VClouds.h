        ...
        class VClouds
        {
          public:
            ...
            struct GeometrySettings
            {
                 ...
                 */
                GeometrySettings()
                    : Height(Ogre::Vector2(10, 50)), Alpha(Ogre::Degree(12)), Beta(Ogre::Degree(40)), Radius(100),
                      NumberOfBlocks(12), Na(10), Nb(8), Nc(6)
                {
                }
                ...
                GeometrySettings(const Ogre::Vector2 &_Height, const float &_Radius,
                    ...
                    : Height(_Height), Alpha(_Alpha), Beta(_Beta), Radius(_Radius), NumberOfBlocks(_NumberOfBlocks), Na(_Na),
                      Nb(_Nb), Nc(_Nc)
                {
                }
            };
            ...
            struct CameraData
            {
              public:
                 ...
                 */
                inline CameraData()
                    : camera(0), lastPosition(Ogre::Vector3(0, 0, 0)), cameraOffset(Ogre::Vector2(0, 0)),
                ...
                inline CameraData(Ogre::Camera *c)
                    : camera(c), lastPosition(c->getDerivedPosition()), cameraOffset(Ogre::Vector2(0, 0)),
                      geometryDisplacement(Ogre::Vector3(0, 0, 0))
                {
                }
            ...
            /** Set distance falling params
                @param DistanceFallingParams
                       DistanceFallingParams.x = Distance falling factor (How much the cloud field geometry falls with the
               distance) Remember that the geometry falling is relative to the distance(height) between the camera and the cloud
               field. Typical range is [0, ~2] 0 = no falling DistanceFallingParams.y = Max falling (in world coords), useful when
               , i.e., you've water and you want to go in. That param will allow you to avoid the cloud field geometry falls into
            ...
            */
            ...
            /** Get distance falling params
                @return DistanceFallingParams.x = Distance falling factor (How much the cloud field geometry falls with the
               distance) Remember that the geometry falling is relative to the distance(height) between the camera and the cloud
               field. Typical range is [0, ~2] 0 = no falling DistanceFallingParams.y = Max falling (in world coords), useful when
               , i.e., you've water and you want to go in. That param will allow you to avoid the cloud field geometry falls into
               the ocean. -1 means not max falling. (default)
            ...
            /** Set wheater parameters
                ...
                @param AverageCloudsSize Average clouds size, for example: if previous wheater clouds size parameter was very
               different from new one(i.e: more little) only the old biggest clouds are going to be keept and the little ones are
