...
namespace SkyX
{
    namespace VClouds
    {
        ...
        Ellipsoid::Ellipsoid(const int &a, const int &b, const int &c, const int &nx, const int &ny, const int &nz, const int &x,
                             const int &y, const int &z, const Ogre::Real &Density)
            : mA(a), mB(b), mC(c), mA2(Ogre::Math::Pow(a, 2)), mB2(Ogre::Math::Pow(b, 2)), mC2(Ogre::Math::Pow(c, 2)), mNx(nx),
              mNy(ny), mNz(nz), mX(x), mY(y), mZ(z), mDensity(Density)
        {
        }
        ...
        {
            if (x == mX && y == mY && z == mZ) { return 0.0f; }

            //  x^2   y^2   z^2
            //  /   + /   + /    = 1  (Ellipsoid ecuation)
            //  a^2   b^2   c^2
            //
            //  maxradatdir = lambda (Xo, Yo, Zo) = lambda; where Xo, Yo and Zo are the components of the normaliced direction
            //  vector
            //
            ...
            Ogre::Vector3 Direction = Ogre::Vector3(x - mX, y - mY, z - mZ), DirectionNormalized = Direction.normalisedCopy();

            Ogre::Real a = Ogre::Math::Pow(DirectionNormalized.x, 2) / mA2 + Ogre::Math::Pow(DirectionNormalized.y, 2) / mB2 +
                ...
                for (v = mY - mB; v < mY + mB; v++)
                {
                    ...
                    for (w = mZ - mC; w < mZ + mC; w++)
                    {
                        ...
                        if (length < 1)
                        {
                            ...
                            if (!delayedResponse) { c[uu][vv][w].cld = Ogre::Math::RangeRandom(0, 1) > length ? true : false; }
                        }
                    }
                }
            }
        ...
        {
            if ((mX + mA) >= mNx || (mX - mA) < 0 || (mY + mB) >= mNy || (mY - mB) < 0 || (mZ + mZ) >= mNz || (mZ - mZ) < 0)
            { return true; }
