...
Water::Water()
    : m_max_ampl(0), m_water_visible(true), m_waterplane_mesh_scale(1.0f), m_refract_rtt_viewport(0), m_reflect_rtt_viewport(0),
      m_render_cam(gEnv->mainCamera), m_bottom_height(0), m_water_height(0), m_waterplane_node(0),
      m_waterplane_force_update_pos(false), m_frame_counter(0), m_refract_rtt_target(0), m_reflect_rtt_target(0),
      m_reflect_cam(0), m_refract_cam(0)
{
    ...
    if (m_map_size.x < 1500 && m_map_size.z < 1500) m_waterplane_mesh_scale = 1.5f;
...
void Water::PrepareWater()
{
    ...
    if (full_gfx || type == GfxWaterMode::REFLECT)
    {
        ...
        m_waterplane_mesh = MeshManager::getSingleton().createPlane(
            "ReflectPlane", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, m_water_plane,
            m_map_size.x * m_waterplane_mesh_scale, m_map_size.z * m_waterplane_mesh_scale, WAVEREZ, WAVEREZ, true, 1, 50, 50,
            Vector3::UNIT_Z, HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE);
    ...
    {
        m_waterplane_mesh = MeshManager::getSingleton().createPlane(
            "WaterPlane", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, m_water_plane,
            m_map_size.x * m_waterplane_mesh_scale, m_map_size.z * m_waterplane_mesh_scale, WAVEREZ, WAVEREZ, true, 1, 50, 50,
            Vector3::UNIT_Z, HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE);
    ...
    m_waterplane_node->setPosition(
        Vector3((m_map_size.x * m_waterplane_mesh_scale) / 2, m_water_height, (m_map_size.z * m_waterplane_mesh_scale) / 2));

    // bottom
    m_bottom_plane.normal = Vector3::UNIT_Y;
    m_bottom_plane.d      = -m_bottom_height; // 30m below waterline
    MeshManager::getSingleton().createPlane("BottomPlane", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, m_bottom_plane,
                                            m_map_size.x * m_waterplane_mesh_scale, m_map_size.z * m_waterplane_mesh_scale, 1, 1,
    ...
    m_bottomplane_node->setPosition(
        Vector3((m_map_size.x * m_waterplane_mesh_scale) / 2, 0, (m_map_size.z * m_waterplane_mesh_scale) / 2));
...
void Water::SetWaterVisible(bool value)
{
    ...
    if (m_waterplane_entity) m_waterplane_entity->setVisible(value);
    if (m_waterplane_node) m_waterplane_node->setVisible(value);
    if (m_bottomplane_node) m_bottomplane_node->setVisible(value);
}
...
{
    if (!m_waterplane_vert_buf_local) return;
    ...
    for (int pz = 0; pz < WAVEREZ + 1; pz++)
    {
        ...
        {
            m_waterplane_vert_buf_local[(pz * (WAVEREZ + 1) + px) * 8 + 1] =
                CalcWavesHeight(refpos + Vector3(xScaled * 0.5 - (float)px * xScaled / WAVEREZ, 0,
                                                 (float)pz * zScaled / WAVEREZ - zScaled * 0.5)) -
                m_water_height;
        }
    }
    ...
    for (int pz = 0; pz < WAVEREZ + 1; pz++)
    {
        for (int px = 0; px < WAVEREZ + 1; px++)
        {
            ...
            Vector3 normal = (Vector3(m_waterplane_vert_buf_local + ((pz * (WAVEREZ + 1) + left) * 8)) -
                              Vector3(m_waterplane_vert_buf_local + ((pz * (WAVEREZ + 1) + right) * 8)))
                                 .crossProduct(Vector3(m_waterplane_vert_buf_local + ((up * (WAVEREZ + 1) + px) * 8)) -
                                               Vector3(m_waterplane_vert_buf_local + ((down * (WAVEREZ + 1) + px) * 8)));
            normal.normalise();
...
{
    if (m_render_cam) { return (m_render_cam->getPosition().y < CalcWavesHeight(m_render_cam->getPosition())); }
    return false;
}
...
void Water::UpdateWater()
{
    ...
    if (m_waterplane_node)
    {
        ...
        if (intersection.first && intersection.second > 0.0f) sightPos = lineOfSight.getPoint(intersection.second);
...
{
    if (m_refract_rtt_target) m_refract_rtt_target->removeListener(&m_refract_listener);
    if (m_reflect_rtt_target) m_reflect_rtt_target->removeListener(&m_reflect_listener);
}
...
float Water::CalcWavesHeight(Vector3 pos)
{
    ...
    // uh, some upper limit?!
    if (pos.y > m_water_height + m_max_ampl) return m_water_height;
    ...
    // now walk through all the wave trains. One 'train' is one sin/cos set that will generate once wave. All the trains together
    // will sum up, so that they generate a 'rough' sea
    for (size_t i = 0; i < m_wavetrain_defs.size(); i++)
    {
        ...
        result += amp * sin(Math::TWO_PI * ((gEnv->mrTime * m_wavetrain_defs[i].wavespeed + m_wavetrain_defs[i].dir_sin * pos.x +
                                             m_wavetrain_defs[i].dir_cos * pos.z) /
                                            m_wavetrain_defs[i].wavelength));
    }
...
bool Water::IsUnderWater(Vector3 pos)
{
    ...
    if (RoR::App::gfx_water_waves.GetActive() && RoR::App::mp_state.GetActive() == RoR::MpState::DISABLED)
    {
        ...
        if (pos.y > m_water_height + m_max_ampl * waveheight || pos.y > m_water_height + m_max_ampl) return false;
...
{
    if (!RoR::App::gfx_water_waves.GetActive() || RoR::App::mp_state.GetActive() == RoR::MpState::CONNECTED) return Vector3::ZERO;

    float waveheight = GetWaveHeight(pos);

    if (pos.y > m_water_height + m_max_ampl) return Vector3::ZERO;
    ...
    for (size_t i = 0; i < m_wavetrain_defs.size(); i++)
    {
        ...
        float coeff = Math::TWO_PI *
                      (gEnv->mrTime * m_wavetrain_defs[i].wavespeed + m_wavetrain_defs[i].dir_sin * pos.x +
                       m_wavetrain_defs[i].dir_cos * pos.z) /
                      m_wavetrain_defs[i].wavelength;
...
void Water::UpdateReflectionPlane(float h)
{
    ...
    if (m_refract_cam) { m_refract_cam->enableCustomNearClipPlane(m_refract_plane); }
}
...
float Water::GetWaveHeight(Vector3 pos)
{
    ...
    float waveheight = (pos - Vector3((m_map_size.x * m_waterplane_mesh_scale) * 0.5, m_water_height,
                                      (m_map_size.z * m_waterplane_mesh_scale) * 0.5))
                           .squaredLength() /
                       3000000.0;
