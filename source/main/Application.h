...
namespace RoR
{
    ...
    template <size_t L> class Str
    {
      public:
        ...
        {
            char buf[50];
            std::snprintf(buf, 50, "%lu", static_cast<unsigned long>(z));
            this->Append(buf);
            return *this;
        }
    ...
    {
      public:
        GVarStr_A(const char *name, const char *conf, const char *active_val) : GVarBase(name, conf), m_value_active(active_val)
        {
        }
    ...
    template <size_t L> class GVarStr_AP : public GVarStr_A<L>
    {
      public:
        ...
        void SetPending(const char *val)
        {
            if (val != m_value_pending)
            {
                if (GVarStr_A<L>::name != "mp_player_token") GVarBase::LogSetPending(val, m_value_pending);
                m_value_pending = val;
            }
        }
        ...
        virtual void SetActive(const char *val) override
        {
            ...
            {
                if (GVarStr_A<L>::name != "mp_player_token") GVarBase::LogSetActive(val, GVarStr_A<L>::m_value_active);
