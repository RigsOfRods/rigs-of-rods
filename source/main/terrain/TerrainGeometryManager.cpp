...
void TerrainGeometryManager::loadOgreTerrainConfig(String filename)
{
	...
	disableCaching = m_terrain_config.GetBoolOrDefault("disableCaching", false);
...
{
	// X, Y and Z scale
	mapsizex    = m_terrain_config.GetIntOrDefault("WorldSizeX", 1024);
	mapsizey    = m_terrain_config.GetIntOrDefault("WorldSizeY", 50);
	mapsizez    = m_terrain_config.GetIntOrDefault("WorldSizeZ", 1024);
	terrainSize = m_terrain_config.GetIntOrDefault("PageSize", 1025);

	worldSize = std::max(mapsizex, mapsizez);

	String filenameSuffix = "mapbin";
	pageMinX = 0;
	pageMaxX = m_terrain_config.GetIntOrDefault("PagesX", 0);
	pageMinZ = 0;
	pageMaxZ = m_terrain_config.GetIntOrDefault("PagesZ", 0);

	pageConfigFormat = m_terrain_config.GetStringOrDefault("PageFileFormat", baseName + "-page-{X}-{Z}.otc");

	bool is_flat = m_terrain_config.GetBoolOrDefault("Flat", false);
...
void TerrainGeometryManager::configureTerrainDefaults()
{
	...
	TerrainGlobalOptions *terrainOptions = TerrainGlobalOptions::getSingletonPtr();
	// Configure global
	terrainOptions->setMaxPixelError(m_terrain_config.GetIntOrDefault("MaxPixelError", 5));
	...
	defaultimp.minBatchSize = m_terrain_config.GetIntOrDefault("minBatchSize", 33);
	defaultimp.maxBatchSize = m_terrain_config.GetIntOrDefault("maxBatchSize", 65);
	...
	if (matProfile)
	{
		matProfile->setLightmapEnabled(m_terrain_config.GetBoolOrDefault("LightmapEnabled", false));
		matProfile->setLayerNormalMappingEnabled(m_terrain_config.GetBoolOrDefault("NormalMappingEnabled", false));
		matProfile->setLayerSpecularMappingEnabled(m_terrain_config.GetBoolOrDefault("SpecularMappingEnabled", false));
		matProfile->setLayerParallaxMappingEnabled(m_terrain_config.GetBoolOrDefault("ParallaxMappingEnabled", false));
		matProfile->setGlobalColourMapEnabled(m_terrain_config.GetBoolOrDefault("GlobalColourMapEnabled", false));
		matProfile->setReceiveDynamicShadowsDepth(m_terrain_config.GetBoolOrDefault("ReceiveDynamicShadowsDepth", false));
	}

	terrainOptions->setLayerBlendMapSize(m_terrain_config.GetIntOrDefault("LayerBlendMapSize", 1024));
	terrainOptions->setCompositeMapSize(m_terrain_config.GetIntOrDefault("CompositeMapSize", 1024));
	terrainOptions->setCompositeMapDistance(m_terrain_config.GetIntOrDefault("CompositeMapDistance", 4000));
	terrainOptions->setSkirtSize(m_terrain_config.GetIntOrDefault("SkirtSize", 30));
	terrainOptions->setLightMapSize(m_terrain_config.GetIntOrDefault("LightMapSize", 1024));
	terrainOptions->setCastsDynamicShadows(m_terrain_config.GetBoolOrDefault("CastsDynamicShadows", false));
...
{
	bool debugBlendMaps = m_terrain_config.GetBoolOrDefault("DebugBlendMaps", false);
...
bool TerrainGeometryManager::getTerrainImage(int x, int z, Image& img)
{
	...
	{
		int rawSize = m_terrain_config.GetIntOrDefault(heightmapString + ".raw.size", 1025);
		int bpp     = m_terrain_config.GetIntOrDefault(heightmapString + ".raw.bpp", 2);
	...
	if (m_terrain_config.GetBoolOrDefault(heightmapString + ".flipX", false))
		img.flipAroundX();
	if (m_terrain_config.GetBoolOrDefault(heightmapString + ".flipY", false))
		img.flipAroundY();
