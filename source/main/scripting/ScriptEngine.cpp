...
String ScriptEngine::composeModuleName(String const& scriptName, ScriptOrigin origin)
{
    switch (origin)
    {
    case ScriptOrigin::TERRAIN: return fmt::format("TERRAIN:{}", scriptName);

    case ScriptOrigin::ADDON: return fmt::format("ADDON:{}", scriptName);

    default: return "";
    }
}

int ScriptEngine::loadScript(String scriptName, ScriptOrigin origin/* = ScriptOrigin::TERRAIN*/)
{
    String moduleName = this->composeModuleName(scriptName, origin);
    if (moduleName == "")
        return -1;
    ...
    String scriptHash;
    if (origin == ScriptOrigin::TERRAIN) // Classic behavior
    ...
    else
    {
        ...
        if (origin == ScriptOrigin::TERRAIN)
        {
...
void ScriptEngine::unloadScript(String scriptName, ScriptOrigin origin)
{
    ROR_ASSERT(m_currently_executing_script_unit == -1);

    String module_name = this->composeModuleName(scriptName, origin);
    if (module_name == "")
        return;
