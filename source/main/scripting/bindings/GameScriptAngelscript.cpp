...
void RoR::RegisterGameScript(asIScriptEngine *engine)
{
    ...
    result = engine->RegisterObjectMethod("GameScriptClass", "void MoveObjectVisuals(const string &in, vector3 &in)", asMETHOD(GameScript,MoveTerrainObjectVisuals), asCALL_THISCALL); ROR_ASSERT(result>=0);
    ...
    result = engine->RegisterObjectMethod("GameScriptClass", "int getCurrentTruckNumber()", asMETHOD(GameScript,GetPlayerActorId), asCALL_THISCALL); ROR_ASSERT(result>=0);
