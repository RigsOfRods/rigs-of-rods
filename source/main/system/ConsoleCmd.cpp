...
class LoadRaceCmd : public ConsoleCmd
{
public:
    LoadRaceCmd() : ConsoleCmd("loadrace", "[filename]", _L("Loads a *.race file")) {}
    ...
    void Run(Ogre::StringVector const& args) override
    {
        ...
        {
            if (!App::GetGameContext()->LoadRace(args[1]))
            {
                reply_type = Console::CONSOLE_SYSTEM_ERROR;
...
void Console::regBuiltinCommands()
{
    ...
    cmd = new LoadRaceCmd();           m_commands.insert(std::make_pair(cmd->getName(), cmd));
