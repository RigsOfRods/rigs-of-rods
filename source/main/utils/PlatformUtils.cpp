...
namespace RoR
{
    ...
    std::wstring MSW_Utf8ToWchar(const char *path)
    {
        ...
        int size_needed =
            MultiByteToWideChar(CP_UTF8, 0, &path[0], -1, NULL,
                                0); // Doc: https://msdn.microsoft.com/en-us/library/windows/desktop/dd319072(v=vs.85).aspx
        std::wstring out_wstr(size_needed, 0);
        int          raw_result = MultiByteToWideChar(CP_UTF8, 0, &path[0], -1, &out_wstr[0], size_needed);
        if (raw_result <= 0)
        {
            RoR::LogFormat(
                "[RoR] Internal error: MSW_Utf8ToWchar() could not convert UTF-8 to UTF-16; MultiByteToWideChar() returned %d",
                raw_result);
            return std::wstring();
        }
        return out_wstr;
    ...
    {
        if (path == nullptr || path[0] == 0) { return INVALID_FILE_ATTRIBUTES; }
    ...
    std::string MSW_WcharToUtf8(const wchar_t *wstr) // wstr _must_ be NUL-terminated!
    {
        ...
        {
            RoR::LogFormat(
                "[RoR] Internal error: MSW_WcharToUtf8() could not convert UTF-16 to UTF-8; WideCharToMultiByte() returned %d",
                raw_result);
            return std::string();
        }
        return dst;
    ...
    {
        std::wstring out_wstr(MAX_PATH, 0); // Length limit imposed by the function, see
        ...
        if (hres != S_OK)
        {
            RoR::LogFormat("[RoR] Internal error: GetUserHomeDirectory() failed; SHGetFolderPathW() returned %ld",
    ...
    std::string GetExecutablePath()
    {
        ...
        if (res <= 0)
        {
            RoR::LogFormat("[RoR] Internal error: GetExecutablePath() failed; GetModuleFileNameW() returned %d",
                           static_cast<int>(res));
            return std::string();
        }
    ...
    std::string GetExecutablePath()
    {
        ...
        {
            RoR::LogFormat("[RoR] Internal error: GetExecutablePath() failed; readlink() sets errno to %d",
