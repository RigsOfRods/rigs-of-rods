...
void ForceFeedback::SetForces(float roll, float pitch, float wspeed, float dircommand, float stress)
{
    ...
    if (hydroConstForce != nullptr)
    {
        float stress_gain = App::io_ffb_stress_gain->GetActiveVal<float>();
        float centering_gain = App::io_ffb_center_gain->GetActiveVal<float>();
...
void ForceFeedback::SetEnabled(bool b)
{
    ...
    if (b != m_enabled)
    {
        float gain = (b) ? App::io_ffb_master_gain->GetActiveVal<float>() : 0.f;
