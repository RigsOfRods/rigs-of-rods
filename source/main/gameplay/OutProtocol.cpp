...
OutProtocol::OutProtocol(void) : delay(0.1f), id(0), mode(0), sockfd(-1), timer(0), working(false)
{
...
bool OutProtocol::Update(float dt, Actor *truck)
{
    ...
    else if (truck && truck->ar_engine)
    {
        ...
        if (truck->ar_engine->HasTurbo()) { gd.Flags |= OG_TURBO; }
        ...
        if (!truck->tc_nodash) gd.DashLights |= DL_TC;
        if (!truck->alb_nodash) gd.DashLights |= DL_ABS;

        gd.ShowLights = 0;
        if (truck->ar_parking_brake) gd.ShowLights |= DL_HANDBRAKE;
        if (truck->ar_lights) gd.ShowLights |= DL_FULLBEAM;
        if (truck->ar_engine->HasStarterContact() && !truck->ar_engine->IsRunning()) gd.ShowLights |= DL_BATTERY;
        if (truck->ar_left_blink_on) gd.ShowLights |= DL_SIGNAL_L;
        ...
        if (truck->ar_design_name.length() > 15) { strncpy(gd.Display2, truck->ar_design_name.c_str() + 15, 15); }
    }
    // send the package
