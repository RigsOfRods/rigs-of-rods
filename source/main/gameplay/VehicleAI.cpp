...
VehicleAI::VehicleAI(Actor *b) : is_waiting(false), wait_time(0.0f)
{
    beam = b;
}
...
{
    if (current_waypoint == Vector3::ZERO) current_waypoint = point;
...
void VehicleAI::AddEvent(String &id, int &ev)
{
    ...
    if (waypointid) waypoint_events.emplace(waypointid, ev);
}
...
void VehicleAI::SetValueAtWaypoint(String &id, int &value_id, float &value)
{
    ...
    if (waypointid)
    {
        switch (value_id)
        {
        case AI_SPEED: waypoint_speed.emplace(waypointid, value); break;
        case AI_POWER: waypoint_power.emplace(waypointid, value); break;
        default: break;
        }
    }
}
...
void VehicleAI::updateWaypoint()
{
    RoR::App::GetConsole()->putMessage(RoR::Console::CONSOLE_MSGTYPE_SCRIPT, RoR::Console::CONSOLE_SYSTEM_NOTICE,
                                       "Reached waypoint: " + waypoint_names[current_waypoint_id], "note.png");
    ...
    if (event)
    {
        switch (event)
        {
        case AI_LIGHTSTOGGLE: beam->ToggleLights(); break;
        case AI_BEACONSTOGGLE: beam->ToggleBeacons(); break;
        default: break;
        }
    }
...
void VehicleAI::update(float dt, int doUpdate)
{
    ...
    {
        wait_time -= dt;
        if (wait_time < 0) { is_waiting = false; }
        return;
    }
    ...
    if (beam->ar_engine)
    {
        ...
        if (!beam->ar_engine->IsRunning()) beam->ar_engine->StartEngine();
