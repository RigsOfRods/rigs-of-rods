...
namespace RoR
{
    namespace ChatSystem
    {
        ...
        Ogre::UTFString GetColouredName(Ogre::UTFString nick, int colour_number)
        {
            ...
            char              tmp[255] = {0};
            sprintf(tmp, "#%02X%02X%02X", (unsigned int)(col_val.r * 255.0f), (unsigned int)(col_val.g * 255.0f),
                    (unsigned int)(col_val.b * 255.0f));
        ...
        {
            if (type != MSG2_UTF8_CHAT && type != MSG2_UTF8_PRIVCHAT) return;
            ...
            if (type == MSG2_UTF8_CHAT)
            {
                ...
                else if (source == RoR::Networking::GetUID())
                {
                    ...
                    Ogre::UTFString local_username =
                        GetColouredName(RoR::Networking::GetUsername(), RoR::Networking::GetUserColor());
            ...
            else if (type == MSG2_UTF8_PRIVCHAT)
            {
                ...
                else
                {
                    ...
                    if (RoR::Networking::GetUserInfo(source, user))
                    {
                        msg = GetColouredName(user.username, user.colournum) + _L(" [whispered] ") + RoR::Color::NormalColour +
                              ": " + msg;
                    }
                }
            }
        ...
        void SendPrivateChat(int target_uid, Ogre::UTFString chatline, Ogre::UTFString target_username)
        {
            ...
            Ogre::UTFString nmsg = local_username + RoR::Color::WhisperColour + _L(" [whispered to ") + RoR::Color::NormalColour +
                                   target_username + RoR::Color::WhisperColour + "]" + RoR::Color::NormalColour + ": " + chatline;
        ...
        void SendPrivateChat(Ogre::UTFString target_username, Ogre::UTFString chatline)
        {
            ...
            if (!found_target)
            {
                RoR::App::GetGuiManager()->pushMessageChatBox(RoR::Color::CommandColour + _L("user not found: ") +
                                                              target_username);
                return;
            }
