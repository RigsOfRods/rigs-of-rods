...
EngineSim::EngineSim(float _min_rpm, float _max_rpm, float torque, std::vector<float> gears, float dratio, Actor *actor)
    : m_air_pressure(0.0f), m_auto_cur_acc(0.0f), m_auto_mode(AUTOMATIC), m_autoselect(DRIVE), m_braking_torque(-torque / 5.0f),
      m_clutch_force(10000.0f), m_clutch_time(0.2f), m_starter_has_contact(false), m_cur_acc(0.0f), m_cur_clutch(0.0f),
      m_cur_clutch_torque(0.0f), m_cur_engine_rpm(0.0f), m_cur_engine_torque(0.0f), m_cur_gear(0), m_cur_gear_range(0),
      m_cur_wheel_revolutions(0.0f), m_ref_wheel_revolutions(0.0f), m_diff_ratio(dratio), m_tcase_ratio(1.0f),
      m_engine_torque(torque), m_gear_ratios(gears), m_engine_has_air(true), m_engine_has_turbo(true), m_hydropump_state(0.0f),
      m_engine_idle_rpm(std::min(std::abs(_min_rpm), 800.0f)), m_engine_inertia(10.0f), m_kickdown_delay_counter(0),
      m_max_idle_mixture(0.1f), m_engine_max_rpm(std::abs(_max_rpm)), m_min_idle_mixture(0.0f),
      m_engine_min_rpm(std::abs(_min_rpm)), m_num_gears((int)gears.size() - 2), m_post_shift_time(0.2f), m_post_shift_clock(0.0f),
      m_post_shifting(0), m_engine_is_priming(false), m_engine_is_running(false), m_shift_behaviour(0.0f), m_shift_time(0.5f),
      m_shift_clock(0.0f), m_shifting(0), m_shift_val(0), m_engine_stall_rpm(300.0f), m_starter(0),
      m_torque_curve(new TorqueCurve()), m_actor(actor), m_engine_turbo_mode(OLD), m_engine_type('t'), m_upshift_delay_counter(0),
      m_engine_is_electric(false), m_turbo_inertia_factor(1), m_num_turbos(1), m_max_turbo_rpm(200000.0f),
      m_turbo_engine_rpm_operation(0.0f), m_turbo_ver(1), m_min_bov_psi(11), m_min_wastegate_psi(20),
      m_turbo_has_wastegate(false), m_turbo_has_bov(false), m_turbo_flutters(false), m_turbo_wg_threshold_p(0),
      m_turbo_wg_threshold_n(0), m_turbo_has_antilag(false), m_antilag_rand_chance(0.9975), m_antilag_min_rpm(3000),
...
void EngineSim::SetTurboOptions(int type, float tinertiaFactor, int nturbos, float param1, float param2, float param3,
                                float param4, float param5, float param6, float param7, float param8, float param9, float param10,
                                float param11)
{
    ...
    if (param2 != 9999) m_turbo_engine_rpm_operation = param2;
    ...
    else
    {
        ...
        if (param3 != 9999) m_min_bov_psi = param4;
        ...
        if (param6 != 9999) m_min_wastegate_psi = param6 * 10000;
        ...
        if (param9 != 9999) m_antilag_rand_chance = param9;

        if (param10 != 9999) m_antilag_min_rpm = param10;

        if (param11 != 9999) m_antilag_power_factor = param11;
    }
}

void EngineSim::SetEngineOptions(float einertia, char etype, float eclutch, float ctime, float stime, float pstime, float irpm,
                                 float srpm, float maximix, float minimix, float ebraking)
{
    m_engine_inertia = einertia;
    m_engine_type    = etype;
    m_clutch_force   = eclutch;

    if (ctime > 0) m_clutch_time = ctime;
    if (stime > 0) m_shift_time = stime;
    if (pstime > 0) m_post_shift_time = pstime;
    if (irpm > 0) m_engine_idle_rpm = irpm;
    if (srpm > 0) m_engine_stall_rpm = srpm;
    if (maximix > 0) m_max_idle_mixture = maximix;
    if (minimix > 0) m_min_idle_mixture = minimix;
    if (ebraking > 0) m_braking_torque = -ebraking;
    ...
    if (etype == 'c')
    {
        ...
        if (m_clutch_force < 0.0f) { m_clutch_force = 5000.0f; }
    }
    ...
    else
    {
        m_engine_is_electric = false;
        // it's a truck
        if (m_clutch_force < 0.0f) { m_clutch_force = 10000.0f; }
    }
}
...
void EngineSim::UpdateEngineSim(float dt, int doUpdate)
{
    ...
    if (doUpdate) { SOUND_MODULATE(m_actor, SS_MOD_INJECTOR, acc); }
    ...
    if (m_engine_has_turbo)
    {
        ...
        else
        {
            for (int i = 0; i < m_num_turbos; i++)
            {
                ...
                if (m_cur_engine_rpm >= m_turbo_engine_rpm_operation)
                {
                    if (m_cur_turbo_rpm[i] <= m_max_turbo_rpm && m_engine_is_running && acc > 0.06f)
                    {
                        if (m_turbo_has_wastegate)
                        {
                            ...
                            {
                                turbotorque += 1.5f * acc *
                                               (((m_cur_engine_rpm - m_turbo_engine_rpm_operation) /
                                                 (m_engine_max_rpm - m_turbo_engine_rpm_operation)));
                            }
                            else
                            {
                    ...
                    {
                        turbotorque += 0.1f * (((m_cur_engine_rpm - m_turbo_engine_rpm_operation) /
                                                (m_engine_max_rpm - m_turbo_engine_rpm_operation)));
                    }
                ...
                if (!m_turbo_has_bov)
                {
                    if (m_cur_turbo_rpm[i] > 13 * 10000 && m_cur_acc < 0.06f) { turbotorque += (turbotorque * 2.5); }
                }
                ...
                // Update BOV
                // It's basicly an other turbo which is limmited to the main one's rpm, but it doesn't affect its rpm.  It only
                // affects the power going into the engine. This one is used to simulate the pressure between the engine and the
                // compressor. I should make the whole turbo code work this way. -Max98
                if (m_turbo_has_bov)
                {
    ...
    if (m_cur_engine_rpm > 100.0f) { totaltorque -= 8.0f * m_hydropump_state / (m_cur_engine_rpm * 0.105f * dt); }
    ...
    {
        if (m_engine_is_running && m_cur_engine_rpm < m_engine_stall_rpm) { this->StopEngine(); }
    ...
    // clutch
    if (m_cur_gear) { totaltorque -= m_cur_clutch_torque / m_gear_ratios[m_cur_gear + 1]; }
    ...
    if (m_auto_mode < MANUAL)
    {
        ...
        if (m_post_shifting)
        {
            ...
            if (m_post_shift_clock > m_post_shift_time) { m_post_shifting = 0; }
        ...
        if (m_cur_gear == 0 || m_cur_engine_rpm < declutchRPM) { m_cur_clutch = 0.0f; }
    ...
    if (doUpdate && !m_shifting && !m_post_shifting)
    {
        ...
        if (hdir != Vector3::ZERO) { velocity = hdir.dotProduct(m_actor->ar_nodes[0].Velocity); }
        ...
        {
            if ((m_cur_engine_rpm > m_engine_max_rpm - 100.0f && m_cur_gear > 1) ||
                m_cur_wheel_revolutions * m_gear_ratios[m_cur_gear + 1] > m_engine_max_rpm - 100.0f)
            {
                if ((m_autoselect == DRIVE && m_cur_gear < m_num_gears && m_cur_clutch > 0.99f) ||
                    (m_autoselect == TWO && m_cur_gear < std::min(2, m_num_gears)))
                {
                    m_kickdown_delay_counter = 100;
                    shift(1);
                }
            }
            else if (m_cur_gear > 1 && m_ref_wheel_revolutions * m_gear_ratios[m_cur_gear] < m_engine_max_rpm &&
                     (m_cur_engine_rpm < m_engine_min_rpm ||
                      (m_cur_engine_rpm < m_engine_min_rpm + m_shift_behaviour * m_half_rpm_range / 2.0f &&
                       getEnginePower(m_cur_wheel_revolutions * m_gear_ratios[m_cur_gear]) >
                           getEnginePower(m_cur_wheel_revolutions * m_gear_ratios[m_cur_gear + 1]))))
            {
                shift(-1);
            }
            ...
            else if (acc < 0.5f && avgAcc50 < 0.5f && avgAcc200 < 0.5f && m_actor->ar_brake < 0.5f && avgBrake50 < 0.5f &&
                     avgBrake200 < 0.5)
            {
            ...
            {
                while (newGear > 1 &&
                       m_cur_wheel_revolutions * m_gear_ratios[newGear] < m_engine_max_rpm - m_one_third_rpm_range &&
            ...
            else if (avgAcc50 > 0.6f && acc < 0.8f && acc > avgAcc50 + 0.1f &&
                     m_cur_engine_rpm < m_engine_min_rpm + m_half_rpm_range)
            {
            ...
            else if (avgAcc50 > 0.4f && acc < 0.8f && acc > avgAcc50 + 0.1f &&
                     m_cur_engine_rpm < m_engine_min_rpm + m_half_rpm_range)
            {
            ...
            else if (m_cur_gear < (m_autoselect == TWO ? std::min(2, m_num_gears) : m_num_gears) && avgBrake200 < 0.2f &&
                     acc < std::min(avgAcc200 + 0.1f, 1.0f) && m_cur_engine_rpm > avgRPM200 - m_full_rpm_range / 20.0f)
            {
                if (avgAcc200 < 0.6f && avgAcc200 > 0.4f && m_cur_engine_rpm > m_engine_min_rpm + m_one_third_rpm_range &&
                    m_cur_engine_rpm < m_engine_max_rpm - m_one_third_rpm_range)
                {
                ...
                else if (avgAcc200 < 0.2f && m_cur_engine_rpm > m_engine_min_rpm + m_one_third_rpm_range / 2.0f &&
                         m_cur_engine_rpm < m_engine_min_rpm + m_half_rpm_range)
                {
                    if (m_cur_wheel_revolutions * m_gear_ratios[newGear + 2] > m_engine_min_rpm + m_one_third_rpm_range / 2.0f)
                    { newGear++; }
                ...
                if (newGear > m_cur_gear)
                {
                    ...
                    if (m_upshift_delay_counter <= 100 * m_shift_behaviour) { newGear = m_cur_gear; }
                }
                else
                {
            ...
            if (newGear < m_cur_gear && m_kickdown_delay_counter > 0) { newGear = m_cur_gear; }
            m_kickdown_delay_counter = std::max(0, m_kickdown_delay_counter - 1);

            if (newGear < m_cur_gear &&
                    std::abs(m_cur_wheel_revolutions * (m_gear_ratios[newGear + 1] - m_gear_ratios[m_cur_gear + 1])) >
                        m_one_third_rpm_range / 6.0f ||
                newGear > m_cur_gear &&
                    std::abs(m_cur_wheel_revolutions * (m_gear_ratios[newGear + 1] - m_gear_ratios[m_cur_gear + 1])) >
                        m_one_third_rpm_range / 3.0f)
            {
                if (std::abs(m_actor->GetGForcesCur().y) < 0.25f) shiftTo(newGear);
            }
            ...
            if (m_auto_mode <= SEMIAUTO && m_cur_gear != 0)
            {
                ...
                {
                    float clutch = 0.0f + 1.0f / (1.0f + std::abs(m_cur_wheel_revolutions * m_gear_ratios[m_cur_gear + 1] -
                                                                  m_engine_max_rpm * 1.25f) /
                                                             2.0f);
...
void EngineSim::UpdateEngineAudio()
{
    ...
    // reverse gear beep
    if (m_cur_gear == -1 && m_engine_is_running) { SOUND_START(m_actor, SS_TRIG_REVERSE_GEAR); }
    else
    {
...
void EngineSim::ToggleAutoShiftMode()
{
    ...
    if (m_auto_mode == MANUAL_RANGES) { m_cur_gear_range = 0; }
}
...
{
    if (m_engine_turbo_mode == OLD) { return m_cur_turbo_rpm[0] / 10000.0f; }
...
void EngineSim::PushNetworkState(float rpm, float acc, float clutch, int gear, bool running, bool contact, char automode,
                                 char autoselect)
{
    ...
    if (automode != -1) { m_auto_mode = automode; }
    if (autoselect != -1) { m_autoselect = (autoswitch)autoselect; }
}
...
void EngineSim::ToggleStarterContact()
{
    ...
    if (m_starter_has_contact) { SOUND_START(m_actor, SS_TRIG_IGNITION); }
    else
    {
...
void EngineSim::StartEngine()
{
    ...
    m_engine_is_running   = true;
    if (m_auto_mode <= SEMIAUTO) { m_cur_gear = 1; }
    if (m_auto_mode == AUTOMATIC) { m_autoselect = DRIVE; }
...
void EngineSim::OffStart()
{
    ...
    m_shifting            = 0;
    m_shift_val           = 0;
    if (m_auto_mode == AUTOMATIC) { m_autoselect = NEUTRAL; }
    for (int i = 0; i < m_num_turbos; i++)
    {
...
{
    m_auto_cur_acc = val;
    if (!m_shifting) { SetAcceleration(val); }
}

void EngineSim::shift(int val)
{
    if (!val || m_cur_gear + val < -1 || m_cur_gear + val > getNumGears()) return;
    if (m_auto_mode < MANUAL)
    {
        m_shift_val   = val;
        m_shifting    = 1;
    ...
    {
        if (m_cur_clutch > 0.25f) { SOUND_PLAY_ONCE(m_actor, SS_TRIG_GEARSLIDE); }
        else
        {
...
{
    if (m_engine_is_electric) return;
    if (m_autoselect == MANUALMODE) return;

    SOUND_PLAY_ONCE(m_actor, SS_TRIG_SHIFT);

    if (m_autoselect == REAR) { m_cur_gear = -1; }
    ...
    else
    {
        ...
        while (newGear < m_num_gears && m_cur_wheel_revolutions > 0.0f &&
               m_cur_wheel_revolutions * m_gear_ratios[newGear + 1] > m_engine_max_rpm - 100.0f)
        {
            newGear++;
        }
        ...
        if (m_autoselect == TWO) { m_cur_gear = std::min(m_cur_gear, 2); }
    }
}
...
{
    if (!m_engine_has_turbo) return 0.0f;
    if (m_engine_turbo_mode != NEW) return 0.0f;
...
float EngineSim::getEnginePower(float rpm)
{
    ...
    if (m_torque_curve) { tqValue = m_torque_curve->getEngineTorque(rpm); }
...
{
    if (m_cur_engine_rpm <= m_engine_idle_rpm) return m_max_idle_mixture;
