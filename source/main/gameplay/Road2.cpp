...
Road2::Road2(int id) : first(true), mid(id), snode(0), tricount(0), vertexcount(0)
{
    msh.setNull();
}
...
Road2::~Road2()
{
    if (snode) { snode->removeAndDestroyAllChildren(); }
    if (!msh.isNull())
    {
...
void Road2::addBlock(Vector3 pos, Quaternion rot, int type, float width, float bwidth, float bheight, int pillartype)
{
    ...
    if (!first)
    {
        ...
        {
            // sides (close)
            addQuad(pts[5], lpts[5], lpts[4], pts[4], TEXFIT_CONCRETEWALLI, true, pos, lastpos, width,
                    (type == ROAD_FLAT || type == ROAD_LEFT));
            addQuad(pts[3], lpts[3], lpts[2], pts[2], TEXFIT_CONCRETEWALLI, true, pos, lastpos, width,
                    !(type == ROAD_FLAT || type == ROAD_RIGHT));
            // sides (far)
            addQuad(pts[6], lpts[6], lpts[5], pts[5], TEXFIT_CONCRETETOP, true, pos, lastpos, width,
                    (type == ROAD_FLAT || type == ROAD_LEFT));
            addQuad(pts[2], lpts[2], lpts[1], pts[1], TEXFIT_CONCRETETOP, true, pos, lastpos, width,
                    !(type == ROAD_FLAT || type == ROAD_RIGHT));
        }
        ...
        if ((type == ROAD_BRIDGE || type == ROAD_MONORAIL) && pillartype > 0)
        {
            ...
            Vector3 leftv        = pos + rot * Vector3(0, 0, bwidth + width / 2.0);
            Vector3 rightv       = pos + rot * Vector3(0, 0, -bwidth - width / 2.0);
            Vector3 middle       = lpts[0] - ((lpts[0] + (pts[1] - lpts[0]) / 2) - (lpts[7] + (pts[6] - lpts[7]) / 2)) * 0.5;
            ...
            if (pillartype == 2)
            {
                ...
                if (pillarcounter % 5) builtpillars = false;
            }

            middle       = lpts[0] - ((lpts[0] + (pts[1] - lpts[0]) / 2) - (lpts[7] + (pts[6] - lpts[7]) / 2)) * sidefactor;
            ...
            if (pillartype == 2) width2 = 0.2;

            if (width2 >= 0.2 && builtpillars)
            {
                // sides
                addQuad(middle + Vector3(-width2, -len, -width2), middle + Vector3(-width2, 0, -width2),
                        middle + Vector3(width2, 0, -width2), middle + Vector3(width2, -len, -width2), TEXFIT_CONCRETETOP, true,
                        pos, lastpos, width2);
...
void Road2::addQuad(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, int texfit, bool collision, Vector3 pos, Vector3 lastpos,
                    float width, bool flip)
{
    ...
    if (collision)
    {
        ...
        if (texfit == TEXFIT_ROAD || texfit == TEXFIT_ROADS1 || texfit == TEXFIT_ROADS2 || texfit == TEXFIT_ROADS3 ||
            texfit == TEXFIT_ROADS4)
...
void Road2::textureFit(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, int texfit, Vector2 *texc, Vector3 pos, Vector3 lastpos,
                       float width)
{
    int i;
    ...
    if (texfit == TEXFIT_ROAD || texfit == TEXFIT_ROADS1 || texfit == TEXFIT_ROADS2 || texfit == TEXFIT_ROADS3 ||
        texfit == TEXFIT_ROADS4 || texfit == TEXFIT_CONCRETETOP || texfit == TEXFIT_CONCRETEUNDER)
    {
        Vector3 ps[4];
        ps[0]         = p1;
        ps[1]         = p2;
