...
///                     this includes static collision objects (or intrusion detection objects), managed by
...
///  OLD: We use threadpool; rendering loop runs on main thread, simulation is updated in between frames via 'Ogre::FrameListener'
///  interface which also provides timing.
...
class SimController : public Ogre::WindowEventListener, public ZeroedMemoryAllocator
{
  public:
    ...
    {
        return m_actor_manager.GetLocalActors();
    }
    Actor *GetActorById(int actor_id)
    {
        return m_actor_manager.GetActorByIdInternal(actor_id);
    }
    std::pair<Actor *, float> GetNearestActor(Ogre::Vector3 position)
    {
        return m_actor_manager.GetNearestActor(position);
    };
    void ChangePlayerActor(Actor *actor);
    void QueueActorSpawn(RoR::ActorSpawnRequest const &rq)
    {
        m_actor_spawn_queue.push_back(rq);
    }
    void QueueActorModify(RoR::ActorModifyRequest const &rq)
    {
        m_actor_modify_queue.push_back(rq);
    }
    void QueueActorRemove(Actor *actor)
    {
    ...
    void   RemoveActorByCollisionBox(
          std::string const &ev_src_instance_name,
          std::string const &box_name); ///< Scripting utility. TODO: Does anybody use it? ~ only_a_ptr, 08/2017
    ...
    {
        return static_cast<float>(m_time - m_race_start_time);
    }
    ...
    // Temporary interface until camera controls are refactored; only for use by SceneMouse; see == SimCam == ~ only_a_ptr,
    // 06/2018
    ...
    RoR::ActorManager *GetBeamFactory()
    {
        return &m_actor_manager;
    } // TODO: Eliminate this. All operations upon actors should be done through above methods. ~ only_a_ptr, 06/2017
    RoR::SkidmarkConfig *GetSkidmarkConf()
    {
        return m_skidmark_conf;
    }
    RoR::GfxScene &GetGfxScene()
    {
        return m_gfx_scene;
    }
    RoR::SceneMouse &GetSceneMouse()
    {
        return m_scene_mouse;
    }
    Ogre::Vector3 GetDirArrowTarget()
    {
        return m_dir_arrow_pointed;
    }
    bool IsPressurizingTyres() const
    {
        return m_pressure_pressed;
    }
    ...
    Actor *GetPrevPlayerActor()
    {
        return m_prev_player_actor;
    };

    void SetPendingPlayerActor(Actor *actor)
    {
        m_pending_player_actor = actor;
    };
    void SetPrevPlayerActorInternal(Actor *actor)
    {
        m_prev_player_actor = actor;
    };

    bool GetPhysicsPaused()
    {
        return m_physics_simulation_paused;
    };
    void SetPhysicsPausedInternal(bool paused)
    {
        m_physics_simulation_paused = paused;
    };
  ...
  private:
    ...
    float m_last_simulation_speed; //!< previously used time ratio between real time (evt.timeSinceLastFrame) and physics time
                                   //!< ('dt' used in calcPhysics)
