...
void RigInspector::InspectProps(std::ofstream & f, Beam* rig)
{
	...
	for (int i = 0; i < rig->free_prop; i++)
	{
		...
		f<<"\n prop "<<i<<":"
			...
			<<" snode="<<ECHO_PTR(data.scene_node)
			<<" wheel="<<ECHO_PTR(data.wheel)
			<<" wheelpos="<<ECHO_V3(data.wheelpos)

			<<" mirror="<<data.mirror
			<<" beacontype="<<data.beacontype
			<<" beacon_flares_billboard_system="
				<<ECHO_PTR(data.beacon_flares_billboard_system[0])
				<<ECHO_PTR(data.beacon_flares_billboard_system[1])
				<<ECHO_PTR(data.beacon_flares_billboard_system[2])
				<<ECHO_PTR(data.beacon_flares_billboard_system[3])
			<<" beacon_flare_billboard_scene_node="
				<<ECHO_PTR(data.beacon_flare_billboard_scene_node[0])
				<<ECHO_PTR(data.beacon_flare_billboard_scene_node[1])
				<<ECHO_PTR(data.beacon_flare_billboard_scene_node[2])
				<<ECHO_PTR(data.beacon_flare_billboard_scene_node[3])
			<<" light="
				<<ECHO_PTR(data.beacon_light[0])
				<<ECHO_PTR(data.beacon_light[1])
				<<ECHO_PTR(data.beacon_light[2])
				<<ECHO_PTR(data.beacon_light[3])
			<<" beacon_light_rotation_rate="
				<<data.beacon_light_rotation_rate[0]<<" "
				<<data.beacon_light_rotation_rate[1]<<" "
				<<data.beacon_light_rotation_rate[2]<<" "
				<<data.beacon_light_rotation_rate[3]<<" "
			<<" beacon_light_rotation_angle="
				<<data.beacon_light_rotation_angle[0]<<" "
				<<data.beacon_light_rotation_angle[1]<<" "
				<<data.beacon_light_rotation_angle[2]<<" "
				<<data.beacon_light_rotation_angle[3]<<" "
			<<" pale="<<data.pale
