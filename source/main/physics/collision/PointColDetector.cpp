...
void PointColDetector::queryrec(int kdindex, int axis)
{
    for (;;)
    {
        if (m_kdtree[kdindex].end < 0) { build_kdtree_incr(axis, kdindex); }

        if (m_kdtree[kdindex].ref != NULL)
        {
            const float *point = m_kdtree[kdindex].ref->point;
            if (point[0] >= m_bbmin.x && point[0] <= m_bbmax.x && point[1] >= m_bbmin.y && point[1] <= m_bbmax.y &&
        ...
        {
            if (m_bbmin[axis] > m_kdtree[kdindex].max) { return; }
            ...
            if (m_bbmin[axis] <= m_kdtree[kdindex].middle) { queryrec(newindex, newaxis); }
        ...
        {
            if (m_bbmax[axis] < m_kdtree[kdindex].min) { return; }
