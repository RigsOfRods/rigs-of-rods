 ...
 * @param nodes
 */
static bool BackfaceCollisionTest(const float distance, const Vector3 &normal, const node_t &surface_point,
                                  const std::vector<int> &neighbour_node_ids, const node_t nodes[])
{
...
void ResolveCollisionForces(const float penetration_depth, node_t &hitnode, node_t &na, node_t &nb, node_t &no, const float alpha,
                            const float beta, const float gamma, const Vector3 &normal, const float dt, const bool remote,
...
void ResolveInterActorCollisions(const float dt, PointColDetector &interPointCD, const int free_collcab, int collcabs[],
                                 int cabs[], collcab_rate_t inter_collcabrate[], node_t nodes[], const float collrange,
                                 ground_model_t &submesh_ground_model)
{
    for (int i = 0; i < free_collcab; i++)
    {
        ...
        interPointCD.query(no->AbsPosition, na->AbsPosition, nb->AbsPosition, collrange);
            ...
            for (auto h : interPointCD.hit_list)
            {
                ...
                if (is_colliding)
                {
                    ...
                    const bool is_backface =
                        BackfaceCollisionTest(distance, normal, *no, neighbour_node_ids, hit_actor->ar_nodes);
                    if (is_backface)
                    {
                    ...
                    ResolveCollisionForces(penetration_depth, *hitnode, *na, *nb, *no, coord.alpha, coord.beta, coord.gamma,
                                           normal, dt, remote, submesh_ground_model);
...
void ResolveIntraActorCollisions(const float dt, PointColDetector &intraPointCD, const int free_collcab, int collcabs[],
                                 int cabs[], collcab_rate_t intra_collcabrate[], node_t nodes[], const float collrange,
                                 ground_model_t &submesh_ground_model)
{
    for (int i = 0; i < free_collcab; i++)
    {
        ...
        intraPointCD.query(no->AbsPosition, na->AbsPosition, nb->AbsPosition, collrange);
        ...
        if (!intraPointCD.hit_list.empty())
        {
            ...
            for (auto h : intraPointCD.hit_list)
            {
                ...
                if (is_colliding)
                {
                    ...
                    ResolveCollisionForces(penetration_depth, *hitnode, *na, *nb, *no, coord.alpha, coord.beta, coord.gamma,
                                           normal, dt, false, submesh_ground_model);
                }
            }
        }

        if (collision) { intra_collcabrate[i].rate = -20000; }
        else
        {
            intra_collcabrate[i].rate++;
        }
