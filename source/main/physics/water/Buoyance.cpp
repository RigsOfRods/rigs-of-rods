...
Buoyance::Buoyance(DustPool *splash, DustPool *ripple) : splashp(splash), ripplep(ripple), sink(0), update(false)
{
}
...
Vector3 Buoyance::computePressureForce(Vector3 a, Vector3 b, Vector3 c, Vector3 vel, int type)
{
    ...
    if (a.y > wha || b.y > wha || c.y > wha)
    {
        ...
        if (a.y < wha && b.y > wha && c.y > wha)
        {
            return computePressureForceSub(a, a + (wha - a.y) / (b.y - a.y) * (b - a), a + (wha - a.y) / (c.y - a.y) * (c - a),
                                           vel, type);
        }
        if (b.y < wha && c.y > wha && a.y > wha)
        {
            return computePressureForceSub(b, b + (wha - b.y) / (c.y - b.y) * (c - b), b + (wha - b.y) / (a.y - b.y) * (a - b),
                                           vel, type);
        }
        if (c.y < wha && a.y > wha && b.y > wha)
        {
            return computePressureForceSub(c, c + (wha - c.y) / (a.y - c.y) * (a - c), c + (wha - c.y) / (b.y - c.y) * (b - c),
                                           vel, type);
        }
        // two dips
        if (a.y > wha && b.y < wha && c.y < wha)
...
void Buoyance::computeNodeForce(node_t *a, node_t *b, node_t *c, bool doUpdate, int type)
{
    ...
    // apply forces
    a->Forces +=
        computePressureForce(a->AbsPosition, mab, m, vel, type) + computePressureForce(a->AbsPosition, m, mca, vel, type);
    b->Forces +=
        computePressureForce(b->AbsPosition, mbc, m, vel, type) + computePressureForce(b->AbsPosition, m, mab, vel, type);
    c->Forces +=
        computePressureForce(c->AbsPosition, mca, m, vel, type) + computePressureForce(c->AbsPosition, m, mbc, vel, type);
}
