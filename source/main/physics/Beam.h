...
class Actor : public ZeroedMemoryAllocator
{
  ...
  public:
    ...
    Actor(int actor_id, unsigned int vector_index, std::shared_ptr<RigDef::File> def, RoR::ActorSpawnRequest rq);

    ~Actor();

    void          ApplyNodeBeamScales();
    void          PushNetwork(char *data,
                              int size); //!< Parses network data; fills actor's data buffers and flips them. Called by the network thread.
    void          CalcNetwork();
    ...
    void RequestRotation(float rotation, Ogre::Vector3 center)
    {
        ...
        m_rotation_request_center = center;
    };
    void RequestAngleSnap(int division)
    {
        m_anglesnap_request = division;
    };
    ...
    Ogre::Vector3 GetGForcesCur()
    {
        return m_camera_local_gforces_cur;
    };
    Ogre::Vector3 GetGForcesMax()
    {
        return m_camera_local_gforces_max;
    };
    ...
    {
        return m_avg_proped_wheel_radius;
    };
    ...
    {
        return m_section_config;
    }
    RoR::PerVehicleCameraContext *GetCameraContext()
    {
        return &m_camera_context;
    }
    std::vector<Actor *> GetAllLinkedActors()
    {
        return m_linked_actors;
    }; //!< Returns a list of all connected (hooked) actors
    Ogre::Vector3 GetCameraDir()
    {
        return (ar_nodes[ar_main_camera_node_pos].RelPosition - ar_nodes[ar_main_camera_node_dir].RelPosition).normalisedCopy();
    }
    Ogre::Vector3 GetCameraRoll()
    {
        return (ar_nodes[ar_main_camera_node_pos].RelPosition - ar_nodes[ar_main_camera_node_roll].RelPosition).normalisedCopy();
    }
    Ogre::Vector3 GetFFbBodyForces() const
    {
        return m_force_sensors.out_body_forces;
    }
    RoR::GfxActor *GetGfxActor()
    {
        return m_gfx_actor.get();
    }
    ...
    {
        return m_force_sensors.out_hydros_forces;
    }
    bool isPreloadedWithTerrain() const
    {
        return m_preloaded_with_terrain;
    };
    bool isBeingReset() const
    {
        return m_ongoing_reset;
    };
    VehicleAI *getVehicleAI()
    {
        return ar_vehicle_ai;
    }
    float getWheelSpeed() const
    {
        return ar_wheel_speed;
    }
    int GetNumNodes() const
    {
        return ar_num_nodes;
    }
    CacheEntry *GetUsedSkin() const
    {
        return m_used_skin_entry;
    }
    void SetUsedSkin(CacheEntry *skin)
    {
        m_used_skin_entry = skin;
    }
    float getSpeed()
    {
        return m_avg_node_velocity.length();
    };
    Ogre::Vector3 getVelocity() const
    {
        return m_avg_node_velocity;
    ...
    std::vector<Ogre::AxisAlignedBox> ar_collision_bounding_boxes; //!< smart bounding boxes, used for determining the state of an
                                                                   //!< actor (every box surrounds only a subset of nodes)
    ...
    bool ar_hide_in_actor_list; //!< Hide in list of spawned actors (available in top menubar). Useful for fixed-place machinery,
                                //!< i.e. cranes.
    ...
    bool ar_camera_node_roll_inv[MAX_CAMERAS]; //!< Physics attr; 'camera' = frame of reference; indicates roll node is right
                                               //!< instead of left
  ...
  private:
    ...
    void CalcAnimators(const int flag_state, float &cstate, int &div, float timer, const float lower_limit,
                       const float upper_limit, const float option3); //!< TIGHT LOOP; Physics;
