...
void ActorSpawner::Setup(Actor *rig, std::shared_ptr<RigDef::File> file, Ogre::SceneNode *parent,
...
void ActorSpawner::CalcMemoryRequirements(ActorMemoryRequirements &req, RigDef::File::Module *module_def)
{
    ...
    {
        if (BITMASK_IS_1(def.options, RigDef::Node::OPTION_h_HOOK_POINT)) { req.num_beams += 1; }
    }
...
void ActorSpawner::InitializeRig()
{
    ...
    if (req.num_shocks > 0) m_actor->ar_shocks = new shock_t[req.num_shocks];

    if (req.num_rotators > 0) m_actor->ar_rotators = new rotator_t[req.num_rotators];

    if (req.num_wings > 0) m_actor->ar_wings = new wing_t[req.num_wings];
    ...
    if (!App::sim_no_collisions.GetActive()) { m_actor->m_inter_point_col_detector = new PointColDetector(m_actor); }

    if (!App::sim_no_self_collisions.GetActive()) { m_actor->m_intra_point_col_detector = new PointColDetector(m_actor); }
    ...
    m_placeholder_managedmat =
        Ogre::MaterialManager::getSingleton().getByName("rigsofrods/managedmaterial-placeholder"); // Built-in
    ...
    if (m_apply_simple_materials)
    {
        ...
        {
            this->AddMessage(Message::TYPE_INTERNAL_ERROR,
...
void ActorSpawner::FinalizeRig()
{
    ...
    {
        int result =
            m_actor->ar_engine->getTorqueCurve()->spaceCurveEvenly(m_actor->ar_engine->getTorqueCurve()->getUsedSpline());
        if (result)
        {
    ...
    if (m_actor->m_num_proped_wheels > 0)
    {
        ...
        for (int i = 0; i < m_actor->ar_num_wheels; i++)
        {
            if (m_actor->ar_wheels[i].wh_propulsed > 0) { proped_wheels_radius_sum += m_actor->ar_wheels[i].wh_radius; }
        }
    ...
    if (m_actor->ar_camera_node_pos[0] > 0)
    {
        ...
        m_actor->ar_posnode_spawn_height =
            m_actor->ar_nodes[m_actor->ar_camera_node_pos[0]].RelPosition.y - m_actor->ar_posnode_spawn_height;
    }
    else
    {
    ...
    {
        // LogManager::getSingleton().logMessage("Camera
        // dir="+StringConverter::toString(ar_nodes[ar_camera_node_dir[i]].RelPosition-ar_nodes[ar_camera_node_pos[i]].RelPosition)+"
        // roll="+StringConverter::toString(ar_nodes[ar_camera_node_roll[i]].RelPosition-ar_nodes[ar_camera_node_pos[i]].RelPosition));
        Ogre::Vector3 dir_node_offset =
            GetNode(m_actor->ar_camera_node_dir[i]).RelPosition - GetNode(m_actor->ar_camera_node_pos[i]).RelPosition;
        Ogre::Vector3 roll_node_offset =
            GetNode(m_actor->ar_camera_node_roll[i]).RelPosition - GetNode(m_actor->ar_camera_node_pos[i]).RelPosition;
        Ogre::Vector3 cross = dir_node_offset.crossProduct(roll_node_offset);

        m_actor->ar_camera_node_roll_inv[i] =
            cross.y >
            0; //(ar_nodes[ar_camera_node_dir[i]].RelPosition-ar_nodes[ar_camera_node_pos[i]].RelPosition).crossProduct(ar_nodes[ar_camera_node_roll[i]].RelPosition-ar_nodes[ar_camera_node_pos[i]].RelPosition).y>0;
        if (m_actor->ar_camera_node_roll_inv[i])
        {
            AddMessage(Message::TYPE_WARNING,
                       "camera definition is probably invalid and has been corrected. It should be center, back, left");
        }
    }
    ...
    if (m_first_wing_index != -1)
    {
        ...
        {
            m_actor->ar_autopilot->setInertialReferences(&GetNode(m_airplane_left_light), &GetNode(m_airplane_right_light),
                                                         m_actor->m_fusealge_back, &GetNode(m_actor->ar_camera_node_pos[0]));
        }
        // inform wing segments
        float span = GetNode(m_actor->ar_wings[m_first_wing_index].fa->nfrd)
                         .RelPosition.distance(GetNode(m_actor->ar_wings[m_actor->ar_num_wings - 1].fa->nfld).RelPosition);
...
void ActorSpawner::WashCalculator()
{
    ...
    for (p = 0; p < m_actor->ar_num_aeroengines; p++)
    {
        ...
        {
            // left wash
            Ogre::Vector3 wcent = ((m_actor->ar_nodes[m_actor->ar_wings[w].fa->nfld].RelPosition +
                                    m_actor->ar_nodes[m_actor->ar_wings[w].fa->nfrd].RelPosition) /
                                   2.0);
            ...
            if (wcent.x > prop.x && wcent.x < prop.x + 15.0)
            {
                ...
                if (wcent.y > prop.y - radius && wcent.y < prop.y + radius)
                {
                    ...
                    if (aright < aleft)
                    {
                        ...
                        Ogre::String msg = "Wing " + TOSTRING(w) + " is washed by prop " + TOSTRING(p) + " at " +
                                           TOSTRING((float)(wratio * 100.0)) + "%";
...
void ActorSpawner::ProcessScrewprop(RigDef::Screwprop &def)
{
    ...
    m_actor->ar_screwprops[m_actor->ar_num_screwprops] =
        new Screwprop(m_actor->ar_nodes, ref_node_idx, back_node_idx, top_node_idx, def.power, m_actor->ar_instance_id);
...
void ActorSpawner::BuildAerialEngine(int ref_node_index, int back_node_index, int blade_1_node_index, int blade_2_node_index,
                                     int blade_3_node_index, int blade_4_node_index, int couplenode_index, bool is_turboprops,
                                     Ogre::String const &airfoil, float power, float pitch)
{
    int aeroengine_index = m_actor->ar_num_aeroengines;

    Turboprop *turbo_prop = new Turboprop(
        this->ComposeName("Turboprop", aeroengine_index).c_str(), m_actor->ar_nodes, ref_node_index, back_node_index,
        blade_1_node_index, blade_2_node_index, blade_3_node_index, blade_4_node_index, couplenode_index, power, airfoil,
        m_actor->ar_num_aeroengines, m_actor->ar_instance_id, m_actor->m_disable_smoke, !is_turboprops, pitch);
...
void ActorSpawner::ProcessTurboprop2(RigDef::Turboprop2 &def)
{
    ...
    BuildAerialEngine(GetNodeIndexOrThrow(def.reference_node), GetNodeIndexOrThrow(def.axis_node),
                      GetNodeIndexOrThrow(def.blade_tip_nodes[0]), GetNodeIndexOrThrow(def.blade_tip_nodes[1]), p3_node_index,
                      p4_node_index, couple_node_index, true, def.airfoil, def.turbine_power_kW, -10);
}
...
void ActorSpawner::ProcessPistonprop(RigDef::Pistonprop &def)
{
    ...
    BuildAerialEngine(GetNodeIndexOrThrow(def.reference_node), GetNodeIndexOrThrow(def.axis_node),
                      GetNodeIndexOrThrow(def.blade_tip_nodes[0]), GetNodeIndexOrThrow(def.blade_tip_nodes[1]), p3_node_index,
                      p4_node_index, couple_node_index, false, def.airfoil, def.turbine_power_kW, def.pitch);
}
...
void ActorSpawner::ProcessAirbrake(RigDef::Airbrake &def)
{
    ...
    m_actor->ar_airbrakes.push_back(new Airbrake(
        m_actor, this->ComposeName("Airbrake", airbrake_idx).c_str(), airbrake_idx, GetNodePointerOrThrow(def.reference_node),
        GetNodePointerOrThrow(def.x_axis_node), GetNodePointerOrThrow(def.y_axis_node), GetNodePointerOrThrow(def.aditional_node),
        def.offset, def.width, def.height, def.max_inclination_angle, m_cab_material_name, def.texcoord_x1, def.texcoord_y1,
        def.texcoord_x2, def.texcoord_y2, def.lift_coefficient));
}
...
void ActorSpawner::ProcessWing(RigDef::Wing &def)
{
    ...
    auto              flex_airfoil = new FlexAirfoil(
        wing_name, m_actor, node_indices[0], node_indices[1], node_indices[2], node_indices[3], node_indices[4], node_indices[5],
        node_indices[6], node_indices[7], m_cab_material_name, Ogre::Vector2(def.tex_coords[0], def.tex_coords[1]),
        Ogre::Vector2(def.tex_coords[2], def.tex_coords[3]), Ogre::Vector2(def.tex_coords[4], def.tex_coords[5]),
        Ogre::Vector2(def.tex_coords[6], def.tex_coords[7]), def.control_surface, def.chord_point, def.min_deflection,
        def.max_deflection, def.airfoil, def.efficacy_coef, m_actor->ar_sim_state != Actor::SimState::NETWORKED_OK);
    ...
    if (m_first_wing_index == -1)
    {
        ...
        m_wing_area =
            ComputeWingArea(this->GetNode(flex_airfoil->nfld).AbsPosition, this->GetNode(flex_airfoil->nfrd).AbsPosition,
    ...
    else
    {
        ...
        if (node_indices[1] != previous_wing.fa->nfld)
        {
            ...
            if (m_generate_wing_position_lights && (m_actor->m_flares_mode != GfxFlaresMode::NONE))
            {
                ...
                // the flare billboard
                left_green_prop.beacon_flare_billboard_scene_node[0] =
                    gEnv->sceneManager->getRootSceneNode()->createChildSceneNode();
                left_green_prop.beacon_flares_billboard_system[0] =
                    gEnv->sceneManager->createBillboardSet(this->ComposeName("Prop", static_cast<int>(m_props.size()) + 1), 1);
                ...
                if (left_green_prop.beacon_flares_billboard_system[0])
                {
                    ...
                    left_green_prop.beacon_flare_billboard_scene_node[0]->attachObject(
                        left_green_prop.beacon_flares_billboard_system[0]);
                }
                ...
                // the flare billboard
                left_flash_prop.beacon_flare_billboard_scene_node[0] =
                    gEnv->sceneManager->getRootSceneNode()->createChildSceneNode();
                ...
                if (left_flash_prop.beacon_flares_billboard_system[0])
                {
                    ...
                    left_flash_prop.beacon_flare_billboard_scene_node[0]->attachObject(
                        left_flash_prop.beacon_flares_billboard_system[0]);
                }
                ...
                // the flare billboard
                right_red_prop.beacon_flare_billboard_scene_node[0] =
                    gEnv->sceneManager->getRootSceneNode()->createChildSceneNode();
                right_red_prop.beacon_flares_billboard_system[0] =
                    gEnv->sceneManager->createBillboardSet(this->ComposeName("Prop", static_cast<int>(m_props.size()) + 1), 1);
                ...
                if (right_red_prop.beacon_flares_billboard_system[0])
                {
                    ...
                    right_red_prop.beacon_flare_billboard_scene_node[0]->attachObject(
                        right_red_prop.beacon_flares_billboard_system[0]);
                }
                ...
                // the flare billboard
                right_flash_prop.beacon_flare_billboard_scene_node[0] =
                    gEnv->sceneManager->getRootSceneNode()->createChildSceneNode();
                ...
                if (right_flash_prop.beacon_flares_billboard_system[0] != nullptr)
                {
                    ...
                    right_flash_prop.beacon_flare_billboard_scene_node[0]->attachObject(
                        right_flash_prop.beacon_flares_billboard_system[0]);
                }
...
float ActorSpawner::ComputeWingArea(Ogre::Vector3 const &ref, Ogre::Vector3 const &x, Ogre::Vector3 const &y,
...
void ActorSpawner::ProcessSoundSource2(RigDef::SoundSource2 &def)
{
    ...
    AddSoundSource(m_actor, SoundScriptManager::getSingleton().createInstance(def.sound_script_name, m_actor->ar_instance_id),
                   node_index, mode);
#endif // USE_OPENAL
}
...
{
#ifdef USE_OPENAL
    AddSoundSource(vehicle,
                   SoundScriptManager::getSingleton().createInstance(sound_script_name, vehicle->ar_instance_id, nullptr),
                   node_index);
#endif // USE_OPENAL
}
...
void ActorSpawner::AddSoundSource(Actor *vehicle, SoundScriptInstance *sound_script, int node_index, int type)
{
    ...
    if (sound_script == nullptr) { return; }
...
#ifdef USE_OPENAL
    AddSoundSource(m_actor, SoundScriptManager::getSingleton().createInstance(def.sound_script_name, m_actor->ar_instance_id),
                   GetNodeIndexOrThrow(def.node), -2);
#endif // USE_OPENAL
}
...
void ActorSpawner::ProcessCameraRail(RigDef::CameraRail &def)
{
    ...
    for (; itor != end; ++itor)
    {
        if (!CheckCameraRailLimit(1)) { return; }
...
void ActorSpawner::ProcessExtCamera(RigDef::ExtCamera &def)
{
    ...
    if (def.node.IsValidAnyState()) { m_actor->ar_extern_camera_node = GetNodeIndexOrThrow(def.node); }
}
...
void ActorSpawner::ProcessGuiSettings(RigDef::GuiSettings &def)
{
    ...
    if (!def.help_material.empty()) { m_actor->ar_help_panel_material = def.help_material; }
    ...
    else
    {
        ...
        msg << "Invalid 'speedo_highest_kph' value (" << def.speedo_highest_kph
            << "), allowed range is <10 -32000>. Falling back to default...";
...
{
    if (m_actor->m_disable_smoke) { return; }
    ...
    {
        if (nfx.nx_node_idx == exhaust.emitterNode || nfx.nx_node_idx == exhaust.directionNode) { nfx.nx_is_hot = true; }
    }
...
std::string ActorSpawner::GetSubmeshGroundmodelName()
{
    ...
    {
        if (!module_itor->get()->submeshes_ground_model_name.empty()) { return module_itor->get()->submeshes_ground_model_name; }
    }
    return std::string();
};
...
void ActorSpawner::ProcessSubmesh(RigDef::Submesh &def)
{
    ...
    for (; cab_itor != cab_itor_end; ++cab_itor)
    {
        ...
        if (cab_itor->GetOption_D_ContactBuoyant() || cab_itor->GetOption_F_10xTougherBuoyant() ||
    ...
    if (def.backmesh)
    {
        ...
        // texcoords
        int uv_start =
            (m_oldstyle_cab_submeshes.size() == 1) ? 0 : static_cast<int>((m_oldstyle_cab_submeshes.rbegin() + 1)->texcoords_pos);
        ...
        // cab
        int cab_start =
            (m_oldstyle_cab_submeshes.size() == 1) ? 0 : static_cast<int>((m_oldstyle_cab_submeshes.rbegin() + 1)->cabs_pos);
...
void ActorSpawner::ProcessFlexbody(std::shared_ptr<RigDef::Flexbody> def)
{
    ...
    try
    {
        auto *flexbody = m_flex_factory.CreateFlexBody(def.get(), reference_node, x_axis_node, y_axis_node, rot, node_indices,
                                                       m_custom_resource_group);

        if (flexbody == nullptr) return; // Error already logged
...
void ActorSpawner::ProcessProp(RigDef::Prop &def)
{
    ...
    if (prop.nodex == -1 || prop.nodey == -1) { return; }
    ...
    if (def.special == RigDef::Prop::SPECIAL_DASHBOARD_LEFT) { steering_wheel_offset = Ogre::Vector3(-0.67, -0.61, 0.24); }
    if (def.special == RigDef::Prop::SPECIAL_DASHBOARD_RIGHT) { steering_wheel_offset = Ogre::Vector3(0.67, -0.61, 0.24); }
    if (steering_wheel_offset != Ogre::Vector3::ZERO)
    {
        if (def.special_prop_dashboard._offset_is_set) { steering_wheel_offset = def.special_prop_dashboard.offset; }
        ...
        prop.wheelmo = new MeshObject(def.special_prop_dashboard.mesh_name, m_custom_resource_group, instance_name, prop.wheel);
    ...
    prop.mo->setCastShadows(true); // Orig code {{ prop.mo->setCastShadows(shadowmode != 0); }}, shadowmode has default value 1
                                   // and changes with undocumented directive 'set_shadows'
    ...
    else if (def.special == RigDef::Prop::SPECIAL_DRIVER_SEAT)
    {
        ...
        {
            this->AddMessage(Message::TYPE_INFO,
                             "Found more than one 'seat[2]' special props. Only the first one will be the driver's seat.");
        }
    }
    else if (def.special == RigDef::Prop::SPECIAL_DRIVER_SEAT_2)
    {
        ...
        if (m_driverseat_prop_index == -1) { m_driverseat_prop_index = prop_index; }
        else
        {
            this->AddMessage(Message::TYPE_INFO,
                             "Found more than one 'seat[2]' special props. Only the first one will be the driver's seat.");
    ...
    {
        std::stringstream msg;
        msg << "Prop (mesh: " << def.mesh_name << ") has too many animations: " << def.animations.size()
            << " (max. is 10). Using first 10 ...";
    ...
    while (anim_itor != def.animations.end() && anim_index < 10)
    {
        ...
        /* Arg #4: source */
        if (BITMASK_IS_1(anim_itor->source, RigDef::Animation::SOURCE_AIRSPEED))
        { /* (NOTE: code formatting relaxed) */
        ...
        if (prop.animFlags[anim_index] == 0) { AddMessage(Message::TYPE_ERROR, "Failed to identify animation source"); }
        ...
        if (prop.animMode[anim_index] == 0) { AddMessage(Message::TYPE_ERROR, "Failed to identify animation mode"); }
        ...
        if (BITMASK_IS_1(anim_itor->mode, RigDef::Animation::MODE_AUTO_ANIMATE))
        {
            ...
            if (BITMASK_IS_1(anim_itor->mode, RigDef::Animation::MODE_ROTATION_X))
            {
                prop.constraints[anim_index].lower_limit =
                    (use_default_lower_limit) ? (-180.f) : (anim_itor->lower_limit + prop.rotaX);
                prop.constraints[anim_index].upper_limit =
                    (use_default_upper_limit) ? (180.f) : (anim_itor->upper_limit + prop.rotaX);
            }
            if (BITMASK_IS_1(anim_itor->mode, RigDef::Animation::MODE_ROTATION_Y))
            {
                prop.constraints[anim_index].lower_limit =
                    (use_default_lower_limit) ? (-180.f) : (anim_itor->lower_limit + prop.rotaY);
                prop.constraints[anim_index].upper_limit =
                    (use_default_upper_limit) ? (180.f) : (anim_itor->upper_limit + prop.rotaY);
            }
            if (BITMASK_IS_1(anim_itor->mode, RigDef::Animation::MODE_ROTATION_Z))
            {
                prop.constraints[anim_index].lower_limit =
                    (use_default_lower_limit) ? (-180.f) : (anim_itor->lower_limit + prop.rotaZ);
                prop.constraints[anim_index].upper_limit =
                    (use_default_upper_limit) ? (180.f) : (anim_itor->upper_limit + prop.rotaZ);
            }
            if (BITMASK_IS_1(anim_itor->mode, RigDef::Animation::MODE_OFFSET_X))
            {
                prop.constraints[anim_index].lower_limit =
                    (use_default_lower_limit) ? (-10.f) : (anim_itor->lower_limit + prop.orgoffsetX);
                prop.constraints[anim_index].upper_limit =
                    (use_default_upper_limit) ? (10.f) : (anim_itor->upper_limit + prop.orgoffsetX);
            }
            if (BITMASK_IS_1(anim_itor->mode, RigDef::Animation::MODE_OFFSET_Y))
            {
                prop.constraints[anim_index].lower_limit =
                    (use_default_lower_limit) ? (-10.f) : (anim_itor->lower_limit + prop.orgoffsetY);
                prop.constraints[anim_index].upper_limit =
                    (use_default_upper_limit) ? (10.f) : (anim_itor->upper_limit + prop.orgoffsetY);
            }
            if (BITMASK_IS_1(anim_itor->mode, RigDef::Animation::MODE_OFFSET_Z))
            {
                prop.constraints[anim_index].lower_limit =
                    (use_default_lower_limit) ? (-10.f) : (anim_itor->lower_limit + prop.orgoffsetZ);
                prop.constraints[anim_index].upper_limit =
                    (use_default_upper_limit) ? (10.f) : (anim_itor->upper_limit + prop.orgoffsetZ);
            }
        }
        ...
        if (!anim_itor->event.empty())
        {
            ...
            if (event_id == -1) { AddMessage(Message::TYPE_ERROR, "Unknown animation event: " + anim_itor->event); }
            else
            {
...
void ActorSpawner::ProcessFlare2(RigDef::Flare2 &def)
{
    ...
    if (size == -2.f && def.type == RigDef::Flare2::TYPE_f_HEADLIGHT) { size = 1.f; }
    ...
    {
        AddMessage(Message::TYPE_WARNING,
                   "Failed to create flare: '" + flare_name + "', continuing without it (compatibility)...");
    }
    else
    {
        ...
        {
            if (def.type == RigDef::Flare2::TYPE_b_BRAKELIGHT) { material_name = "tracks/brakeflare"; }
...
void ActorSpawner::ProcessManagedMaterial(RigDef::ManagedMaterial &def)
{
    ...
    if (def.type == RigDef::ManagedMaterial::TYPE_FLEXMESH_STANDARD ||
        def.type == RigDef::ManagedMaterial::TYPE_FLEXMESH_TRANSPARENT)
    {
        std::string mat_name_base = (def.type == RigDef::ManagedMaterial::TYPE_FLEXMESH_STANDARD)
        ...
        if (def.HasDamagedDiffuseMap())
        {
            if (def.HasSpecularMap())
            {
                ...
                material->getTechnique("BaseTechnique")
                    ->getPass("BaseRender")
                    ->getTextureUnitState("Diffuse_Map")
                    ->setTextureName(def.diffuse_map);
                material->getTechnique("BaseTechnique")
                    ->getPass("BaseRender")
                    ->getTextureUnitState("Dmg_Diffuse_Map")
                    ->setTextureName(def.damaged_diffuse_map);
                material->getTechnique("BaseTechnique")
                    ->getPass("SpecularMapping1")
                    ->getTextureUnitState("SpecularMapping1_Tex")
                    ->setTextureName(def.specular_map);
            }
            else
            {
    ...
    else if (def.type == RigDef::ManagedMaterial::TYPE_MESH_STANDARD ||
             def.type == RigDef::ManagedMaterial::TYPE_MESH_TRANSPARENT)
    {
        Ogre::String mat_name_base =
            (def.type == RigDef::ManagedMaterial::TYPE_MESH_STANDARD) ? "managed/mesh_standard" : "managed/mesh_transparent";
...
void ActorSpawner::ProcessAxle(RigDef::Axle &def)
{
    ...
    {
        std::stringstream msg;
        msg << "Couldn't find wheel with axis nodes '" << def.wheels[0][0].ToString() << "' and '" << def.wheels[0][1].ToString()
            << "'";
        AddMessage(Message::TYPE_WARNING, msg.str());
    }
    ...
    {
        std::stringstream msg;
        msg << "Couldn't find wheel with axis nodes '" << def.wheels[1][0].ToString() << "' and '" << def.wheels[1][1].ToString()
            << "'";
        AddMessage(Message::TYPE_WARNING, msg.str());
    }
    ...
    else
    {
        ...
        for (auto itor = def.options.begin(); itor != end; ++itor)
        {
            switch (*itor)
            {
            case RigDef::Axle::OPTION_l_LOCKED: diff->AddDifferentialType(LOCKED_DIFF); break;
            case RigDef::Axle::OPTION_o_OPEN: diff->AddDifferentialType(OPEN_DIFF); break;
            case RigDef::Axle::OPTION_s_SPLIT: diff->AddDifferentialType(SPLIT_DIFF); break;
            case RigDef::Axle::OPTION_s_VISCOUS: diff->AddDifferentialType(VISCOUS_DIFF); break;
            default: AddMessage(Message::TYPE_WARNING, "Unknown differential type: " + *itor); break;
            }
        }
    }
...
void ActorSpawner::ProcessTorqueCurve(RigDef::TorqueCurve &def)
{
    ...
    if (def.predefined_func_name.length() != 0) { target_torque_curve->setTorqueModel(def.predefined_func_name); }
    else
    {
...
void ActorSpawner::ProcessTie(RigDef::Tie &def)
{
    ...
    if (!def.is_invisible) { this->CreateBeamVisuals(beam, beam_index, false, def.beam_defaults); }
...
void ActorSpawner::ProcessSlidenode(RigDef::SlideNode &def)
{
    ...
    else if (def.rail_node_ranges.size() > 0)
    {
        ...
        if (rail_group != nullptr) { m_actor->m_railgroups.push_back(rail_group); }
    }
    else
    {
...
int ActorSpawner::FindNodeIndex(RigDef::Node::Ref &node_ref, bool silent /* Default: false */)
{
    ...
    if (result.second) { return static_cast<int>(result.first); }
    else
    {
        if (!silent)
        {
                                          ...
                                          std::vector<unsigned int> &       out_node_indices)
{
    ...
    for (; itor != node_ranges.end(); itor++)
    {
        if (itor->IsRange())
        {
            ...
            {
                std::stringstream msg;
                msg << "Encountered non-existent node '" << itor->end.ToString() << "' in range [" << itor->start.ToString()
                    << " - " << itor->end.ToString() << "], "
...
beam_t *ActorSpawner::FindBeamInRig(unsigned int node_a_index, unsigned int node_b_index)
{
    ...
    {
        if ((GetBeam(i).p1 == node_a && GetBeam(i).p2 == node_b) || (GetBeam(i).p2 == node_a && GetBeam(i).p1 == node_b))
        { return &GetBeam(i); }
    }
    return nullptr;
}
...
void ActorSpawner::ProcessHook(RigDef::Hook &def)
{
    ...
    if (def.flag_auto_lock)
    {
        ...
        if (hook->hk_group == -1) { hook->hk_group = -2; /* only overwrite hgroup when its still default (-1) */ }
    }
    if (def.flag_no_rope) { hook->hk_beam->bounded = NOSHOCK; }
...
void ActorSpawner::ProcessTrigger(RigDef::Trigger &def)
{
    ...
    if (def.HasFlag_B_TriggerBlocker()) { BITMASK_SET_1(shock_flags, SHOCK_FLAG_TRG_BLOCKER); }
    if (def.HasFlag_s_CmdNumSwitch()) // switch that exchanges cmdshort/cmdshort for all triggers with the same commandnumbers,
                                      // default false
    ...
    if (def.HasFlag_A_InvTriggerBlocker()) // Blocker that enable/disable other triggers, reversed activation method (inverted
                                           // Blocker style, auto-ON)
    { BITMASK_SET_1(shock_flags, SHOCK_FLAG_TRG_BLOCKER_A); }
    if (def.HasFlag_h_UnlocksHookGroup()) { BITMASK_SET_1(shock_flags, SHOCK_FLAG_TRG_HOOK_UNLOCK); }
    if (def.HasFlag_H_LocksHookGroup()) { BITMASK_SET_1(shock_flags, SHOCK_FLAG_TRG_HOOK_LOCK); }
    if (def.HasFlag_t_Continuous())
    {
        BITMASK_SET_1(shock_flags, SHOCK_FLAG_TRG_CONTINUOUS); // this trigger sends values between 0 and 1
    }
    if (def.HasFlag_E_EngineTrigger()) { BITMASK_SET_1(shock_flags, SHOCK_FLAG_TRG_ENGINE); }
    ...
    if (!def.IsTriggerBlockerAnyType() && !def.IsHookToggleTrigger() && !def.HasFlag_E_EngineTrigger())
    {
        ...
        {
            std::stringstream msg;
            msg << "Invalid value of 'shortbound_trigger_action': '" << def.shortbound_trigger_action
                << "'. Must be between 1 and " << MAX_COMMANDS << ". Ignoring trigger.";
            AddMessage(Message::TYPE_ERROR, msg.str());
            return;
        }
    }
    ...
    if (node_1_index == -1 || node_2_index == -1) { return; }
    int     beam_index = m_actor->ar_num_beams;
    ...
    if (!def.HasFlag_i_Invisible()) { this->CreateBeamVisuals(beam, beam_index, true, def.beam_defaults); }

    if (m_actor->m_trigger_debug_enabled) { LOG("Trigger added. BeamID " + TOSTRING(beam_index)); }
...
void ActorSpawner::ProcessRotator(RigDef::Rotator &def)
{
    ...
    this->_ProcessKeyInertia(def.inertia, *def.inertia_defaults, m_actor->ar_command_key[def.spin_left_key].rotator_inertia,
...
void ActorSpawner::ProcessRotator2(RigDef::Rotator2 &def)
{
    ...
    // Rotate left key
    m_actor->ar_command_key[def.spin_left_key].rotators.push_back(-(m_actor->ar_num_rotators + 1));
    if (!def.description.empty()) { m_actor->ar_command_key[def.spin_left_key].description = def.description; }
    else
    {
        m_actor->ar_command_key[def.spin_left_key].description = "Rotate_Left/Right";
    }
...
void ActorSpawner::_ProcessKeyInertia(RigDef::Inertia &inertia, RigDef::Inertia &inertia_defaults,
                                      RoR::CmdKeyInertia &contract_cmd, RoR::CmdKeyInertia &extend_cmd)
{
    /* Handle placeholders */
    ...
    {
        contract_cmd.SetCmdKeyDelay(App::GetSimController()->GetBeamFactory()->GetInertiaConfig(), inertia.start_delay_factor,
                                    inertia.stop_delay_factor, start_function, stop_function);

        extend_cmd.SetCmdKeyDelay(App::GetSimController()->GetBeamFactory()->GetInertiaConfig(), inertia.start_delay_factor,
                                  inertia.stop_delay_factor, start_function, stop_function);
    }
    else if (inertia_defaults.start_delay_factor > 0 || inertia_defaults.stop_delay_factor > 0)
    {
        contract_cmd.SetCmdKeyDelay(App::GetSimController()->GetBeamFactory()->GetInertiaConfig(),
                                    inertia_defaults.start_delay_factor, inertia_defaults.stop_delay_factor,
                                    inertia_defaults.start_function, inertia_defaults.stop_function);

        extend_cmd.SetCmdKeyDelay(App::GetSimController()->GetBeamFactory()->GetInertiaConfig(),
                                  inertia_defaults.start_delay_factor, inertia_defaults.stop_delay_factor,
                                  inertia_defaults.start_function, inertia_defaults.stop_function);
    }
}
...
void ActorSpawner::ProcessCommand(RigDef::Command2 &def)
{
    ...
    beam_t &beam =
        AddBeam(m_actor->ar_nodes[node_1_index], m_actor->ar_nodes[node_2_index], def.beam_defaults, def.detacher_group);
    CalculateBeamLength(beam);
    ...
    if (contract_command->description.empty()) { contract_command->description = def.description; }
    ...
    if (extend_command->description.empty()) { extend_command->description = def.description; }

    this->_ProcessKeyInertia(def.inertia, *def.inertia_defaults, contract_command->command_inertia,
                             extend_command->command_inertia);

    if (!def.option_i_invisible) { this->CreateBeamVisuals(beam, beam_index, true, def.beam_defaults); }
...
void ActorSpawner::ProcessAnimator(RigDef::Animator &def)
{
    ...
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_AIRSPEED)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_AIRSPEED); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_VERTICAL_VELOCITY)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_VVI); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_ANGLE_OF_ATTACK)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_AOA); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_FLAP)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_FLAP); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_AIR_BRAKE)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_AIRBRAKE); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_ROLL)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_ROLL); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_PITCH)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_PITCH); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_BRAKES)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_BRAKE); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_ACCEL)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_ACCEL); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_CLUTCH)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_CLUTCH); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_SPEEDO)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_SPEEDO); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_TACHO)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_TACHO); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_TURBO)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_TURBO); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_PARKING)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_PBRAKE); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_TORQUE)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_TORQUE); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_BOAT_THROTTLE)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_BTHROTTLE); }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_BOAT_RUDDER)) { BITMASK_SET_1(anim_flags, ANIM_FLAG_BRUDDER); }
    ...
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_SHORT_LIMIT)) { beam.shortbound = def.short_limit; }
    if (BITMASK_IS_1(def.flags, RigDef::Animator::OPTION_LONG_LIMIT)) { beam.longbound = def.long_limit; }
    ...
    {
        hb.hb_inertia.SetCmdKeyDelay(App::GetSimController()->GetBeamFactory()->GetInertiaConfig(),
                                     def.inertia_defaults->start_delay_factor, def.inertia_defaults->stop_delay_factor,
                                     def.inertia_defaults->start_function, def.inertia_defaults->stop_function);
    }

    m_actor->ar_hydros.push_back(hb);
}

beam_t &ActorSpawner::AddBeam(node_t &node_1, node_t &node_2, std::shared_ptr<RigDef::BeamDefaults> &beam_defaults,
                              int detacher_group)
{
    /* Init */
...
void ActorSpawner::ProcessHydro(RigDef::Hydro &def)
{
    ...
    else
    {
        for (unsigned int i = 0; i < def.options.length(); ++i)
        {
            ...
            case RigDef::Hydro::OPTION_h_INPUT_InvELEVATOR_RUDDER: // 'h':
                ...
                break;
            default: this->AddMessage(Message::TYPE_WARNING, std::string("Ignoring invalid flag:") + c); break;
            }
    ...
    if (!invisible) { this->CreateBeamVisuals(beam, beam_index, true, def.beam_defaults); }
...
void ActorSpawner::ProcessShock3(RigDef::Shock3 &def)
{
    ...
    if (BITMASK_IS_1(def.options, RigDef::Shock3::OPTION_M_ABSOLUTE_METRIC))
    {
        ...
        if (long_bound < 0.f)
        {
            AddMessage(Message::TYPE_WARNING, "Metric shock length calculation failed, 'short_bound' less than beams spawn "
                                              "length. Resetting to beam's spawn length (short_bound = 0)");
            long_bound = 0.f;
        }

        if (short_bound > 1.f)
        {
            AddMessage(Message::TYPE_WARNING, "Metric shock length calculation failed, 'short_bound' less than 0 meters. "
                                              "Resetting to 0 meters (short_bound = 1)");
            short_bound = 1.f;
        }
    }
...
void ActorSpawner::FetchAxisNodes(node_t *&axis_node_1, node_t *&axis_node_2, RigDef::Node::Ref const &axis_node_1_id,
...
void ActorSpawner::ProcessFlexBodyWheel(RigDef::FlexBodyWheel &def)
{
    ...
    for (unsigned int i = 0; i < def.num_rays; i++)
    {
        ...
        // reinforcement (tire tread)
        {
            ...
            AddWheelBeam(&m_actor->ar_nodes[rimnode + rays * 2], &m_actor->ar_nodes[base_node_index + i * 2 + 1 + rays * 2],
                         tread_spring, tread_damp, def.beam_defaults);
            AddWheelBeam(&m_actor->ar_nodes[rimnode + rays * 2],
                         &m_actor->ar_nodes[base_node_index + ((i + 1) % rays) * 2 + rays * 2], tread_spring, tread_damp,
                         def.beam_defaults);
            AddWheelBeam(&m_actor->ar_nodes[base_node_index + i * 2 + 1 + rays * 2],
                         &m_actor->ar_nodes[base_node_index + ((i + 1) % rays) * 2 + 1 + rays * 2], tread_spring, tread_damp,
                         def.beam_defaults);
            AddWheelBeam(&m_actor->ar_nodes[rimnode + 1 + rays * 2],
                         &m_actor->ar_nodes[base_node_index + ((i + 1) % rays) * 2 + rays * 2], tread_spring, tread_damp,
                         def.beam_defaults);
            ...
            if (rigidity_node != nullptr)
            {
                ...
                unsigned int beam_index =
                    AddWheelBeam(rigidity_node, axis_node_closest_to_rigidity_node, tyre_spring, tyre_damp, def.beam_defaults);
...
void ActorSpawner::ProcessMeshWheel(RigDef::MeshWheel &meshwheel_def)
{
    ...
    unsigned int wheel_index = BuildWheelObjectAndNodes(
        meshwheel_def.num_rays, axis_node_1, axis_node_2, GetNodePointer(meshwheel_def.reference_arm_node),
        meshwheel_def.num_rays * 2, meshwheel_def.num_rays * 8, meshwheel_def.tyre_radius, meshwheel_def.propulsion,
        meshwheel_def.braking, meshwheel_def.node_defaults, meshwheel_def.mass);

    BuildWheelBeams(meshwheel_def.num_rays, base_node_index, axis_node_1, axis_node_2, meshwheel_def.spring, /* Tyre */
                    meshwheel_def.damping,                                                                   /* Tyre */
                    meshwheel_def.spring,                                                                    /* Rim */
                    meshwheel_def.damping,                                                                   /* Rim */
                    meshwheel_def.beam_defaults, meshwheel_def.rigidity_node);
...
void ActorSpawner::ProcessMeshWheel2(RigDef::MeshWheel &def)
{
    ...
    unsigned int wheel_index =
        BuildWheelObjectAndNodes(def.num_rays, axis_node_1, axis_node_2, GetNodePointer(def.reference_arm_node), def.num_rays * 2,
                                 def.num_rays * 8, def.tyre_radius, def.propulsion, def.braking, def.node_defaults, def.mass);
    ...
    BuildWheelBeams(def.num_rays, base_node_index, axis_node_1, axis_node_2, tyre_spring, tyre_damp, rim_spring, rim_damp,
                    def.beam_defaults, def.rigidity_node,
                    0.15 // max_extension
    );
...
void ActorSpawner::BuildMeshWheelVisuals(unsigned int wheel_index, unsigned int base_node_index, unsigned int axis_node_1_index,
                                         unsigned int axis_node_2_index, unsigned int num_rays, Ogre::String mesh_name,
                                         Ogre::String material_name, float rim_radius, bool rim_reverse)
{
    try
    {
        FlexMeshWheel *flexmesh_wheel =
            m_flex_factory.CreateFlexMeshWheel(wheel_index, axis_node_1_index, axis_node_2_index, base_node_index, num_rays,
                                               rim_radius, rim_reverse, mesh_name, material_name);
...
unsigned int ActorSpawner::BuildWheelObjectAndNodes(unsigned int num_rays, node_t *axis_node_1, node_t *axis_node_2,
                                                    node_t *reference_arm_node, unsigned int reserve_nodes,
                                                    unsigned int reserve_beams, float wheel_radius,
                                                    RigDef::Wheels::Propulsion propulsion, RigDef::Wheels::Braking braking,
                                                    std::shared_ptr<RigDef::NodeDefaults> node_defaults, float wheel_mass,
...
)
{
    ...
    std::stringstream msg;
    msg << "\nDBG ActorSpawner::BuildWheelObjectAndNodes()\nDBG nodebase:" << m_actor->ar_num_nodes
        << ", axis-node-0:" << axis_node_1->pos << ", axis-node-1:" << axis_node_2->pos << "\n";
    ...
    for (unsigned int i = 0; i < num_rays; i++)
    {
        ...
        msg << "\nDBG\tN1: index=" << outer_node.pos + modifier << ", iswheel=" << WHEEL_DEFAULT
            << ", X=" << outer_node.AbsPosition.x << ", Y=" << outer_node.AbsPosition.y << ", Z=" << outer_node.AbsPosition.z
            << std::endl
...
void ActorSpawner::BuildWheelBeams(unsigned int num_rays, unsigned int base_node_index, node_t *axis_node_1, node_t *axis_node_2,
                                   float tyre_spring, float tyre_damping, float rim_spring, float rim_damping,
                                   std::shared_ptr<RigDef::BeamDefaults> beam_defaults, RigDef::Node::Ref const &rigidity_node_id,
...
)
{
    ...
    for (unsigned int i = 0; i < num_rays; i++)
    {
        ...
        if (rigidity_node != nullptr)
        {
            ...
            unsigned int beam_index =
                AddWheelBeam(rigidity_node, target_node, tyre_spring, tyre_damping, beam_defaults, -1.f, -1.f, BEAM_VIRTUAL);
...
unsigned int ActorSpawner::AddWheel(RigDef::Wheel &wheel_def)
{
    ...
    unsigned int wheel_index =
        BuildWheelObjectAndNodes(wheel_def.num_rays, axis_node_1, axis_node_2, GetNodePointer(wheel_def.reference_arm_node),
                                 wheel_def.num_rays * 2, wheel_def.num_rays * 8, wheel_def.radius, wheel_def.propulsion,
                                 wheel_def.braking, wheel_def.node_defaults, wheel_def.mass,
                                 -1.f // Set width to axis length (width in definition is ignored)
        );

    BuildWheelBeams(wheel_def.num_rays, base_node_index, axis_node_1, axis_node_2, wheel_def.springiness, /* Tyre */
                    wheel_def.damping,                                                                    /* Tyre */
                    wheel_def.springiness,                                                                /* Rim */
                    wheel_def.damping,                                                                    /* Rim */
                    wheel_def.beam_defaults, wheel_def.rigidity_node);
...
unsigned int ActorSpawner::AddWheel2(RigDef::Wheel2 &wheel_2_def)
{
    ...
    for (unsigned int i = 0; i < wheel_2_def.num_rays; i++)
    {
        ...
        if (wheel_2_def.rigidity_node.IsValidAnyState())
        {
            unsigned int rig_beam_index               = AddWheelRimBeam(wheel_2_def, GetNodePointer(wheel_2_def.rigidity_node),
...
void ActorSpawner::CreateWheelVisuals(unsigned int wheel_index, unsigned int node_base_index, unsigned int num_rays,
                                      Ogre::String const &rim_material_name, Ogre::String const &band_material_name,
                                      bool separate_rim, float rim_ratio)
{
    ...
    try
    {
        ...
        visual_wheel.wx_is_meshwheel      = false;
        visual_wheel.wx_flex_mesh =
            new FlexMesh(wheel_mesh_name, m_actor->m_gfx_actor.get(), wheel.wh_axis_node_0->pos, wheel.wh_axis_node_1->pos,
                         node_base_index, num_rays, rim_material_name, band_material_name, separate_rim, rim_ratio);
...
unsigned int ActorSpawner::AddWheelBeam(node_t *node_1, node_t *node_2, float spring, float damping,
...
void ActorSpawner::ProcessTractionControl(RigDef::TractionControl &def)
{
    ...
    if (pulse <= 1.0f || pulse >= 2000.0f) { pulse = 2000.0f; }
...
void ActorSpawner::ProcessBrakes(RigDef::Brakes &def)
{
    ...
    if (def.parking_brake_force != -1.f) { m_actor->m_handbrake_force = def.parking_brake_force; }
};
...
void ActorSpawner::ProcessEngturbo(RigDef::Engturbo &def)
{
    ...
    {
        if (module_itor->get()->engturbo != nullptr) { engturbo = module_itor->get()->engturbo; }
    }

    /* Process it */
    m_actor->ar_engine->SetTurboOptions(engturbo->version, engturbo->tinertiaFactor, engturbo->nturbos, engturbo->param1,
                                        engturbo->param2, engturbo->param3, engturbo->param4, engturbo->param5, engturbo->param6,
                                        engturbo->param7, engturbo->param8, engturbo->param9, engturbo->param10,
                                        engturbo->param11);
};
...
void ActorSpawner::ProcessEngoption(RigDef::Engoption &def)
{
    ...
    for (; module_itor != m_selected_modules.end(); module_itor++)
    {
        if (module_itor->get()->engoption != nullptr) { engoption = module_itor->get()->engoption; }
    }
    ...
    /* Process it */
    m_actor->ar_engine->SetEngineOptions(engoption->inertia, engoption->type, engoption->clutch_force, engoption->shift_time,
                                         engoption->clutch_time, engoption->post_shift_time, engoption->idle_rpm,
                                         engoption->stall_rpm, engoption->max_idle_mixture, engoption->min_idle_mixture,
...
void ActorSpawner::ProcessEngine(RigDef::Engine &def)
{
    ...
    m_actor->ar_engine =
        new EngineSim(def.shift_down_rpm, def.shift_up_rpm, def.torque, gears_compat, def.global_gear_ratio, m_actor);
...
void ActorSpawner::ProcessAuthors()
{
    ...
    for (; author_itor != m_file->authors.end(); author_itor++)
    {
        ...
        if (author_itor->_has_forum_account) { author.id = author_itor->forum_account_id; }
...
void ActorSpawner::ProcessCamera(RigDef::Camera &def)
{
    ...
    { m_actor->ar_camera_node_pos[m_actor->ar_num_cameras] = static_cast<int>(GetNodeIndexOrThrow(def.center_node)); }
    else
    {
    ...
    { m_actor->ar_camera_node_dir[m_actor->ar_num_cameras] = static_cast<int>(GetNodeIndexOrThrow(def.back_node)); }
    else
    {
    ...
    { m_actor->ar_camera_node_roll[m_actor->ar_num_cameras] = static_cast<int>(GetNodeIndexOrThrow(def.left_node)); }
    else
    {
...
void ActorSpawner::ProcessBeam(RigDef::Beam &def)
{
    ...
    /* Options */
    if (BITMASK_IS_1(def.options, RigDef::Beam::OPTION_r_ROPE)) { beam.bounded = ROPE; }
...
void ActorSpawner::SetBeamDeformationThreshold(beam_t &beam, std::shared_ptr<RigDef::BeamDefaults> beam_defaults)
{
    ...
    */
    ...
    if (beam_defaults->_is_user_defined)
    {
        ...
        if (!beam_defaults->_enable_advanced_deformation && default_deform < BEAM_DEFORM) { default_deform = BEAM_DEFORM; }
...
void ActorSpawner::CreateBeamVisuals(beam_t const &beam, int beam_index, bool visible,
                                     std::shared_ptr<RigDef::BeamDefaults> const &beam_defaults, std::string material_override)
{
    std::string material_name = material_override;
    if (material_name.empty())
    {
        if (beam.bm_type == BEAM_HYDRO) { material_name = "tracks/Chrome"; }
        else
        {
            ...
            if (it != m_managed_materials.end())
            {
                ...
                if (!material.isNull()) { material_name = material->getName(); }
            }
        }
    }
...
std::pair<unsigned int, bool> ActorSpawner::GetNodeIndex(RigDef::Node::Ref const &node_ref, bool quiet /* Default: false */)
{
    ...
    if (is_named)
    {
        ...
        if (result != m_named_nodes.end()) { return std::make_pair(result->second, true); }
        else if (!quiet)
        {
            std::stringstream msg;
    ...
    else
    {
        ...
        if (!is_imported && (node_ref.Num() >= static_cast<unsigned int>(m_actor->ar_num_nodes)))
        {
            ...
            {
                std::stringstream msg;
                msg << "Failed to resolve node-ref (node index too big, node count is: " << m_actor->ar_num_nodes
                    << "): " << node_ref.ToString();
                AddMessage(Message::TYPE_ERROR, msg.str());
            }
            return std::make_pair(0, false);
...
{
    std::pair<unsigned int, bool> result = GetNodeIndex(node_ref);
    if (result.second) { return &m_actor->ar_nodes[result.first]; }
    else
    {
        return nullptr;
    }
...
std::pair<unsigned int, bool> ActorSpawner::AddNode(RigDef::Node::Id &id)
{
    ...
    {
        std::stringstream msg;
        msg << "Attempt to add node with 'INVALID' flag: " << id.ToString()
            << " (number of nodes at this point: " << m_actor->ar_num_nodes << ")";
    ...
    if (id.IsTypeNamed())
    {
        ...
        {
            std::stringstream msg;
            msg << "Ignoring named node! Duplicate name: " << id.Str()
                << " (number of nodes at this point: " << m_actor->ar_num_nodes << ")";
            this->AddMessage(Message::TYPE_ERROR, msg.str());
            return std::make_pair(0, false);
        }
    ...
    if (id.IsTypeNumbered())
    {
        ...
        {
            std::stringstream msg;
            msg << "Duplicate node number, previous definition will be overriden! - " << id.ToString()
                << " (number of nodes at this point: " << m_actor->ar_num_nodes << ")";
...
void ActorSpawner::ProcessNode(RigDef::Node &def)
{
    ...
    if (!inserted_node.second) { return; }
    ...
    /* Lockgroup */
    node.nd_lockgroup =
        (m_file->lockgroup_default_nolock) ? RigDef::Lockgroup::LOCKGROUP_NOLOCK : RigDef::Lockgroup::LOCKGROUP_DEFAULT;
...
void ActorSpawner::AddExhaust(unsigned int emitter_node_idx, unsigned int direction_node_idx)
{
    if (m_actor->m_disable_smoke) { return; }
    exhaust_t exhaust;
    ...
    exhaust.smoker =
        gEnv->sceneManager->createParticleSystem(this->ComposeName("Exhaust", static_cast<int>(m_actor->exhausts.size())),
    ...
    for (GfxActor::NodeGfx &nfx : m_gfx_nodes)
    {
        if (nfx.nx_node_idx == emitter_node_idx || nfx.nx_node_idx == direction_node_idx) { nfx.nx_is_hot = true; }
    }
...
void ActorSpawner::ProcessCinecam(RigDef::Cinecam &def)
{
    ...
    // NOTE: Not applying the 'node_mass' value here for backwards compatibility - this node must go through initial
    // `Actor::RecalculateNodeMasses()` pass with default weight.
...
void ActorSpawner::InitNode(node_t &node, Ogre::Vector3 const &position, std::shared_ptr<RigDef::NodeDefaults> node_defaults)
{
...
void ActorSpawner::ProcessGlobals(RigDef::Globals &def)
{
    ...
    if (!def.material_name.empty())
    {
        Ogre::MaterialPtr mat =
            Ogre::MaterialManager::getSingleton().getByName(def.material_name); // Check if exists (compatibility)
        if (!mat.isNull()) { m_cab_material_name = def.material_name; }
        else
        {
            std::stringstream msg;
            msg << "Material '" << def.material_name
...
void ActorSpawner::SetupDefaultSoundSources(Actor *vehicle)
{
    ...
    if (vehicle->ar_engine != nullptr) /* Land vehicle */
    {
        ...
        if (vehicle->ar_engine->m_engine_type == 'c') AddSoundSourceInstance(vehicle, "tracks/default_car", ar_exhaust_pos_node);
        ...
        if (vehicle->ar_engine->m_engine_has_air) AddSoundSourceInstance(vehicle, "tracks/default_air_purge", 0);
        // starter
    ...
    // pump
    if (vehicle->m_has_command_beams) { AddSoundSourceInstance(vehicle, "tracks/default_pump", 0); }
    // antilock brake
    if (vehicle->alb_mode || !vehicle->alb_notoggle) { AddSoundSourceInstance(vehicle, "tracks/default_antilock", 0); }
    // tractioncontrol
    if (vehicle->tc_mode || !vehicle->tc_notoggle) { AddSoundSourceInstance(vehicle, "tracks/default_tractioncontrol", 0); }
    // screetch
    ...
    {
        AddSoundSource(vehicle,
                       SoundScriptManager::getSingleton().createInstance(Ogre::String("tracks/linked/default_command/extend"),
                                                                         trucknum, NULL, SL_COMMAND, i),
                       0);
        AddSoundSource(vehicle,
                       SoundScriptManager::getSingleton().createInstance(Ogre::String("tracks/linked/default_command/retract"),
                                                                         trucknum, NULL, SL_COMMAND, -i),
                       0);
    }
...
std::string ActorSpawner::ProcessMessagesToString()
{
    ...
    for (; itor != end; ++itor)
    {
        switch (itor->type)
        {
        case (Message::TYPE_INTERNAL_ERROR): report << "#FF3300 INTERNAL ERROR #FFFFFF"; break;

        case (Message::TYPE_ERROR): report << "#FF3300 ERROR #FFFFFF"; break;

        case (Message::TYPE_WARNING): report << "#FFFF00 WARNING #FFFFFF"; break;
...
RigDef::MaterialFlareBinding *ActorSpawner::FindFlareBindingForMaterial(std::string const &material_name)
{
    for (auto &module : m_selected_modules)
    {
        ...
        {
            if (def.material_name == material_name) { return &def; }
        }
    }
    return nullptr;
}
...
Ogre::MaterialPtr ActorSpawner::FindOrCreateCustomizedMaterial(std::string mat_lookup_name)
{
    try
    {
        ...
        if (lookup_res != m_material_substitutions.end()) { return lookup_res->second.material; }
        ...
        if (mat_lookup_name == "mirror")
        {
            ...
            ++mirror_counter;
            lookup_entry.material =
                Ogre::MaterialManager::getSingleton().getByName("mirror")->clone(new_mat_name, true, m_custom_resource_group);
        ...
        if (videocam_def != nullptr)
        {
            ...
            if (found_managedmat != m_managed_materials.end()) { video_mat_shared = found_managedmat->second; }
            else
            {
        ...
        if (mat_flare_def != nullptr) { lookup_entry.material_flare_def = mat_flare_def; }
        ...
        if (m_actor->m_used_skin_entry != nullptr)
        {
            ...
            {
                Ogre::MaterialPtr skin_mat =
                    Ogre::MaterialManager::getSingleton().getByName(skin_res->second, m_actor->m_used_skin_entry->resource_group);
                if (!skin_mat.isNull())
                {
                ...
                else
                {
                    ...
                    buf << "Material '" << skin_res->second << "' from skin '" << m_actor->m_used_skin_entry->dname
                        << "' not found (filename: '" << m_actor->m_used_skin_entry->fname << "', resource group: '"
                        << m_actor->m_used_skin_entry->resource_group << "')! Ignoring it...";
        ...
        else
        {
            ...
            Ogre::MaterialPtr orig_mat =
                Ogre::MaterialManager::getSingleton().getByName(mat_lookup_name, m_custom_resource_group);
            if (orig_mat.isNull())
            {
                std::stringstream buf;
                ...
                for (auto &tex_unit : pass->getTextureUnitStates())
                {
                    ...
                    else if (m_actor->m_used_skin_entry != nullptr)
                    {
                        ...
                        for (size_t i = 0; i < num_frames; ++i)
                        {
                            ...
                            const auto query =
                                m_actor->m_used_skin_entry->skin_def->replace_textures.find(tex_unit->getFrameTextureName(i));
                            if (query != end)
                            {
                                // Skin has replacement for this texture
                                if (m_actor->m_used_skin_entry->resource_group !=
                                    m_custom_resource_group) // The skin comes from a SkinZip bundle (different resource group)
                                {
...
void ActorSpawner::SetupNewEntity(Ogre::Entity *ent, Ogre::ColourValue simple_color)
{
    ...
    for (size_t i = 0; i < subent_max; ++i)
    {
        ...
        if (!subent->getMaterial().isNull())
        {
            ...
            if (!own_mat.isNull()) { subent->setMaterial(own_mat); }
        }
    }
}
...
{
    // Create the actor
    m_actor->m_gfx_actor = std::unique_ptr<RoR::GfxActor>(new RoR::GfxActor(
        m_actor, this, m_custom_resource_group, m_gfx_nodes, m_props, m_driverseat_prop_index, m_oldstyle_renderdash));
...
void ActorSpawner::FinalizeGfxSetup()
{
    ...
    for (auto &entry : m_material_substitutions)
    {
        ...
        { m_actor->m_gfx_actor->AddMaterialFlare(entry.second.material_flare_def->flare_number, entry.second.material); }
        else if (entry.second.mirror_prop_type != CustomMaterial::MirrorPropType::MPROP_NONE) // special 'prop' - rear view mirror
        {
            this->CreateMirrorPropVideoCam(entry.second.material, entry.second.mirror_prop_type,
                                           entry.second.mirror_prop_scenenode);
        }
        ...
        if (m_actor->ar_driveable == TRUCK) // load default for a truck
        {
            if (App::gfx_speedo_digital.GetActive())
            {
                if (App::gfx_speedo_imperial.GetActive())
                {
                    ...
                    {
                        m_actor->ar_dashboard->loadDashBoard("default_dashboard7000_mph.layout",
                                                             false); // 7000 rpm tachometer thanks to Klink
                ...
                else
                {
                    if (m_actor->ar_engine->getMaxRPM() > 3500)
                    {
                        m_actor->ar_dashboard->loadDashBoard("default_dashboard7000.layout",
                                                             false); // 7000 rpm tachometer thanks to Klink
            ...
            else // Analog speedometer
            {
                if (App::gfx_speedo_imperial.GetActive())
                {
                    if (m_actor->ar_engine->getMaxRPM() > 3500)
                    {
                        m_actor->ar_dashboard->loadDashBoard("default_dashboard7000_analog_mph.layout",
                                                             false); // 7000 rpm tachometer thanks to Klink
                ...
                else
                {
                    if (m_actor->ar_engine->getMaxRPM() > 3500)
                    {
                        m_actor->ar_dashboard->loadDashBoard("default_dashboard7000_analog.layout",
                                                             false); // 7000 rpm tachometer thanks to Klink
    ...
    if (!m_oldstyle_cab_texcoords.empty() && m_actor->ar_num_cabs > 0)
    {
        ...
        if (transmat->getTechnique(0)->getPass(0)->getNumTextureUnitStates() > 0)
        {
            backmat->getTechnique(0)->getPass(0)->getTextureUnitState(0)->setColourOperationEx(
                Ogre::LBX_SOURCE1, Ogre::LBS_MANUAL, Ogre::LBS_MANUAL, Ogre::ColourValue(0, 0, 0), Ogre::ColourValue(0, 0, 0));
        }
        if (App::gfx_reduce_shadows.GetActive()) { backmat->setReceiveShadows(false); }
        backmat->compile();
        ...
        FlexObj *   cab_mesh  = new FlexObj(m_actor->m_gfx_actor.get(), m_actor->ar_nodes, m_oldstyle_cab_texcoords,
                                        m_actor->ar_num_cabs, m_actor->ar_cabs, m_oldstyle_cab_submeshes, cab_material_name_cstr,
                                        mesh_name.c_str(), backmatname, transmatname);
        ...
        try
        {
            ...
            if (ec) { cab_scene_node->attachObject(ec); }
    ...
    for (WheelVisualsTicket &ticket : m_wheel_visuals_queue)
    {
        ...
        {
            this->CreateWheelVisuals(ticket.wheel_index, ticket.base_node_index, ticket.wheel_def->num_rays,
                                     ticket.wheel_def->face_material_name, ticket.wheel_def->band_material_name, false);
        }
        else if (ticket.wheel2_def != nullptr)
        {
            this->CreateWheelVisuals(ticket.wheel_index, ticket.base_node_index, ticket.wheel2_def->num_rays,
                                     ticket.wheel2_def->face_material_name, ticket.wheel2_def->band_material_name, true,
                                     ticket.wheel2_def->rim_radius / ticket.wheel2_def->tyre_radius);
        }
        else if (ticket.meshwheel_def != nullptr)
        {
            this->BuildMeshWheelVisuals(ticket.wheel_index, ticket.base_node_index, ticket.axis_node_1, ticket.axis_node_2,
                                        ticket.meshwheel_def->num_rays, ticket.meshwheel_def->mesh_name,
                                        ticket.meshwheel_def->material_name, ticket.meshwheel_def->rim_radius,
        ...
        else if (ticket.flexbodywheel_def != nullptr)
        {
            ...
            try
            {
                auto *flexbody = m_flex_factory.CreateFlexBody(
                    &flexbody_def, ticket.axis_node_1, ticket.axis_node_2, static_cast<int>(ticket.base_node_index),
                    Ogre::Quaternion(Ogre::Degree(90), Ogre::Vector3::UNIT_Y), node_indices, m_custom_resource_group);
            ...
            {
                this->AddMessage(Message::TYPE_ERROR, "Failed to create flexbodywheel visuals '" + def.tyre_mesh_name +
                                                          "', reason:" + e.getFullDescription());
            }
        }
    }
...
void ActorSpawner::ValidateRotator(int id, int axis1, int axis2, int *nodes1, int *nodes2)
{
    ...
    if ((std::max(rot1, rot2) / std::min(rot1, rot2) > 1.f + eps) || (std::max(rot2, rot3) / std::min(rot2, rot3) > 1.f + eps) ||
        (std::max(rot3, rot4) / std::min(rot3, rot4) > 1.f + eps) || (std::max(rot4, rot1) / std::min(rot4, rot1) > 1.f + eps))
    {
...
Ogre::ManualObject *CreateVideocameraDebugMesh()
{
    ...
    Ogre::MaterialPtr mat     = Ogre::MaterialManager::getSingleton().create("VideoCamDebugMat-" + TOSTRING(counter),
...
void ActorSpawner::CreateVideoCamera(RigDef::VideoCamera *def)
{
    try
    {
        ...
        default:
            this->AddMessage(Message::TYPE_ERROR,
                             "VideoCamera (mat: " + def->material_name + ") has invalid 'role': " + TOSTRING(def->camera_role));
            return;
        }
        ...
        vcam.vcam_rotation = Ogre::Quaternion(Ogre::Degree(rotation_z), Ogre::Vector3::UNIT_Z) *
        ...
        if (!App::gfx_window_videocams.GetActive())
        {
            vcam.vcam_render_tex = Ogre::TextureManager::getSingleton().createManual(
                vcam.vcam_material->getName() + "_texture", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
                Ogre::TEX_TYPE_2D, def->texture_width, def->texture_height,
                0, // no mip maps
        ...
        else
        {
            ...
            vcam.vcam_render_window =
                Ogre::Root::getSingleton().createRenderWindow(window_name, def->texture_width, def->texture_height, false, &misc);
        ...
        if (vcam.vcam_render_target)
        {
            ...
            vcam.vcam_material->getTechnique(0)->getPass(0)->getTextureUnitState(0)->setTextureName(
                vcam.vcam_render_tex->getName());
...
void ActorSpawner::CreateMirrorPropVideoCam(Ogre::MaterialPtr custom_mat, CustomMaterial::MirrorPropType type,
                                            Ogre::SceneNode *prop_scenenode)
{
    ...
    try
    {
        ...
        switch (type)
        {
        case CustomMaterial::MirrorPropType::MPROP_LEFT: vcam.vcam_type = GfxActor::VideoCamType::VCTYPE_MIRROR_PROP_LEFT; break;

        case CustomMaterial::MirrorPropType::MPROP_RIGHT:
            vcam.vcam_type = GfxActor::VideoCamType::VCTYPE_MIRROR_PROP_RIGHT;
            break;

        default: this->AddMessage(Message::TYPE_ERROR, "Cannot create mirror prop of type 'MPROP_NONE'"); return;
        }

        // Create rendering texture
        const std::string mirror_tex_name = this->ComposeName("MirrorPropTexture-", static_cast<int>(mprop_counter));
        vcam.vcam_render_tex              = Ogre::TextureManager::getSingleton().createManual(
            mirror_tex_name, m_custom_resource_group, Ogre::TEX_TYPE_2D, 128, 256, 0, Ogre::PF_R8G8B8, Ogre::TU_RENDERTARGET);

        // Create OGRE camera
        vcam.vcam_ogre_camera =
            gEnv->sceneManager->createCamera(this->ComposeName("MirrorPropCamera-", static_cast<int>(mprop_counter)));
...
Ogre::ParticleSystem *ActorSpawner::CreateParticleSystem(std::string const &name, std::string const &template_name)
{
    ...
    Ogre::MovableObject *obj =
        gEnv->sceneManager->createMovableObject(name, Ogre::ParticleSystemFactory::FACTORY_TYPE_NAME, &params);
