...
struct commandbeam_state_t
{
    commandbeam_state_t()
    {
        memset(this, 0, sizeof(commandbeam_state_t));
    }
...
struct prop_t
{
    ...
    char beacontype; //<! Special prop: beacon {0 = none, 'b' = user-specified, 'r' = red, 'p' = police lightbar, 'L'/'R'/'w' -
                     // aircraft wings}
    ...
    float animratio[10]; //!< A coefficient for the animation, prop degree if used with mode: rotation and propoffset if used with
                         //!< mode: offset.
    int         animFlags[10];
    int         animMode[10];
