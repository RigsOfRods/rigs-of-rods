...
SlideNode::SlideNode(node_t *slidingNode, RailGroup *slidingRail)
    :
      ...
      m_sliding_node(slidingNode), m_sliding_beam(NULL), m_initial_railgroup(slidingRail), m_cur_railgroup(m_initial_railgroup),
      m_cur_rail_seg(NULL), m_node_forces_ratio(0.0f),

      m_initial_threshold(0.0f), m_cur_threshold(0.0f), m_spring_rate(9000000),
      m_break_force(std::numeric_limits<Ogre::Real>::infinity()), // beam won't break

      m_attach_rate(1.0f), m_attach_distance(0.1f), sn_attach_foreign(false), sn_attach_self(false), sn_slide_broken(false)
{
...
void SlideNode::UpdateForces(float dt)
{
    ...
    if (!m_sliding_beam || m_sliding_beam->bm_broken || sn_slide_broken) { return; }
    ...
    if (m_cur_threshold > m_initial_threshold) { m_cur_threshold -= (m_attach_rate * dt); }
    ...
    if (perpForces.length() > m_break_force) { sn_slide_broken = true; }
...
RailSegment *RailSegment::CheckCurSlideSegment(const Ogre::Vector3 &point)
{
    ...
    if (this->rs_next != nullptr)
    {
        ...
        if (dist_sq < closest_dist_sq) { closest_seg = this->rs_next; }
    }
