...
Actor::~Actor()
{
    ...
    if (m_fusealge_airfoil) delete m_fusealge_airfoil;
    m_fusealge_airfoil = 0;

    if (m_replay_handler) delete m_replay_handler;
    m_replay_handler = nullptr;

    if (ar_vehicle_ai) delete ar_vehicle_ai;
    ar_vehicle_ai = 0;
    ...
    if (m_deletion_scene_nodes.size() > 0)
    {
        ...
        {
            if (!m_deletion_scene_nodes[i]) continue;
    ...
    if (m_deletion_entities.size() > 0)
    {
        ...
        {
            if (!m_deletion_entities[i]) continue;
    ...
    {
        // flexAirfoil, airfoil
        if (ar_wings[i].fa) delete ar_wings[i].fa;
        if (ar_wings[i].cnode)
        {
    ...
    {
        if (ar_aeroengines[i]) delete ar_aeroengines[i];
    }
    ...
    for (size_t i = 0; i < this->ar_flares.size(); i++)
    {
        ...
        if (ar_flares[i].bbs) gEnv->sceneManager->destroyBillboardSet(ar_flares[i].bbs);
        if (ar_flares[i].light) gEnv->sceneManager->destroyLight(ar_flares[i].light);
    }
    ...
    if (m_transfer_case) delete m_transfer_case;

    for (int i = 0; i < m_num_axle_diffs; ++i)
    {
        if (m_axle_diffs[i] != nullptr) delete m_axle_diffs[i];
    }

    for (int i = 0; i < m_num_wheel_diffs; ++i)
    {
        if (m_wheel_diffs[i] != nullptr) delete m_wheel_diffs[i];
    }
...
void Actor::PushNetwork(char *data, int size)
{
    ...
    else
    {
        if (!m_net_initialized)
        {
            ...
            Networking::AddPacket(reg.origin_streamid, RoRnet::MSG2_STREAM_REGISTER_RESULT, sizeof(RoRnet::StreamRegister),
                                  (char *)&reg);
            ...
            UTFString message = RoR::ChatSystem::GetColouredName(info.username, info.colournum) + RoR::Color::WarningColour +
                                _L(" content mismatch: ") + RoR::Color::NormalColour + reg.name;
...
void Actor::CalcNetwork()
{
    ...
    if (m_net_updates.size() < 2) return;
    ...
    for (int i = 0; i < ar_num_wheels; i++)
    {
        ...
        Vector3 ortho =
            -pplan.projectVector(ar_wheels[i].wh_near_attach_node->AbsPosition) - ar_wheels[i].wh_axis_node_0->AbsPosition;
    ...
    if (ar_engine) { SOUND_MODULATE(ar_instance_id, SS_MOD_ENGINE, engspeed); }
    ...
    if (ar_engine)
    {
        ...
        {
            automode = static_cast<int>(SimGearboxMode::SEMI_AUTO);
        }
        else if ((flagmask & NETMASK_ENGINE_MODE_MANUAL) != 0)
        {
            automode = static_cast<int>(SimGearboxMode::MANUAL);
        }
        else if ((flagmask & NETMASK_ENGINE_MODE_MANUAL_STICK) != 0)
        {
            automode = static_cast<int>(SimGearboxMode::MANUAL_STICK);
        }
        else if ((flagmask & NETMASK_ENGINE_MODE_MANUAL_RANGES) != 0)
        {
            automode = static_cast<int>(SimGearboxMode::MANUAL_RANGES);
        }
    ...
    // set particle cannon
    if (((flagmask & NETMASK_PARTICLE) != 0) != m_custom_particles_enabled) ToggleCustomParticles();

    // set lights
    if (((flagmask & NETMASK_LIGHTS) != 0) != ar_lights) ToggleLights();
    if (((flagmask & NETMASK_BEACONS) != 0) != m_beacon_light_is_active) ToggleBeacons();
...
{
    if (!ar_free_pressure_beam) return false;
    ...
    if (newpressure == m_ref_tyre_pressure) return false;
...
{
    if (ar_free_pressure_beam) return m_ref_tyre_pressure;
    return 0;
}
...
void Actor::RecalculateNodeMasses(Real total)
{
    ...
    for (int i = 0; i < ar_num_nodes; i++)
    {
        ...
        {
            if (!ar_nodes[i].nd_loaded_mass) { ar_nodes[i].mass = 0; }
    ...
    for (int i = 0; i < ar_num_beams; i++)
    {
        if (ar_beams[i].bm_type != BEAM_VIRTUAL)
        {
            ...
            if (!ar_beams[i].p1->nd_tyre_node) len += half_newlen;
            if (!ar_beams[i].p2->nd_tyre_node) len += half_newlen;
        }
    }
    ...
    for (int i = 0; i < ar_num_beams; i++)
    {
        if (ar_beams[i].bm_type != BEAM_VIRTUAL)
        {
            ...
            if (!ar_beams[i].p1->nd_tyre_node) ar_beams[i].p1->mass += half_mass;
            if (!ar_beams[i].p2->nd_tyre_node) ar_beams[i].p2->mass += half_mass;
        }
    }
    // fix rope masses
    ...
    for (int i = 0; i < ar_num_nodes; i++)
    {
        if (!ar_nodes[i].nd_tyre_node && !(m_definition->minimass_skip_loaded_nodes && ar_nodes[i].nd_loaded_mass) &&
            ar_nodes[i].mass < ar_minimass[i])
        {
            if (App::diag_truck_mass.GetActive())
            {
                char buf[300];
                snprintf(buf, 300, "Node '%d' mass (%f Kg) is too light. Resetting to 'minimass' (%f Kg)", i, ar_nodes[i].mass,
                         ar_minimass[i]);
                LOG(buf);
            }
            ar_nodes[i].mass = ar_minimass[i];
...
{
    if (!withLocked) return m_total_mass; // already computed in RecalculateNodeMasses
...
void Actor::DetermineLinkedActors() // TODO: Refactor this - logic iterating over all actors should be in `ActorManager`! ~
                                    // only_a_ptr, 01/2018
{
    ...
    auto inter_actor_links =
        App::GetSimController()->GetBeamFactory()->inter_actor_links; // TODO: Shouldn't this have been a reference?? Also, ugly,
                                                                      // see the TODO note above ~ only_a_ptr, 01/2018
...
bool Actor::Intersects(Actor *actor, Vector3 offset)
{
    ...
    if (!bb.intersects(actor->ar_bounding_box)) return false;
    ...
    for (int i = 0; i < ar_num_beams; i++)
    {
        ...
        for (int j = 0; j < actor->ar_num_collcabs; j++)
        {
            ...
            if (result.first && result.second < 1.0f) { return true; }
        }
    }
...
Vector3 Actor::calculateCollisionOffset(Vector3 direction)
{
    ...
    while (collision_offset.length() < max_distance)
    {
        ...
        for (auto actor : App::GetSimController()->GetActors())
        {
            ...
            if (!bb.intersects(actor->ar_bounding_box)) continue;
            ...
            if (m_intra_point_col_detector)
            {
                for (int i = 0; i < actor->ar_num_collcabs; i++)
                {
                    ...
                    m_intra_point_col_detector->query(no->AbsPosition - collision_offset, na->AbsPosition - collision_offset,
                                                      nb->AbsPosition - collision_offset, actor->ar_collision_range * 3.0f);

                    if (collision = !m_intra_point_col_detector->hit_list.empty()) break;
                }
            ...
            for (int i = 0; i < ar_num_nodes; i++)
            {
                if (!ar_nodes[i].nd_contacter && !ar_nodes[i].nd_contactable) continue;

                Vector3 query_position = ar_nodes[i].AbsPosition + collision_offset;

                for (int j = 0; j < actor->ar_num_nodes; j++)
                {
                    if (!actor->ar_nodes[j].nd_contacter && !actor->ar_nodes[j].nd_contactable) continue;

                    if (collision = query_position.squaredDistance(actor->ar_nodes[j].AbsPosition) < proximity) break;
                }
        ...
        if (!collision && m_inter_point_col_detector)
        {
            for (int i = 0; i < ar_num_collcabs; i++)
            {
                ...
                m_inter_point_col_detector->query(no->AbsPosition + collision_offset, na->AbsPosition + collision_offset,
                                                  nb->AbsPosition + collision_offset, ar_collision_range * 3.0f);

                if (collision = !m_inter_point_col_detector->hit_list.empty()) break;
            }
        }
        ...
        if (!collision)
        {
            ...
            {
                if (actor == this) continue;
                if (collision = this->Intersects(actor, collision_offset)) break;
            }
        }
...
{
    if (m_intra_point_col_detector) m_intra_point_col_detector->UpdateIntraPoint(true);

    if (m_inter_point_col_detector) m_inter_point_col_detector->UpdateInterPoint(true);
    ...
    ResetPosition(ar_nodes[0].AbsPosition.x + offset.x, ar_nodes[0].AbsPosition.z + offset.z, false,
                  this->GetMinHeight() + offset.y);
}
...
void Actor::resolveCollisions(float max_distance, bool consider_up)
{
    ...
    if (consider_up)
    {
        ...
        if (up.length() * 1.2f < offset.length()) offset = up;
    }
    ...
    ResetPosition(ar_nodes[0].AbsPosition.x + offset.x, ar_nodes[0].AbsPosition.z + offset.z, true,
                  this->GetMinHeight() + offset.y);
}
...
void Actor::calculateAveragePosition()
{
    ...
    if (ar_custom_camera_node >= 0) { m_avg_node_position = ar_nodes[ar_custom_camera_node].AbsPosition; }
...
void Actor::ResetPosition(float px, float pz, bool setInitPosition, float miny)
{
    ...
    for (int i = 1; i < ar_num_nodes; i++)
    {
        if (ar_nodes[i].nd_no_ground_contact) continue;
...
{
    if (!ar_engine || !m_transfer_case || m_transfer_case->tr_ax_2 < 0 || !m_transfer_case->tr_2wd) return;
    ...
    if (m_transfer_case->tr_4wd_mode && !m_transfer_case->tr_2wd_lo)
    {
        for (int i = 0; i < m_transfer_case->tr_gear_ratios.size(); i++)
        {
            ...
            if (m_transfer_case->tr_gear_ratios[0] == 1.0f) break;
        }
    }
...
{
    if (!ar_engine || !m_transfer_case || m_transfer_case->tr_gear_ratios.size() < 2) return;
...
Ogre::Vector3 Actor::GetRotationCenter()
{
    ...
    for (int i = 0; i < ar_num_nodes; i++)
    {
        Vector3    pos = ar_nodes[i].AbsPosition;
        const auto it =
            std::find_if(positions.begin(), positions.end(), [pos](const Vector3 ref) { return pos.positionEquals(ref, 0.01f); });
        if (it == positions.end())
        {
            sum += pos;
...
float Actor::GetMinHeight(bool skip_virtual_nodes)
{
    ...
    for (int i = 0; i < ar_num_nodes; i++)
    {
        if (!skip_virtual_nodes || !ar_nodes[i].nd_no_ground_contact) { height = std::min(ar_nodes[i].AbsPosition.y, height); }
    }
...
float Actor::GetMaxHeight(bool skip_virtual_nodes)
{
    ...
    for (int i = 0; i < ar_num_nodes; i++)
    {
        if (!skip_virtual_nodes || !ar_nodes[i].nd_no_ground_contact) { height = std::max(height, ar_nodes[i].AbsPosition.y); }
    }
...
void Actor::SyncReset(bool reset_position)
{
    ...
    if (ar_engine)
    {
        if (App::sim_spawn_running.GetActive()) { ar_engine->StartEngine(); }
    ...
    if (ar_autopilot) this->ar_autopilot->reset();
    if (m_buoyance) m_buoyance->sink = false;
    ...
    if (!reset_position)
    {
        ...
        if (App::GetSimTerrain()->getWater())
        { agl = std::min(this->GetMinHeight(true) - App::GetSimTerrain()->getWater()->GetStaticWaterHeight(), agl); }
        if (agl < 0.0f) { this->ResetPosition(ar_nodes[0].AbsPosition - agl * Vector3::UNIT_Y, false); }
    }
    else
    {
        this->UpdateBoundingBoxes();
    ...
    if (m_slidenodes_locked) { this->ToggleSlideNodeLock(); }
...
{
    if (!ar_replay_mode || !m_replay_handler || !m_replay_handler->isValid()) return false;
...
void Actor::SearchBeamDefaults()
{
    ...
    for (int k = 0; k < ar_nb_measure_steps; k++)
    {
        ...
        for (int i = 0; i < ar_num_beams; i++)
        {
            ...
            if (k == 0 && ar_beams[i].bm_broken) { sum_broken++; }
        }
        if (sum_broken > ar_nb_reference[6] || stress > ar_nb_reference[0] || velocity > ar_nb_reference[2] ||
            movement > ar_nb_optimum[4] || sum_stress > ar_nb_reference[1] || sum_velocity > ar_nb_reference[3] ||
            sum_movement > ar_nb_optimum[5] * 2.f)
        {
    ...
    if (!ar_nb_initialized) { ar_nb_reference = ar_nb_optimum; }
    ar_nb_initialized = true;
}

void Actor::HandleInputEvents(float dt)
{
    if (!m_ongoing_reset) return;
...
void Actor::sendStreamSetup()
{
    ...
    strncpy(reg.name, ar_filename.c_str(), 128);
    if (m_used_skin_entry != nullptr) { strncpy(reg.skin, m_used_skin_entry->dname.c_str(), 60); }
...
void Actor::sendStreamData()
{
    using namespace RoRnet;
#ifdef USE_SOCKETW
    if (ar_net_timer.getMilliseconds() - ar_net_last_update_time < 100) return;
    ...
    // RoRnet::VehicleState is at the beginning of the buffer
    {
        ...
        if (ar_engine)
        {
            ...
            if (ar_engine->HasStarterContact()) send_oob->flagmask += NETMASK_ENGINE_CONT;
            if (ar_engine->IsRunning()) send_oob->flagmask += NETMASK_ENGINE_RUN;

            switch (ar_engine->GetAutoShiftMode())
            {
            case RoR::SimGearboxMode::AUTO: send_oob->flagmask += NETMASK_ENGINE_MODE_AUTOMATIC; break;
            case RoR::SimGearboxMode::SEMI_AUTO: send_oob->flagmask += NETMASK_ENGINE_MODE_SEMIAUTO; break;
            case RoR::SimGearboxMode::MANUAL: send_oob->flagmask += NETMASK_ENGINE_MODE_MANUAL; break;
            case RoR::SimGearboxMode::MANUAL_STICK: send_oob->flagmask += NETMASK_ENGINE_MODE_MANUAL_STICK; break;
            case RoR::SimGearboxMode::MANUAL_RANGES: send_oob->flagmask += NETMASK_ENGINE_MODE_MANUAL_RANGES; break;
            }
        }
        if (ar_num_aeroengines > 0)
        {
        ...
        if (ar_lights) send_oob->flagmask += NETMASK_LIGHTS;
        ...
        if (ar_parking_brake) send_oob->flagmask += NETMASK_PBRAKE;
        if (m_tractioncontrol) send_oob->flagmask += NETMASK_TC_ACTIVE;
        if (m_antilockbrake) send_oob->flagmask += NETMASK_ALB_ACTIVE;

        if (SOUND_GET_STATE(ar_instance_id, SS_TRIG_HORN)) send_oob->flagmask += NETMASK_HORN;
    }
...
void Actor::CalcAnimators(const int flag_state, float &cstate, int &div, Real timer, const float lower_limit,
                          const float upper_limit, const float option3)
{
    // ## DEV NOTE:
    // ## Until 06/2018, this function was used for both animator-beams (physics, part of softbody) and animated props
    // (visual-only).
    ...
    if (flag_state & ANIM_FLAG_DIFFLOCK)
    {
        if (m_num_wheel_diffs && m_wheel_diffs[0])
        {
            ...
            if (name == "Open") cstate = 0.0f;
            if (name == "Split") cstate = 0.5f;
            if (name == "Locked") cstate = 1.0f;
        }
    ...
    if (ar_engine && (flag_state & ANIM_FLAG_SHIFTER) && option3 == 3.0f)
    {
        ...
        if (!lower_limit && !upper_limit)
        {
            ...
            if (ar_anim_shift_timer > 0.0f)
            {
                ...
                if (ar_anim_shift_timer < 0.0f) ar_anim_shift_timer = 0.0f;
            }
            if (ar_anim_shift_timer < 0.0f)
            {
                cstate = -1.0f;
        ...
        else
        {
            ...
            if (lower_limit >= 1.0f && lower_limit <= 48.0)
                if (ar_command_key[int(lower_limit)].commandValue > 0) cstate += 1.0f;
            // check if upper_limit is a valid to get commandvalue, then get commandvalue
            if (upper_limit >= 1.0f && upper_limit <= 48.0)
                if (ar_command_key[int(upper_limit)].commandValue > 0) cstate -= 1.0f;
        }
    ...
    if (ftp > option3 - 1.0f)
    {
        ...
        if (flag_state & ANIM_FLAG_AESTATUS)
        {
            if (!ar_aeroengines[aenum]->getIgnition())
                ...
                cstate = 0.5f;
            if (ar_aeroengines[aenum]->isFailed()) cstate = 1.0f;
            div++;
        }
    }
    ...
    {
        float aoa = 0;
        if (ar_num_wings > 4) aoa = (ar_wings[4].fa->aoa) / 25.0f;
        if ((ar_nodes[0].Velocity.length() * 1.9438) < 10.0f) aoa = 0;
        cstate -= aoa;
        if (cstate <= -1.0f) cstate = -1.0f;
        if (cstate >= 1.0f) cstate = 1.0f;
        div++;
...
void Actor::CalcCabCollisions()
{
    ...
    if (m_intra_point_col_detector != nullptr)
    {
        ...
        ResolveIntraActorCollisions(PHYSICS_DT, *m_intra_point_col_detector, ar_num_collcabs, ar_collcabs, ar_cabs,
                                    ar_intra_collcabrate, ar_nodes, ar_collision_range, *ar_submesh_ground_model);
    }
}
...
{
    if ((ar_beams[i].shock->flags & SHOCK_FLAG_ISTRIGGER) &&
        ar_beams[i].shock->trigger_enabled) // this is a trigger and its enabled
    {
        const float dt = PHYSICS_DT;

        if (difftoBeamL > ar_beams[i].longbound * ar_beams[i].L ||
            difftoBeamL < -ar_beams[i].shortbound * ar_beams[i].L) // that has hit boundary
        {
            ...
            {
                for (int scount = i + 1; scount <= i + ar_beams[i].shock->trigger_cmdshort;
                     scount++) // (cycle blockerbeamID +1) to (blockerbeamID + beams to lock)
                {
                    if (ar_beams[scount].shock &&
                        (ar_beams[scount].shock->flags &
                         SHOCK_FLAG_ISTRIGGER)) // don't mess anything up if the user set the number too big
                    {
                        if (m_trigger_debug_enabled && !ar_beams[scount].shock->trigger_enabled &&
            ...
            else if (ar_beams[i].shock->flags &
                     SHOCK_FLAG_TRG_BLOCKER_A) // this is an enabled inverted blocker and inside boundary
            {
                for (int scount = i + 1; scount <= i + ar_beams[i].shock->trigger_cmdlong;
                     scount++) // (cycle blockerbeamID + 1) to (blockerbeamID + beams to release)
                {
                    if (ar_beams[scount].shock &&
                        (ar_beams[scount].shock->flags &
                         SHOCK_FLAG_ISTRIGGER)) // don't mess anything up if the user set the number too big
                    {
                        ...
                        {
                            LOG(" Trigger enabled. Inverted Blocker BeamID " + TOSTRING(i) + " disabled trigger " +
                                TOSTRING(scount));
                            ar_beams[i].shock->last_debug_state = 9;
                        }
            ...
            else if (ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CMD_BLOCKER) // this an enabled cmd-key-blocker and past a boundary
            {
                ...
                {
                    LOG(" F-key trigger block released. Blocker BeamID " + TOSTRING(i) + " Released F" +
            ...
            else if (ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CMD_SWITCH) // this is an enabled cmdkey switch and past a boundary
            {
                if (!ar_beams[i].shock->trigger_switch_state) // this switch is triggered first time in this boundary
                {
                    ...
                    {
                        int short1 =
                            ar_beams[ar_shocks[scount].beamid].shock->trigger_cmdshort; // cmdshort of checked trigger beam
                        ...
                        if (((short1 == short2 && long1 == long2) || (short1 == long2 && long1 == short2)) &&
                            i != tmpi) // found both command triggers then swap if its not the switching trigger
                        {
                            int tmpcmdkey = ar_beams[ar_shocks[scount].beamid].shock->trigger_cmdlong;
                            ar_beams[ar_shocks[scount].beamid].shock->trigger_cmdlong =
                                ar_beams[ar_shocks[scount].beamid].shock->trigger_cmdshort;
                            ar_beams[ar_shocks[scount].beamid].shock->trigger_cmdshort = tmpcmdkey;
                            ar_beams[i].shock->trigger_switch_state =
                                ar_beams[i].shock->trigger_boundary_t; // prevent trigger switching again before leaving
                                                                       // boundaries or timeout
                            if (m_trigger_debug_enabled && ar_beams[i].shock->last_debug_state != 3)
                            {
                                LOG(" Trigger F-key commands switched. Switch BeamID " + TOSTRING(i) +
                                    " switched commands of Trigger BeamID " +
                                    TOSTRING(ar_beams[ar_shocks[scount].beamid].shock->beamid) + " to cmdShort: F" +
                                    TOSTRING(ar_beams[ar_shocks[scount].beamid].shock->trigger_cmdshort) + ", cmdlong: F" +
            ...
            {                                                            // just a trigger, check high/low boundary and set action
                if (difftoBeamL > ar_beams[i].longbound * ar_beams[i].L) // trigger past longbound
                {
                    ...
                    {
                        // just a trigger
                        if (!ar_command_key[ar_beams[i].shock->trigger_cmdlong]
                                 .trigger_cmdkeyblock_state) // related cmdkey is not blocked
                        {
                            if (ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CONTINUOUS)
                                ar_command_key[ar_beams[i].shock->trigger_cmdshort].triggerInputValue =
                                    1; // continuous trigger only operates on trigger_cmdshort
                            else
                            ...
                            {
                                LOG(" Trigger Longbound activated. Trigger BeamID " + TOSTRING(i) + " Triggered F" +
                ...
                else // trigger past short bound
                {
                    ...
                    {
                        bool triggerValue =
                            !(ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CONTINUOUS); // 0 if trigger is continuous, 1 otherwise

                        EngineTriggerHelper(ar_beams[i].shock->trigger_cmdshort, ar_beams[i].shock->trigger_cmdlong,
                                            triggerValue);
                    }
                    else
                    {
                        // just a trigger
                        if (!ar_command_key[ar_beams[i].shock->trigger_cmdshort]
                                 .trigger_cmdkeyblock_state) // related cmdkey is not blocked
                        {
                            if (ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CONTINUOUS)
                                ar_command_key[ar_beams[i].shock->trigger_cmdshort].triggerInputValue =
                                    0; // continuous trigger only operates on trigger_cmdshort
                            else
                            ...
                            {
                                LOG(" Trigger Shortbound activated. Trigger BeamID " + TOSTRING(i) + " Triggered F" +
                                    TOSTRING(ar_beams[i].shock->trigger_cmdshort));
        ...
        else // this is a trigger inside boundaries and its enabled
        {
            if (ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CONTINUOUS) // this is an enabled continuous trigger
            {
                if (ar_beams[i].longbound - ar_beams[i].shortbound > 0.0f)
                {
                    ...
                    float triggerValue =
                        (diffPercentage - ar_beams[i].shortbound) / (ar_beams[i].longbound - ar_beams[i].shortbound);
            ...
            {
                for (int scount = i + 1; scount <= i + ar_beams[i].shock->trigger_cmdshort;
                     scount++) // (cylce blockerbeamID +1) to (blockerbeamID + beams tob lock)
                {
                    if (ar_beams[scount].shock &&
                        (ar_beams[scount].shock->flags &
                         SHOCK_FLAG_ISTRIGGER)) // dont mess anything up if the user set the number too big
                    {
                        if (m_trigger_debug_enabled && !ar_beams[scount].shock->trigger_enabled &&
                            ar_beams[i].shock->last_debug_state != 10)
                        {
                            LOG(" Trigger disabled. Inverted Blocker BeamID " + TOSTRING(i) + " enabled trigger " +
                                TOSTRING(scount));
                            ar_beams[i].shock->last_debug_state = 10;
                        }
                        ar_beams[scount].shock->trigger_enabled = false; // disable the trigger
            ...
            else if ((ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CMD_SWITCH) &&
                     ar_beams[i]
                         .shock->trigger_switch_state) // this is a switch that was activated and is back inside boundaries again
            {
            ...
            else if ((ar_beams[i].shock->flags & SHOCK_FLAG_TRG_CMD_BLOCKER) &&
                     !ar_command_key[ar_beams[i].shock->trigger_cmdshort]
                          .trigger_cmdkeyblock_state) // this cmdkeyblocker is inside boundaries and cmdkeystate is diabled
            {
                ...
                {
                    LOG(" F-key trigger blocked. Blocker BeamID " + TOSTRING(i) + " Blocked F" +
                        TOSTRING(ar_beams[i].shock->trigger_cmdshort));
...
void Actor::prepareInside(bool inside)
{
    ...
    {
        if (ar_dashboard) { ar_dashboard->setVisible(false); }
    ...
    if (App::gfx_reduce_shadows.GetActive()) { m_gfx_actor->SetCastShadows(!inside); }
}
...
void Actor::ToggleLights()
{
    ...
    if (!ar_lights)
    {
        for (size_t i = 0; i < ar_flares.size(); i++)
        {
            if (ar_flares[i].fl_type == FlareType::HEADLIGHT)
            {
                ...
                if (ar_flares[i].bbs) ar_flares[i].snode->detachAllObjects();
                if (ar_flares[i].light) ar_flares[i].light->setVisible(false);
    ...
    else
    {
        for (size_t i = 0; i < ar_flares.size(); i++)
        {
            if (ar_flares[i].fl_type == FlareType::HEADLIGHT)
            {
                if (ar_flares[i].light) ar_flares[i].light->setVisible(true);
                ar_flares[i].isVisible = true;
                if (ar_flares[i].bbs) ar_flares[i].snode->attachObject(ar_flares[i].bbs);
            }
        }
    }
...
{
    if (m_custom_light_toggle_countdown > -1) m_custom_light_toggle_countdown -= dt;
    ...
    for (size_t i = 0; i < this->ar_flares.size(); i++)
    {
        ...
        else if (ar_flares[i].fl_type == FlareType::REVERSE_LIGHT)
        {
            if (ar_engine || m_reverse_light_active)
                ...
                isvisible = false;
                 ...
                 ar_flares[i].controlnumber != -1) // controlnumber = read only attribute
        {
            if (ar_sim_state == Actor::SimState::LOCAL_SIMULATED &&
                this == App::GetSimController()->GetPlayerActor()) // no network!!
            {
                // networked customs are set directly, so skip this
                if (RoR::App::GetInputEngine()->getEventBoolValue(EV_TRUCK_LIGHTTOGGLE01 + (ar_flares[i].controlnumber - 1)) &&
        ...
        if (ar_flares[i].fl_type == FlareType::BLINKER_LEFT && m_blink_type == BLINK_LEFT)
        {
            ...
            if (ar_left_blink_on) SOUND_PLAY_ONCE(ar_instance_id, SS_TRIG_TURN_SIGNAL_TICK);
        ...
        else if (ar_flares[i].fl_type == FlareType::BLINKER_RIGHT && m_blink_type == BLINK_RIGHT)
        {
            ...
            if (ar_right_blink_on) SOUND_PLAY_ONCE(ar_instance_id, SS_TRIG_TURN_SIGNAL_TICK);
    ...
    if (keysleep) m_custom_light_toggle_countdown = 0.2;
}
...
void Actor::setBlinkType(blinktype blink)
{
    ...
    if (blink == BLINK_NONE) { SOUND_STOP(ar_instance_id, SS_TRIG_TURN_SIGNAL); }
    else
    {
...
void Actor::autoBlinkReset()
{
    ...
    if (m_blink_type == BLINK_RIGHT && ar_hydro_dir_state > blink_lock_range) m_blinker_autoreset = true;
...
void Actor::UpdateSoundSources()
{
    ...
    for (int i = 0; i < ar_num_soundsources; i++)
    {
        ...
        ar_soundsources[i].ssi->setPosition(ar_nodes[ar_soundsources[i].nodenum].AbsPosition,
                                            ar_nodes[ar_soundsources[i].nodenum].Velocity);
    }
...
void Actor::updateVisual(float dt)
{
    ...
    for (int i = 0; i < ar_num_wings; i++)
    {
        if (ar_wings[i].fa->type == 'a') ar_wings[i].fa->setControlDeflection(autoaileron);
        if (ar_wings[i].fa->type == 'b') ar_wings[i].fa->setControlDeflection(-autoaileron);
        if (ar_wings[i].fa->type == 'r') ar_wings[i].fa->setControlDeflection(autorudder);
        if (ar_wings[i].fa->type == 'e' || ar_wings[i].fa->type == 'S' || ar_wings[i].fa->type == 'T')
            ar_wings[i].fa->setControlDeflection(autoelevator);
        if (ar_wings[i].fa->type == 'f') ar_wings[i].fa->setControlDeflection(flapangles[ar_aerial_flap]);
        ...
        if (ar_wings[i].fa->type == 'd' || ar_wings[i].fa->type == 'U')
            ar_wings[i].fa->setControlDeflection((-autoaileron + autoelevator) / 2.0);
        if (ar_wings[i].fa->type == 'g') ar_wings[i].fa->setControlDeflection((autoaileron + flapangles[ar_aerial_flap]) / 2.0);
        if (ar_wings[i].fa->type == 'h') ar_wings[i].fa->setControlDeflection((-autoaileron + flapangles[ar_aerial_flap]) / 2.0);
        if (ar_wings[i].fa->type == 'i') ar_wings[i].fa->setControlDeflection((-autoelevator + autorudder) / 2.0);
        if (ar_wings[i].fa->type == 'j') ar_wings[i].fa->setControlDeflection((autoelevator + autorudder) / 2.0);
...
void Actor::AddInterActorBeam(beam_t *beam, Actor *a, Actor *b)
{
    ...
    if (pos == ar_inter_beams.end()) { ar_inter_beams.push_back(beam); }
...
void Actor::ToggleTies(int group)
{
    ...
    for (std::vector<tie_t>::iterator it = ar_ties.begin(); it != ar_ties.end(); it++)
    {
        ...
        if (it->ti_tied)
        {
            ...
            it->ti_tied  = false;
            it->ti_tying = false;
            if (it->ti_locked_ropable) it->ti_locked_ropable->attached_ties--;
    ...
    if (!istied)
    {
        for (std::vector<tie_t>::iterator it = ar_ties.begin(); it != ar_ties.end(); it++)
        {
            ...
            if (!it->ti_tied)
            {
                ...
                for (auto actor : App::GetSimController()->GetActors())
                {
                    if (actor->ar_sim_state == SimState::LOCAL_SLEEPING || (actor == this && it->ti_no_self_lock)) { continue; }

                    // and their ropables
                    for (std::vector<ropable_t>::iterator itr = actor->ar_ropables.begin(); itr != actor->ar_ropables.end();
                         itr++)
                    {
                        // if the ropable is not multilock and used, then discard this ropable
                        if (!itr->multilock && itr->attached_ties > 0) continue;

                        // skip if tienode is ropable too (no selflock)
                        if (this == actor && itr->node->pos == it->ti_beam->p1->pos) continue;
...
void Actor::ToggleRopes(int group)
{
    ...
    for (std::vector<rope_t>::iterator it = ar_ropes.begin(); it != ar_ropes.end(); it++)
    {
        ...
        if (it->rp_locked == LOCKED || it->rp_locked == PRELOCK)
        {
            ...
            // remove node locking
            if (it->rp_locked_ropable) it->rp_locked_ropable->attached_ropes--;
        ...
        else
        {
            ...
            for (auto actor : App::GetSimController()->GetActors())
            {
                if (actor->ar_sim_state == SimState::LOCAL_SLEEPING) continue;
                // and their ropables
                for (std::vector<ropable_t>::iterator itr = actor->ar_ropables.begin(); itr != actor->ar_ropables.end(); itr++)
                {
                    // if the ropable is not multilock and used, then discard this ropable
                    if (!itr->multilock && itr->attached_ropes > 0) continue;
...
void Actor::ToggleHooks(int group, hook_states mode, int node_number)
{
    ...
    for (std::vector<hook_t>::iterator it = ar_hooks.begin(); it != ar_hooks.end(); it++)
    {
        ...
        if (mode == HOOK_TOGGLE && group == -1)
        {
            ...
            if (it->hk_group <= -2) continue;
        }
        if (mode == HOOK_LOCK && group == -2)
        {
            // automatic lock attempt (cyclic with doupdate). Toggle all hooks groups with group#: -2, -3, -4 --, skip the ones
            // which are not autolock (triggered only)
            if (it->hk_group >= -1 || !it->hk_autolock) continue;
        }
        ...
        if ((mode == HOOK_LOCK || mode == HOOK_UNLOCK) && group <= -3)
        {
            ...
            if (it->hk_group != group) continue;
        }
        if ((mode == HOOK_LOCK || mode == HOOK_UNLOCK) && group >= -1) { continue; }
        ...
        if (mode != HOOK_UNLOCK && it->hk_locked == UNLOCKED)
        {
            ...
            {
                if (actor->ar_sim_state == SimState::LOCAL_SLEEPING) continue;
                if (this == actor && !it->hk_selflock) continue; // don't lock to self

                node_t *nearest_node = nullptr;
                for (int i = 0; i < actor->ar_num_nodes; i++)
                {
                    // skip all nodes with lockgroup 9999 (deny lock)
                    if (actor->ar_nodes[i].nd_lockgroup == 9999) continue;

                    // exclude this truck and its current hooknode from the locking search
                    if (this == actor && i == it->hk_hook_node->pos) continue;

                    // a lockgroup for this hooknode is set -> skip all nodes that do not have the same lockgroup (-1 =
                    // default(all nodes))
                    if (it->hk_lockgroup != -1 && it->hk_lockgroup != actor->ar_nodes[i].nd_lockgroup) continue;
        ...
        // this is a locked or prelocked hook and its not a locking attempt or the locked actor was removed (bm_inter_actor ==
        // false)
...
{
    if (!alb_notoggle) alb_mode = !alb_mode;
}

void Actor::ToggleTractionControl()
{
    if (!tc_notoggle) tc_mode = !tc_mode;
}
...
{
    if (!m_replay_handler || !m_replay_handler->isValid()) return;
...
{
    if (ar_sim_state == SimState::NETWORKED_OK) return m_net_reverse_light;

    if (ar_engine) return (ar_engine->GetGear() < 0);
...
void Actor::StopAllSounds()
{
    ...
    for (int i = 0; i < ar_num_soundsources; i++)
    {
        if (ar_soundsources[i].ssi) ar_soundsources[i].ssi->setEnabled(false);
    }
#endif // USE_OPENAL
}
...
int Actor::GetNumActiveConnectedBeams(int nodeid)
{
    ...
    {
        if (!ar_beams[ar_node_to_beam_connections[nodeid][ni]].bm_disabled &&
            !ar_beams[ar_node_to_beam_connections[nodeid][ni]].bounded)
            totallivebeams++;
    }
...
{
    for (std::vector<hook_t>::iterator it = ar_hooks.begin(); it != ar_hooks.end(); it++)
        if (it->hk_locked == LOCKED) return true;
    return false;
}

void Actor::updateDashBoards(float dt)
{
    if (!ar_dashboard) return;
    // some temp vars
    Vector3 dir;
    ...
    if (cam_dir != Vector3::ZERO) { velocity = cam_dir.dotProduct(ar_nodes[0].Velocity); }
...
void Actor::EngineTriggerHelper(int engineNumber, int type, float triggerValue)
{
    ...
    case TRG_ENGINE_CLUTCH:
        ...
        break;
    case TRG_ENGINE_BRAKE: ar_brake = triggerValue; break;
    case TRG_ENGINE_ACC:
        if (e) e->SetAcceleration(triggerValue);
        break;
    case TRG_ENGINE_RPM:
...
Actor::Actor(int actor_id, unsigned int vector_index, std::shared_ptr<RigDef::File> def, RoR::ActorSpawnRequest rq)
    : ar_nodes(nullptr), ar_num_nodes(0), ar_beams(nullptr), ar_num_beams(0), ar_shocks(nullptr), ar_num_shocks(0),
      ar_has_active_shocks(false), ar_rotators(nullptr), ar_num_rotators(0), ar_wings(nullptr), ar_num_wings(0),
      m_hud_features_ok(false), ar_aileron(0), m_avionic_chatter_timer(11.0f) // some pseudo random number,  doesn't matter
      ,
      m_beacon_light_is_active(false), m_blink_type(BLINK_NONE), m_blinker_autoreset(false), ar_brake(0.0),
      m_camera_gforces_accu(Ogre::Vector3::ZERO), m_camera_gforces(Ogre::Vector3::ZERO),
      m_camera_local_gforces_cur(Ogre::Vector3::ZERO), m_camera_local_gforces_max(Ogre::Vector3::ZERO),
      ar_engine_hydraulics_ready(true), m_custom_particles_enabled(false), ar_scale(1), ar_current_cinecam(-1) // -1 = external
      ,
      ar_dashboard(nullptr), ar_disable_aerodyn_turbulent_drag(false), ar_elevator(0), ar_aerial_flap(0),
      ar_fusedrag(Ogre::Vector3::ZERO), ar_hydro_aileron_command(0), ar_hydro_aileron_state(0), ar_hydro_dir_command(0),
      ar_hydro_dir_state(0), ar_hydro_dir_wheel_display(0.0), ar_hydro_elevator_command(0), ar_hydro_elevator_state(0),
      ar_hydro_rudder_command(0), ar_hydro_rudder_state(0), ar_nb_initialized(false), ar_nb_skip_steps(0),
      ar_nb_measure_steps(500), ar_nb_optimum(7, std::numeric_limits<float>::max()),
      ar_nb_reference(7, std::numeric_limits<float>::max()), ar_nb_mass_scale(1.0f),
      ar_nb_beams_scale(std::make_pair(1.0f, 1.0f)), ar_nb_shocks_scale(std::make_pair(1.0f, 1.0f)),
      ar_nb_wheels_scale(std::make_pair(1.0f, 1.0f)), ar_nb_beams_k_interval(std::make_pair(0.1f, 2.0f)),
      ar_nb_beams_d_interval(std::make_pair(0.1f, 2.0f)), ar_nb_shocks_k_interval(std::make_pair(0.1f, 8.0f)),
      ar_nb_shocks_d_interval(std::make_pair(0.1f, 8.0f)), ar_nb_wheels_k_interval(std::make_pair(1.0f, 1.0f)),
      ar_nb_wheels_d_interval(std::make_pair(1.0f, 1.0f)), m_inter_point_col_detector(nullptr),
      m_intra_point_col_detector(nullptr), ar_net_last_update_time(0), m_avg_node_position_prev(rq.asr_position),
      ar_left_mirror_angle(0.52), ar_lights(1), m_avg_node_velocity(Ogre::Vector3::ZERO), ar_custom_camera_node(-1),
      ar_main_camera_dir_corr(Ogre::Quaternion::IDENTITY), ar_main_camera_node_pos(0), ar_main_camera_node_dir(0),
      ar_main_camera_node_roll(0), m_preloaded_with_terrain(rq.asr_origin == RoR::ActorSpawnRequest::Origin::TERRN_DEF),
      ar_net_source_id(0), m_spawn_rotation(0.0), ar_net_stream_id(0), m_custom_light_toggle_countdown(0),
      m_min_camera_radius(0.0f), m_mouse_grab_move_force(0.0f), m_mouse_grab_node(-1), m_mouse_grab_pos(Ogre::Vector3::ZERO),
      m_net_initialized(false), m_net_brake_light(false), m_net_label_node(0), m_net_label_mt(0), m_net_reverse_light(false),
      ar_initial_total_mass(0), m_replay_pos_prev(-1), ar_parking_brake(false), ar_trailer_parking_brake(false),
      m_avg_node_position(rq.asr_position), m_previous_gear(0), m_ref_tyre_pressure(50.0), m_replay_handler(nullptr),
      ar_replay_precision(0), m_replay_timer(0), ar_replay_length(10000), ar_replay_mode(false), ar_replay_pos(0),
      m_reverse_light_active(false), ar_right_mirror_angle(-0.52), ar_rudder(0), ar_update_physics(false), ar_sleep_counter(0.0f),
      m_stabilizer_shock_request(0), m_stabilizer_shock_ratio(0.0), m_stabilizer_shock_sleep(0.0), m_total_mass(0),
      m_water_contact(false), m_water_contact_old(false), m_num_axle_diffs(0), m_axle_diffs{} // Init array to nullptr
      ,
      m_num_wheel_diffs(0), m_wheel_diffs{} // Init array to nullptr
      ,
      m_has_command_beams(false), m_num_command_beams(0), m_load_mass(0.f), m_dry_mass(0.f), ar_gui_use_engine_max_rpm(false),
      ar_autopilot(nullptr), ar_is_police(false), m_disable_default_sounds(false), ar_engine(nullptr),
      ar_driveable(NOT_DRIVEABLE), m_skid_trails{} // Init array to nullptr
      ,
      ar_collision_range(DEFAULT_COLLISION_RANGE), ar_instance_id(actor_id), ar_vector_index(vector_index),
      ar_rescuer_flag(false), m_antilockbrake(false), m_tractioncontrol(false), ar_forward_commands(false),
      ar_import_commands(false), ar_toggle_ties(false), ar_airbrakes{} // Init array to nullptr
      ,
      ar_cabs{} // Init array to 0
      ,
      ar_num_cabs(0), ar_num_contactable_nodes(0), ar_num_contacters(0), ar_screwprops{} // Init array to nullptr
      ,
      ar_num_screwprops(0), ar_num_camera_rails(0), ar_aeroengines() // Zero-init array
      ,
      ...
      ar_num_wheels() // int
      ,
      m_avg_proped_wheel_radius(0.2f), ar_filename(rq.asr_filename), m_section_config(rq.asr_config), m_ongoing_reset(false),
      ar_top_speed(0.0f), ar_last_fuzzy_ground_model(nullptr), m_transfer_case(nullptr)
{
}
...
{
    if (ar_sim_state == SimState::NETWORKED_OK) return m_net_brake_light;
...
void Actor::setCustomLightVisible(int number, bool visible)
{
    ...
    if (flareID < ar_flares.size() && ar_flares[flareID].snode) { ar_flares[flareID].controltoggle_status = visible; }
}
...
{
    if (nodeNumber >= 0 && nodeNumber < ar_num_nodes) { return ar_nodes[nodeNumber].AbsPosition; }
    else
    {
        return Ogre::Vector3();
    }
