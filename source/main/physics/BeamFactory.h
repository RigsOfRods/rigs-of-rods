...
namespace RoR
{
    ...
    class ActorManager
    {
      ...
      public:
        ...
        {
            return m_net_timer.getMilliseconds();
        };
        ...
        {
            m_stream_mismatches[sourceid].insert(streamid);
        };
        ...
        {
            m_forced_awake = forced;
        };
        bool AreTrucksForcedAwake() const
        {
            return m_forced_awake;
        }
        void SetSimulationSpeed(float speed)
        {
            m_simulation_speed = std::max(0.0f, speed);
        };
      ...
      private:
        ...
        bool CheckActorCollAabbIntersect(int a, int b); //!< Returns whether or not the bounding boxes of truck a and truck b
                                                        //!< intersect. Based on the truck collision bounding boxes.
        bool PredictActorCollAabbIntersect(
            int a, int b); //!< Returns whether or not the bounding boxes of truck a and truck b might intersect during the next
                           //!< framestep. Based on the truck collision bounding boxes.
        ...
        std::map<int, std::set<int>> m_stream_mismatches;  //!< Networking: A set of streams without a corresponding actor in the
                                                           //!< actor-array for each stream source
