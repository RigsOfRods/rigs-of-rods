...
void ActorSpawner::ProcessNewActor(Actor* actor, ActorSpawnRequest rq, RigDef::DocumentPtr def)
{
    ...
    {
        // Skip lines with only a keyword starting/ending a block.
        ...
        try
        {
            ...
            {
            case Keyword::ADD_ANIMATION:                 break;
            ...
            case Keyword::END_SECTION: this->AddMessage(Message::TYPE_WARNING,
                "`end_section` not supported by this game version"); break;
            ...
            case Keyword::GUID:                          break; // Unused by Actor, only by GUI::MainSelector
            ...
            case Keyword::PROP_CAMERA_MODE: this->AddMessage(Message::TYPE_WARNING,
                "`prop_camera_mode` not supported by this game version"); break;
            ...
            case Keyword::SECTION:  this->AddMessage(Message::TYPE_WARNING,
                "`section` not supported by this game version"); break;
            case Keyword::SECTIONCONFIG: this->AddMessage(Message::TYPE_WARNING,
                "`sectionconfig` not supported by this game version"); break;
...
void ActorSpawner::CalcMemoryRequirements(ActorMemoryRequirements& req)
{
    ...
    // 'nodes2'
    req.num_nodes += m_document->nodes2.size();
    for (auto& def : m_document->nodes2)
    {
        if (BITMASK_IS_1(def.options, RigDef::Node::OPTION_h_HOOK_POINT))
        {
            req.num_beams += 1;
    ...
    // 'wheels2'
    for (RigDef::Wheel2& wheel2 : m_document->wheels2)
    {
    ...
    // 'meshwheels'
    for (RigDef::MeshWheel& meshwheel : m_document->meshwheels)
    {
    ...
    // 'meshwheels2'
    for (RigDef::MeshWheel2& meshwheel : m_document->meshwheels2)
    {
        req.num_nodes += meshwheel.num_rays * 2; // BuildWheelObjectAndNodes()
        req.num_beams += meshwheel.num_rays * ((meshwheel.rigidity_node != NODEREF_INVALID) ? 9 : 8); // BuildWheelBeams()
    }
    ...
    // 'flexbodywheels'
    for (RigDef::FlexBodyWheel& flexwheel : m_document->flexbodywheels)
    {
...
{
    ManagedMaterial& def = m_document->managedmaterials[pos];
