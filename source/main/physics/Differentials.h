...
class Differential
{
  public:
    ...
    {
        return m_available_diffs[0];
    }
    int GetNumDiffTypes()
    {
        return static_cast<int>(m_available_diffs.size());
    }

    static void CalcSeparateDiff(
        DifferentialData &diff_data); //!< a differential that always splits the torque evenly, this is the original method
