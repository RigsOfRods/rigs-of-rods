...
Turboprop::Turboprop(const char *propname, node_t *nd, int nr, int nb, int np1, int np2, int np3, int np4, int tqn, float power,
                     Ogre::String const &propfoilname, int mnumber, int trucknum, bool disable_smoke, bool ispiston, float fpitch)
{
    ...
    {
    case 0:
        mod_id = SS_MOD_AEROENGINE1;
        src_id = SS_TRIG_AEROENGINE1;
        thr_id = SS_MOD_THROTTLE1;
        break;
    case 1:
        mod_id = SS_MOD_AEROENGINE2;
        src_id = SS_TRIG_AEROENGINE2;
        thr_id = SS_MOD_THROTTLE2;
        break;
    case 2:
        mod_id = SS_MOD_AEROENGINE3;
        src_id = SS_TRIG_AEROENGINE3;
        thr_id = SS_MOD_THROTTLE3;
        break;
    case 3:
        mod_id = SS_MOD_AEROENGINE4;
        src_id = SS_TRIG_AEROENGINE4;
        thr_id = SS_MOD_THROTTLE4;
        break;
    case 4:
        mod_id = SS_MOD_AEROENGINE5;
        src_id = SS_TRIG_AEROENGINE5;
        thr_id = SS_MOD_THROTTLE5;
        break;
    case 5:
        mod_id = SS_MOD_AEROENGINE6;
        src_id = SS_TRIG_AEROENGINE6;
        thr_id = SS_MOD_THROTTLE6;
        break;
    case 6:
        mod_id = SS_MOD_AEROENGINE7;
        src_id = SS_TRIG_AEROENGINE7;
        thr_id = SS_MOD_THROTTLE7;
        break;
    case 7:
        mod_id = SS_MOD_AEROENGINE8;
        src_id = SS_TRIG_AEROENGINE8;
        thr_id = SS_MOD_THROTTLE8;
        break;
    default:
        mod_id = SS_MOD_NONE;
        src_id = SS_TRIG_NONE;
...
Turboprop::~Turboprop()
{
    ...
    if (smokePS != nullptr) { smokePS->removeAllEmitters(); }
}
...
void Turboprop::updateForces(float dt, int doUpdate)
{
    ...
    avg = avg / numblades;
    if ((avg - nodes[noderef].RelPosition).length() > 0.4) { failed = true; }
    ...
    else
    {
        ...
        else
        {
            ...
            if (rpm > regspeed * 1.11) { pitch -= pitchspeed * dt; }
        }
    }
    if (!failed)
    {
    ...
    float estrotenergy =
        0.5 * numblades * nodes[nodep[0]].mass * radius * radius * (rpm / RAD_PER_SEC_TO_RPM) * (rpm / RAD_PER_SEC_TO_RPM);
    // for each blade
    float totthrust = 0;
    float tottorque = 0;
    for (int i = 0; i < numblades; i++)
    {
        ...
        else if (!nodes[noderef].Velocity.isZeroLength())
        {
            ...
            // determine nodes speed and divide by engines speed (with some magic numbers for tuning) to keep it rotating longer
            // when shutoff in flight and stop after a while when plane is stopped (on the ground)
    ...
    //	sprintf(debug, "pitch %i thrust %i totenergy=%i apparentenergy=%i", (int)pitch, (int)totthrust, (int)rotenergy,
    //(int)estrotenergy); prop wash
