...
Turbojet::Turbojet(Actor *actor, int tnodefront, int tnodeback, int tnoderef, RigDef::Turbojet &def)
{
    ...
    {
    case 0:
        m_sound_mod = SS_MOD_AEROENGINE1;
        m_sound_src = SS_TRIG_AEROENGINE1;
        m_sound_thr = SS_MOD_THROTTLE1;
        m_sound_ab  = SS_TRIG_AFTERBURNER1;
        break;
    case 1:
        m_sound_mod = SS_MOD_AEROENGINE2;
        m_sound_src = SS_TRIG_AEROENGINE2;
        m_sound_thr = SS_MOD_THROTTLE2;
        m_sound_ab  = SS_TRIG_AFTERBURNER2;
        break;
    case 2:
        m_sound_mod = SS_MOD_AEROENGINE3;
        m_sound_src = SS_TRIG_AEROENGINE3;
        m_sound_thr = SS_MOD_THROTTLE3;
        m_sound_ab  = SS_TRIG_AFTERBURNER3;
        break;
    case 3:
        m_sound_mod = SS_MOD_AEROENGINE4;
        m_sound_src = SS_TRIG_AEROENGINE4;
        m_sound_thr = SS_MOD_THROTTLE4;
        m_sound_ab  = SS_TRIG_AFTERBURNER4;
        break;
    case 4:
        m_sound_mod = SS_MOD_AEROENGINE5;
        m_sound_src = SS_TRIG_AEROENGINE5;
        m_sound_thr = SS_MOD_THROTTLE5;
        m_sound_ab  = SS_TRIG_AFTERBURNER5;
        break;
    case 5:
        m_sound_mod = SS_MOD_AEROENGINE6;
        m_sound_src = SS_TRIG_AEROENGINE6;
        m_sound_thr = SS_MOD_THROTTLE6;
        m_sound_ab  = SS_TRIG_AFTERBURNER6;
        break;
    case 6:
        m_sound_mod = SS_MOD_AEROENGINE7;
        m_sound_src = SS_TRIG_AEROENGINE7;
        m_sound_thr = SS_MOD_THROTTLE7;
        m_sound_ab  = SS_TRIG_AFTERBURNER7;
        break;
    case 7:
        m_sound_mod = SS_MOD_AEROENGINE8;
        m_sound_src = SS_TRIG_AEROENGINE8;
        m_sound_thr = SS_MOD_THROTTLE8;
        m_sound_ab  = SS_TRIG_AFTERBURNER8;
        break;
    default:
        m_sound_mod = SS_MOD_NONE;
        m_sound_src = SS_TRIG_NONE;
        m_sound_thr = SS_MOD_NONE;
        m_sound_ab  = SS_TRIG_NONE;
    }
#endif
...
void TurbojetVisual::SetupVisuals(RigDef::Turbojet &def, int num, std::string const &propname, Ogre::Entity *nozzle,
...
{
    if (m_flame_entity != nullptr) { m_flame_entity->setVisible(false); }

    if (m_nozzle_entity != nullptr) { m_nozzle_entity->setVisible(false); }

    if (m_smoke_particle != nullptr) { m_smoke_particle->removeAllEmitters(); }
}
...
void TurbojetVisual::UpdateVisuals(RoR::GfxActor *gfx_actor)
{
    ...
    RoR::GfxActor::SimBuffer::AeroEngineSB &ae_buf   = gfx_actor->GetSimDataBuffer().simbuf_aeroengines.at(m_number);
...
{
    if (doUpdate) { SOUND_MODULATE(m_actor, m_sound_mod, m_rpm_percent); }
    m_timer += dt;
    m_axis      = m_actor->ar_nodes[m_node_front].RelPosition - m_actor->ar_nodes[m_node_back].RelPosition;
    ...
    if (m_warmup)
    {
        ...
        if (warmupfactor >= 1.0) m_warmup = false;
    }

    // turbine RPM
    // braking (compression)
    if (m_rpm_percent < 0) m_rpm_percent = 0;
    float torque = -m_rpm_percent / 100.0;
    // powering with limiter
    if (m_rpm_percent < 100.0 && !m_is_failed && m_ignition) torque += ((0.2 + m_throtle * 0.8) * warmupfactor);
    // integration
    ...
    if (!m_is_failed && m_ignition)
    {
        ...
        if (m_afterburner_active) enginethrust += (m_afterburn_thrust - m_max_dry_thrust);
    }
    else
        m_afterburner_active = false;
...
{
    if (m_timer - m_last_flip < 0.3) return;
