...
FlexAirfoil::FlexAirfoil(
        Ogre::String const & name, ActorPtr actor, 
...
{
    Vector3 center;
    node_t* nodes = fa_actor->ar_nodes;
...
void FlexAirfoil::updateForces()
{
    ...
    for (i=0; i<free_wash; i++)
        wind-=(0.5*washpropratio[i]*fa_actor->ar_aeroengines[washpropnum[i]]->getpropwash())*fa_actor->ar_aeroengines[washpropnum[i]]->getAxis();
