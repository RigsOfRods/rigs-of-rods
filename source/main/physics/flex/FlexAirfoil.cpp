...
float refairfoilpos[90] = {0.00, 0.50, 0.00, 1.00, 0.50, 0.00,

                           0.00, 0.70, 0.03, 1.00, 0.70, 0.03, 0.00, 0.30, 0.03, 1.00, 0.30, 0.03,

                           0.00, 0.90, 0.10, 1.00, 0.90, 0.10, 0.00, 0.10, 0.10, 1.00, 0.10, 0.10,

                           0.00, 1.00, 0.25, 1.00, 1.00, 0.25, 0.00, 0.00, 0.25, 1.00, 0.00, 0.25,

                           0.00, 1.00, 0.50, 1.00, 1.00, 0.50, 0.00, 0.00, 0.50, 1.00, 0.00, 0.50,

                           // updated with control surface chord ratio
                           0.00, 0.75, 0.75, 1.00, 0.75, 0.75, 0.00, 0.25, 0.75, 1.00, 0.25, 0.75,

                           0.00, 0.75, 0.75, 1.00, 0.75, 0.75, 0.00, 0.25, 0.75, 1.00, 0.25, 0.75,

                           // moving with control surface
                           0.00, 0.50, 1.00, 1.00, 0.50, 1.00,

                           0.00, 0.50, 1.00, 1.00, 0.50, 1.00};

using namespace Ogre;

FlexAirfoil::FlexAirfoil(Ogre::String const &name, Actor *actor, int pnfld, int pnfrd, int pnflu, int pnfru, int pnbld, int pnbrd,
                         int pnblu, int pnbru, std::string const &texband, Vector2 texlf, Vector2 texrf, Vector2 texlb,
                         Vector2 texrb, char mtype, float controlratio, float mind, float maxd, Ogre::String const &afname,
                         float lift_coef, bool break_able)
{
    ...
    if (hascontrol)
    {
        ...
        for (i = 0; i < 12; i++)
            airfoilpos[54 + 12 + i] = airfoilpos[54 + i];
    }
    ...
    for (i = 0; i < 24; i++)
        covertices[i + 30].texcoord = covertices[i].texcoord;
    ...
    float tsref = 2.0 * (nodes[nfrd].RelPosition - nodes[nfld].RelPosition)
                            .crossProduct(nodes[nbld].RelPosition - nodes[nfld].RelPosition)
                            .length();
    sref = 2.0 * (nodes[nfrd].RelPosition - nodes[nfld].RelPosition)
                     .crossProduct(nodes[nbrd].RelPosition - nodes[nfrd].RelPosition)
                     .length();
    if (tsref > sref) sref = tsref;
    sref = sref * sref;

    lratio = (nodes[nfld].RelPosition - nodes[nflu].RelPosition).length() /
             (nodes[nfld].RelPosition - nodes[nbld].RelPosition).length();
    rratio = (nodes[nfrd].RelPosition - nodes[nfru].RelPosition).length() /
             (nodes[nfrd].RelPosition - nodes[nbrd].RelPosition).length();
    ...
    HardwareIndexBufferSharedPtr faceibuf = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, faceibufCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
    ...
    // for the band
    /// Allocate index buffer of the requested number of vertices (ibufCount)
    HardwareIndexBufferSharedPtr bandibuf = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, bandibufCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
    ...
    // for the aileron up
    /// Allocate index buffer of the requested number of vertices (ibufCount)
    HardwareIndexBufferSharedPtr cupibuf = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, cupibufCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
    ...
    /// Allocate index buffer of the requested number of vertices (ibufCount)
    HardwareIndexBufferSharedPtr cdnibuf = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, cdnibufCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
...
void FlexAirfoil::updateVerticesPhysics()
{
    ...
    if (breakable)
    { broken = broken || (vx.crossProduct(vzl).squaredLength() > sref) || (vx.crossProduct(vzr).squaredLength() > sref); }
    else
    {
        broken = (vx.crossProduct(vzl).squaredLength() > sref) || (vx.crossProduct(vzr).squaredLength() > sref);
    }
...
Vector3 FlexAirfoil::updateVerticesGfx(RoR::GfxActor *gfx_actor)
{
    ...
    else
    {
        ...
        {
            if (i % 2)
                covertices[i].vertex = airfoilpos[i * 3] * Vector3(0.01, 0, 0) + airfoilpos[i * 3 + 1] * Vector3(0, 0.01, 0) +
                                       airfoilpos[i * 3 + 2] * Vector3(0, 0, 0.01);
            else
                covertices[i].vertex = airfoilpos[i * 3] * Vector3(0.01, 0, 0) + airfoilpos[i * 3 + 1] * Vector3(0, 0.01, 0) +
                                       airfoilpos[i * 3 + 2] * Vector3(0, 0, 0.01);
            if (i < 22) covertices[i + 30].vertex = covertices[i].vertex;
    ...
    if (isstabilator)
    {
        ...
        if (!stabilleft)
        {
            rcent = ((gfx_nodes[nflu].AbsPosition + gfx_nodes[nbld].AbsPosition) / 2.0 +
                     (gfx_nodes[nflu].AbsPosition - gfx_nodes[nblu].AbsPosition) / 4.0) -
                    center;
            raxis = (gfx_nodes[nflu].AbsPosition - gfx_nodes[nfld].AbsPosition)
                        .crossProduct(gfx_nodes[nflu].AbsPosition - gfx_nodes[nblu].AbsPosition);
        }
        else
        {
            rcent = ((gfx_nodes[nfru].AbsPosition + gfx_nodes[nbrd].AbsPosition) / 2.0 +
                     (gfx_nodes[nfru].AbsPosition - gfx_nodes[nbru].AbsPosition) / 4.0) -
                    center;
            raxis = (gfx_nodes[nfru].AbsPosition - gfx_nodes[nfrd].AbsPosition)
                        .crossProduct(gfx_nodes[nfru].AbsPosition - gfx_nodes[nbru].AbsPosition);
        }
        raxis.normalise();
...
{
    if (val < 0)
        deflection = -val * mindef;
    else
        deflection = val * maxdef;
}
...
void FlexAirfoil::updateForces()
{
    ...
    Vector3 chordv =
        ((nodes[nbld].RelPosition - nodes[nfld].RelPosition) + (nodes[nbrd].RelPosition - nodes[nfrd].RelPosition)) / 2.0;
    float chord = chordv.length();
    // span vector, left to right
    Vector3 spanv =
        ((nodes[nfrd].RelPosition - nodes[nfld].RelPosition) + (nodes[nbrd].RelPosition - nodes[nbld].RelPosition)) / 2.0;
