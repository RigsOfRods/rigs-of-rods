...
FlexMeshWheel::FlexMeshWheel(Ogre::Entity *rim_prop_entity, RoR::GfxActor *gfx_actor, int axis_node_1_index,
                             int axis_node_2_index, int nstart, int nrays, std::string const &tire_mesh_name,
                             std::string const &tire_material_name, float rimradius, bool rimreverse)
    : m_axis_node0_idx(axis_node_1_index), m_axis_node1_idx(axis_node_2_index), m_start_node_idx(nstart),
      m_num_rays(static_cast<size_t>(nrays)), m_gfx_actor(gfx_actor), m_is_rim_reverse(rimreverse), m_rim_radius(rimradius)
{
    ...
    for (i = 0; i < nrays; i++)
    {
        m_indices[3 * (i * 10)]         = i * 6;
        m_indices[3 * (i * 10) + 1]     = i * 6 + 1;
        m_indices[3 * (i * 10) + 2]     = (i + 1) * 6;
        m_indices[3 * (i * 10 + 1)]     = i * 6 + 1;
        m_indices[3 * (i * 10 + 1) + 1] = (i + 1) * 6 + 1;
        m_indices[3 * (i * 10 + 1) + 2] = (i + 1) * 6;

        m_indices[3 * (i * 10 + 2)]     = i * 6 + 1;
        m_indices[3 * (i * 10 + 2) + 1] = i * 6 + 2;
        m_indices[3 * (i * 10 + 2) + 2] = (i + 1) * 6 + 1;
        m_indices[3 * (i * 10 + 3)]     = i * 6 + 2;
        m_indices[3 * (i * 10 + 3) + 1] = (i + 1) * 6 + 2;
        m_indices[3 * (i * 10 + 3) + 2] = (i + 1) * 6 + 1;

        m_indices[3 * (i * 10 + 4)]     = i * 6 + 2;
        m_indices[3 * (i * 10 + 4) + 1] = i * 6 + 3;
        m_indices[3 * (i * 10 + 4) + 2] = (i + 1) * 6 + 2;
        m_indices[3 * (i * 10 + 5)]     = i * 6 + 3;
        m_indices[3 * (i * 10 + 5) + 1] = (i + 1) * 6 + 3;
        m_indices[3 * (i * 10 + 5) + 2] = (i + 1) * 6 + 2;

        m_indices[3 * (i * 10 + 6)]     = i * 6 + 3;
        m_indices[3 * (i * 10 + 6) + 1] = i * 6 + 4;
        m_indices[3 * (i * 10 + 6) + 2] = (i + 1) * 6 + 3;
        m_indices[3 * (i * 10 + 7)]     = i * 6 + 4;
        m_indices[3 * (i * 10 + 7) + 1] = (i + 1) * 6 + 4;
        m_indices[3 * (i * 10 + 7) + 2] = (i + 1) * 6 + 3;

        m_indices[3 * (i * 10 + 8)]     = i * 6 + 4;
        m_indices[3 * (i * 10 + 8) + 1] = i * 6 + 5;
        m_indices[3 * (i * 10 + 8) + 2] = (i + 1) * 6 + 4;
        m_indices[3 * (i * 10 + 9)]     = i * 6 + 5;
        m_indices[3 * (i * 10 + 9) + 1] = (i + 1) * 6 + 5;
        m_indices[3 * (i * 10 + 9) + 2] = (i + 1) * 6 + 4;
    }
    ...
    // compute m_norm_y;
    m_norm_y =
        ((m_vertices[0].position - m_vertices[1].position).crossProduct(m_vertices[1].position - m_vertices[6 + 1].position))
            .length();
    ...
    HardwareIndexBufferSharedPtr ibuf = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, m_index_count, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
...
Vector3 FlexMeshWheel::updateVertices()
{
    ...
    for (size_t i = 0; i < m_num_rays; i++)
    {
        ...
        m_vertices[i * 6 + 1].position =
            all_nodes[m_start_node_idx + i * 2].AbsPosition -
            0.05 * (all_nodes[m_start_node_idx + i * 2].AbsPosition - all_nodes[m_axis_node0_idx].AbsPosition) - center;
        m_vertices[i * 6 + 2].position =
            all_nodes[m_start_node_idx + i * 2].AbsPosition -
            0.1 * (all_nodes[m_start_node_idx + i * 2].AbsPosition - all_nodes[m_start_node_idx + i * 2 + 1].AbsPosition) -
            center;
        m_vertices[i * 6 + 3].position =
            all_nodes[m_start_node_idx + i * 2 + 1].AbsPosition -
            0.1 * (all_nodes[m_start_node_idx + i * 2 + 1].AbsPosition - all_nodes[m_start_node_idx + i * 2].AbsPosition) -
            center;
        m_vertices[i * 6 + 4].position =
            all_nodes[m_start_node_idx + i * 2 + 1].AbsPosition -
            0.05 * (all_nodes[m_start_node_idx + i * 2 + 1].AbsPosition - all_nodes[m_axis_node1_idx].AbsPosition) - center;
        ...
        m_vertices[i * 6 + 1].normal =
            (m_vertices[i * 6].position - m_vertices[i * 6 + 1].position)
                .crossProduct(m_vertices[i * 6].position - m_vertices[((i + 1) % m_num_rays) * 6 + 1].position) /
            m_norm_y;
        m_vertices[i * 6 + 2].normal = ray;
        m_vertices[i * 6 + 3].normal = ray;
        m_vertices[i * 6 + 4].normal =
            (m_vertices[i * 6 + 4].position - m_vertices[i * 6 + 5].position)
                .crossProduct(m_vertices[i * 6 + 4].position - m_vertices[((i + 1) % m_num_rays) * 6 + 4].position) /
            m_norm_y;
