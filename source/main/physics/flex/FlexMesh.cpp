...
FlexMesh::FlexMesh(Ogre::String const &name, RoR::GfxActor *gfx_actor, int n1, int n2, int nstart, int nrays,
                   Ogre::String const &face_material_name, Ogre::String const &band_material_name, bool rimmed, float rim_ratio)
    : m_is_rimmed(rimmed), m_num_rays(nrays), m_gfx_actor(gfx_actor)
{
    ...
    for (i = 0; i < nrays; i++)
    {
        ...
        if (i % 2 == 0) // Even index
        {
            if (odd_num_rays && ((i + 1) == nrays))
            {
                ...
                m_vertices[band_vert].texcoord =
                    Vector2(0.5, 0.0); // Stretch the texture over 2 quads instead of 1... ugly, but best we can do here
        ...
        if (m_is_rimmed)
        {
            m_vertices[2 + i * 2].texcoord             = Vector2(0.5 + 0.5 * rim_ratio * sin((float)i * 2.0 * 3.14159 / nrays),
                                                     0.5 + 0.5 * rim_ratio * cos((float)i * 2.0 * 3.14159 / nrays));
            m_vertices[2 + i * 2 + 1].texcoord         = m_vertices[2 + i * 2].texcoord;
            m_vertices[2 + 4 * nrays + i * 2].texcoord = Vector2(0.5 + 0.5 * sin(((float)i + 0.5) * 2.0 * 3.14159 / nrays),
                                                                 0.5 + 0.5 * cos(((float)i + 0.5) * 2.0 * 3.14159 / nrays));
            m_vertices[2 + 4 * nrays + i * 2 + 1].texcoord = m_vertices[2 + 4 * nrays + i * 2].texcoord;
        }
        else
        {
            m_vertices[2 + i * 2].texcoord =
                Vector2(0.5 + 0.5 * sin(i * 2.0 * 3.14159 / nrays), 0.5 + 0.5 * cos(i * 2.0 * 3.14159 / nrays));
            m_vertices[2 + i * 2 + 1].texcoord = m_vertices[2 + i * 2].texcoord;
        }
    }
    ...
    {
        // wheel sides
        m_wheelface_indices[3 * (i * 2)]         = 0;
        m_wheelface_indices[3 * (i * 2) + 1]     = 2 + i * 2;
        m_wheelface_indices[3 * (i * 2) + 2]     = 2 + ((i + 1) % nrays) * 2;
        m_wheelface_indices[3 * (i * 2 + 1)]     = 1;
        m_wheelface_indices[3 * (i * 2 + 1) + 2] = 2 + i * 2 + 1;
        m_wheelface_indices[3 * (i * 2 + 1) + 1] = 2 + ((i + 1) % nrays) * 2 + 1;
        if (m_is_rimmed)
        {
            m_wheelface_indices[3 * (i * 4 + 0 + 2 * nrays)]     = 2 + i * 2;
            m_wheelface_indices[3 * (i * 4 + 0 + 2 * nrays) + 1] = 2 + 4 * nrays + i * 2;
            m_wheelface_indices[3 * (i * 4 + 0 + 2 * nrays) + 2] = 2 + ((i + 1) % nrays) * 2;
            m_wheelface_indices[3 * (i * 4 + 1 + 2 * nrays)]     = 2 + 4 * nrays + i * 2;
            m_wheelface_indices[3 * (i * 4 + 1 + 2 * nrays) + 1] = 2 + 4 * nrays + ((i + 1) % nrays) * 2;
            m_wheelface_indices[3 * (i * 4 + 1 + 2 * nrays) + 2] = 2 + ((i + 1) % nrays) * 2;
            m_wheelface_indices[3 * (i * 4 + 2 + 2 * nrays)]     = 2 + i * 2 + 1;
            m_wheelface_indices[3 * (i * 4 + 2 + 2 * nrays) + 2] = 2 + 4 * nrays + i * 2 + 1;
            m_wheelface_indices[3 * (i * 4 + 2 + 2 * nrays) + 1] = 2 + ((i + 1) % nrays) * 2 + 1;
            m_wheelface_indices[3 * (i * 4 + 3 + 2 * nrays)]     = 2 + 4 * nrays + i * 2 + 1;
            m_wheelface_indices[3 * (i * 4 + 3 + 2 * nrays) + 2] = 2 + 4 * nrays + ((i + 1) % nrays) * 2 + 1;
            m_wheelface_indices[3 * (i * 4 + 3 + 2 * nrays) + 1] = 2 + ((i + 1) % nrays) * 2 + 1;
        }
        // wheel band
        m_tiretread_indices[3 * (i * 2)]         = 2 + 2 * nrays + i * 2;
        m_tiretread_indices[3 * (i * 2) + 1]     = 2 + 2 * nrays + i * 2 + 1;
        m_tiretread_indices[3 * (i * 2) + 2]     = 2 + 2 * nrays + ((i + 1) % nrays) * 2;
        m_tiretread_indices[3 * (i * 2 + 1)]     = 2 + 2 * nrays + ((i + 1) % nrays) * 2;
        m_tiretread_indices[3 * (i * 2 + 1) + 2] = 2 + 2 * nrays + ((i + 1) % nrays) * 2 + 1;
        m_tiretread_indices[3 * (i * 2 + 1) + 1] = 2 + 2 * nrays + i * 2 + 1;
    }
    ...
    m_hw_vbuf = HardwareBufferManager::getSingleton().createVertexBuffer(offset, m_mesh->sharedVertexData->vertexCount,
    ...
    HardwareIndexBufferSharedPtr faceibuf = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, wheelface_num_indices, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
    ...
    HardwareIndexBufferSharedPtr bandibuf = HardwareBufferManager::getSingleton().createIndexBuffer(
        HardwareIndexBuffer::IT_16BIT, tiretread_num_indices, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
...
Vector3 FlexMesh::updateVertices()
{
    ...
    for (int i = 0; i < m_num_rays * 2; i++)
    {
        ...
        if ((i % 2) == 0)
        {
            m_vertices[2 + i].normal =
                approx_normalise(all_nodes[m_vertex_nodes[0]].AbsPosition - all_nodes[m_vertex_nodes[1]].AbsPosition);
        }
        else
        {
        ...
        if (m_is_rimmed)
        {
            ...
            if ((i % 2) == 0)
            {
                m_vertices[2 + 4 * m_num_rays + i].normal =
                    approx_normalise(all_nodes[m_vertex_nodes[2 + 4 * m_num_rays + i]].AbsPosition -
                                     all_nodes[m_vertex_nodes[2 + 4 * m_num_rays + i + 1]].AbsPosition);
            }
            else
            {
