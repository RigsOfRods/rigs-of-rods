...
namespace RoR
{
    ...
    {
        FlexBodyCacheData() : dst_pos(nullptr), src_normals(nullptr), src_colors(nullptr), locators(nullptr)
        {
        }
    ...
    class FlexBodyFileIO
    {
      public:
        ...
        inline void              AddItemToSave(FlexBody *fb)
        {
            m_items_to_save.push_back(fb);
        }
        inline FlexBodyCacheData *GetLoadedItem(unsigned index)
        {
            return &m_loaded_items[index];
        }
        ResultCode SaveFile();
        ResultCode LoadFile();
    ...
    class FlexFactory
    {
      public:
        ...
        FlexBody *CreateFlexBody(RigDef::Flexbody *def, const int ref_node, const int x_node, const int y_node,
                                 Ogre::Quaternion const &rot, std::vector<unsigned int> &node_indices,
                                 std::string resource_group_name);

        FlexMeshWheel *CreateFlexMeshWheel(unsigned int wheel_index, int axis_node_1_index, int axis_node_2_index, int nstart,
                                           int nrays, float rim_radius, bool rim_reverse, std::string const &rim_mesh_name,
