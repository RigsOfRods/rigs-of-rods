...
void RoR::DrawImGuiSpinner(float &counter, const ImVec2 size, const float spacing, const float step_sec)
{
    ...
    const ImU32 COLORS[] = {ImColor(255, 255, 255, 255), ImColor(210, 210, 210, 255), ImColor(120, 120, 120, 255),
    ...
    if (counter > (step_sec * 3.f)) { color_start = 3; }
    ...
    // NOTE: Enter vertices in clockwise order, otherwise anti-aliasing doesn't work and polygon is rasterized larger! -- Observed
    // under OpenGL2 / OGRE 1.9

    // Top triangle, vertices: mid, left, right
    draw_list->AddTriangleFilled(ImVec2(mid_x, mid_y - spacing), ImVec2(left + spacing, top), ImVec2(right - spacing, top),
                                 COLORS[color_start]);
    // Right triangle, vertices: mid, top, bottom
    draw_list->AddTriangleFilled(ImVec2(mid_x + spacing, mid_y), ImVec2(right, top + spacing), ImVec2(right, bottom - spacing),
                                 COLORS[(color_start + 3) % 4]);
    // Bottom triangle, vertices: mid, right, left
    draw_list->AddTriangleFilled(ImVec2(mid_x, mid_y + spacing), ImVec2(right - spacing, bottom), ImVec2(left + spacing, bottom),
                                 COLORS[(color_start + 2) % 4]);
    // Left triangle, vertices: mid, bottom, top
    draw_list->AddTriangleFilled(ImVec2(mid_x - spacing, mid_y), ImVec2(left, bottom - spacing), ImVec2(left, top + spacing),
                                 COLORS[(color_start + 1) % 4]);
}

void RoR::DrawGCheckbox(GVarPod_A<bool> &gvar, const char *label)
{
    bool val = gvar.GetActive();
    if (ImGui::Checkbox(label, &val)) { gvar.SetActive(val); }
}
...
void RoR::DrawGIntCheck(GVarPod_A<int> &gvar, const char *label)
{
    ...
    if (ImGui::Checkbox(label, &val)) { gvar.SetActive(val ? 1 : 0); }
}

void RoR::DrawGIntBox(GVarPod_A<int> &gvar, const char *label)
{
    int val = gvar.GetActive();
    if (ImGui::InputInt(label, &val, 1, 100, ImGuiInputTextFlags_EnterReturnsTrue)) { gvar.SetActive(val); }
}

void RoR::DrawGIntSlider(GVarPod_A<int> &gvar, const char *label, int v_min, int v_max)
{
    int val = gvar.GetActive();
    if (ImGui::SliderInt(label, &val, v_min, v_max)) { gvar.SetActive(val); }
}

void RoR::DrawGFloatSlider(GVarPod_A<float> &gvar, const char *label, float v_min, float v_max)
{
    float val = gvar.GetActive();
    if (ImGui::SliderFloat(label, &val, v_min, v_max, "%.2f")) { gvar.SetActive(val); }
}

void RoR::DrawGFloatBox(GVarPod_A<float> &gvar, const char *label)
{
    float fval = gvar.GetActive();
    if (ImGui::InputFloat(label, &fval, 0.f, 0.f, -1, ImGuiInputTextFlags_EnterReturnsTrue)) { gvar.SetActive(fval); }
}
