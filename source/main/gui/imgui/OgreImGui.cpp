...
void OgreImGui::Init(Ogre::SceneManager *scenemgr)
{
    ...
    io.KeyMap[ImGuiKey_Tab] = OIS::KC_TAB; // Keyboard mapping. ImGui will use those indices to peek into the io.KeyDown[] array
...
void OgreImGui::InjectKeyPressed(const OIS::KeyEvent &arg)
{
    ...
    if (arg.text > 0) { io.AddInputCharacter((unsigned short)arg.text); }
}
...
void OgreImGui::Render()
{
    ...
    Ogre::Matrix4 projMatrix(2.0f / (R - L), 0.0f, 0.0f, (L + R) / (L - R), 0.0f, -2.0f / (B - T), 0.0f, (T + B) / (B - T), 0.0f,
    ...
    if (vp != nullptr) // Viewport is sometimes 'null' on Windows/Dx9 right after game startup or map load - probably related to
                       // switching Ogre::Camera-s ~ only_a_ptr, 01/2018
    {
    ...
    for (int i = 0; i < draw_data->CmdListsCount; ++i)
    {
        ...
        for (int j = 0; j < draw_list->CmdBuffer.Size; ++j)
        {
            ...
            renderable.updateVertexData(draw_list->VtxBuffer.Data, &draw_list->IdxBuffer.Data[startIdx],
                                        draw_list->VtxBuffer.Size, drawCmd->ElemCount);
...
void OgreImGui::createMaterial()
{
    ...
    {
        vertexShaderUnified = mgr.createProgram("imgui/VP", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "unified",
                                                Ogre::GPT_VERTEX_PROGRAM);
    }
    if (pixelShaderUnified.isNull())
    {
        pixelShaderUnified = mgr.createProgram("imgui/FP", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "unified",
    ...
    {
        vertexShaderD3D11 = mgr.createProgram("imgui/VP/D3D11", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "hlsl",
                                              Ogre::GPT_VERTEX_PROGRAM);
    ...
    {
        pixelShaderD3D11 = mgr.createProgram("imgui/FP/D3D11", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "hlsl",
                                             Ogre::GPT_FRAGMENT_PROGRAM);
    ...
    {
        vertexShaderD3D9 = mgr.createProgram("imgui/VP/D3D9", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "hlsl",
                                             Ogre::GPT_VERTEX_PROGRAM);
    ...
    {
        pixelShaderD3D9 = mgr.createProgram("imgui/FP/D3D9", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "hlsl",
                                            Ogre::GPT_FRAGMENT_PROGRAM);
    ...
    {
        vertexShaderGL = mgr.createProgram("imgui/VP/GL130", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "glsl",
                                           Ogre::GPT_VERTEX_PROGRAM);
    ...
    {
        pixelShaderGL = mgr.createProgram("imgui/FP/GL130", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "glsl",
                                          Ogre::GPT_FRAGMENT_PROGRAM);
    ...
    Ogre::MaterialPtr imguiMaterial =
        Ogre::MaterialManager::getSingleton().create("imgui/material", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
    ...
    mPass->setSeparateSceneBlending(Ogre::SBF_SOURCE_ALPHA, Ogre::SBF_ONE_MINUS_SOURCE_ALPHA, Ogre::SBF_ONE_MINUS_SOURCE_ALPHA,
                                    Ogre::SBF_ZERO);
...
void OgreImGui::createFontTexture()
{
    ...
    mFontTex =
        Ogre::TextureManager::getSingleton().createManual("ImguiFontTex", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
                                                          Ogre::TEX_TYPE_2D, width, height, 1, 1, Ogre::PF_BYTE_RGBA);

    // Lock texture for writing
    const Ogre::PixelBox &lockBox =
        mFontTex->getBuffer()->lock(Ogre::Box(0, 0, width, height), Ogre::HardwareBuffer::HBL_DISCARD);
...
OgreImGui::ImGUIRenderable::ImGUIRenderable() : mVertexBufferSize(5000), mIndexBufferSize(10000)
{
...
/// @author
/// https://bitbucket.org/ChaosCreator/imgui-ogre2.1-binding/src/8f1a01db510f543a987c3c16859d0a33400d9097/ImguiRenderable.cpp?at=master&fileviewer=file-view-default
/// Commentary on OGRE forums: http://www.ogre3d.org/forums/viewtopic.php?f=5&t=89081#p531059
void OgreImGui::ImGUIRenderable::updateVertexData(const ImDrawVert *vtxBuf, const ImDrawIdx *idxBuf, unsigned int vtxCount,
                                                  unsigned int idxCount)
{
    ...
    if (bind->getBindings().empty() || mVertexBufferSize != vtxCount)
    {
        ...
        bind->setBinding(0, Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(sizeof(ImDrawVert), mVertexBufferSize,
    ...
    if (mRenderOp.indexData->indexBuffer.isNull() || mIndexBufferSize != idxCount)
    {
        ...
        mRenderOp.indexData->indexBuffer = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(
            Ogre::HardwareIndexBuffer::IT_16BIT, mIndexBufferSize, Ogre::HardwareBuffer::HBU_WRITE_ONLY);
    }
