...
// Removing this file from your project is hindering your access to documentation, likely leading you to poorer usage of the
// library.
...
#include <stdlib.h>                       // NULL, malloc, free, qsort, atoi
    ...
    #pragma clang diagnostic ignored                                                                                             \
        "-Wdeprecated-declarations" // warning : 'xx' is deprecated: The POSIX name for this item.. // for strdup used in demo
...
void ImGui::ShowTestWindow(bool *p_open)
{
    ...
    {
        ImGui::TextWrapped("This window is being created by the ShowTestWindow() function. Please refer to the code for "
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        {
            ImGui::TextWrapped("The logging API redirects all text output so you can easily capture the content of a window or a "
                               "block. Tree nodes can be automatically expanded. You can also call ImGui::LogText() to output "
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        if (ImGui::TreeNode("Trees"))
        {
            ...
            if (ImGui::TreeNode("Advanced, with Selectable nodes"))
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    // Disable the default open on single-click behavior and pass in Selected flag according to our selection
                    // state.
                    ...
                    {
                        // Leaf: The only reason we have a TreeNode at all is to allow selection of the leaf. Otherwise we can use
        ...
        if (ImGui::TreeNode("UTF-8 Text"))
        {
            ...
            // http://mplus-fonts.sourceforge.jp/mplus-outline-fonts/index-en.html) Most compiler appears to support UTF-8 in
            // source code (with Visual Studio you need to save your file as 'UTF-8 without signature') However for the sake for
            // maximum portability here we are *not* including raw UTF-8 character in this source file, instead we encode the
            // string with hexadecimal constants. In your own application be reasonable and use UTF-8 in source or retrieve the
            // data from file system! Note that characters values are preserved even if the font cannot be displayed, so you can
        ...
        ImGui::Combo(
            "combo", &item,
            "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0"); // Combo using values packed in a single constant string (for really quick combo)

        const char *items[] = {"AAAA", "BBBB", "CCCC", "DDDD", "EEEE", "FFFF", "GGGG", "HHHH", "IIII", "JJJJ", "KKKK"};
        static int  item2   = -1;
        ImGui::Combo("combo scroll", &item2, items,
                     IM_ARRAYSIZE(items)); // Combo using proper array. You can also pass a callback to retrieve array value, no
                                           // need to create/copy an array just for that.
    ...
    if (ImGui::CollapsingHeader("Graphs widgets"))
    {
        ...
        // FIXME: This is rather awkward because current plot API only pass in indices. We probably want an API passing floats and
        ...
        // Typically we would use ImVec2(-1.0f,0.0f) to use all available width, or ImVec2(width,0.0f) for a specified width.
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        {
            ImGui::TextWrapped(
                "(Using ImGui::BeginGroup()/EndGroup() to layout items. BeginGroup() basically locks the horizontal position. "
                "EndGroup() bundles the whole group so that you can use functions such as IsItemHovered() on it.)");
            ImGui::BeginGroup();
            {
                ImGui::BeginGroup();
                ImGui::Button("AAA");
        ...
        {
            ImGui::TextWrapped(
                "(This is testing the vertical alignment that occurs on text to keep it at the same baseline as widgets. Lines "
            ...
            ImGui::AlignFirstTextHeightToWidgets(); // Vertically align text node a bit lower so it'll be vertically centered with
                                                    // upcoming widget. Otherwise you can use SmallButton (smaller fit).
            bool node_open = ImGui::TreeNode("Node##2"); // Common mistake to avoid: if we want to SameLine after TreeNode we need
                                                         // to do it before we add child content.
            ImGui::SameLine(0.0f, spacing);
        ...
        {
            ImGui::Bullet();
            ImGui::TextWrapped("Horizontal scrolling for a window has to be enabled explicitly via the "
            ...
            {
                // Display random stuff (for the sake of this trivial demo we are using basic Button+SameLine. If you want to
                // create your own time line for a real application you may be better off manipulating the cursor position
                // yourself, aka using SetCursorPos/SetCursorScreenPos to position the widgets yourself. You may also want to use
                ...
                for (int n = 0; n < num_buttons; n++)
                {
                    ...
                    const char *label = (!(n % 15))
                                            ? "FizzBuzz"
                                            : (!(n % 3)) ? "Fizz" : (!(n % 5)) ? "Buzz" : (sprintf(num_buf, "%d", n), num_buf);
                    float hue = n * 0.05f;
        ...
        if (ImGui::TreeNode("Clipping"))
        {
            ...
            ImGui::TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it won't fit in its frame. "
                               "Otherwise we are doing coarser clipping + passing a scissor rectangle to the renderer. The "
                               "system is designed to try minimizing both execution and CPU/GPU rendering cost.");
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows"))
    {
        if (ImGui::TreeNode("Popups"))
        {
            ...
            ImGui::Separator();
            // NB: As a quirk in this very specific example, we want to differentiate the parent of this menu from the parent of
            // the various popup menus above. To do so we are encloding the items in a PushID()/PopID() block to make them two
            // different menusets. If we don't, opening any popup above and hovering our menu here would open it. This is because
            // once a menu is active, we allow to switch to a sibling menu by just hovering on it, which is the desired behavior
            // for regular menus.
            ImGui::PushID("foo");
    ...
    if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus"))
    {
        ...
        if (ImGui::TreeNode("Dragging"))
        {
            ...
            if (ImGui::IsItemActive())
            {
                ...
                ImGui::SameLine();
                ImGui::Text("Raw (%.1f, %.1f), WithLockThresold (%.1f, %.1f), MouseDelta (%.1f, %.1f)", value_raw.x, value_raw.y,
                            value_with_lock_threshold.x, value_with_lock_threshold.y, mouse_delta.x, mouse_delta.y);
            }
            ImGui::TreePop();
        }
        ...
        {
            ImGui::TextWrapped(
                "Your application can render a different mouse cursor based on what ImGui::GetMouseCursor() returns. You can "
                "also set io.MouseDrawCursor to ask ImGui to render the cursor for you in software.");
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref)
{
    ...
    // You can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it compares to the default
    // style)
    ...
    if (ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size))
    {
        ...
        for (int i = 0; i < atlas->Fonts.Size; i++)
        {
            ...
            if (ImGui::TreeNode("Details"))
            {
                ...
                ImGui::SameLine();
                ShowHelpMarker(
                    "Note than the default embedded font is NOT meant to be scaled.\n\nFont are currently rendered into bitmaps "
                    "at a given size at the time of building the atlas. You may oversample them to get some flexibility with "
                    "scaling. You can also render at multiple sizes and select which one to use at runtime.\n\n(Glimmer of hope: "
                    "the atlas system should hopefully be rewritten in the future to make scaling more natural and automatic.)");
                ImGui::Text("Ascent: %f, Descent: %f, Height: %f", font->Ascent, font->Descent, font->Ascent - font->Descent);
                ImGui::Text("Fallback character: '%c' (%d)", font->FallbackChar, font->FallbackChar);
                ImGui::Text("Texture surface: %d pixels (approx) ~ %dx%d", font->MetricsTotalSurface,
                            (int)sqrtf((int)font->MetricsTotalSurface), (int)sqrtf((int)font->MetricsTotalSurface));
                ...
                if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size))
                {
                    ...
                    const ImFont::Glyph *glyph_fallback = font->FallbackGlyph; // Forcefully/dodgily make FindGlyph() return NULL
                                                                               // on fallback, which isn't the default behavior.
                    ...
                    for (int base = 0; base < 0x10000; base += 256)
                    {
                        ...
                        if (count > 0 && ImGui::TreeNode((void *)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base + 255,
                                                         count, count > 1 ? "glyphs" : "glyph"))
                        {
                            ...
                            for (int n = 0; n < 256; n++)
                            {
                                ...
                                font->RenderChar(
                                    ...
                                    (ImWchar)(base + n)); // We use ImFont::RenderChar as a shortcut because we don't have UTF-8
                                                          // conversion functions available to generate a string.
...
// Read section "How can I have multiple widgets with the same label? Can I have widget without a label? (Yes). A primer on the
// purpose of labels/IDs." about ID.
...
static void ShowExampleAppCustomRendering(bool *p_open)
{
    ...
    ImGui::ColorEdit3("Color", &col.x);
    {
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            draw_list->AddBezierCurve(ImVec2(x, y), ImVec2(x + sz * 1.3f, y + sz * 0.3f),
                                      ImVec2(x + sz - sz * 1.3f, y + sz - sz * 0.3f), ImVec2(x + sz, y + sz), col32, thickness);
            x = p.x + 4;
            y += sz + spacing;
        }
    ...
    ImGui::Separator();
    {
        ...
        // However you can draw directly and poll mouse/keyboard by yourself. You can manipulate the cursor using GetCursorPos()
        // and SetCursorPos(). If you only use the ImDrawList API, you can notify the owner window of its extends by using
        // SetCursorPos(max).
...
struct ExampleAppConsole
{
    ...
    void Draw(const char *title, bool *p_open)
    {
        ...
        // lines that are visible using the ImGuiListClipper helper, if your elements are evenly spaced and you have cheap random
        // access to the elements. To use the clipper we could replace the 'for (int i = 0; i < Items.Size; i++)' loop with:
        //     ImGuiListClipper clipper(Items.Size);
        //     while (clipper.Step())
        //         for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
        // However take note that you can not use this code as is if a filter is active because it breaks the 'cheap
        // random-access' property. We would need random-access on the post-filtered list. A typical application wanting coarse
        // clipping and filtering may want to pre-compute an array of indices that passed the filtering test, recomputing this
        // array when user changes the filter, and appending newly elements as they are inserted. This is left as a task to the
        // user until we can manage to improve this example code! If your items are of variable size you may want to implement
        // code similar to what ImGuiListClipper does. Or split your data into fixed height items to allow random-seeking into
        // your list.
...
static void ShowExampleAppLayout(bool *p_open)
{
    ...
    if (ImGui::Begin("Example: Layout", p_open, ImGuiWindowFlags_MenuBar))
    {
        ...
        ImGui::Separator();
        ImGui::TextWrapped("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore "
...
static void ShowExampleAppPropertyEditor(bool *p_open)
{
    ...
    struct funcs
    {
        static void ShowDummyObject(const char *prefix, int uid)
        {
            ...
            ImGui::AlignFirstTextHeightToWidgets(); // Text and Tree nodes are less high than regular widgets, here we add
