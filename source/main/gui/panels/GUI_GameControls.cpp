...
void GameControls::Draw()
{
    ...
    if (m_tab == ControlsTab::AIRPLANE)
    {
        ...
        for (auto& event: App::GetInputEngine()->getEvents())
        {
            if (App::GetInputEngine()->eventIDToName(event.first).find("AIRPLANE") == 0)
            {
                ImGui::AlignFirstTextHeightToWidgets();
                ImGui::TextColored(theme.value_blue_text_color, "%s", App::GetInputEngine()->eventIDToName(event.first).c_str());
            }
        }
        ...
        for (auto& event: App::GetInputEngine()->getEvents())
        {
            if (App::GetInputEngine()->eventIDToName(event.first).find("AIRPLANE") == 0)
            {
                ImGui::PushID(App::GetInputEngine()->eventIDToName(event.first).c_str());
                App::GetInputEngine()->EditInputConfig(App::GetInputEngine()->getEventCommand(event.first).c_str(), App::GetInputEngine()->eventIDToName(event.first).c_str(), buf);
                ImGui::PopID();
            }
        }
        ...
        for (auto& event: App::GetInputEngine()->getEvents())
        {
            ...
            {
                ImGui::AlignFirstTextHeightToWidgets();
                ImGui::TextColored(GRAY_HINT_TEXT, "%s", App::GetInputEngine()->eventIDToDescription(event.first).c_str());
           }
       } 
    }
