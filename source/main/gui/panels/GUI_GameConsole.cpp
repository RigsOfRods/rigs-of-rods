...
Console::Console()
{
    ...
    ((MyGUI::Window *)mMainWidget)
        ->setPosition((parentSize.width - windowSize.width) / 2, (parentSize.height - windowSize.height) / 2);
...
void Console::eventButtonPressed(MyGUI::Widget *_sender, MyGUI::KeyCode _key, MyGUI::Char _char)
{
    ...
    {
        if (HistoryCursor > iText) HistoryCursor = 0;

        if (sTextHistory[HistoryCursor] != "") m_Console_TextBox->setCaption(sTextHistory[HistoryCursor]);
    ...
    {
        if (HistoryCursor < 0) HistoryCursor = iText;
...
void Console::eventCommandAccept(MyGUI::Edit *_sender)
{
    ...
    if (msg[0] == '/' || msg[0] == '\\')
    {
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP,
                   _L("Using slashes before commands are deprecated, you can now type command without any slashes"), "help.png");
        msg.erase(0, 1);
    }
    ...
    if (args[0] == "help")
    {
        ...
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP, _L("terrainheight - get height of terrain at current position"),
                   "world.png");
...
#ifdef USE_ANGELSCRIPT
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP, _L("as <code here> - interpret angel code using console"),
                   "script_go.png");
#endif // USE_ANGELSCRIPT

        putMessage(
            CONSOLE_MSGTYPE_INFO, CONSOLE_HELP,
            _L("gravity <real> or <text string> - changes gravity constant. Outputs current value if no argument is given"),
            "script_go.png");
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP,
                   _L("Possible values: \n earth \n moon \n jupiter \n A random number (use negative)"), "script_go.png");

        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP, _L("setwaterlevel <real> or default - changes water's level"),
                   "script_go.png");

        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP, _L("spawnobject <odef name> - spawn a object at the player position"),
                   "script_go.png");

        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_TITLE, _L("Tips:"), "help.png");
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP, _L("- use Arrow Up/Down Keys in the InputBox to reuse old messages"),
                   "information.png");
        return;
    }
    else if (args[0] == "gravity")
    {
        ...
        else
        {
            putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                       _L("Current gravity is: ") + StringConverter::toString(App::GetSimTerrain()->getGravity()),
                       "information.png");
            return;
        }

        App::GetSimTerrain()->setGravity(gValue);
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY, _L("Gravity set to: ") + StringConverter::toString(gValue),
                   "information.png");
        return;
    }
    else if (args[0] == "setwaterlevel" && is_appstate_sim)
    {
        if (args.size() > 1)
        {
            ...
            if (water != nullptr)
            {
                ...
                water->UpdateWater();
                putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                           _L("Water level set to: ") + StringConverter::toString(water->GetStaticWaterHeight()),
                           "information.png");
            }
            else
            {
                putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_ERROR, _L("This terrain does not have water."),
                           "information.png");
            }
        }
        else
    ...
    else if (args[0] == "pos" && (is_appstate_sim && !is_sim_select))
    {
        ...
        if (!b && gEnv->player)
        {
            ...
            putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                       _L("Character position: ") + String("x: ") + TOSTRING(pos.x) + String(" y: ") + TOSTRING(pos.y) +
        ...
        else if (b)
        {
            ...
            putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                       _L("Vehicle position: ") + String("x: ") + TOSTRING(pos.x) + String(" y: ") + TOSTRING(pos.y) +
                           String(" z: ") + TOSTRING(pos.z),
                       "world.png");
        }
    ...
    else if (args[0] == "goto" && (is_appstate_sim && !is_sim_select))
    {
        if (args.size() != 4)
        {
            putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_HELP, RoR::Color::CommandColour + _L("usage: goto x y z"),
                       "information.png");
            return;
        }
        ...
        if (!b && gEnv->player)
        {
            ...
            putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                       _L("Character position set to: ") + String("x: ") + TOSTRING(pos.x) + String(" y: ") + TOSTRING(pos.y) +
                           String(" z: ") + TOSTRING(pos.z),
                       "world.png");
        }
        else if (b)
        {
            ...
            putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                       _L("Vehicle position set to: ") + String("x: ") + TOSTRING(pos.x) + String(" y: ") + TOSTRING(pos.y) +
                           String(" z: ") + TOSTRING(pos.z),
                       "world.png");
        }
    ...
    {
        if (!App::GetSimTerrain()) return;
        Vector3 pos = Vector3::ZERO;

        Actor *b = App::GetSimController()->GetPlayerActor();
        if (!b && gEnv->player) { pos = gEnv->player->getPosition(); }
        else if (b)
        {
            pos = b->getPosition();
        }
        ...
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                   _L("Terrain height at position: ") + String("x: ") + TOSTRING(pos.x) + String("z: ") + TOSTRING(pos.z) +
                       _L(" = ") + TOSTRING(h),
                   "world.png");
    ...
    else if (args[0] == "ver")
    {
        ...
        putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                   " build time: " + String(ROR_BUILD_DATE) + ", " + String(ROR_BUILD_TIME), "information.png");
        return;
    }
    else if (args[0] == "quit")
    {
    ...
    else if (args[0] == "spawnobject" && (is_appstate_sim && !is_sim_select))
    {
        ...
        if (gEnv->player) { pos = gEnv->player->getPosition(); }
        ...
        try
        {
            ...
            putMessage(CONSOLE_MSGTYPE_INFO, CONSOLE_SYSTEM_REPLY,
                       _L("Spawned object at position: ") + String("x: ") + TOSTRING(pos.x) + String("z: ") + TOSTRING(pos.z),
                       "world.png");
        }
