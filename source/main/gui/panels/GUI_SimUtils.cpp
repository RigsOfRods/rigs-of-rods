...
void CLASS::FrameStepSimGui(float dt)
{
    ...
    if (m_notification->getVisible())
    {
        ...
        if (ot < startTime) { m_notifi_box_alpha = 1.0f; }
        else
        {
        ...
        if (m_notifi_box_alpha <= 0.1) m_notification->setVisible(false);
    }
}
...
void CLASS::UpdateStats(float dt, Actor *actor)
{
    ...
    {
        if (!m_truckinfo_box->getVisible()) m_truckinfo_box->setVisible(true);
        ...
        {
            if (beam->bm_broken != 0) { beambroken++; }
        ...
        float health =
            ((float)beambroken / (float)actor->ar_num_beams) * 10.0f + ((float)beamdeformed / (float)actor->ar_num_beams);
        if (health < 1.0f)
        {
            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Vehicle health: ") + WhiteColor +
                                TOUTFSTRING(Round((1.0f - health) * 100.0f, 2)) + U("%") + "\n";
        }
        else if (health >= 1.0f)
        {
        ...
        m_actor_stats_str =
            m_actor_stats_str + MainThemeColor + _L("Beam count: ") + WhiteColor + TOUTFSTRING(actor->ar_num_beams) + "\n";
        m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Broken beams count: ") + WhiteColor +
                            TOUTFSTRING(beambroken) + U(" (") +
                            TOUTFSTRING(Round((float)beambroken / (float)actor->ar_num_beams, 2) * 100.0f) + U("%)") + "\n";
        m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Deformed beams count: ") + WhiteColor +
                            TOUTFSTRING(beamdeformed) + U(" (") +
                            TOUTFSTRING(Round((float)beamdeformed / (float)actor->ar_num_beams, 2) * 100.0f) + U("%)") + "\n";
        m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Average deformation: ") + WhiteColor +
                            TOUTFSTRING(Round((float)average_deformation / (float)actor->ar_num_beams, 4) * 100.0f) + "\n";
        ...
        m_actor_stats_str =
            m_actor_stats_str + MainThemeColor + _L("Average stress: ") + WhiteColor + Ogre::UTFString(beamstressstr) + "\n";
        ...
        m_actor_stats_str =
            m_actor_stats_str + MainThemeColor + _L("Node count: ") + WhiteColor + Ogre::UTFString(nodecountstr) + "\n";

        wchar_t truckmassstr[256];
        swprintf(truckmassstr, 256, L"%8.2f kg (%.2f tons)", mass, mass / 1000.0f);
        m_actor_stats_str =
            m_actor_stats_str + MainThemeColor + _L("Total mass: ") + WhiteColor + Ogre::UTFString(truckmassstr) + "\n";
        ...
        if (actor->ar_driveable == TRUCK && actor->ar_engine)
        {
            if (actor->ar_engine->GetEngineRpm() > actor->ar_engine->getMaxRPM())
                m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Engine RPM: ") + RedColor +
                                    TOUTFSTRING(Round(actor->ar_engine->GetEngineRpm())) + U(" / ") +
                                    TOUTFSTRING(Round(actor->ar_engine->getMaxRPM())) + "\n";
            else
                m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Engine RPM: ") + WhiteColor +
                                    TOUTFSTRING(Round(actor->ar_engine->GetEngineRpm())) + U(" / ") +
                                    TOUTFSTRING(Round(actor->ar_engine->getMaxRPM())) + "\n";

            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Input shaft RPM: ") + WhiteColor +
                                TOUTFSTRING(Round(std::max(0.0f, actor->ar_engine->GetInputShaftRpm()))) + "\n\n";

            float currentTorque = actor->ar_engine->GetEngineTorque();

            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Current torque: ") + WhiteColor +
                                TOUTFSTRING(Round(currentTorque)) + U(" Nm") + "\n";

            float currentKw = currentTorque * Ogre::Math::TWO_PI * (actor->ar_engine->GetEngineRpm() / 60.0f) / 1000.0f;

            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Current power: ") + WhiteColor +
                                TOUTFSTRING(Round(currentKw / 0.73549875f)) + U(" hp / ") + TOUTFSTRING(Round(currentKw)) +
                                U(" Kw") + "\n\n";

            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Current gear: ") + WhiteColor +
                                TOUTFSTRING(Round(actor->ar_engine->GetGear())) + "\n";

            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Drive ratio: ") + WhiteColor +
                                TOUTFSTRING(Round(actor->ar_engine->GetDriveRatio())) + ":1\n\n";
            ...
            if (fabs(velocityMPH) < 0.5f) { velocityKMH = velocityMPH = 0.0f; }
            if (fabs(carSpeedMPH) < 0.5f) { carSpeedKPH = carSpeedMPH = 0.0f; }

            Ogre::UTFString wsmsg = _L("Wheel speed: ");
            // Some kind of wheel skidding detection? lol
            if (velocityKMH > carSpeedKPH * 1.25f)
                m_actor_stats_str = m_actor_stats_str + MainThemeColor + wsmsg + RedColor + TOUTFSTRING(Round(velocityKMH)) +
                                    U(" km/h (") + TOUTFSTRING(Round(velocityMPH)) + U(" mph)") + "\n";
            else if (velocityKMH < carSpeedKPH / 1.25f)
                m_actor_stats_str = m_actor_stats_str + MainThemeColor + wsmsg + BlueColor + TOUTFSTRING(Round(velocityKMH)) +
                                    U(" km/h (") + TOUTFSTRING(Round(velocityMPH)) + U(" mph)") + "\n";
            else
                m_actor_stats_str = m_actor_stats_str + MainThemeColor + wsmsg + WhiteColor + TOUTFSTRING(Round(velocityKMH)) +
                                    U(" km/h (") + TOUTFSTRING(Round(velocityMPH)) + U(" mph)") + "\n";

            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Vehicle speed: ") + WhiteColor +
                                TOUTFSTRING(Round(carSpeedKPH)) + U(" km/h (") + TOUTFSTRING(Round(carSpeedMPH)) + U(" mph)") +
                                "\n";
        }
        else
        {
            float speedKN     = actor->ar_nodes[0].Velocity.length() * 1.94384449f;
            m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Current speed: ") + WhiteColor +
                                TOUTFSTRING(Round(speedKN)) + U(" kn (") + TOUTFSTRING(Round(speedKN * 1.852)) + U(" km/h) (") +
                                TOUTFSTRING(Round(speedKN * 1.151)) + U(" mph)") + "\n";

            Ogre::UTFString engmsg = _L("Engine ");
            if (actor->ar_driveable == AIRPLANE)
            {
                float altitude    = actor->ar_nodes[0].AbsPosition.y / 30.48 * 100;
                m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Altitude: ") + WhiteColor +
                                    TOUTFSTRING(Round(altitude)) + U(" feet (") + TOUTFSTRING(Round(altitude * 0.30480)) +
                                    U(" meters)") + "\n";
                for (int i = 0; i < 8; i++)
                {
                    if (actor->ar_aeroengines[i] && actor->ar_aeroengines[i]->getType() == AeroEngine::AEROENGINE_TYPE_TURBOJET)
                        m_actor_stats_str = m_actor_stats_str + MainThemeColor + engmsg +
                                            TOUTFSTRING(i + 1 /*not to start with 0, players wont like it i guess*/) + " : " +
                                            WhiteColor + TOUTFSTRING(Round(actor->ar_aeroengines[i]->getRPM())) + "%" + "\n";
                    else if (actor->ar_aeroengines[i] &&
                             actor->ar_aeroengines[i]->getType() == AeroEngine::AEROENGINE_TYPE_TURBOPROP)
                        m_actor_stats_str = m_actor_stats_str + MainThemeColor + engmsg +
                                            TOUTFSTRING(i + 1 /*not to start with 0, players wont like it i guess*/) + " : " +
                                            WhiteColor + TOUTFSTRING(Round(actor->ar_aeroengines[i]->getRPM())) + " RPM" + "\n";
                }
            }
            else if (actor->ar_driveable == BOAT)
            {
                for (int i = 0; i < 8; i++)
                {
                    if (actor->ar_screwprops[i])
                        m_actor_stats_str = m_actor_stats_str + MainThemeColor + engmsg +
                                            TOUTFSTRING(i + 1 /*not to start with 0, players wont like it i guess*/) + " : " +
                                            WhiteColor + TOUTFSTRING(Round(actor->ar_screwprops[i]->getThrottle() * 100)) + "%" +
                                            "\n";
                }
            }
        }
        ...
        m_actor_stats_str = m_actor_stats_str + MainThemeColor + _L("Top speed: ") + WhiteColor + TOUTFSTRING(Round(speedKPH)) +
                            U(" km/h (") + TOUTFSTRING(Round(speedMPH)) + U(" mph)") + "\n";
