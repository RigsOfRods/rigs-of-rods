...
#define MAIN_WIDGET ((MyGUI::Window *)mMainWidget)
        ...
        void TextureToolWindow::fillCombo()
        {
            ...
            if (mCBo->getItemCount() > 0)
            {
                int        index = 0;
                const auto it =
                    std::find_if(texture_names.begin(), texture_names.end(), [](const std::pair<std::string, std::string> &t) {
                        return t.second.substr(0, 9) == "MapRttTex";
                    });
                if (it != texture_names.end()) { index = std::distance(texture_names.begin(), it); }
        ...
        void TextureToolWindow::saveTexture(String texName, bool usePNG)
        {
            try
            {
                ...
                RoR::App::GetConsole()->putMessage(Console::CONSOLE_MSGTYPE_INFO, Console::CONSOLE_MSGTYPE_INFO, msg,
                                                   "information.png");
            }
            catch (Exception &e)
            {
                UTFString str = "Exception while saving image: " + e.getFullDescription();
                RoR::App::GetConsole()->putMessage(Console::CONSOLE_MSGTYPE_INFO, Console::CONSOLE_MSGTYPE_INFO, str,
                                                   "error.png");
            }
        }
        ...
        void TextureToolWindow::updateControls(String texName)
        {
            try
            {
                ...
                String usageStr = "";
                if (tex->getUsage() & TU_STATIC) usageStr += "static,\n";
