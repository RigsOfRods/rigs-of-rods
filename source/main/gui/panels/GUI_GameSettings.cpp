...
void RoR::GUI::GameSettings::Draw()
{
    ...
    if (!is_visible)
    {
        ...
        if (App::app_state.GetActive() == RoR::AppState::MAIN_MENU) { App::GetGuiManager()->SetVisible_GameMainMenu(true); }
        ImGui::End();
        return;
    }
    ...
    ImGui::SameLine();
    if (ImGui::Button(_LC("GameSettings", "Update cache"))) { App::app_force_cache_udpate.SetActive(true); }
    ...
    if (m_tab == SettingsTab::RENDER_SYSTEM)
    {
        ...
        std::set<std::string> filter         = {
            "Allow NVPerfHUD",          "Colour Depth",   "Fixed Pipeline Enabled", "Floating-point mode",
            "Resource Creation Policy", "VSync Interval", "sRGB Gamma Conversion"};
        for (auto opt : config_options)
        {
            auto co = opt.second;
            if (co.immutable) continue;
            if (co.possibleValues.empty()) continue;
            if (filter.find(co.name) != filter.end()) continue;
    ...
    else if (m_tab == SettingsTab::GENERAL)
    {
        ...
        const auto it = std::find_if(languages.begin(), languages.end(), [](const std::pair<std::string, std::string> &l) {
            return l.second == App::app_language.GetActive();
        });
        ...
        int sshot_select =
            (std::strcmp(App::app_screenshot_format.GetActive(), "jpg") == 0) ? 1 : 0; // Hardcoded; TODO: list available formats.
    ...
    else if (m_tab == SettingsTab::GRAPHICS)
    {
        ...
        if (App::gfx_texture_filter.GetActive() == GfxTexFilter::ANISOTROPIC)
        {
            ...
            if (ImGui::Combo(_LC("GameSettings", "Anisotropy"), &selection,
                             "1\0"
                             "2\0"
                             "4\0"
                             "8\0"
    ...
    else if (m_tab == SettingsTab::DIAG)
    {
        ...
        if (ImGui::Button(_LC("GameSettings", "Rebuild cache"))) { App::app_force_cache_purge.SetActive(true); }
    }
