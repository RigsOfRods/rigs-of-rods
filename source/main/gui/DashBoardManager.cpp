...
int DashBoardManager::getLinkIDForName(Ogre::String &str)
{
    ...
    {
        if (!strcmp(data[i].name, s)) return i;
    }
    return -1;
}
...
float DashBoardManager::getNumeric(size_t key)
{
    ...
    {
    case DC_BOOL: return data[key].data.value_bool ? 1.0f : 0.0f;
    case (DC_INT): return (float)data[key].data.value_int;
    case (DC_FLOAT): return data[key].data.value_float;
    }
    return 0;
}
...
void DashBoardManager::setVisible(bool visibility)
{
    ...
    for (int i = 0; i < free_dashboard; i++)
    {
        if (!dashboards[i]->getIsTextureLayer()) dashboards[i]->setVisible(visibility);
    }
}
...
void DashBoardManager::setVisible3d(bool visibility)
{
    for (int i = 0; i < free_dashboard; i++)
    {
        if (dashboards[i]->getIsTextureLayer()) dashboards[i]->setVisible(visibility, false);
    }
}
...
void DashBoardManager::windowResized()
{
    for (int i = 0; i < free_dashboard; i++)
    {
        if (dashboards[i]) dashboards[i]->windowResized();
    }
}
...
DashBoard::DashBoard(DashBoardManager *manager, Ogre::String filename, bool _textureLayer)
    : manager(manager), filename(filename), free_controls(0), visible(false), mainWidget(nullptr), textureLayer(_textureLayer)
{
...
void DashBoard::update(float &dt)
{
    ...
    for (int i = 0; i < free_controls; i++)
    {
        ...
        if (controls[i].animationType == ANIM_ROTATE)
        {
            ...
            // calculate the angle
            float angle =
                (val - controls[i].vmin) * (controls[i].wmax - controls[i].wmin) / (controls[i].vmax - controls[i].vmin) +
                controls[i].wmin;
        ...
        else if (controls[i].animationType == ANIM_LAMP)
        {
            ...
            // switch states
            if (state) { controls[i].img->setImageTexture(String(controls[i].texture) + "-on.png"); }
            else
            {
        ...
        else if (controls[i].animationType == ANIM_TEXTFORMAT)
        {
            ...
            MyGUI::UString s;
            if (strlen(controls[i].format) == 0) { s = Ogre::StringConverter::toString(val); }
            else
            {
                char tmp[1024] = "";
                ...
                if (strcmp(tmp, controls[i].format_neg_zero) == 0) { sprintf(tmp, controls[i].format, 0.f); }
...
void DashBoard::windowResized()
{
    ...
    if (textureLayer)
    {
        ...
        if (!tex.isNull()) mainWidget->setSize(tex->getWidth(), tex->getHeight());
    }
    else
    {
...
void DashBoard::loadLayoutRecursive(MyGUI::WidgetPtr w)
{
    ...
    if (!linkArgs.empty())
    {
        ...
        else if (anim == "textformat")
        {
            ...
            if (std::strlen(ctrl.format)) { std::snprintf(ctrl.format_neg_zero, 255, ctrl.format, -0.f); }
        }
