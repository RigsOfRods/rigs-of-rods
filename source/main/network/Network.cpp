...
namespace RoR
{
    ...
    #define LOG_THREAD(_MSG_)                                                                                                    \
        {                                                                                                                        \
            std::stringstream s;                                                                                                 \
            s << _MSG_ << " (Thread ID: " << std::this_thread::get_id() << ")";                                                  \
            LOG(s.str());                                                                                                        \
        }
    #define LOGSTREAM Ogre::LogManager().getSingleton().stream()
        ...
        Ogre::ColourValue GetPlayerColor(int color_num)
        {
            ...
            if (color_num < 0 || color_num >= numColours) return Ogre::ColourValue::ZERO;
        ...
        void DebugPacket(const char *name, RoRnet::Header *header, char *buffer)
        {
            ...
            msg << "++ " << name << ": " << header->source << ", " << header->streamid << ", " << header->command << ", "
                << header->size << ", hash: " << HashData(buffer, header->size);
            LOG(msg.str());
        }
        ...
        bool SendNetMessage(int type, unsigned int streamid, int len, char *content)
        {
            ...
            if (msgsize >= RORNET_MAX_MESSAGE_LENGTH) { return false; }
        ...
        int ReceiveMessage(RoRnet::Header *head, char *content, int bufferlen)
        {
            ...
            if (head->size > uint32_t(bufferlen)) { return -3; }
        ...
        void RecvThread()
        {
            ...
            while (!m_shutdown)
            {
                ...
                // LOG("Received data: " + TOSTRING(header.command) + ", source: " + TOSTRING(header.source) + ":" +
                // TOSTRING(header.streamid) + ", size: " + TOSTRING(header.size));
                if (err != 0)
                {
                ...
                {
                    if (header.source == m_uid) continue;

                    RoRnet::StreamRegister *reg = (RoRnet::StreamRegister *)buffer;

                    LOG(" * received stream registration: " + TOSTRING(header.source) + ": " + TOSTRING(header.streamid) +
                ...
                {
                    if (header.size != sizeof(int)) { continue; }
                ...
                else if (header.command == MSG2_USER_LEAVE)
                {
                    ...
                    {
                        m_shutdown =
                            true; // Atomic; stop sending/receiving data - server closes the connection quickly after kick.

                        std::stringstream msg;
                        msg << _L("disconnected: remote side closed the connection");
                        msg << " ** ";
                        msg << buffer;
                        if (std::strstr(buffer, "disconnected on request") !=
                            nullptr) // FIXME: Add a reason code to MSG2_USER_LEAVE, this is ugly!
                    ...
                    else
                    {
                        ...
                        auto user = std::find_if(m_users.begin(), m_users.end(), [header](const RoRnet::UserInfo &u) {
                            return static_cast<int>(u.uniqueid) == header.source;
                        });
                        if (user != m_users.end())
                        {
                            Ogre::UTFString msg = RoR::ChatSystem::GetColouredName(user->username, user->colournum) +
                                                  RoR::Color::CommandColour + _L(" left the game");
                    ...
                    else
                    {
                        ...
                        bool user_exists = false;
                        {
                            ...
                            if (!user_exists)
                            {
                                ...
                                Ogre::UTFString msg = RoR::ChatSystem::GetColouredName(user_info.username, user_info.colournum) +
                                                      RoR::Color::CommandColour + _L(" joined the game");
                                const char *   utf8_line = msg.asUTF8_c_str();
                                RoRnet::Header head;
                                head.command = MSG2_UTF8_CHAT;
                                head.source  = -1;
        ...
        {
            RoR::LogFormat("[RoR|Networking] Failed to connect to server [%s:%d], message: %s", m_net_host.GetBuffer(),
                           m_net_port, msg.c_str());
        ...
        NetEventQueue CheckEvents()
        {
            ...
            while (!m_event_queue.empty())
            {
                ...
                case NetEvent::Type::CONNECT_SUCCESS:
                    if (m_connect_thread.joinable()) m_connect_thread.join(); // Clean up
                    break;
                default:;
                }
        ...
        bool ConnectThread()
        {
            ...
            if (header.command == MSG2_WRONG_VER_LEGACY) // Pre-RoRnet_2.40 server
            {
                ...
                Ogre::UTFString format_wstr = _L("Establishing network session: wrong server version, you are using version '%s' "
                                                 "and the server is using '%s'");
                const char *    server_ver =
                    (info.protocolversion[0] != 0) ? info.protocolversion : "~ RoRnet_2.38 or earlier (not detected) ~";
                char msg_buf[500];
            ...
            {
                wchar_t         tmp[512] = L"";
                Ogre::UTFString tmp2 = _L("Establishing network session: wrong server version, you are using version '%s' and "
            ...
            strncpy((char *)c.username, m_username.substr(0, RORNET_MAX_USERNAME_LEN * 0.5f).asUTF8_c_str(),
        ...
        void Disconnect()
        {
            ...
            if (is_clean_disconnect) { SendNetMessage(MSG2_USER_LEAVE, 0, 0, 0); }

            m_recv_thread.join();
            LOG("[RoR|Networking] Disconnect() receiver thread cleaned up");

            if (is_clean_disconnect) { socket.disconnect(); }
            else
            {
                socket.close_fd();
            }
        ...
        void AddPacket(int streamid, int type, int len, char *content)
        {
            ...
            if (len > max_len)
            {
                LOGSTREAM << "[RoR|Networking] Discarding network packet (StreamID: " << streamid << ", Type: " << type
                          << "), length is " << len << ", max is " << max_len;
                return;
            }
            ...
            { // Lock scope
                ...
                if (type == MSG2_STREAM_DATA_DISCARDABLE)
                {
                    ...
                    auto search =
                        std::find_if(m_send_packet_buffer.begin(), m_send_packet_buffer.end(), [&](const send_packet_t &p) {
                            return !memcmp(packet.buffer, p.buffer, sizeof(RoRnet::Header));
                        });
