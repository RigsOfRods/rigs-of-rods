...
RoR::OTCParser::OTCParser() : m_def(std::make_shared<RoR::OTCFile>())
{
}
    ...
    try
    {
        ...
        for (int x = 0; x <= m_def->pages_max_x; ++x)
        {
            for (int z = 0; z <= m_def->pages_max_z; ++z)
            {
                ...
                snprintf(key, 75, "%s.raw.size", base);
                int raw_size = cfg.GetInt(key, 1025);
                snprintf(key, 75, "%s.raw.bpp", base);
                int raw_bpp = cfg.GetInt(key, 2);
                snprintf(key, 75, "%s.flipX", base);
                bool flip_x = cfg.GetBool(key, false);
                snprintf(key, 75, "%s.flipY", base);
                bool flip_y = cfg.GetBool(key, false);
...
bool RoR::OTCParser::LoadPageConfig(Ogre::DataStreamPtr &ds, RoR::OTCPage &page, const char *filename)
{
    try
    {
        ...
        while (!ds->eof())
        {
            ...
            if (line_sane.empty() || line_sane[0] == ';' || line_sane[0] == '/') { continue; }
            ...
            if (args.size() > 2) { layer.diffusespecular_filename = Utils::TrimStr(args[1]); }
            if (args.size() > 1) { layer.normalheight_filename = Utils::TrimStr(args[2]); }
            if (args.size() > 3) { layer.blendmap_filename = Utils::TrimStr(args[3]); }
            if (args.size() > 4) { layer.blend_mode = Utils::TrimStr(args[4])[0]; }
            if (args.size() > 5) { layer.alpha = PARSEREAL(args[5]); }

            page.layers.push_back(layer);
        }

        if (page.heightmap_filename.find(".raw") != std::string::npos) { page.is_heightmap_raw = true; }
...
RoR::OTCFile::OTCFile()
    : world_size_x(0), world_size_y(0), world_size_z(0), world_size(0), page_size(0), pages_max_x(0), pages_max_z(0),
      origin_pos(Ogre::Vector3::ZERO), batch_size_min(0), batch_size_max(0), layer_blendmap_size(0), max_pixel_error(0),
      composite_map_size(0), composite_map_distance(0), skirt_size(0), lightmap_size(0), lightmap_enabled(false),
      norm_map_enabled(false), spec_map_enabled(false), parallax_enabled(false), global_colormap_enabled(false),
      recv_dyn_shadows_depth(false), disable_cache(false), is_flat(true)
{
}

RoR::OTCPage::OTCPage(int x_pos, int z_pos, std::string const &conf_filename, bool flipX, bool flipY, int rawsize, int rawbpp)
    : pageconf_filename(conf_filename), pos_x(x_pos), pos_z(z_pos), is_heightmap_raw(false), raw_flip_x(flipX), raw_flip_y(flipY),
      raw_size(rawsize), raw_bpp(rawbpp), num_layers(0)
{
}

RoR::OTCLayer::OTCLayer() : blend_mode('R'), alpha(static_cast<float>('R')) // Backwards compatibility, probably old typo
{
}
