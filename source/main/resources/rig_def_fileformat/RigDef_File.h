...
namespace RigDef
{
    ...
    {
        CameraSettings() : mode(MODE_ALWAYS), cinecam_index(0)
        {
        }
    ...
    {
        BeamDefaultsScale()
            : springiness(1), damping_constant(1), deformation_threshold_constant(1), breaking_threshold_constant(1)
        {
        }
    ...
    struct BeamDefaults
    {
        BeamDefaults()
            : // NOTE: -1.f is 'empty value'; replaced by constant in parser.
              springiness(-1.f), damping_constant(-1.f), deformation_threshold(-1.f), visual_beam_diameter(-1.f),
              beam_material_name("tracks/beam"), plastic_deform_coef(0.f), // This is a default
              breaking_threshold(-1.f), _enable_advanced_deformation(false), _is_plastic_deform_coef_user_defined(false),
        ...
        bool _is_user_defined; ///< Informs whether these data were read from "set_beam_defaults" directive or filled in by the
                               ///< parser on startup.
    ...
    struct Inertia
    {
        Inertia() : start_delay_factor(0), stop_delay_factor(0)
        {
        }
    ...
    {
        ManagedMaterialsOptions() : double_sided(false)
        {
        }
    ...
    {
        GuiSettings()
            : speedo_highest_kph(DEFAULT_SPEEDO_MAX), use_max_rpm(false), /* This is default */
    ...
    struct Animation
    {
        ...
        Animation() : ratio(0), lower_limit(-1.f), upper_limit(-1.f), source(0), mode(0)
        {
        }
    ...
    struct TransferCase
    {
        TransferCase() : a1(0), a2(-1), has_2wd(true), has_2wd_lo(false), gear_ratios({1.0f})
        {
        }
    ...
    {
        Beam()
            : options(0), extension_break_limit(0),                /* This is default */
              _has_extension_break_limit(false), detacher_group(0) /* 0 = Default detacher group */
        {
        }
    ...
    struct Cinecam
    {
        Cinecam() : position(Ogre::Vector3::ZERO), spring(8000.f), damping(800.f), node_mass(20.f)
        {
        }
    ...
    {
        CruiseControl() : min_speed(0), autobrake(0)
        {
        }
    ...
    struct Author
    {
        Author() : forum_account_id(0), _has_forum_account(false)
        {
        }
    ...
    struct Fileinfo
    {
        Fileinfo() : category_id(-1), file_version(0) /* Default */
        {
        }
    ...
    {
        Engine()
            : shift_down_rpm(0), shift_up_rpm(0), torque(0), global_gear_ratio(0), reverse_gear_ratio(0), neutral_gear_ratio(0)
        {
    ...
    struct SlopeBrake
    {
        SlopeBrake() : regulating_force(6.f), attach_angle(5.f), release_angle(10.f)
        {
        }
    ...
    {
        WheelDetacher() : wheel_id(0), detacher_group(0)
        {
        }
    ...
    struct BaseWheel
    {
        BaseWheel() : width(0), num_rays(0), braking(Wheels::BRAKING_NO), mass(0)
        {
        }
    ...
    {
        Wheel() : BaseWheel(), radius(0), face_material_name("tracks/wheelface"), band_material_name("tracks/wheelband1")
        {
        }
    ...
    {
        BaseWheel2() : BaseWheel(), rim_radius(0), tyre_radius(0), tyre_springiness(0), tyre_damping(0)
        {
        }
    ...
    {
        Wheel2()
            : BaseWheel2(), rim_springiness(0), rim_damping(0), face_material_name("tracks/wheelface"),
    ...
    {
        MeshWheel() : BaseWheel(), side(SIDE_INVALID), rim_radius(0), tyre_radius(0), _is_meshwheel2(false)
        {
        }
    ...
    struct Flare2
    {
        Flare2()
            : offset(0, 0, 1), /* Section 'flares(1)' has offset.z hardcoded to 1 */
              type(TYPE_f_HEADLIGHT), control_number(-1), blink_delay_milis(-2), size(-1)
        {
        }
    ...
    struct Flexbody
    {
        Flexbody() : offset(Ogre::Vector3::ZERO), rotation(Ogre::Vector3::ZERO)
        {
        }
    ...
    {
        FlexBodyWheel() : side(MeshWheel::SIDE_INVALID), rim_springiness(0), rim_damping(0)
        {
        }
    ...
    struct Shock
    {
        ...
        // Stability active suspension can be made with "L" for suspension on the truck's left and "R" for suspension on the
        // truck's right.
        ...
        float damping; ///< The 'resistance to motion' of the shock. The best value is given by this equation:  2 * sqrt(suspended
                       ///< mass * springness)
        float short_bound; ///< Maximum contraction. The shortest length the shock can be, as a proportion of its original length.
                           ///< "0" means the shock will not be able to contract at all, "1" will let it contract all the way to
                           ///< zero length. If the shock tries to shorten more than this value allows, it will become as rigid as
                           ///< a normal beam.
        float long_bound;  ///< Maximum extension. The longest length a shock can be, as a proportion of its original length. "0"
                           ///< means the shock will not be able to extend at all. "1" means the shock will be able to double its
                           ///< length. Higher values allow for longer extension.
        float precompression; ///< Changes compression or extension of the suspension when the truck spawns. This can be used to
                              ///< "level" the suspension of a truck if it sags in game. The default value is 1.0.
        unsigned int                  options; ///< Bit flags.
        std::shared_ptr<BeamDefaults> beam_defaults;
        int                           detacher_group;
    };
    ...
    struct Shock2
    {
        ...
        // Absolute metric values for shortbound/longbound, settings apply without regarding to the original length of the
        // beam.(Use with caution, check ror.log for errors)
        ...
        float     progress_factor_spring_in; ///< Progression factor for springin. A value of 0 disables this option. 1...x as
                                             ///< multipliers, example:maximum springrate == springrate + (factor*springrate)
        float progress_factor_damp_in;    ///< Progression factor for dampin. 0 = disabled, 1...x as multipliers, example:maximum
                                          ///< dampingrate == springrate + (factor*dampingrate)
        float spring_out;                 ///< spring value applied when shock extending
        float damp_out;                   ///< damping value applied when shock extending
        float progress_factor_spring_out; ///< Progression factor springout, 0 = disabled, 1...x as multipliers, example:maximum
                                          ///< springrate == springrate + (factor*springrate)
        float progress_factor_damp_out;   ///< Progression factor dampout, 0 = disabled, 1...x as multipliers, example:maximum
                                          ///< dampingrate == springrate + (factor*dampingrate)
    ...
    {
        SkeletonSettings() : visibility_range_meters(150.f), beam_thickness_meters(BEAM_SKELETON_DIAMETER)
        {
        }
    ...
    struct Hydro
    {
        Hydro() : lenghtening_factor(0), detacher_group(0)
        {
        }
        ...
        inline bool HasFlag_a()
        {
            return options.find(RigDef::Hydro::OPTION_a_INPUT_AILERON) != std::string::npos;
        }
        inline bool HasFlag_e()
        {
            return options.find(RigDef::Hydro::OPTION_e_INPUT_ELEVATOR) != std::string::npos;
        }
        inline bool HasFlag_g()
        {
            return options.find(RigDef::Hydro::OPTION_g_INPUT_ELEVATOR_RUDDER) != std::string::npos;
        }
        inline bool HasFlag_h()
        {
            return options.find(RigDef::Hydro::OPTION_h_INPUT_InvELEVATOR_RUDDER) != std::string::npos;
        }
        inline bool HasFlag_i()
        {
            return options.find(RigDef::Hydro::OPTION_i_INVISIBLE) != std::string::npos;
        }
        inline bool HasFlag_r()
        {
            return options.find(RigDef::Hydro::OPTION_r_INPUT_RUDDER) != std::string::npos;
        }
        inline bool HasFlag_s()
        {
            return options.find(RigDef::Hydro::OPTION_s_DISABLE_ON_HIGH_SPEED) != std::string::npos;
        }
        inline bool HasFlag_u()
        {
            return options.find(RigDef::Hydro::OPTION_u_INPUT_AILERON_ELEVATOR) != std::string::npos;
        }
        inline bool HasFlag_v()
        {
            return options.find(RigDef::Hydro::OPTION_v_INPUT_InvAILERON_ELEVATOR) != std::string::npos;
        }
        inline bool HasFlag_x()
        {
            return options.find(RigDef::Hydro::OPTION_x_INPUT_AILERON_RUDDER) != std::string::npos;
        }
        inline bool HasFlag_y()
        {
            return options.find(RigDef::Hydro::OPTION_y_INPUT_InvAILERON_RUDDER) != std::string::npos;
        }

        inline void AddFlag(char flag)
        {
            options += flag;
        }
    ...
    struct Animator
    {
        Animator() : lenghtening_factor(0), flags(0), short_limit(0), long_limit(0), detacher_group(0)
        {
        }
    ...
    {
        Rotator()
            : rate(0), spin_left_key(0), spin_right_key(0), engine_coupling(1), /* Default */
    ...
    {
        Rotator2()
            : Rotator(), rotating_force(10000000), /* Default */
              tolerance(0)
        {
        }
    ...
    struct Trigger
    {
        ...
        inline bool IsHookToggleTrigger()
        {
            return HasFlag_H_LocksHookGroup() || HasFlag_h_UnlocksHookGroup();
        }

        inline bool IsTriggerBlockerAnyType()
        {
            return HasFlag_B_TriggerBlocker() || HasFlag_A_InvTriggerBlocker();
        }
        ...
        {
            assert(BITMASK_IS_0(options, OPTION_B_BLOCK_TRIGGERS | OPTION_A_INV_BLOCK_TRIGGERS | OPTION_h_UNLOCK_HOOKGROUPS_KEY |
                                             OPTION_H_LOCK_HOOKGROUPS_KEY | OPTION_E_ENGINE_TRIGGER));
    ...
    {
        MaterialFlareBinding() : flare_number(0)
        {
        }
    ...
    struct Pistonprop
    {
        Pistonprop() : turbine_power_kW(0), pitch(0)
        {
        }
    ...
    struct Prop
    {
        struct DashboardSpecial
        {
            DashboardSpecial()
                : offset(Ogre::Vector3::ZERO), // Default depends on right/left-hand dashboard placement
                  rotation_angle(160.f), _offset_is_set(false), mesh_name("dirwheel.mesh")
            {
            }
        ...
        {
            BeaconSpecial() : color(1.0, 0.5, 0.0), flare_material_name("tracks/beaconflare")
            {
            }
        ...
        Prop() : offset(Ogre::Vector3::ZERO), rotation(Ogre::Vector3::ZERO), special(SPECIAL_INVALID)
        {
        }
    ...
    struct Rope
    {
        Rope() : invisible(false), detacher_group(0) /* Global detacher group */
        {
        }
    ...
    struct SlideNode
    {
        ...
        BITMASK_PROPERTY(constraint_flags, 2, CONSTRAINT_ATTACH_FOREIGN, HasConstraint_f_AttachForeign,
                         SetConstraint_f_AttachForeign)
    ...
    struct SoundSource2 : SoundSource
    {
        ...
        SoundSource2() : mode(MODE_INVALID), cinecam_index(0)
        {
        }
    ...
    struct SpeedLimiter
    {
        SpeedLimiter() : max_speed(0.f), is_enabled(false)
        {
        }
    ...
    struct TorqueCurve
    {
        ...
        struct Sample
        {
            Sample() : power(0), torque_percent(0)
            {
            }
    ...
    struct Turbojet
    {
        Turbojet() : is_reversable(0), dry_thrust(0), wet_thrust(0), front_diameter(0), back_diameter(0), nozzle_length(0)
        {
        }
    ...
    struct Turboprop2
    {
        Turboprop2() : turbine_power_kW(0), _format_version(2)
        {
        }
    ...
    struct File
    {
        ...
        /** IMPORTANT! If you add a value here, you must also modify Regexes::IDENTIFY_KEYWORD, it relies on numeric values of
         * this enum. */
        enum Keyword
        {
