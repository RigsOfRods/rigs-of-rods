...
Serializer::Serializer(std::shared_ptr<RigDef::File> def_file, Ogre::String const &file_path)
    : m_file_path(file_path), m_rig_def(def_file), m_node_id_width(5), m_float_precision(6), m_inertia_function_width(10),
      m_bool_width(5), // strlen("false") == 5
      m_command_key_width(2), m_float_width(10)
{
}
...
{
    if (module->pistonprops.empty()) { return; }
    ...
    for (auto itor = module->pistonprops.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.reference_node.ToString() << ", " << def.axis_node.ToString() << ", "
                 << def.blade_tip_nodes[0].ToString() << ", " << def.blade_tip_nodes[1].ToString() << ", "
                 << def.blade_tip_nodes[2].ToString() << ", " << def.blade_tip_nodes[3].ToString() << ", "
                 << (def.couple_node.IsValidAnyState() ? def.couple_node.ToString() : "-1") << ", " << def.turbine_power_kW
                 << ", " << def.pitch << ", " << def.airfoil;
    }
...
{
    if (module->turbojets.empty()) { return; }
    ...
    for (auto itor = module->turbojets.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.front_node.ToString() << ", " << def.back_node.ToString() << ", " << def.side_node.ToString()
                 << ", " << def.is_reversable << ", " << def.dry_thrust << ", " << def.wet_thrust << ", " << def.front_diameter
                 << ", " << def.back_diameter << ", " << def.nozzle_length;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->screwprops.empty()) { return; }
    ...
    for (auto itor = module->screwprops.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.prop_node.ToString() << ", " << def.back_node.ToString() << ", " << def.top_node.ToString()
                 << ", " << def.power;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->turboprops_2.empty()) { return; }
    ...
    for (auto itor = module->turboprops_2.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.reference_node.ToString() << ", " << def.axis_node.ToString() << ", "
                 << def.blade_tip_nodes[0].ToString() << ", " << def.blade_tip_nodes[1].ToString() << ", "
                 << def.blade_tip_nodes[2].ToString() << ", " << def.blade_tip_nodes[3].ToString() << ", " << def.turbine_power_kW
                 << ", " << def.airfoil;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->airbrakes.empty()) { return; }
    ...
    for (auto itor = module->airbrakes.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.reference_node.ToString() << ", " << def.x_axis_node.ToString() << ", "
                 << def.y_axis_node.ToString() << ", "
                 << (def.aditional_node.IsValidAnyState() ? def.aditional_node.ToString() : "-1") << ", " << def.offset.x << ", "
                 << def.offset.y << ", " << def.offset.z << ", " << def.width << ", " << def.height << ", "
                 << def.max_inclination_angle << ", " << def.texcoord_x1 << ", " << def.texcoord_y1 << ", " << def.texcoord_x2
                 << ", " << def.texcoord_y2 << ", " << def.lift_coefficient;
    }
    m_stream << endl << endl; // Empty line
}
...
void Serializer::ProcessWings(File::Module *module)
{
    ...
    for (auto itor = module->wings.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.nodes[0].ToString() << ", " << def.nodes[1].ToString() << ", " << def.nodes[2].ToString()
                 << ", " << def.nodes[3].ToString() << ", " << def.nodes[4].ToString() << ", " << def.nodes[5].ToString() << ", "
                 << def.nodes[6].ToString() << ", " << def.nodes[7].ToString() << ", " << def.tex_coords[0] << ", "
                 << def.tex_coords[1] << ", " << def.tex_coords[2] << ", " << def.tex_coords[3] << ", " << def.tex_coords[4]
                 << ", " << def.tex_coords[5] << ", " << def.tex_coords[6] << ", " << def.tex_coords[7] << ", "
                 << def.control_surface << ", " << def.chord_point << ", " << def.min_deflection << ", " << def.max_deflection
                 << ", " << def.airfoil << ", " << def.efficacy_coef;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->soundsources.empty()) { return; }
...
{
    if (module->soundsources2.empty()) { return; }
    ...
    for (auto itor = module->soundsources2.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.node.ToString() << ", " << def.mode << ", " << def.sound_script_name;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (!module->ext_camera) { return; }
    RigDef::ExtCamera *def = module->ext_camera.get();
    m_stream << "extcamera ";

    switch (def->mode)
    {
    case ExtCamera::MODE_NODE: m_stream << "node " << def->node.ToString(); break;
    case ExtCamera::MODE_CINECAM: m_stream << "cinecam"; break;
    case ExtCamera::MODE_CLASSIC:
    default: m_stream << "classic"; break;
    }
    m_stream << "\n\n";
}
...
{
    if (module->videocameras.empty()) { return; }
    ...
    for (auto itor = module->videocameras.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.reference_node.ToString() << ", " << def.left_node.ToString() << ", "
                 ...
                 << def.offset.x << ", " << def.offset.y << ", " << def.offset.z << ", " << def.rotation.x << ", "
                 << def.rotation.y << ", " << def.rotation.z << ", " << def.field_of_view << ", " << def.texture_width << ", "
                 << def.texture_height << ", " << def.min_clip_distance << ", " << def.max_clip_distance << ", "
                 << def.camera_role << ", " << def.camera_mode << ", " << def.material_name;
        if (!def.camera_name.empty()) { m_stream << ", " << def.camera_name; }
    }
    m_stream << endl << endl; // Empty line
}
...
void Serializer::ProcessGuiSettings(File::Module *module)
{
    ...
    {
    case GuiSettings::MAP_MODE_OFF: m_stream << "\n\tinteractiveOverviewMap: off"; break;
    case GuiSettings::MAP_MODE_SIMPLE: m_stream << "\n\tinteractiveOverviewMap: simple"; break;
    case GuiSettings::MAP_MODE_ZOOM: m_stream << "\n\tinteractiveOverviewMap: zoom"; break;
    default:;
    }
    if (!def->tacho_material.empty()) { m_stream << "\n\ttachoMaterial: " << def->tacho_material; }
    if (!def->speedo_material.empty()) { m_stream << "\n\tspeedoMaterial: " << def->speedo_material; }
    if (!def->help_material.empty()) { m_stream << "\n\thelpMaterial: " << def->help_material; }
...
{
    if (module->exhausts.empty()) { return; }
    ...
    for (auto itor = module->exhausts.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.reference_node.ToString() << ", " << def.direction_node.ToString() << ", " << def.particle_name;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->submeshes.empty()) { return; }
    ...
    for (auto itor = module->submeshes.begin(); itor != end_itor; ++itor)
    {
        ...
        if (def.cab_triangles.size() > 0)
        {
            ...
            {
                m_stream << "\n\t" << cab_itor->nodes[0].ToString() << ", " << cab_itor->nodes[1].ToString() << ", "
                         << cab_itor->nodes[2].ToString() << ", n";
...
{
    m_stream << "\n\tadd_animation " << anim.ratio << ", " << anim.lower_limit << ", " << anim.upper_limit << ", source: ";
    ...
    if (anim.HasSource_Event()) { m_stream << ", event: " << anim.event; }
}

void Serializer::ProcessFlexbodies(File::Module *module)
{
    if (module->flexbodies.empty()) { return; }
    ...
    for (auto itor = module->flexbodies.begin(); itor != end_itor; ++itor)
    {
        ...
        // Prop-like line
        m_stream << "\n\t" << def->reference_node.ToString() << ", " << def->x_axis_node.ToString() << ", "
                 << def->y_axis_node.ToString() << ", " << def->offset.x << ", " << def->offset.y << ", " << def->offset.z << ", "
                 << def->rotation.x << ", " << def->rotation.y << ", " << def->rotation.z << ", " << def->mesh_name;
...
void Serializer::ProcessPropsAndAnimations(File::Module *module)
{
    ...
    for (auto itor = module->props.begin(); itor != end_itor; ++itor)
    {
        ...
        {
            m_stream << def.mesh_name << " " << def.special_prop_beacon.flare_material_name << " "
                     << def.special_prop_beacon.color.r << ", " << def.special_prop_beacon.color.g << ", "
                     << def.special_prop_beacon.color.b;
        }
        else if (def.special == Prop::SPECIAL_DASHBOARD_LEFT || def.special == Prop::SPECIAL_DASHBOARD_RIGHT)
        {
            m_stream << " " << def.special_prop_dashboard.mesh_name << " " << def.special_prop_dashboard.offset.x << ", "
                     << def.special_prop_dashboard.offset.y << ", " << def.special_prop_dashboard.offset.z << ", "
...
{
    if (module->material_flare_bindings.empty()) { return; }
...
{
    if (module->flares_2.empty()) { return; }
    ...
    for (auto itor = module->flares_2.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.reference_node.ToString() << ", " << def.node_axis_x.ToString() << ", "
                 << def.node_axis_y.ToString() << ", " << def.offset.x << ", " << def.offset.y << ", " << def.type << ", "
                 << def.control_number << ", " << def.blink_delay_milis << ", " << def.size << " " << def.material_name;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->managed_materials.empty()) { return; }
    ...
    for (auto itor = module->managed_materials.begin(); itor != end_itor; ++itor)
    {
        ...
        switch (def.type)
        {
        case ManagedMaterial::TYPE_FLEXMESH_STANDARD: m_stream << "flexmesh_standard "; break;
        case ManagedMaterial::TYPE_FLEXMESH_TRANSPARENT: m_stream << "flexmesh_transparent "; break;
        case ManagedMaterial::TYPE_MESH_STANDARD: m_stream << "mesh_standard "; break;
        case ManagedMaterial::TYPE_MESH_TRANSPARENT: m_stream << "mesh_transparent "; break;
        default:;
        }
...
{
    if (module->collision_boxes.empty()) { return; }
...
{
    if (module->interaxles.empty()) { return; }
...
{
    if (!module->transfer_case) { return; }
    m_stream << "transfercase\t" << module->transfer_case->a1 << ", " << module->transfer_case->a2 << ", "
             << module->transfer_case->has_2wd << ", " << module->transfer_case->has_2wd_lo;
...
{
    if (!module->cruise_control) { return; }
    m_stream << "cruisecontrol " << module->cruise_control->min_speed << ", " << (int)module->cruise_control->autobrake << endl
             << endl;
}

void Serializer::ProcessSpeedLimiter(File::Module *module)
{
    if (!module->speed_limiter.is_enabled) { return; }
    m_stream << "speedlimiter " << module->speed_limiter.max_speed << endl << endl;
}

void Serializer::ProcessTorqueCurve(File::Module *module)
{
    if (!module->torque_curve) { return; }
...
{
    if (module->particles.empty()) { return; }
    ...
    for (auto itor = module->particles.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << setw(m_node_id_width) << def.emitter_node.ToString() << ", " << setw(m_node_id_width)
                 << def.reference_node.ToString() << ", " << def.particle_system_name;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->ropables.empty()) { return; }
    ...
    for (auto itor = module->ropables.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.node.ToString() << ", " << def.group << ", " << (int)def.has_multilock;
    }
    m_stream << endl << endl; // Empty line
}
...
void Serializer::ProcessTies(File::Module *module)
{
    ...
    for (auto itor = module->ties.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.root_node.ToString() << ", " << setw(m_float_width) << def.max_reach_length << ", "
                 << setw(m_float_width) << def.auto_shorten_rate << ", " << setw(m_float_width) << def.min_length << ", "
                 << setw(m_float_width) << def.max_length << ", " << (def.is_invisible ? "i" : "n") << ", " << setw(m_float_width)
                 << def.max_stress << ", " << def.group;
    }
    m_stream << endl << endl; // Empty line
}
...
void Serializer::ProcessRopes(File::Module *module)
{
    ...
    for (auto itor = module->ropes.begin(); itor != end_itor; ++itor)
    {
        ...
        if (first || (def.beam_defaults.get() != beam_defaults)) { ProcessBeamDefaults(def.beam_defaults.get(), "\t"); }

        m_stream << "\n\t" << setw(m_node_id_width) << def.root_node.ToString() << ", " << setw(m_node_id_width)
                 << def.end_node.ToString();
        if (def.invisible) { m_stream << ", i"; }
        first = false;
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->railgroups.empty()) { return; }
    ...
    for (auto itor = module->railgroups.begin(); itor != end_itor; ++itor)
    {
        ...
        for (auto node_itor = def.node_list.begin(); node_itor != node_end; ++node_itor)
        {
            ...
            if (node_itor->IsRange()) { m_stream << " - " << node_itor->end.ToString(); }
        }
    }
    m_stream << endl << endl; // Empty line
}
...
{
    if (module->slidenodes.empty()) { return; }
    ...
    for (auto itor = module->slidenodes.begin(); itor != end_itor; ++itor)
    {
        ...
        if (!def.rail_node_ranges.empty())
        {
            ...
            for (; itor != end; ++itor)
            {
                ...
                if (itor->IsRange()) { m_stream << " - " << itor->end.ToString(); }
            }
        }
        else
        {
        ...
        // Params
        m_stream << ", s" << def.spring_rate << ", b" << def.break_force << ", t" << def.tolerance << ", r" << def.attachment_rate
                 << ", d" << def.max_attachment_distance;

        // Constraint flags (cX)
        if (def.HasConstraint_a_AttachAll()) { m_stream << ", ca"; }
        else if (def.HasConstraint_f_AttachForeign())
        {
            m_stream << ", cf";
        }
        else if (def.HasConstraint_s_AttachSelf())
        {
            m_stream << ", cs";
        }
        else if (def.HasConstraint_n_AttachNone())
        {
            m_stream << ", cn";
        }
    }
...
void Serializer::ProcessHooks(File::Module *module)
{
    ...
    for (auto itor = module->hooks.begin(); itor != end_itor; ++itor)
    {
        ...
        // Key-value options
        m_stream << ", hookrange: " << def.option_hook_range << ", speedcoef: " << def.option_speed_coef
                 << ", maxforce: " << def.option_max_force << ", hookgroup: " << def.option_hookgroup
                 << ", lockgroup: " << def.option_lockgroup << ", timer: " << def.option_timer
...
{
    if (module->lockgroups.empty()) { return; }
...
{
    if (module->triggers.empty()) { return; }
    ...
    for (auto itor = module->triggers.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\n\t" << def.nodes[0].ToString() << ", " << def.nodes[1].ToString() << ", " << def.contraction_trigger_limit
                 << ", " << def.expansion_trigger_limit << ", " << def.shortbound_trigger_action << ", "
...
{
    if (module->animators.empty()) { return; }
    m_stream << "animators" << endl << endl;
    ...
    for (auto itor = module->animators.begin(); itor != end_itor; ++itor)
    {
        ...
        m_stream << "\t" << def.nodes[0].ToString() << ", " << def.nodes[1].ToString() << ", " << def.lenghtening_factor << ", ";
...
{
    if (module->contacters.empty()) { return; }
...
{
    if (module->rotators.empty()) { return; }
    ...
    for (auto itor = module->rotators.begin(); itor != end_itor; ++itor)
    {
        ...
        // Axis nodes
        m_stream << def.axis_nodes[0].ToString() << ", " << def.axis_nodes[1].ToString() << ", ";
        ...
        // Inertia
        m_stream << def.inertia.start_delay_factor << ", " << def.inertia.stop_delay_factor << ", " << def.inertia.start_function
                 << ", " << def.inertia.stop_function << ", " << def.engine_coupling << ", "
...
{
    if (module->rotators_2.empty()) { return; }
    ...
    for (auto itor = module->rotators_2.begin(); itor != end_itor; ++itor)
    {
        ...
        // Attributes
        m_stream << def.rate << ", " << def.spin_left_key << ", " << def.spin_right_key << ", " << def.rotating_force << ", "
                 << def.tolerance << ", " << def.description << ", ";
...
{
    if (module->flex_body_wheels.empty()) { return; }
    ...
    {
        m_stream << "\t" << setw(m_float_width) << itor->tyre_radius << ", " << setw(m_float_width) << itor->rim_radius << ", "
                 << setw(m_float_width) << itor->width << ", " << setw(3) << itor->num_rays << ", " << setw(m_node_id_width)
                 << itor->nodes[0].ToString() << ", " << setw(m_node_id_width) << itor->nodes[1].ToString() << ", "
                 << setw(m_node_id_width) << itor->rigidity_node.ToString() << ", " << setw(3) << itor->braking << ", " << setw(3)
                 << itor->propulsion << ", " << setw(m_node_id_width) << itor->reference_arm_node.ToString() << ", "
                 << setw(m_float_width) << itor->mass << ", " << setw(m_float_width) << itor->tyre_springiness << ", "
                 << setw(m_float_width) << itor->tyre_damping << ", " << (static_cast<char>(itor->side)) << ", "
                 << itor->rim_mesh_name << " " // Separator = space!
                 << itor->tyre_mesh_name << endl;
    }
...
void Serializer::ProcessSlopeBrake(File::Module *module)
{
    ...
    {
        m_stream << "SlopeBrake " << module->slope_brake->regulating_force << ", " << module->slope_brake->attach_angle << ", "
                 << module->slope_brake->release_angle << endl
                 << endl;
    }
}
...
void Serializer::ProcessTractionControl(File::Module *module)
{
    ...
    m_stream << "TractionControl " << tc->regulation_force << ", " << tc->wheel_slip << ", " << tc->fade_speed << ", "
...
{
    m_stream << "brakes\n\t" << module->brakes->default_braking_force << ", " << module->brakes->parking_brake_force;
}
...
void Serializer::ProcessAntiLockBrakes(File::Module *module)
{
    ...
    m_stream << "AntiLockBrakes " << alb->regulation_force << ", " << alb->min_speed << ", " << alb->pulse_per_sec
...
{
    if (module->engine.get() == nullptr) { return; }
    ...
    m_stream << "engine"
                ...
                " NeutralGear,"
                " Forward gears...\n\t"
             << setw(12) << module->engine->shift_down_rpm << ", " << setw(10) << module->engine->shift_up_rpm << ", " << setw(10)
             << module->engine->torque << ", " << setw(10) << module->engine->global_gear_ratio << ", " << setw(11)
             << module->engine->reverse_gear_ratio << ", " << setw(11) << module->engine->neutral_gear_ratio;
...
{
    if (module->engoption.get() == nullptr) { return; }
    ...
    m_stream << "engoption"
                ...
                " BrakingForce"
                "\n\t"
             << setw(10) << module->engoption->inertia << ", " << setw(10) << (char)module->engoption->type << ", " << setw(11)
             << module->engoption->clutch_force << ", " << setw(9) << module->engoption->shift_time << ", " << setw(10)
             << module->engoption->clutch_time << ", " << setw(13) << module->engoption->post_shift_time << ", " << setw(8)
             << module->engoption->stall_rpm << ", " << setw(7) << module->engoption->idle_rpm << ", " << setw(14)
             << module->engoption->max_idle_mixture << ", " << setw(14) << module->engoption->min_idle_mixture << ", " << setw(15)
             << module->engoption->braking_torque;
...
void Serializer::ProcessWheels2(File::Module *module)
{
    ...
    for (auto itor = module->wheels_2.begin(); itor != end_itor; ++itor)
    {
        m_stream << "\t" << setw(m_float_width) << itor->tyre_radius << ", " << setw(m_float_width) << itor->rim_radius << ", "
                 ...
                 << setw(m_node_id_width) << itor->rigidity_node.ToString() << ", " << setw(3) << itor->braking << ", " << setw(3)
                 << itor->propulsion << ", " << setw(m_node_id_width) << itor->reference_arm_node.ToString() << ", "
                 << setw(m_float_width) << itor->mass << ", " << setw(m_float_width) << itor->rim_springiness << ", "
                 << setw(m_float_width) << itor->rim_damping << ", " << setw(m_float_width) << itor->tyre_springiness << ", "
                 << setw(m_float_width) << itor->tyre_damping << ", " << itor->face_material_name << " " // Separator = space!
...
{
    if (module->wheels.empty()) { return; }
    m_stream << "wheels" << endl << endl;
    auto end_itor = module->wheels.end();
    for (auto itor = module->wheels.begin(); itor != end_itor; ++itor)
    {
        m_stream << "\t" << setw(m_float_width) << itor->radius << ", " << setw(m_float_width) << itor->width << ", " << setw(3)
                 << itor->num_rays << ", " << setw(m_node_id_width) << itor->nodes[0].ToString() << ", " << setw(m_node_id_width)
                 << itor->nodes[1].ToString() << ", " << setw(m_node_id_width) << itor->rigidity_node.ToString() << ", "
                 << setw(3) << itor->braking << ", " << setw(3) << itor->propulsion << ", " << setw(m_node_id_width)
                 << itor->reference_arm_node.ToString() << ", " << setw(m_float_width) << itor->mass << ", "
                 << setw(m_float_width) << itor->springiness << ", " << setw(m_float_width) << itor->damping << ", "
...
void Serializer::ProcessMeshWheels(File::Module *module)
{
    ...
    for (int i = 1; i <= 2; ++i)
    {
        ...
        {
            m_stream << "\t" << setw(m_float_width) << itor->tyre_radius << ", " << setw(m_float_width) << itor->rim_radius
                     << ", " << setw(m_float_width) << itor->width << ", " << setw(3) << itor->num_rays << ", "
                     << setw(m_node_id_width) << itor->nodes[0].ToString() << ", " << setw(m_node_id_width)
                     << itor->nodes[1].ToString() << ", " << setw(m_node_id_width) << itor->rigidity_node.ToString() << ", "
                     << setw(3) << itor->braking << ", " << setw(3) << itor->propulsion << ", " << setw(m_node_id_width)
                     << itor->reference_arm_node.ToString() << ", " << setw(m_float_width) << itor->mass << ", "
                     << setw(m_float_width) << itor->spring << ", " << setw(m_float_width) << itor->damping << ", "
                     << (static_cast<char>(itor->side)) << ", " << itor->mesh_name << " " // Separator = space!
                     << itor->material_name;
            m_stream << endl;
        }
    }
...
{
    if (module->cinecam.empty()) { return; }

    m_stream << "cinecam" << endl << endl;

    for (auto itor = module->cinecam.begin(); itor != module->cinecam.end(); ++itor)
    {
        m_stream << "\t" << setw(m_float_width) << itor->position.x << ", " << setw(m_float_width) << itor->position.y << ", "
                 << setw(m_float_width) << itor->position.z << ", ";
        m_stream << setw(m_node_id_width) << itor->nodes[0].ToString() << ", " << setw(m_node_id_width)
                 << itor->nodes[1].ToString() << ", " << setw(m_node_id_width) << itor->nodes[2].ToString() << ", "
                 << setw(m_node_id_width) << itor->nodes[3].ToString() << ", " << setw(m_node_id_width)
                 << itor->nodes[4].ToString() << ", " << setw(m_node_id_width) << itor->nodes[5].ToString() << ", "
                 << setw(m_node_id_width) << itor->nodes[6].ToString() << ", " << setw(m_node_id_width)
                 << itor->nodes[7].ToString() << ", ";
        m_stream << setw(m_float_width) << itor->spring << ", " << setw(m_float_width) << itor->damping;
    }
...
{
    if (module->shocks_2.empty()) { return; }
...
{
    if (module->shocks_3.empty()) { return; }
...
{
    if (module->commands_2.empty()) { return; }
...
{
    m_stream << "\t" << std::setw(m_node_id_width) << def.nodes[0].ToString() << ", " << std::setw(m_node_id_width)
    ...
    // Options
    bool dummy = true;
    if (def.option_c_auto_center)
    {
        m_stream << "c";
        dummy = false;
    }
    if (def.option_f_not_faster)
    {
        m_stream << "f";
        dummy = false;
    }
    ...
    if (def.option_o_1press_center)
    {
        m_stream << "o";
        dummy = false;
    }
    if (def.option_p_1press)
    {
        m_stream << "p";
        dummy = false;
    }
    if (def.option_r_rope)
    {
        m_stream << "r";
        dummy = false;
    }
...
void Serializer::ProcessHydro(Hydro &def)
{
    ...
    if (!inertia.start_function.empty())
    {
        ...
        if (!inertia.stop_function.empty()) { m_stream << ", " << std::setw(m_inertia_function_width) << inertia.stop_function; }
    }
    m_stream << endl;
}
...
void Serializer::ProcessShock(Shock &def)
{
    ...
    {
        if (BITMASK_IS_1(def.options, Shock::OPTION_i_INVISIBLE)) { m_stream << "i"; }
        if (BITMASK_IS_1(def.options, Shock::OPTION_m_METRIC)) { m_stream << "m"; }
        if (BITMASK_IS_1(def.options, Shock::OPTION_L_ACTIVE_LEFT)) { m_stream << "L"; }
        if (BITMASK_IS_1(def.options, Shock::OPTION_R_ACTIVE_RIGHT)) { m_stream << "R"; }
    }
...
void Serializer::ProcessShock2(Shock2 &def)
{
    ...
    {
        if (BITMASK_IS_1(def.options, Shock2::OPTION_i_INVISIBLE)) { m_stream << "i"; }
        if (BITMASK_IS_1(def.options, Shock2::OPTION_m_METRIC)) { m_stream << "m"; }
        if (BITMASK_IS_1(def.options, Shock2::OPTION_M_ABSOLUTE_METRIC)) { m_stream << "M"; }
        if (BITMASK_IS_1(def.options, Shock2::OPTION_s_SOFT_BUMP_BOUNDS)) { m_stream << "s"; }
    }
...
void Serializer::ProcessShock3(Shock3 &def)
{
    ...
    {
        if (BITMASK_IS_1(def.options, Shock3::OPTION_i_INVISIBLE)) { m_stream << "i"; }
        if (BITMASK_IS_1(def.options, Shock3::OPTION_m_METRIC)) { m_stream << "m"; }
        if (BITMASK_IS_1(def.options, Shock3::OPTION_M_ABSOLUTE_METRIC)) { m_stream << "M"; }
    }
...
{
    if (beam_defaults == nullptr) { return; }
    m_stream << prefix << "set_beam_defaults       " // Align with "set_beam_defaults_scale"
             ...
             << beam_defaults->deformation_threshold << ", " << beam_defaults->breaking_threshold << ", "
             << beam_defaults->visual_beam_diameter << ", " << beam_defaults->beam_material_name << ", "
             << beam_defaults->plastic_deform_coef << endl;

    BeamDefaultsScale &scale = beam_defaults->scale;
    m_stream << prefix << "set_beam_defaults_scale " << scale.springiness << ", " << scale.damping_constant << ", "
             << scale.deformation_threshold_constant << ", " << scale.breaking_threshold_constant << endl;
}

void Serializer::ProcessBeam(Beam &beam)
{
    m_stream << "\t" << std::setw(m_node_id_width) << beam.nodes[0].ToString() << ", " << std::setw(m_node_id_width)
             << beam.nodes[1].ToString() << ", ";
    ...
    {
        if (BITMASK_IS_1(beam.options, Beam::OPTION_i_INVISIBLE)) { m_stream << "i"; }
        if (BITMASK_IS_1(beam.options, Beam::OPTION_r_ROPE)) { m_stream << "r"; }
        if (BITMASK_IS_1(beam.options, Beam::OPTION_s_SUPPORT)) { m_stream << "s"; }
    }

    if (beam._has_extension_break_limit) { m_stream << ", " << beam.extension_break_limit; }
...
void Serializer::ProcessNodes(File::Module *module)
{
    ...
    for (auto itor = module->nodes.begin(); itor != itor_end; ++itor)
    {
        ...
        {
            if (node_zero != nullptr) { throw std::runtime_error("FATAL: Multiple nodes zero!!!"); }
            node_zero = &node;
            continue;
        }
    ...
    // Node zero first
    if (node_zero == nullptr) { throw std::runtime_error("FATAL: Node zero not defined!!!"); }
...
void Serializer::ProcessNodeDefaults(NodeDefaults *node_defaults)
{
    ...
    m_stream << node_defaults->load_weight << ", " << node_defaults->friction << ", " << node_defaults->volume << ", "
...
void Serializer::ProcessNodeOptions(unsigned int options)
{
    ...
    if (BITMASK_IS_1(options, Node::OPTION_b_EXTRA_BUOYANCY)) { m_stream << "b"; }
    if (BITMASK_IS_1(options, Node::OPTION_c_NO_GROUND_CONTACT)) { m_stream << "c"; }
    if (BITMASK_IS_1(options, Node::OPTION_f_NO_SPARKS)) { m_stream << "f"; }
    if (BITMASK_IS_1(options, Node::OPTION_h_HOOK_POINT)) { m_stream << "h"; }
    if (BITMASK_IS_1(options, Node::OPTION_l_LOAD_WEIGHT)) { m_stream << "l"; }
    if (BITMASK_IS_1(options, Node::OPTION_L_LOG)) { m_stream << "L"; }
    if (BITMASK_IS_1(options, Node::OPTION_p_NO_PARTICLES)) { m_stream << "p"; }
    if (BITMASK_IS_1(options, Node::OPTION_x_EXHAUST_POINT)) { m_stream << "x"; }
    if (BITMASK_IS_1(options, Node::OPTION_y_EXHAUST_DIRECTION)) { m_stream << "y"; }
}

void Serializer::ProcessNode(Node &node)
{
    m_stream << "\t" << std::setw(m_node_id_width) << node.id.ToString() << ", " << std::setw(m_float_width) << node.position.x
             << ", " << std::setw(m_float_width) << node.position.y << ", " << std::setw(m_float_width) << node.position.z
             << ", ";

    ProcessNodeOptions(node.options);

    // Load mass
    if (node._has_load_weight_override) { m_stream << " " << node.load_weight_override; }
    m_stream << endl;
}

void Serializer::WriteFlags()
{
    if (m_rig_def->enable_advanced_deformation) { m_stream << "enable_advanced_deformation" << endl << endl; }
    if (m_rig_def->hide_in_chooser) { m_stream << "hideInChooser" << endl << endl; }
    if (m_rig_def->slide_nodes_connect_instantly) { m_stream << "slidenode_connect_instantly" << endl << endl; }
    if (m_rig_def->lockgroup_default_nolock) { m_stream << "lockgroup_default_nolock" << endl << endl; }
    if (m_rig_def->rollon) { m_stream << "rollon" << endl << endl; }
    if (m_rig_def->rescuer) { m_stream << "rescuer" << endl << endl; }
    if (m_rig_def->disable_default_sounds) { m_stream << "disabledefaultsounds" << endl << endl; }
    if (m_rig_def->forward_commands) { m_stream << "forwardcommands" << endl << endl; }
    if (m_rig_def->import_commands) { m_stream << "importcommands" << endl << endl; }
}
...
{
    if (!m_rig_def->guid.empty()) { m_stream << "guid " << m_rig_def->guid << endl << endl; }
}
...
void Serializer::ProcessAuthors()
{
    for (auto itor = m_rig_def->authors.begin(); itor != m_rig_def->authors.end(); ++itor)
    {
        ...
        if (def._has_forum_account) { m_stream << def.forum_account_id << " "; }
        else
        {
            m_stream << "-1 ";
        }
...
{
    if (module->globals.get() == nullptr) { return; }

    m_stream << "globals\n\t" << module->globals->dry_mass << ", " << module->globals->cargo_mass;
    if (!module->globals->material_name.empty()) { m_stream << ", " << module->globals->material_name; }
    m_stream << endl << endl;
}
