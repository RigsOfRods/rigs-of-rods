...
#define STR_PARSE_BOOL(_STR_) Ogre::StringConverter::parseBool(_STR_)
    ...
    void Parser::ProcessCurrentLine()
    {
        ...
        {
            if (StrEqualsNocase(m_current_line, "end_comment")) { m_in_block_comment = false; }
            return;
        }
        ...
        {
            if (StrEqualsNocase(m_current_line, "end_description")) { m_in_description_section = false; }
            else
            {
        ...
        case File::KEYWORD_RIGIDIFIERS:
            this->AddMessage(Message::TYPE_WARNING, "Rigidifiers are not supported, ignoring...");
            return;
    ...
    void Parser::ParseWing()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            wing.nodes[i] = this->GetArgNodeRef(i);
        }
        for (int i = 8; i < 16; i++)
        {
            wing.tex_coords[i - 8] = this->GetArgFloat(i);
        }
    ...
    void Parser::ParseSetCollisionRange()
    {
        ...
        if (value >= 0) { m_definition->collision_range = value; }
    }
    ...
    void Parser::ParseWheel2()
    {
        ...
        {
            m_sequential_importer.GenerateNodesForWheel(File::KEYWORD_WHEELS2, wheel_2.num_rays,
                                                        wheel_2.rigidity_node.IsValidAnyState());
        }
    ...
    void Parser::ParseWheel()
    {
        ...
        if (m_sequential_importer.IsEnabled())
        {
            m_sequential_importer.GenerateNodesForWheel(File::KEYWORD_WHEELS, wheel.num_rays,
                                                        wheel.rigidity_node.IsValidAnyState());
        }
        ...
        for (unsigned int i = 4; i < tokens.size(); i++)
        {
            ...
            if (args2[0] == "mode" && args2.size() == 2)
            {
                ...
                for (; itor != endi; ++itor)
                {
                    ...
                    else if (strncmp(attr.c_str(), "notoggle", 8) == 0)
                    {
                        tc.attr_no_toggle = true;
                    }
                    else if (strncmp(attr.c_str(), "on", 2) == 0)
                    {
                        tc.attr_is_on = true;
                    }
                    else if (strncmp(attr.c_str(), "off", 3) == 0)
                    {
                        tc.attr_is_on = false;
                    }
                }
    ...
    void Parser::ParseTransferCase()
    {
        ...
        for (int i = 4; i < m_num_args; i++)
        {
            tc.gear_ratios.push_back(this->GetArgFloat(i));
        }
    ...
    void Parser::LogParsedDirectiveSetNodeDefaultsData(float load_weight, float friction, float volume, float surface,
    ...
    void Parser::_ParseNodeOptions(unsigned int &options, const std::string &options_str)
    {
        ...
        for (unsigned int i = 0; i < options_str.length(); i++)
        {
            ...
            switch (c)
            {
            case 'l': BITMASK_SET_1(options, Node::OPTION_l_LOAD_WEIGHT); break;
            case 'n':
            ...
            case 'm':
                ...
                break;
            case 'f': BITMASK_SET_1(options, Node::OPTION_f_NO_SPARKS); break;
            case 'x': BITMASK_SET_1(options, Node::OPTION_x_EXHAUST_POINT); break;
            case 'y': BITMASK_SET_1(options, Node::OPTION_y_EXHAUST_DIRECTION); break;
            case 'c': BITMASK_SET_1(options, Node::OPTION_c_NO_GROUND_CONTACT); break;
            case 'h': BITMASK_SET_1(options, Node::OPTION_h_HOOK_POINT); break;
            case 'e': BITMASK_SET_1(options, Node::OPTION_e_TERRAIN_EDIT_POINT); break;
            case 'b': BITMASK_SET_1(options, Node::OPTION_b_EXTRA_BUOYANCY); break;
            case 'p': BITMASK_SET_1(options, Node::OPTION_p_NO_PARTICLES); break;
            case 'L': BITMASK_SET_1(options, Node::OPTION_L_LOG); break;

            default: this->AddMessage(options_str, Message::TYPE_WARNING, std::string("Ignoring invalid option: ") + c); break;
            }
        }
    }
    ...
    {
        if (m_current_section == File::SECTION_SUBMESH) { m_current_submesh->backmesh = true; }
        else
        {
            this->AddMessage(Message::TYPE_ERROR,
                             "Misplaced sub-directive 'backmesh' (belongs in section 'submesh'), ignoring...");
        }
    }

    void Parser::ProcessKeywordTexcoords()
    {
        if (m_current_section == File::SECTION_SUBMESH) { m_current_subsection = File::SUBSECTION__SUBMESH__TEXCOORDS; }
        else
        {
            this->AddMessage(Message::TYPE_WARNING, "Misplaced sub-section 'texcoords' (belongs in section 'submesh'), falling "
                                                    "back to classic unsafe parsing method.");
    ...
    void Parser::ProcessGlobalDirective(File::Keyword keyword) // Directives that should only appear in root module
    {
        ...
        default:
            this->AddMessage(Message::TYPE_ERROR,
                             "INTERNAL ERROR: '" + std::string(File::KeywordToString(keyword)) + "' is not a global directive");
            return;
        }
    }
    ...
    void Parser::VerifyModuleIsRoot(File::Keyword keyword)
    {
        ...
        {
            char buf[200];
            snprintf(buf, 200, "Keyword '%s' has global effect and should not appear in a module",
                     File::KeywordToString(keyword));
    ...
    void Parser::ParseMeshWheelUnified()
    {
        ...
        if (m_sequential_importer.IsEnabled())
        {
            File::Keyword keyword = (mesh_wheel._is_meshwheel2) ? File::KEYWORD_MESHWHEELS2 : File::KEYWORD_MESHWHEELS;
    ...
    void Parser::ParseHook()
    {
        ...
        while (i < m_num_args)
        {
            ...
            else if (has_value && (attr == "speedcoef"))
            {
                hook.option_speed_coef = this->GetArgFloat(++i);
            }
            else if (has_value && (attr == "maxforce"))
            {
                hook.option_max_force = this->GetArgFloat(++i);
            }
            else if (has_value && (attr == "timer"))
            {
                hook.option_timer = this->GetArgFloat(++i);
            }
            else if (has_value && (attr == "hookgroup" || attr == "hgroup"))
            {
                hook.option_hookgroup = this->GetArgInt(++i);
            }
            else if (has_value && (attr == "lockgroup" || attr == "lgroup"))
            {
                hook.option_lockgroup = this->GetArgInt(++i);
            }
            else if (has_value && (attr == "shortlimit" || attr == "short_limit"))
            {
                hook.option_min_range_meters = this->GetArgFloat(++i);
            }
            // Flags
            else if ((attr == "selflock") || (attr == "self-lock") || (attr == "self_lock"))
            {
                hook.flag_self_lock = true;
            }
            else if ((attr == "autolock") || (attr == "auto-lock") || (attr == "auto_lock"))
            {
                hook.flag_auto_lock = true;
            }
            else if ((attr == "nodisable") || (attr == "no-disable") || (attr == "no_disable"))
            {
                hook.flag_no_disable = true;
            }
            else if ((attr == "norope") || (attr == "no-rope") || (attr == "no_rope"))
            {
                hook.flag_no_rope = true;
            }
            else if ((attr == "visible") || (attr == "vis"))
            {
                hook.flag_visible = true;
            }
            else
            {
    ...
    void Parser::ParseGuiSettings()
    {
        ...
        else if (key == "tachoMaterial")
        {
            gui_settings->tacho_material = this->GetArgStr(1);
        }
        else if (key == "speedoMaterial")
        {
            gui_settings->speedo_material = this->GetArgStr(1);
        }
        else if (key == "ar_speedo_max_kph")
        {
            gui_settings->speedo_highest_kph = this->GetArgInt(1);
        }
        else if (key == "useMaxRPM")
        {
            gui_settings->use_max_rpm = (this->GetArgInt(1) == 1);
        }
        else if (key == "helpMaterial")
        {
            gui_settings->help_material = this->GetArgStr(1);
        }

        else if (key == "dashboard")
        {
            gui_settings->dashboard_layouts.push_back(this->GetArgStr(1));
        }
        else if (key == "texturedashboard")
        {
            gui_settings->rtt_dashboard_layouts.push_back(this->GetArgStr(1));
        }
        ...
        else if (key == "interactiveOverviewMap")
        {
            ...
            {
                gui_settings->interactive_overview_map_mode = GuiSettings::MAP_MODE_SIMPLE;
            }
            else if (val == "zoom")
            {
                gui_settings->interactive_overview_map_mode = GuiSettings::MAP_MODE_ZOOM;
            }

            else
            {
                this->AddMessage(Message::TYPE_ERROR, "Unknown map mode [" + val + "], ignoring...");
            }
        }
        else
        {
    ...
    void Parser::ParseGuid()
    {
        ...
        if (!m_definition->guid.empty()) { this->AddMessage(Message::TYPE_WARNING, "Guid defined multiple times."); }
    ...
    void Parser::ParseGlobals()
    {
        ...
        if (m_current_module->globals != nullptr) { this->AddMessage(Message::TYPE_WARNING, "Globals defined more than once."); }
            ...
            while (item != nullptr)
            {
                ...
                if (hyphen != nullptr)
                {
                    ...
                    m_last_flexbody->node_list_to_import.push_back(
                        Node::Range(Node::Ref(a_text, a, Node::Ref::IMPORT_STATE_IS_VALID, m_current_line_number),
                ...
                else
                {
                    ...
                    // Add interval [a-a]
                    Node::Range range_a =
                        Node::Range(Node::Ref(std::string(item), a, Node::Ref::IMPORT_STATE_IS_VALID, m_current_line_number));
    ...
    void Parser::ParseFlaresUnified()
    {
        ...
        if (m_current_section == File::SECTION_FLARES_2) { flare2.offset.z = this->GetArgFloat(pos++); }
    ...
    void Parser::ParseExtCamera()
    {
        ...
        if (mode_str == "classic") { extcam->mode = ExtCamera::MODE_CLASSIC; }
    ...
    void Parser::ParseFileFormatVersion()
    {
        ...
        {
            this->AddMessage(Message::TYPE_WARNING,
                             "Inline section 'fileformatversion' has global effect and should not appear in a module");
        }

        m_definition->file_format_version = this->GetArgUint(1);

        if (m_definition->file_format_version >= 450) { m_sequential_importer.Disable(); }
    ...
    void Parser::ParseDirectiveDetacherGroup()
    {
        ...
        if (this->GetArgStr(1) == "end") { m_current_detacher_group = 0; }
        else
        {
    ...
    void Parser::ParseDirectiveAddAnimation()
    {
        ...
        for (auto itor = tokens.begin() + 3; itor != tokens.end(); ++itor)
        {
            ...
            if (entry.size() == 1) // Single keyword
            {
                ...
                else if (entry[0] == "noflip")
                {
                    animation.mode |= Animation::MODE_NO_FLIP;
                }
                else if (entry[0] == "bounce")
                {
                    animation.mode |= Animation::MODE_BOUNCE;
                }
                else if (entry[0] == "eventlock")
                {
                    animation.mode |= Animation::MODE_EVENT_LOCK;
                }

                else
                {
                    snprintf(warn_msg, WARN_LEN, "Invalid keyword: %s", entry[0].c_str());
                }
            }
            else if (entry.size() == 2 && (entry[0] == "mode" || entry[0] == "event" || entry[0] == "source"))
            {
                ...
                if (entry[0] == "mode")
                {
                    for (auto itor = values.begin(); itor != values.end(); ++itor)
                    {
                        ...
                        else if (value == "y-rotation")
                        {
                            animation.mode |= Animation::MODE_ROTATION_Y;
                        }
                        else if (value == "z-rotation")
                        {
                            animation.mode |= Animation::MODE_ROTATION_Z;
                        }
                        else if (value == "x-offset")
                        {
                            animation.mode |= Animation::MODE_OFFSET_X;
                        }
                        else if (value == "y-offset")
                        {
                            animation.mode |= Animation::MODE_OFFSET_Y;
                        }
                        else if (value == "z-offset")
                        {
                            animation.mode |= Animation::MODE_OFFSET_Z;
                        }

                        else
                        {
                            snprintf(warn_msg, WARN_LEN, "Invalid 'mode': %s, ignoring...", entry[1].c_str());
                        }
                    }
                }
                ...
                else if (entry[0] == "source")
                {
                    for (auto itor = values.begin(); itor != values.end(); ++itor)
                    {
                        ...
                        else if (value == "shifterman1")
                        {
                            animation.source |= Animation::SOURCE_SHIFT_LEFT_RIGHT;
                        }
                        else if (value == "shifterman2")
                        {
    ...
    void Parser::ParseEngoption()
    {
        ...
        if (m_num_args > 1) { engoption.type = Engoption::EngineType(this->GetArgChar(1)); }
    ...
    void Parser::ParseCommandsUnified()
    {
        ...
        if (is_commands2) { command2.lengthen_rate = this->GetArgFloat(pos++); }
        else
        {
        ...
        for (auto itor = options_str.begin(); itor != options_str.end(); ++itor)
        {
            ...
            {
                command2.option_i_invisible = true;
            }
            else if (c == 'r')
            {
                command2.option_r_rope = true;
            }
            else if (c == 'f')
            {
                command2.option_f_not_faster = true;
            }
            else if (c == 'c')
            {
                command2.option_c_auto_center = true;
            }
            else if (c == 'p')
            {
                command2.option_p_1press = true;
            }
            else if (c == 'o')
            {
                command2.option_o_1press_center = true;
            }
            else
            {
        ...
        {
            snprintf(warn_msg, WARN_LEN, "Command cannot be one-pressed and self centering at the same time, ignoring flag '%c'",
                     ignored);
        ...
        if (m_num_args > pos)
        {
            ParseOptionalInertia(command2.inertia, pos);
            pos += 4;
        }
    ...
    void Parser::ParseCinecam()
    {
        ...
        if (m_sequential_importer.IsEnabled()) { m_sequential_importer.AddGeneratedNode(File::KEYWORD_CINECAM); }
    ...
    void Parser::ParseBrakes()
    {
        ...
        if (m_current_module->brakes == nullptr) { m_current_module->brakes = std::shared_ptr<Brakes>(new Brakes()); }

        m_current_module->brakes->default_braking_force = this->GetArgFloat(0);

        if (m_num_args > 1) { m_current_module->brakes->parking_brake_force = this->GetArgFloat(1); }
    }
        ...
        for (; iter != tokens.end(); iter++)
        {
            ...
            else if (results[5].matched)
            {
                ...
                for (unsigned int i = 0; i < options_str.length(); i++)
                {
                    switch (options_str.at(i))
                    {
                    case 'o': axle.options.push_back(Axle::OPTION_o_OPEN); break;
                    case 'l': axle.options.push_back(Axle::OPTION_l_LOCKED); break;
                    case 's': axle.options.push_back(Axle::OPTION_s_SPLIT); break;
                    case 'v': axle.options.push_back(Axle::OPTION_s_VISCOUS); break;
    ...
    void Parser::ParseInterAxles()
    {
        ...
        if (results[5].matched)
        {
            ...
            for (unsigned int i = 0; i < options_str.length(); i++)
            {
                switch (options_str.at(i))
                {
                case 'o': interaxle.options.push_back(Axle::OPTION_o_OPEN); break;
                case 'l': interaxle.options.push_back(Axle::OPTION_l_LOCKED); break;
                case 's': interaxle.options.push_back(Axle::OPTION_s_SPLIT); break;
                case 'v': interaxle.options.push_back(Axle::OPTION_s_VISCOUS); break;
    ...
    void Parser::ParseTriggers()
    {
        ...
        if (m_num_args > 6)
        {
            ...
            for (unsigned int i = 0; i < options_str.length(); i++)
            {
                ...
                default: this->AddMessage(Message::TYPE_WARNING, Ogre::String("Invalid trigger option: " + options_str.at(i)));
                }
            }
        }
        ...
        if (m_num_args > 7)
        {
            ...
            if (boundary_timer > 0.0f) trigger.boundary_timer = boundary_timer;
        }
    ...
    void Parser::ParseTorqueCurve()
    {
        ...
        if (args.size() == 1u) { m_current_module->torque_curve->predefined_func_name = args[0]; }
    ...
    void Parser::ParseTies()
    {
        ...
        if (m_num_args > 5)
        {
            for (char c : this->GetArgStr(5))
            {
                ...
                case Tie::OPTION_v_FILLER: break;

                case Tie::OPTION_i_INVISIBLE: tie.is_invisible = true; break;

                case Tie::OPTION_s_NO_SELF_LOCK: tie.disable_self_lock = true; break;

                default: this->AddMessage(Message::TYPE_WARNING, std::string("Invalid option: ") + c + ", ignoring..."); break;
                }
            }
        }
    ...
    {
        Ogre::StringVector args = Ogre::StringUtil::split(m_current_line, ", ");
        if (args.size() < 2u) { this->AddMessage(Message::TYPE_ERROR, "Too few arguments"); }
        ...
        for (auto itor = args.begin() + 1; itor != args.end(); ++itor)
        {
            ...
            case 'C':
                switch (itor->at(1))
                {
                case 'a': BITMASK_SET_1(slidenode.constraint_flags, SlideNode::CONSTRAINT_ATTACH_ALL); break;
                case 'f': BITMASK_SET_1(slidenode.constraint_flags, SlideNode::CONSTRAINT_ATTACH_FOREIGN); break;
                case 's': BITMASK_SET_1(slidenode.constraint_flags, SlideNode::CONSTRAINT_ATTACH_SELF); break;
                case 'n': BITMASK_SET_1(slidenode.constraint_flags, SlideNode::CONSTRAINT_ATTACH_NONE); break;
                default: this->AddMessage(Message::TYPE_WARNING, std::string("Ignoring invalid option: ") + itor->at(1)); break;
                }
                in_rail_node_list = false;
                break;
            default:
                if (in_rail_node_list) slidenode.rail_node_ranges.push_back(_ParseNodeRef(*itor));
                break;
            }
        }
    ...
    void Parser::ParseShock3()
    {
        ...
        if (m_num_args > 15)
        {
            ...
            while (itor != endi)
            {
                ...
                {
                case 'n':
                case 'v': break; // Placeholder, does nothing.
                case 'i': BITMASK_SET_1(shock_3.options, Shock3::OPTION_i_INVISIBLE); break;
                case 'm': BITMASK_SET_1(shock_3.options, Shock3::OPTION_m_METRIC); break;
                case 'M': BITMASK_SET_1(shock_3.options, Shock3::OPTION_M_ABSOLUTE_METRIC); break;
                default:
                {
                    char msg[100] = "";
    ...
    void Parser::ParseShock2()
    {
        ...
        if (m_num_args > 13)
        {
            ...
            while (itor != endi)
            {
                ...
                case 'n':
                case 'v': break; // Placeholder, does nothing.
                case 'i': BITMASK_SET_1(shock_2.options, Shock2::OPTION_i_INVISIBLE); break;
                case 'm': BITMASK_SET_1(shock_2.options, Shock2::OPTION_m_METRIC); break;
                case 'M': BITMASK_SET_1(shock_2.options, Shock2::OPTION_M_ABSOLUTE_METRIC); break;
                case 's': BITMASK_SET_1(shock_2.options, Shock2::OPTION_s_SOFT_BUMP_BOUNDS); break;
                default:
                {
                    char msg[100] = "";
                    snprintf(msg, 100, "Invalid option: '%c', ignoring...", c);
    ...
    void Parser::ParseShock()
    {
        ...
        if (m_num_args > 7)
        {
            ...
            while (itor != endi)
            {
                ...
                case 'n':
                case 'v': break; // Placeholder, does nothing.
                case 'i': BITMASK_SET_1(shock.options, Shock::OPTION_i_INVISIBLE); break;
                case 'm': BITMASK_SET_1(shock.options, Shock::OPTION_m_METRIC); break;
                case 'r':
                case 'R': BITMASK_SET_1(shock.options, Shock::OPTION_R_ACTIVE_RIGHT); break;
                case 'l':
                case 'L': BITMASK_SET_1(shock.options, Shock::OPTION_L_ACTIVE_LEFT); break;
                default:
                {
                    char msg[100] = "";
                    snprintf(msg, 100, "Invalid option: '%c', ignoring...", c);
    ...
    Node::Ref Parser::_ParseNodeRef(std::string const &node_id_str)
    {
        if (m_sequential_importer.IsEnabled())
        {
            ...
            {
                std::stringstream msg;
                msg << "Encountered node with illegal negative number: '" << node_id_num << "', parsing as positive '"
                    << node_id_num * -1 << "' for backwards compatibility. Please fix as soon as possible.";
            ...
            unsigned int flags = Node::Ref::IMPORT_STATE_IS_VALID | // Import state
                                 ...
                                 Node::Ref::REGULAR_STATE_IS_NAMED; // Regular state (fileformatversion >= 450)
            if (m_any_named_node_defined) { flags |= Node::Ref::IMPORT_STATE_MUST_CHECK_NAMED_FIRST; }
    ...
    void Parser::ParseRotatorsUnified()
    {
        ...
        if (m_current_section == File::SECTION_ROTATORS_2) { m_current_module->rotators_2.push_back(rotator); }
        else
        {
    ...
    void Parser::ParseFileinfo()
    {
        ...
        {
            this->AddMessage(Message::TYPE_WARNING,
                             "Inline-section 'fileinfo' has global effect and should not appear in a module");
        }
    ...
    void Parser::ParseProps()
    {
        ...
        else if (prop.mesh_name.find("rightmirror") != std::string::npos)
        {
            prop.special = Prop::SPECIAL_MIRROR_RIGHT;
        }
        else if (prop.mesh_name.find("dashboard-rh") != std::string::npos)
        {
            prop.special = Prop::SPECIAL_DASHBOARD_RIGHT;
            is_dash      = true;
        }
        else if (prop.mesh_name.find("dashboard") != std::string::npos)
        {
            prop.special = Prop::SPECIAL_DASHBOARD_LEFT;
            is_dash      = true;
        }
        else if (Ogre::StringUtil::startsWith(prop.mesh_name, "spinprop", false))
        {
            prop.special = Prop::SPECIAL_AERO_PROP_SPIN;
        }
        else if (Ogre::StringUtil::startsWith(prop.mesh_name, "pale", false))
        {
            prop.special = Prop::SPECIAL_AERO_PROP_BLADE;
        }
        else if (Ogre::StringUtil::startsWith(prop.mesh_name, "seat", false))
        {
            prop.special = Prop::SPECIAL_DRIVER_SEAT;
        }
        else if (Ogre::StringUtil::startsWith(prop.mesh_name, "seat2", false))
        {
            prop.special = Prop::SPECIAL_DRIVER_SEAT_2;
        }
        else if (Ogre::StringUtil::startsWith(prop.mesh_name, "beacon", false))
        {
            prop.special = Prop::SPECIAL_BEACON;
        }
        else if (Ogre::StringUtil::startsWith(prop.mesh_name, "redbeacon", false))
        {
            prop.special = Prop::SPECIAL_REDBEACON;
        }
        else if (Ogre::StringUtil::startsWith(prop.mesh_name, "lightb", false))
        {
            prop.special = Prop::SPECIAL_LIGHTBAR;
        } // Previously: 'strncmp("lightbar", meshname, 6)'
        ...
        if ((prop.special == Prop::SPECIAL_BEACON) && (m_num_args >= 14))
        {
            ...
            prop.special_prop_beacon.color =
                Ogre::ColourValue(this->GetArgFloat(11), this->GetArgFloat(12), this->GetArgFloat(13));
        }
        else if (is_dash)
        {
            ...
            if (m_num_args > 13)
            {
                prop.special_prop_dashboard.offset =
                    Ogre::Vector3(this->GetArgFloat(11), this->GetArgFloat(12), this->GetArgFloat(13));
    ...
    void Parser::ParseNodesUnified()
    {
        ...
        if (m_current_section == File::SECTION_NODES_2)
        {
            ...
            if (m_sequential_importer.IsEnabled()) { m_sequential_importer.AddNamedNode(node_name); }
        ...
        else
        {
            ...
            if (m_sequential_importer.IsEnabled()) { m_sequential_importer.AddNumberedNode(node_num); }
        }

        node.position.x = this->GetArgFloat(1);
        node.position.y = this->GetArgFloat(2);
        node.position.z = this->GetArgFloat(3);
        if (m_num_args > 4) { this->_ParseNodeOptions(node.options, this->GetArgStr(4)); }
        if (m_num_args > 5)
        {
    ...
    void Parser::ParseMinimass()
    {
        ...
        if (m_num_args > 1)
        {
            ...
            for (char c : options_str)
            {
                ...
                case (MinimassPreset::OPTION_n_FILLER): break;

                case (MinimassPreset::OPTION_l_SKIP_LOADED): m_definition->minimass_skip_loaded_nodes = true; break;

                default: this->AddMessage(Message::TYPE_WARNING, std::string("Unknown option: ") + c); break;
                }
            }
        }
    ...
    void Parser::ParseFlexBodyWheel()
    {
        ...
        if (m_sequential_importer.IsEnabled())
        {
            m_sequential_importer.GenerateNodesForWheel(File::KEYWORD_FLEXBODYWHEELS, flexbody_wheel.num_rays,
                                                        flexbody_wheel.rigidity_node.IsValidAnyState());
        }
    ...
    void Parser::ParseManagedMaterials()
    {
        ...
        if (type_str == "mesh_standard" || type_str == "mesh_transparent")
        {
            ...
            managed_mat.type =
                (type_str == "mesh_standard") ? ManagedMaterial::TYPE_MESH_STANDARD : ManagedMaterial::TYPE_MESH_TRANSPARENT;
        ...
        else if (type_str == "flexmesh_standard" || type_str == "flexmesh_transparent")
        {
            ...
            managed_mat.type = (type_str == "flexmesh_standard") ? ManagedMaterial::TYPE_FLEXMESH_STANDARD
    ...
    void Parser::ParseBeams()
    {
        ...
        if ((m_num_args > 3) && (beam.options & Beam::OPTION_s_SUPPORT))
        {
            ...
            if (support_break_factor > 0.0f) { support_break_limit = support_break_factor; }
    ...
    void Parser::ParseAnimator()
    {
        ...
        for (; itor != endi; ++itor)
        {
            ...
            {
                if (results[1] == "throttle")
                    animator.aero_animator.flags |= AeroAnimator::OPTION_THROTTLE;
                else if (results[1] == "rpm")
                    animator.aero_animator.flags |= AeroAnimator::OPTION_RPM;
                else if (results[1] == "aerotorq")
                    animator.aero_animator.flags |= AeroAnimator::OPTION_TORQUE;
                else if (results[1] == "aeropit")
                    animator.aero_animator.flags |= AeroAnimator::OPTION_PITCH;
                else if (results[1] == "aerostatus")
                    animator.aero_animator.flags |= AeroAnimator::OPTION_STATUS;
            ...
            else
            {
                // Standalone keywords
                if (token == "vis")
                    animator.flags |= Animator::OPTION_VISIBLE;
                else if (token == "inv")
                    animator.flags |= Animator::OPTION_INVISIBLE;
                else if (token == "airspeed")
                    animator.flags |= Animator::OPTION_AIRSPEED;
                else if (token == "vvi")
                    animator.flags |= Animator::OPTION_VERTICAL_VELOCITY;
                else if (token == "altimeter100k")
                    animator.flags |= Animator::OPTION_ALTIMETER_100K;
                else if (token == "altimeter10k")
                    animator.flags |= Animator::OPTION_ALTIMETER_10K;
                else if (token == "altimeter1k")
                    animator.flags |= Animator::OPTION_ALTIMETER_1K;
                else if (token == "aoa")
                    animator.flags |= Animator::OPTION_ANGLE_OF_ATTACK;
                else if (token == "flap")
                    animator.flags |= Animator::OPTION_FLAP;
                else if (token == "airbrake")
                    animator.flags |= Animator::OPTION_AIR_BRAKE;
                else if (token == "roll")
                    animator.flags |= Animator::OPTION_ROLL;
                else if (token == "pitch")
                    animator.flags |= Animator::OPTION_PITCH;
                else if (token == "brakes")
                    animator.flags |= Animator::OPTION_BRAKES;
                else if (token == "accel")
                    animator.flags |= Animator::OPTION_ACCEL;
                else if (token == "clutch")
                    animator.flags |= Animator::OPTION_CLUTCH;
                else if (token == "speedo")
                    animator.flags |= Animator::OPTION_SPEEDO;
                else if (token == "tacho")
                    animator.flags |= Animator::OPTION_TACHO;
                else if (token == "turbo")
                    animator.flags |= Animator::OPTION_TURBO;
                else if (token == "parking")
                    animator.flags |= Animator::OPTION_PARKING;
                else if (token == "shifterman1")
                    animator.flags |= Animator::OPTION_SHIFT_LEFT_RIGHT;
                else if (token == "shifterman2")
                    animator.flags |= Animator::OPTION_SHIFT_BACK_FORTH;
                else if (token == "sequential")
                    animator.flags |= Animator::OPTION_SEQUENTIAL_SHIFT;
                else if (token == "shifterlin")
                    animator.flags |= Animator::OPTION_GEAR_SELECT;
                else if (token == "torque")
                    animator.flags |= Animator::OPTION_TORQUE;
                else if (token == "difflock")
                    animator.flags |= Animator::OPTION_DIFFLOCK;
                else if (token == "rudderboat")
                    animator.flags |= Animator::OPTION_BOAT_RUDDER;
                else if (token == "throttleboat")
                    animator.flags |= Animator::OPTION_BOAT_THROTTLE;
            }
        }
    ...
    void Parser::ParseAuthor()
    {
        ...
        {
            this->AddMessage(Message::TYPE_WARNING,
                             "Inline-section 'author' has global effect and should not appear in a module");
        }
        ...
        if (m_num_args > 2)
        {
            author.forum_account_id   = this->GetArgInt(2);
    ...
    void Parser::AddMessage(std::string const &line, Message::Type type, std::string const &message)
    {
        ...
        case Message::TYPE_FATAL_ERROR: ++m_messages_num_errors; break;
        case Message::TYPE_WARNING: ++m_messages_num_warnings; break;
        default: ++m_messages_num_other; break;
        }
    }
    ...
    File::Keyword Parser::IdentifyKeywordInCurrentLine()
    {
        ...
        if (c > 'z' || c < 'a') { return File::KEYWORD_INVALID; }
        ...
        if (keyword != File::KEYWORD_INVALID) { return keyword; }
    ...
    void Parser::ChangeSection(RigDef::File::Section new_section)
    {
        ...
        if (m_current_section == File::SECTION_FLEXBODIES) { m_current_subsection = File::SUBSECTION_NONE; }

        // Enter sections
        m_current_section = new_section;
        if (new_section == File::SECTION_SUBMESH) { m_current_submesh = std::shared_ptr<Submesh>(new Submesh()); }
    ...
    void Parser::ProcessChangeModuleLine(File::Keyword keyword)
    {
        ...
        if (search_itor != m_definition->user_modules.end()) { m_current_module = search_itor->second; }
        else
        {
    ...
    void Parser::Finalize()
    {
        ...
        if (m_sequential_importer.IsEnabled()) { m_sequential_importer.Process(m_definition); }
    }
    ...
    std::string Parser::ProcessMessagesToString()
    {
        ...
        for (; itor != end; ++itor)
        {
            switch (itor->type)
            {
            case (RigDef::Parser::Message::TYPE_FATAL_ERROR): report << "#FF3300 FATAL_ERROR #FFFFFF"; break;

            case (RigDef::Parser::Message::TYPE_ERROR): report << "#FF3300 ERROR #FFFFFF"; break;

            case (RigDef::Parser::Message::TYPE_WARNING): report << "#FFFF00 WARNING #FFFFFF"; break;
    ...
    MeshWheel::Side Parser::GetArgWheelSide(int index)
    {
        ...
        if (side_char != 'r')
        {
            ...
            {
                char msg[200] = "";
                snprintf(msg, 200, "Bad arg~%d 'side' (value: %c), parsing as 'l' for backwards compatibility.", index + 1,
                         side_char);
                this->AddMessage(Message::TYPE_WARNING, msg);
            }
    ...
    {
        if (!(Ogre::StringConverter::parseReal(this->GetArgStr(index)) == -1.f)) { return this->GetArgNodeRef(index); }
    ...
    Wing::Control Parser::GetArgWingSurface(int index)
    {
        ...
        if (str.size() > 1)
        {
            snprintf(msg_buf, MSG_LEN, "Argument ~%d 'control surface' (value: %s), should be only 1 letter.", index,
                     str.c_str());
            this->AddMessage(Message::TYPE_WARNING, msg_buf);
        }
