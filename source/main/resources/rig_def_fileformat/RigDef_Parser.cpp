...
void Parser::ParseDirectiveSetBeamDefaults()
{
    ...
    d._is_user_defined = true;

                          d.springiness                     = this->GetArgFloat(1);
    if (m_num_args > 2) { d.damping_constant                = this->GetArgFloat(2); }
    if (m_num_args > 3) { d.deformation_threshold_constant  = this->GetArgFloat(3); }
    if (m_num_args > 4) { d.breaking_threshold_constant     = this->GetArgFloat(4); }
    if (m_num_args > 5) { d.visual_beam_diameter            = this->GetArgFloat(5); }
    if (m_num_args > 6) { d.beam_material_name              = this->GetArgStr  (6); }
    if (m_num_args > 7) { d.plastic_deformation_coefficient = this->GetArgFloat(7); }

    if (d.springiness                    < 0.f) { d.springiness                    = DEFAULT_SPRING;        }
    if (d.damping_constant               < 0.f) { d.damping_constant               = DEFAULT_DAMP;          }
    if (d.deformation_threshold_constant < 0.f) { d.deformation_threshold_constant = BEAM_DEFORM;           }
    if (d.breaking_threshold_constant    < 0.f) { d.breaking_threshold_constant    = BEAM_BREAK;            }
...
void Parser::ParseDirectiveAddAnimation()
{
    ...
    for (auto itor = tokens.begin() + 3; itor != tokens.end(); ++itor)
    {
        ...
        if (entry.size() == 1) // Single keyword
        {
            ...
            else { sprintf_s(warn_msg, "Invalid keyword: %s", entry[0].c_str()); }
        }
        else if (entry.size() == 2 && (entry[0] == "mode" || entry[0] == "event" || entry[0] == "source"))
        {
            ...
            if (entry[0] == "mode")
            {
                for (auto itor = values.begin(); itor != values.end(); ++itor)
                {
                    ...
                    else { sprintf_s(warn_msg, "Invalid 'mode': %s, ignoring...", entry[1].c_str()); }
                }
            }
            ...
            else if (entry[0] == "source")
            {
                ...
                for (auto itor = values.begin(); itor != values.end(); ++itor)
                {
                    ...
                    else
                    {
                        ...
                        {
                            sprintf_s(warn_msg, "Invalid 'source': %s, ignoring...", entry[1].c_str());
                            continue;
                        }
            ...
            {
                sprintf_s(warn_msg, "Invalid keyword: %s, ignoring...", entry[0].c_str());
            }
        }
        else
        {
            sprintf_s(warn_msg, "Invalid item: %s, ignoring...", entry[0].c_str());
        }

        if (warn_msg[0] != '\0')
        {
            char msg[100];
            sprintf_s(msg, "Invalid token: %s, ignoring....", itor->c_str());
            this->AddMessage(Message::TYPE_WARNING, msg);
        }
    }
    ...
    if (mode < 0)
    {
        if (mode < -2)
        {
            this->AddMessage(Message::TYPE_ERROR,
                this->GetArgStr(1) + " is invalid soundsources2.mode, falling back to default -2");
            mode = -2;
        }
    ...
    if (m_num_args > 13)
    {
        ...
        while (itor != endi)
        {
            ...
            switch (c)
            {
                ...
                default: {
                        char msg[100] = "";
                        sprintf_s(msg, "Invalid option: '%c', ignoring...", c);
...
MeshWheel::Side Parser::GetArgWheelSide(int index)
{
    ...
    if (side_char != 'r')
    {
        ...
        {
            char msg[200] = "";
            sprintf_s(msg, "Bad arg~%d 'side' (value: %c), parsing as 'l' for backwards compatibility.", 
                index + 1, side_char);
            this->AddMessage(Message::TYPE_WARNING, msg);
        }
...
Wing::Control Parser::GetArgWingSurface(int index)
{
    ...
    {
        char msg[250];
        sprintf_s(msg, "Invalid argument ~%d 'control surface' (value: %s), allowed are: <%s>, ignoring...",
    ...
    {
        char msg[100];
        sprintf_s(msg, "Argument ~%d 'control surface' (value: %s), should be only 1 letter.", index, str.c_str());
        this->AddMessage(Message::TYPE_WARNING, msg);
    }
