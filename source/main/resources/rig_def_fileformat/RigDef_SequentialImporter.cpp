...
void SequentialImporter::AddMessage(Message::Type msg_type, std::string text)
{
    ...
    m.type    = msg_type;
    m.message = text;
    if (m_current_module.get() != nullptr) { m.module_name = m_current_module->name; }
...
Node::Ref SequentialImporter::ResolveNodeByIndex(unsigned int index_in, unsigned int def_line_number)
{
    if (index_in >= m_all_nodes.size())
    {
        ...
        msg << "Cannot resolve node by index [" << index_in
            << "], node is not defined, highest available number is: " << m_all_nodes.size() - 1;
    ...
    return Node::Ref(TOSTRING(index_out), index_out,
                     Node::Ref::IMPORT_STATE_IS_VALID | Node::Ref::IMPORT_STATE_IS_RESOLVED_NUMBERED, def_line_number);
}

Node::Ref SequentialImporter::ResolveNode(Node::Ref const &noderef_in)
{
    ++m_total_resolved;
    if (!noderef_in.GetImportState_IsValid()) { return noderef_in; }
    ...
    if (noderef_in.Num() >= m_all_nodes.size())
    {
        ...
        Node::Ref         out_ref("0", 0, Node::Ref::IMPORT_STATE_IS_VALID | Node::Ref::IMPORT_STATE_IS_RESOLVED_NUMBERED,
                          noderef_in.GetLineNumber());
        std::stringstream msg;
        msg << "Cannot resolve " << noderef_in.ToString()
            << " - not a named node, and index is not defined (highest is: " << m_all_nodes.size() - 1
            << "). For backwards compatibility, converting to: " << out_ref.ToString();
        this->AddMessage(Message::TYPE_ERROR, msg.str());
    ...
    {
        Node::Ref out_ref(entry.node_id.Str(), 0, Node::Ref::IMPORT_STATE_IS_VALID | Node::Ref::IMPORT_STATE_IS_RESOLVED_NAMED,
                          noderef_in.GetLineNumber());
        std::stringstream msg;
    ...
    else if (entry.node_id.IsTypeNumbered())
    {
        ...
        Node::Ref         out_ref(TOSTRING(out_index), out_index,
                          Node::Ref::IMPORT_STATE_IS_VALID | Node::Ref::IMPORT_STATE_IS_RESOLVED_NUMBERED,
                          noderef_in.GetLineNumber());
        std::stringstream msg;
        msg << "Node resolved\n\tSource: " << noderef_in.ToString() << "\n\tResult: " << out_ref.ToString()
...
void SequentialImporter::ResolveFlexbodyForset(std::vector<Node::Range> &in_ranges, std::vector<Node::Ref> &out_nodes)
{
    ...
    for (; range_itor != range_end; ++range_itor)
    {
        ...
        if (!range.IsRange())
        {
            ...
            {
                Node::Ref node_ref =
                    this->ResolveNodeByIndex(range.start.Num(), range.start.GetLineNumber()); // Forset nodes are numbered-only
                ...
                if (node_ref.IsValidAnyState()) { out_nodes.push_back(node_ref); }
                else
                {
                    std::stringstream msg;
                    msg << "ResolveFlexbodyForset(): Stand-alone node [" << range.start.ToString()
        ...
        else // It's a range
        {
            ...
            {
                std::stringstream msg;
                msg << "ResolveFlexbodyForset(): Skipping range because some nodes are marked INVALID, start: ["
                    << range.start.ToString() << "], end: [" << range.end.ToString() << "]";
                this->AddMessage(Message::TYPE_ERROR, msg.str());
            }
            else if (range.start.GetImportState_IsResolvedNamed() || range.end.GetImportState_IsResolvedNamed())
            {
                std::stringstream msg;
                msg << "ResolveFlexbodyForset(): Some nodes in range are already resolved as named, unable to process, start: ["
                    << range.start.ToString() << "], end: [" << range.end.ToString() << "]";
                this->AddMessage(Message::TYPE_ERROR, msg.str());
            }
            else
            {
                ...
                for (unsigned int i = range.start.Num(); i <= end_index; ++i)
                {
                    ...
                    // Invalid nodes are simply thrown away, for backwards compatibility
                    // (FlexBody loops through existing nodes first, and then evaluates if they are in the SET -> invalid nodes
                    // are silently ignored)
                    if (node_ref.IsValidAnyState()) { out_nodes.push_back(node_ref); }
                    else
                    {
                        std::stringstream msg;
                        msg << "ResolveFlexbodyForset(): Node [" << i << "] from range [" << range.start.ToString() << " - "
                            << range.end.ToString() << "] resolved invalid, removing from FORSET";
                        this->AddMessage(Message::TYPE_WARNING, msg.str());
                    }
                }
            }
...
void SequentialImporter::ResolveNodeRanges(std::vector<Node::Range> &ranges)
{
    ...
    {
        Node::Range &range = *range_itor;
        if (!range.IsRange()) { ranges.push_back(Node::Range(this->ResolveNode(range.start))); }
        ...
        {
            std::stringstream msg;
            msg << "Some nodes in range are already resolved as named, unable to process, start: [" << range.start.ToString()
...
void SequentialImporter::ProcessModule(std::shared_ptr<RigDef::File::Module> module)
{
    ...
    FOR_EACH(File::KEYWORD_CAMERARAIL, module->camera_rails, rail, { RESOLVE_VECTOR(rail.nodes); });
    ...
    FOR_EACH(File::KEYWORD_COLLISIONBOXES, module->collision_boxes, box, { RESOLVE_VECTOR(box.nodes); });
    ...
    FOR_EACH(File::KEYWORD_CONTACTERS, module->contacters, contacter_node, { RESOLVE(contacter_node); });
    ...
    FOR_EACH(File::KEYWORD_FIXES, module->fixes, fixed_node, { RESOLVE(fixed_node); });
    ...
    FOR_EACH(File::KEYWORD_HOOKS, module->hooks, hook, { RESOLVE(hook.node); });
    ...
    FOR_EACH(File::KEYWORD_RAILGROUPS, module->railgroups, railgroup, { ResolveNodeRanges(railgroup.node_list); });

    FOR_EACH(File::KEYWORD_ROPABLES, module->ropables, ropable, { RESOLVE(ropable.node); });
    ...
    FOR_EACH(File::KEYWORD_SOUNDSOURCES, module->soundsources, soundsource, { RESOLVE(soundsource.node); });

    FOR_EACH(File::KEYWORD_SOUNDSOURCES2, module->soundsources2, soundsource2, { RESOLVE(soundsource2.node); });
    ...
    FOR_EACH(File::KEYWORD_TIES, module->ties, tie, { RESOLVE(tie.root_node); });
...
std::string SequentialImporter::ProcessMessagesToString()
{
    ...
    for (; itor != end; ++itor)
    {
        switch (itor->type)
        {
        case (Message::TYPE_FATAL_ERROR): report << "#FF3300 FATAL ERROR #FFFFFF"; break;
        ...
        report << " (module: " << itor->module_name << ", keyword: \"" << File::KeywordToString(itor->keyword)
               << "\"): " << itor->message << std::endl;
    }
...
bool SequentialImporter::AddNumberedNode(unsigned int number)
{
    ...
    {
        std::stringstream msg;
        msg << "Lost sync in node numbers, got numbered node [" << number << "], expected [" << m_all_nodes.size()
            << "]. Ignoring node.";
...
void SequentialImporter::AddGeneratedNode(File::Keyword              generated_from,
...
#define STAT_LINE(STREAM, TITLE, COUNT_VAR, KEYWORD)                                                                             \
    {                                                                                                                            \
        ...
        if (COUNT_VAR != 0)                                                                                                      \
        {                                                                                                                        \
            STREAM << " (after conversion: start index = " << std::setw(4) << offset                                             \
                   << ", end index = " << (offset + COUNT_VAR) - 1 << ")";                                                       \
        }                                                                                                                        \
    }
...
std::string SequentialImporter::IterateAndPrintAllNodes()
{
    ...
    for (; itor != end; ++itor)
    {
        ...
        case File::KEYWORD_FLEXBODYWHEELS: is_wheel = true; break;
        default: break;
        }
        // Add line entry
        report << "\n\t" << std::setw(3) << index << ": " << entry.node_id.ToString()
               << " (from=" << File::KeywordToString(entry.origin_keyword) << ", sub-index=" << entry.node_sub_index;
        if (is_wheel)
        {
