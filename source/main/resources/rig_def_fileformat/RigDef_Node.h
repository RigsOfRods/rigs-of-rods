...
namespace RigDef
{
    ...
    struct Node
    {
        ...
        /// Legacy parser resolved references on-the-fly and the condition to check named nodes was "are there any named nodes
        /// defined at this point?" The new parser defers node resolution, so every ref. must keep track whether named nodes were
        ...
        class Ref
        {
          public:
            ...
            inline unsigned int Num() const
            {
                return m_id_as_number;
            }

            inline bool Compare(Ref const &rhs) const
            {
                return m_id == rhs.m_id;
            }
            inline bool operator==(Ref const &rhs) const
            {
                return Compare(rhs);
            }
            inline bool operator!=(Ref const &rhs) const
            ...
            inline bool IsValidAnyState() const
            {
        ...
        struct Range
        {
            Range(Node::Ref const &start, Node::Ref const &end) : start(start), end(end)
            {
            }

            Range(Node::Ref const &single) : start(single), end(single)
            {
            }
        ...
        Node()
            : position(Ogre::Vector3::ZERO), options(0), load_weight_override(0), _has_load_weight_override(false),
