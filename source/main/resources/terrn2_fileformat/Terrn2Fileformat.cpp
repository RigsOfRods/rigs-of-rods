...
bool Terrn2Parser::LoadTerrn2(Terrn2Def &def, Ogre::DataStreamPtr &ds)
{
    ...
    def.start_position =
        StringConverter::parseVector3(file.GetStringEx("StartPosition", "General"), Vector3(512.0f, 0.0f, 512.0f));
...
void Terrn2Parser::ProcessTeleport(Terrn2Def &def, RoR::ConfigFile *file)
{
    ...
    for (;;)
    {
        ...
        {
            char msg_buf[500];
            snprintf(msg_buf, 500, "ERROR: Field '[Teleport]/%s' ('%s') is not valid XYZ position. Skipping telepoint %u.",
...
Terrn2Def::Terrn2Def()
    : ambient_color(Ogre::ColourValue::Black), category_id(-1), start_position(Ogre::Vector3::ZERO), version(1),
      gravity(DEFAULT_GRAVITY), water_height(0), water_bottom_height(0), caelum_fog_start(0), caelum_fog_end(0), has_water(false)
{
}
