...
CacheEntry::CacheEntry()
    : addtimestamp(0), beamcount(0), categoryid(0), commandscount(0), custom_particles(false), customtach(false), deleted(false),
      driveable(0),    // driveable = 0 = NOT_DRIVEABLE
      enginetype('t'), // enginetype = t = truck is default
      exhaustscount(0), fileformatversion(0), filetime(0), fixescount(0), flarescount(0), flexbodiescount(0),
      forwardcommands(false), hasSubmeshs(false), hydroscount(0), importcommands(false), loadmass(0), maxrpm(0), minrpm(0),
      nodecount(0), number(0), numgears(0), propscount(0), propwheelcount(0), rescuer(false), rotatorscount(0), shockcount(0),
      soundsourcescount(0), torque(0), truckmass(0), turbojetcount(0), turbopropscount(0), usagecounter(0), version(0),
      wheelcount(0), wingscount(0)
{
}
...
CacheEntry *CacheSystem::FindEntryByFilename(std::string filename)
{
    ...
    for (CacheEntry &entry : m_entries)
    {
        ...
        if (fname == filename || fname_without_uid == filename) return &entry;
    }
    return nullptr;
}
...
CacheSystem::CacheValidityState CacheSystem::EvaluateCacheValidity()
{
    ...
    if (!j_doc.IsObject() || !j_doc.HasMember("global_hash") || !j_doc["global_hash"].IsString() ||
        !j_doc.HasMember("format_version") || !j_doc["format_version"].IsNumber() || !j_doc.HasMember("entries") ||
        !j_doc["entries"].IsArray())
    {
...
void CacheSystem::PruneCache()
{
    ...
    for (auto &entry : m_entries)
    {
        ...
        if (entry.resource_bundle_type == "FileSystem") { fn = PathCombine(fn, entry.fname); }
...
void CacheSystem::DetectDuplicates()
{
    ...
    {
        if (m_entries[i].deleted) continue;
        ...
        for (int j = i + 1; j < m_entries.size(); j++)
        {
            ...
            basenameB = Ogre::StringUtil::replaceAll(basenameB, "-", "_");
            if (StripSHA1fromString(basenameA) != StripSHA1fromString(basenameB)) continue;

            if (m_entries[i].resource_bundle_path == m_entries[j].resource_bundle_path)
            {
                LOG("- duplicate: " + m_entries[i].fpath + m_entries[i].fname + " <--> " + m_entries[j].fpath +
                    m_entries[j].fname);
...
String CacheSystem::GetPrettyName(String fname)
{
    for (std::vector<CacheEntry>::iterator it = m_entries.begin(); it != m_entries.end(); it++)
    {
        if (fname == it->fname) return it->dname;
    }
    return "";
}
...
void CacheSystem::WriteCacheFileJson()
{
    ...
    {
        if (!entry.deleted) { this->ExportEntryToJson(j_entries, j_doc, entry); }
    }
    ...
    if (written_ok) { RoR::LogFormat("[RoR|ModCache] File '%s' written OK", path.c_str()); }
    else
    {
...
Ogre::String CacheSystem::StripSHA1fromString(Ogre::String sha1str)
{
    ...
    if (pos != String::npos && pos >= 20) return sha1str.substr(pos + 1, sha1str.length() - pos);
    return sha1str;
}
...
void CacheSystem::AddFile(String group, Ogre::FileInfo f, String ext)
{
    ...
    if (std::find_if(m_entries.begin(), m_entries.end(), [&](CacheEntry &e) {
            return !e.deleted && e.fname == f.filename && e.resource_bundle_path == path;
        }) != m_entries.end())
        return;
    ...
    try
    {
        ...
        for (auto &entry : new_entries)
        {
            ...
            entry.fext              = ext;
            if (type == "Zip") { entry.filetime = RoR::GetFileLastModifiedTime(path); }
            else
            {
    ...
    {
        RoR::LogFormat("[RoR|CacheSystem] Error processing file '%s', message :%s", f.filename.c_str(),
...
void CacheSystem::FillTruckDetailInfo(CacheEntry &entry, Ogre::DataStreamPtr stream, String file_name, String group)
{
    ...
    if (parser.GetMessages().size() > 0)
    {
        ...
        for (; iter != parser.GetMessages().end(); iter++)
        {
            switch (iter->type)
            {
            case (RigDef::Parser::Message::TYPE_FATAL_ERROR): report << "FATAL_ERROR"; break;

            case (RigDef::Parser::Message::TYPE_ERROR): report << "ERROR"; break;

            case (RigDef::Parser::Message::TYPE_WARNING): report << "WARNING"; break;
    ...
    if (def->root_module->engine != nullptr)
    {
        ...
        if (def->root_module->engoption != nullptr && def->root_module->engoption->type == RigDef::Engoption::ENGINE_TYPE_c_CAR)
        { entry.enginetype = 'c'; }
    }
    ...
    */
    ...
    for (; module_itor != def->user_modules.end(); module_itor++)
    {
        if (module_itor->second->engine != nullptr) { vehicle_type = TRUCK; }
        else if (module_itor->second->screwprops.size() > 0)
        {
            vehicle_type = BOAT;
        }
        /* Note: Sections 'turboprops' and 'turboprops2' are unified in TruckParser2013 */
        else if (module_itor->second->turbojets.size() > 0 || module_itor->second->pistonprops.size() > 0 ||
                 module_itor->second->turboprops_2.size() > 0)
        {
            vehicle_type = AIRPLANE;
        }
    }
    /* Root module */
    if (def->root_module->engine != nullptr) { vehicle_type = TRUCK; }
    else if (def->root_module->screwprops.size() > 0)
    {
        vehicle_type = BOAT;
    }
    /* Note: Sections 'turboprops' and 'turboprops2' are unified in TruckParser2013 */
    else if (def->root_module->turbojets.size() > 0 || def->root_module->pistonprops.size() > 0 ||
             def->root_module->turboprops_2.size() > 0)
    {
        vehicle_type = AIRPLANE;
    }
    ...
    {
        entry.wheelcount++;
        if (w.propulsion != RigDef::Wheels::PROPULSION_NONE) entry.propwheelcount++;
    }
    ...
    if (!def->root_module->axles.empty()) { entry.propwheelcount = static_cast<int>(def->root_module->axles.size() * 2); }
...
{
    if (ResourceGroupManager::getSingleton().resourceExists(group, filename + "dds")) return "dds";

    if (ResourceGroupManager::getSingleton().resourceExists(group, filename + "png")) return "png";

    if (ResourceGroupManager::getSingleton().resourceExists(group, filename + "jpg")) return "jpg";
...
void CacheSystem::GenerateFileCache(CacheEntry &entry, String group)
{
    ...
    if (entry.fext == "skin")
    {
        if (entry.skin_def->thumbnail.empty()) return;
...
void CacheSystem::ParseSingleZip(String path)
{
    if (std::find(m_resource_paths.begin(), m_resource_paths.end(), path) == m_resource_paths.end())
    {
        ...
        try
        {
            ...
            if (ParseKnownFiles(RGN_TEMP)) { LOG("No usable content in: '" + path + "'"); }
        }
        catch (Ogre::Exception &e)
        {
...
void CacheSystem::LoadResource(CacheEntry &t)
{
    ...
    {
        RoR::LogFormat("[RoR] Error while loading '%s', message: %s", t.resource_bundle_path.c_str(),
                       e.getFullDescription().c_str());
        if (ResourceGroupManager::getSingleton().resourceGroupExists(group))
        { ResourceGroupManager::getSingleton().destroyResourceGroup(group); }
    }
...
{
    if (guid.empty()) return std::vector<CacheEntry>();

    std::vector<CacheEntry> result;
    for (CacheEntry const &entry : m_entries)
    {
        if (entry.guid == guid && entry.fext == "skin") { result.push_back(entry); }
    }
    return result;
}
...
CacheEntry *CacheSystem::FetchSkinByName(std::string const &skin_name)
{
    for (CacheEntry &entry : m_entries)
    {
        if (entry.dname == skin_name && entry.fext == "skin") { return &entry; }
    }
    return nullptr;
}
...
std::shared_ptr<SkinDef> CacheSystem::FetchSkinDef(CacheEntry *cache_entry)
{
    ...
    try
    {
        ...
        Ogre::DataStreamPtr ds =
            Ogre::ResourceGroupManager::getSingleton().openResource(cache_entry->fname, cache_entry->resource_group);
        ...
        for (auto def : new_skins)
        {
            for (CacheEntry &e : m_entries)
            {
                if (e.resource_bundle_path == cache_entry->resource_bundle_path &&
                    e.resource_bundle_type == cache_entry->resource_bundle_type && e.fname == cache_entry->fname &&
                    e.dname == def->name)
                {
                    e.skin_def       = def;
        ...
        {
            RoR::LogFormat("Definition of skin '%s' was not found in file '%s'", cache_entry->dname.c_str(),
                           cache_entry->fname.c_str());
        }
        return cache_entry->skin_def;
    }
    catch (Ogre::Exception &oex)
    {
        RoR::LogFormat("[RoR] Error loading skin file '%s', message: %s", cache_entry->fname.c_str(),
