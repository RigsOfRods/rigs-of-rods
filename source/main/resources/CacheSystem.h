...
///    Associated media live in a "resource bundle" (ZIP archive or subdirectory) in content directories (ROR_HOME/vehicles,
///    ROR_HOME/terrains etc...)
...
class CacheSystem : public ZeroedMemoryAllocator
{
  public:
    ...
    bool CheckResourceLoaded(
        Ogre::String &in_out_filename); //!< Finds + loads the associated resource bundle if not already done.
    bool CheckResourceLoaded(Ogre::String &in_out_filename,
                             Ogre::String &out_group); //!< Finds given resource, outputs group name. Also loads the associated
                                                       //!< resource bundle if not already done.

    const std::map<int, Ogre::String> &GetCategories() const
    {
        return m_categories;
    }
  ...
  private:
    ...
    std::map<int, Ogre::String> m_categories = {// these are the category numbers from the repository. do not modify them!
