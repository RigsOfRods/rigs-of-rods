...
MeshWheel2::MeshWheel2(RigDef::MeshWheel2 const & def,  Node* inner, Node* outer, Node* rigidity, Node* reference_arm_node):
...
void MeshWheel2::Update(AllWheelsAggregateData *all_data)
{
    ...
    if (data.IsBreakThresholdUniform())   { m_definition.beam_defaults->breaking_threshold_constant = data.rim_breaking_threshold; }
    if (data.IsDeformThresholdUniform())  { m_definition.beam_defaults->deformation_threshold_constant = data.rim_deform_threshold; }
