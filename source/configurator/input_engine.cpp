...
//  ================================================
//
//  Ugly copy-paste from RoR, by only_a_ptr, 09/2016
//  TODO: Clean up!
//
//  ================================================
...
#ifdef LOG // TODO: UGLY! It's here because we still include files from other project. ~ only_a_ptr, 02/2017
#   undef LOG
#endif
#define LOG(_LITERAL_)    wxLogStatus(wxT(_LITERAL_))
#define LOG_OBJ(_STROBJ_) wxLogStatus(_STROBJ_)
...
bool InputEngine::setup(std::string hwnd, bool capture, bool capturemouse)
{
	...
	if (captureMode)
	{
		...
		{
			LOG_OBJ(wxString(wxT("Exception raised on keyboard creation: ")) + wxString::FromUTF8(ex.eText));
		}
		...
		if (capturemouse)
		{
			...
			{
				LOG_OBJ(wxString(wxT("Exception raised on mouse creation: ")) + wxString::FromUTF8(ex.eText));
			}
		}
...
// Helper
inline bool IsModifierKey(OIS::KeyCode kc)
{
    switch (kc)
    {
    case KC_RSHIFT:
...
// Helper
inline void AppendKeyName(std::string& dest, std::string const & src)
{
    if (dest.empty())
        dest = src;
    else
...
int InputEngine::GetCurrentKeyCombo(std::string *combo)
{
	int key_counter = 0;
    int mod_counter = 0;
    std::string plain_keys;
    std::string mod_keys;

    for (auto& state : keyState)
    {
        if (!state.second) // Skip inactive keys
            continue;
        
        auto kc = OIS::KeyCode(state.first);
        if (IsModifierKey(kc))
            AppendKeyName(mod_keys, this->getKeyNameForKeyCode(kc));
        else
            AppendKeyName(plain_keys, this->getKeyNameForKeyCode(kc));
    }

    if ((key_counter == 0) && (mod_counter == 0))
    {
...
bool InputEngine::saveMapping(std::string outfile)
{
    String hwnd;
    int joyNum = -10;
	// -10 = all
